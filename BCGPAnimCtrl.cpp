...
void CBCGPAnimCtrl::OnDraw(CDC* pDC)
{
	...
	if (m_pImagesAnim->GetSafeHandle () != NULL)
	{
...
//**************************************************************************************
BOOL CBCGPAnimCtrl::SetBitmap (UINT uiBmpId, int nFrameWidth, COLORREF clrTransparent, BOOL bSizeToContent, BOOL bDPIAutoScale)
{
#ifdef _BCGSUITE_
	...
	images.SetImageSize(CSize(nFrameWidth, images.GetImageSize().cy), TRUE);
	...
	if (!images.ExportToImageList(m_imagesAnim))
	{
		return FALSE;
	}
#else
...
//*******************************************************************************
LRESULT CBCGPAnimCtrl::OnPrint(WPARAM wp, LPARAM lp)
{
