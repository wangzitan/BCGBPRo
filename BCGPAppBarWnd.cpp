...
//*******************************************************************************
// BCGPAppBarWnd.cpp : implementation file
//
...
IMPLEMENT_SERIAL(CBCGPAppBarWnd,CWnd,VERSIONABLE_SCHEMA | 2)
...
UINT CBCGPAppBarWnd::m_uAppBarNotifyMsg = BCGM_APPBAR_CALLBACK;

static const CString strAppBarsProfile = _T ("BCGPAppBars");

#define REG_SECTION_FMT	_T("%sBCGPAppBar-%d")

/////////////////////////////////////////////////////////////////////////////
// CBCGPAppBarWnd

CBCGPAppBarWnd::CBCGPAppBarWnd()
{
...
BEGIN_MESSAGE_MAP(CBCGPAppBarWnd, CWnd)
	//{{AFX_MSG_MAP(CBCGPAppBarWnd)
	ON_WM_DESTROY()
	ON_WM_ACTIVATE()
...
BOOL CBCGPAppBarWnd::Create (LPCTSTR lpszWindowName, CRect rect, CWnd* pParentWnd)
{
	...
	if (!CreateEx (WS_EX_TOOLWINDOW,
				  globalData.RegisterWindowClass (_T("BCGPAppBar")), 
				  strWindowName,
...
BOOL CBCGPAppBarWnd::UnRegister ()
{
	ASSERT_VALID (this);
...
// uEdge can be ABE_TOP, ABE_BOTTOM, ABE_LEFT, or ABE_RIGHT.
BOOL CBCGPAppBarWnd::SetAutoHideBar (UINT uEdge, BOOL bRegister)
{
	ASSERT_VALID (this);
...
void CBCGPAppBarWnd::DoSetPos (UINT uEdge, CRect& rect, BOOL bMove, BOOL bNoSizeMove)
{
	ASSERT_VALID (this);
	...
	if (m_bAppRegistered)
	{
		...
		// Tell the system we're moving to this new approved position.
		if (!bNoSizeMove && !m_bInsideSetPos)
		{
			SetPos (rect);
		}
...
void CBCGPAppBarWnd::DoQueryPos (CRect& rect)
{
	ASSERT_VALID (this);
	ASSERT (m_bAppRegistered);
...
BOOL CBCGPAppBarWnd::DoSetSide (UINT uSide, BOOL bMove, BOOL bNoSizeMove)
{
	if (!m_bAppRegistered)
	{
		ASSERT (FALSE);
...
void CBCGPAppBarWnd::OnSizeMove ()
{
	ASSERT_VALID (this);
...
// If bNoSizeMove TRUE, prevent resizing the desktop.
BOOL CBCGPAppBarWnd::DoSetAutoHide (BOOL bEnable, BOOL bMove, BOOL bNoSizeMove)
{
	if (!m_bAppRegistered)
	{
	...
	if (bEnable)
	{
		...
		if (hwndAutoHide != NULL && hwndAutoHide != GetSafeHwnd ())
		{
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::GetScreenRect (CRect &rectScreen) const
{
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) 
{
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::OnWindowPosChanged(WINDOWPOS FAR* lpwndpos) 
{
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::OnMove(int x, int y) 
{
	CWnd::OnMove(x, y);
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::OnSize(UINT nType, int cx, int cy) 
{
...
//---------------------------------------------------------------------------------------------
LRESULT CBCGPAppBarWnd::OnEnterSizeMove(WPARAM,LPARAM)
{
	ASSERT_VALID (this);
...
//---------------------------------------------------------------------------------------------
LRESULT CBCGPAppBarWnd::OnExitSizeMove(WPARAM,LPARAM)
{
	ASSERT_VALID (this);
...
//---------------------------------------------------------------------------------------------
BCGNcHitTestType CBCGPAppBarWnd::OnNcHitTest(CPoint point) 
{
	ASSERT_VALID (this);

	UINT uHitTest = HitTest (point, TRUE);
...
LRESULT CBCGPAppBarWnd::AppBarCallback (WPARAM wParam, LPARAM lParam)
{
...
//---------------------------------------------------------------------------------------------
// Handles updating the appbar's size and position.
void CBCGPAppBarWnd::OnAppBarPosChanged ()
{
	if (!m_bSizing && !m_bMoving)
	{
		OnSizeMove ();
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::OnAppBarFullScreenApp (BOOL bOpen)
{
	ASSERT_VALID (this);
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::OnAppBarWindowArrange (BOOL)
{
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::OnAppBarStateChange (BOOL bAutoHide, BOOL bAlwaysOnTop)
{
	ASSERT_VALID (this);
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::OnLButtonDown(UINT nFlags, CPoint point) 
{
	CRect rect;
	GetWindowRect (rect);
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::OnMouseMove(UINT nFlags, CPoint point) 
{
	ASSERT_VALID (this);
	...
	if (m_bMoving)
	{
		...
		if (uSide != ABE_FLOAT)
		{
			CRect rectScreenCurr;
			GetScreenRect (rectScreenCurr);
			int nCXScreen = rectScreenCurr.Width ();
			int nCYScreen = rectScreenCurr.Height ();
		...
		case ABE_FLOAT:
			{
				...
				if (!rectFloat.PtInRect (ptCursor))
				{
					...
					ptOffset = ptCursor - rectFloat.TopLeft ();
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::OnNcMouseMove(UINT nHitTest, CPoint point) 
{
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::OnLButtonUp(UINT nFlags, CPoint point) 
{
	ASSERT_VALID (this);
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::OnCancelMode() 
{
	CWnd::OnCancelMode();
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::SlideWindow(LPRECT prc)
{
	const int nSlideShow = 200;
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR* lpncsp) 
{
	ASSERT_VALID (this);
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::OnTimer(UINT_PTR nIDEvent)
{
	if (!m_bMoving)
	{
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::CalcBorderSize (CRect& rectBorderSize) const
{
	rectBorderSize.SetRectEmpty ();
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::GetCaptionRect (CRect& rectCaption) const
{
	CRect rectBorderSize;
	rectBorderSize.SetRectEmpty ();
	CalcBorderSize (rectBorderSize);
...
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::OnDrawBorder (CDC* pDC)
{
	ASSERT_VALID (pDC);
	...
	CBCGPVisualManager::GetInstance ()->OnDrawAppBarBorder (pDC, this, rectWnd, rectBorderSize);
}
//---------------------------------------------------------------------------------------------
void CBCGPAppBarWnd::OnDrawCaption (CDC* pDC)
{
	ASSERT_VALID (this);
	...
	CBCGPVisualManager::GetInstance ()->OnDrawAppBarCaption (pDC, this, rectCaption, 
		GetCaptionText ());
}
//---------------------------------------------------------------------------------------------
CString CBCGPAppBarWnd::GetCaptionText () const
{
...
//---------------------------------------------------------------------------------------------
UINT CBCGPAppBarWnd::HitTest (CPoint point, BOOL bDetectCaption)
{
	CRect rectWnd;
	GetWindowRect (&rectWnd);
...
//---------------------------------------------------------------------------------------------
LRESULT CBCGPAppBarWnd::OnIdleUpdateCmdUI(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
...
//*************************************************************************************
BOOL CBCGPAppBarWnd::LoadState (LPCTSTR lpszProfileName, int nIndex)
{
	CString strProfileName = ::BCGPGetRegPath (strAppBarsProfile, lpszProfileName);
...
//*************************************************************************************
BOOL CBCGPAppBarWnd::SaveState (LPCTSTR lpszProfileName, int nIndex)
{
	CString strProfileName = ::BCGPGetRegPath (strAppBarsProfile, lpszProfileName);
...
//****************************************************************************************
void CBCGPAppBarWnd::SetFlags (DWORD dwFlags)
{
	ASSERT_VALID (this);
	m_dwFlags = dwFlags;
}
//****************************************************************************************
void CBCGPAppBarWnd::OnSettingChange(UINT uFlags, LPCTSTR lpszSection) 
{
