...
inline
int CompareAppointments (const CBCGPAppointment* pApp1, const CBCGPAppointment* pApp2, BCGP_SORTAPPOINTMENTS_TYPE type = BCGP_SORTAPPOINTMENTS_BY_TYPE_AND_DATE)
{
	if (pApp1 == pApp2)
	{
		return 0;
	...
	BOOL bAllOrMultiFirst = type == BCGP_SORTAPPOINTMENTS_BY_TYPE_AND_DATE;

	if (bAllOrMultiFirst && CBCGPPlannerView::IsOneDay (pApp1->GetStart (), pApp2->GetStart ()))
	{
	...
	if (!(bAllOrMulti1 && bAllOrMulti2) || (bAllOrMulti1 && bAllOrMulti2) || !bAllOrMultiFirst)
	{
...
void SortAppointments (XBCGPAppointmentArray& a, int size, BCGP_SORTAPPOINTMENTS_TYPE type/* = BCGP_SORTAPPOINTMENTS_BY_TYPE_AND_DATE*/)
{
	if (size == 0 || a.GetSize() < size)
	{
		return;
	}
	...
	do
	{
		...
		do
		{
			...
			do
			{
				while (CompareAppointments (a[i], pivot, type) == -1) i++;
				while (CompareAppointments (pivot, a[j], type) == -1) j--;
...
{
	return HitTestEx(point, IsAllDay () || IsMultiDay () ? BCGP_APPOINTMENT_HITTEST_TYPE_HORZ : BCGP_APPOINTMENT_HITTEST_TYPE_VERT);
}

CBCGPAppointment::BCGP_APPOINTMENT_HITTEST 
CBCGPAppointment::HitTestEx (const CPoint& point, DWORD dwFlags) const
{
	const int nHtPrec = globalUtils.ScaleByDPI(c_HT_PREC);

	const BOOL bHorz = (dwFlags & BCGP_APPOINTMENT_HITTEST_TYPE_HORZ) == BCGP_APPOINTMENT_HITTEST_TYPE_HORZ;
	const BOOL bVert = (dwFlags & BCGP_APPOINTMENT_HITTEST_TYPE_VERT) == BCGP_APPOINTMENT_HITTEST_TYPE_VERT;
	...
	if (m_DSDraw.IsEmpty ())
	{
		if (m_bVisibleDraw && m_rectDraw.PtInRect (point))
		{
			...
			if (bVert)
			{
				if (rt.top <= point.y && point.y <= (rt.top + nHtPrec))
				{
					hit = BCGP_APPOINTMENT_HITTEST_TOP;
				}
				else if (point.y <= rt.bottom && (rt.bottom - nHtPrec) <= point.y)
				{
					hit = BCGP_APPOINTMENT_HITTEST_BOTTOM;
				}
			}

			if (bHorz && hit == BCGP_APPOINTMENT_HITTEST_INSIDE)
			{
				if (rt.left <= point.x && point.x <= (rt.left + nHtPrec))
				{
					hit = BCGP_APPOINTMENT_HITTEST_LEFT;
				}
				else if (point.x <= rt.right && (rt.right - nHtPrec) <= point.x)
				{
		...
		for (int i = 0; i < m_DSDraw.GetCount (); i++)
		{
			...
			if (pDS->IsVisible () && pDS->GetRect ().PtInRect (point))
			{
				...
				if (bVert)
				{
					...
					{
						if (point.y <= rt.bottom && (rt.bottom - nHtPrec) <= point.y)
						{
							hit = BCGP_APPOINTMENT_HITTEST_BOTTOM;
						}
					}
				...
				if (bHorz && hit == BCGP_APPOINTMENT_HITTEST_INSIDE)
				{
					...
					if (pDS->GetBorder () & CBCGPAppointmentDrawStruct::BCGP_APPOINTMENT_DS_BORDER_FINISH)
					{
						if (point.x <= rt.right && (rt.right - nHtPrec) <= point.x)
						{
							hit = BCGP_APPOINTMENT_HITTEST_RIGHT;
						}
					}
...
void CBCGPAppointment::AdjustToolTipRect (CRect& rect, BOOL bResizeVert) const
{
	...
	{
		rect.DeflateRect (nHtPrec, 0);
	}
	else if (bResizeVert)
	{
		rect.DeflateRect (0, nHtPrec);
	}
}
