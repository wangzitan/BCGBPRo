...
int CBCGPRibbonSeparator::AddToListBox (CBCGPRibbonCommandsListBox* pWndListBox, BOOL /*bDeep*/)
{
	...
	int nIndex = nCurSel >= 0 ? pWndListBox->InsertString(nCurSel + 1, strText) : pWndListBox->AddString(strText);
...
void CBCGPBaseRibbonElement::SetDroppedDown (CBCGPPopupMenu* pPopupMenu)
{
	...
	if (m_pParentMenu->GetSafeHwnd () != NULL && ::IsWindow(m_pParentMenu->GetSafeHwnd()) && pPopupMenu == NULL)
	{
		ASSERT_VALID (m_pParentMenu);
...
//******************************************************************************
BOOL CBCGPBaseRibbonElement::HasParentMenu(BOOL bCheckForAutohidePopup /*= FALSE*/) const
{
	ASSERT_VALID(this);

	if (!bCheckForAutohidePopup)
	{
		return m_pParentMenu != NULL;
	}
	
	return m_pParentMenu != NULL && !m_pParentMenu->IsRibbonPopupMode();
}
//******************************************************************************
...
CWnd* CBCGPBaseRibbonElement::GetParentWnd () const
{
	...
	if (m_bIsAutohidePopupMode && m_pParentMenu != NULL)
	{
		ASSERT_VALID (m_pParentMenu);
...
void CBCGPBaseRibbonElement::Redraw ()
{
	...
	CWnd* pWndRedraw = m_pWndRedraw;
	if (pWndRedraw->GetSafeHwnd() == NULL)
	{
		rect.InflateRect(2, 2);

		if (m_pParentMenu->GetSafeHwnd () != NULL)
		{
			pWndRedraw = m_pParentMenu;
		}
		else
		{
	...
	if (pWndRedraw->GetSafeHwnd() != NULL)
	{
		pWndRedraw->RedrawWindow(rect);
	}
}
//******************************************************************************
void CBCGPBaseRibbonElement::RedrawWnd(CWnd* pWnd)
{
	CBCGPRAII<CWnd*> raii(m_pWndRedraw, pWnd);
	Redraw();
}
//******************************************************************************
CString CBCGPBaseRibbonElement::GetToolTipText () const
{
	...
	if (m_bQuickAccessMode && strTipText.IsEmpty ())
	{
		...
		globalUtils.RemoveSingleAmp(strTipText);
	}
...
void CBCGPBaseRibbonElement::UpdateTooltipInfo ()
{
	...
	globalUtils.RemoveSingleAmp(m_strToolTip);
}
//******************************************************************************
...
CSize CBCGPBaseRibbonElement::GetKeyTipSize (CDC* pDC)
{
	...
	if (!m_bQuickAccessMode && HasParentMenu() && m_strKeys.GetLength () < 2 && !IsPaletteIcon())
	{
...
BOOL CBCGPBaseRibbonElement::SetACCData (CWnd* pParent, CBCGPAccessibilityData& data)
{
	...
	if (!m_bQuickAccessMode && HasParentMenu() && strKeys.GetLength () < 2 && !IsPaletteIcon ())
	{
		// Try to find key from label:
		int nIndexAmp = m_strText.Find (_T('&'));
...
//******************************************************************************
int CBCGPBaseRibbonElement::DoDrawText (CDC* pDC, const CString& strText, CRect rectText, UINT uiDTFlags, COLORREF clrText)
{
	ASSERT_VALID (this);
	ASSERT_VALID (pDC);
	...
	if (CBCGPToolBarImages::m_bIsDrawOnGlass)
	{
		...
		{
			CWnd* pParent = GetParentRibbonBar ()->GetParent();

			bIsZoomed = pParent->IsZoomed();
			bActive = CBCGPVisualManager::GetInstance()->IsWindowActive(pParent);
		}

		COLORREF clrTextGlass = (bActive && !IsTabElement() && clrText == (COLORREF)-1) ? globalData.GetDWMTextColor() : clrText;
		
		if (!globalData.bIsWindows7 && bIsZoomed)
		{
			clrTextGlass = RGB(255, 255, 255);
		}

		CBCGPVisualManager::GetInstance ()->DrawTextOnGlass(pDC, strText, rectText, uiDTFlags, m_nTextGlassGlowSize, 
			clrTextGlass == (COLORREF)-1 ? clrText : clrTextGlass);
	...
	if (m_bSearchResultMode)
	{
		...
		globalUtils.RemoveSingleAmp(strWithoutAmp);
