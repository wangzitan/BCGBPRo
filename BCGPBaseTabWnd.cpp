...
BOOL CBCGPBaseTabWnd::m_bAlwaysDrawTabIcon = TRUE;
...
CBCGPBaseTabWnd::CBCGPBaseTabWnd()
{
	...
	m_bIsMDITab = FALSE;
	m_bCloseMDITabOnMiddleClick = TRUE;
...
BOOL CBCGPBaseTabWnd::RemoveTab (int iTab, BOOL bRecalcLayout)
{
	...
	StopTabStateAnimation(iTab, FALSE);
...
void CBCGPBaseTabWnd::RemoveAllTabs ()
{
	...
	while (m_iTabsNum > 0)
	{
		...
		StopTabStateAnimation(m_iTabsNum - 1, FALSE);
...
BOOL CBCGPBaseTabWnd::DetachTab (BCGP_DOCK_METHOD dockMethod, int nTabNum, BOOL bHide)
{
	...
	if (pDockingBar != NULL && pDockingBar->CanBeAttached ())
	{
		...
		if (GetTabsNum () == 1 && m_bHideSingleTab)
		{
			...
			CBCGPDockingControlBar* pDockingBarCurr = 
				DYNAMIC_DOWNCAST (CBCGPDockingControlBar, GetTabWnd (0));	

			pParent->InsertControlBar (pDockingBarCurr, pParent);
			pParent->ReplaceControlBar (pDockingBarCurr, dockMethod);
			pDockingBarCurr->SetBarAlignment (pParent->GetCurrentAlignment ());
			RemoveTab (0);
			CWnd* pNewParent = (pMiniFrame == NULL) ? pParent->GetDockSite () 
													: pMiniFrame;

			pDockingBarCurr->EnableGripper (TRUE);
			pNewParent->ScreenToClient (rectWnd);

			pDockingBarCurr->SetParent (pNewParent);
			pDockingBarCurr->SetWindowPos (NULL, rectWnd.left, rectWnd.top,
										rectWnd.Width (), rectWnd.Height (),
										SWP_NOZORDER | SWP_NOACTIVATE | SWP_HIDEWINDOW);
		
			pDockingBarCurr->ShowControlBar (TRUE, FALSE, FALSE);
			pParent->ShowWindow (SW_HIDE);
			pDockingBarCurr->GetParent ()->InvalidateRect (NULL);
			pDockingBarCurr->GetParent ()->UpdateWindow ();
...
void CBCGPBaseTabWnd::ApplyRestoredTabInfo (BOOL bUseTabIndexes)
{
	...
	if (pTabBar != NULL)
	{
		...
		bRestoreTabNames = pTabBar->IsRestoreTabNames();
	}
	...
	if (nVisibleTabNumber == 0)
	{
		CBCGPBaseTabbedBar* pTabBar1 = DYNAMIC_DOWNCAST (CBCGPBaseTabbedBar, GetParent ());	
		if (pTabBar1 != NULL)
		{
			pTabBar1->ShowControlBar (FALSE, FALSE, FALSE);
		}
	}
...
BOOL CBCGPBaseTabWnd::PreTranslateMessage(MSG* pMsg) 
{
	if (m_pInPlaceEdit != NULL)
	{
		...
		{
			if (pMsg->message == WM_KEYDOWN && globalUtils.ProcessCtrlEditAccelerators(m_pInPlaceEdit, (UINT)pMsg->wParam, TRUE))
			{
				return TRUE;
			}
...
//***************************************************************************************
void CBCGPBaseTabWnd::SetImageAutoScale(BOOL bSet /*= TRUE*/)
{
	m_bImageAutoScale = bSet;
}
//***************************************************************************************
BOOL CBCGPBaseTabWnd::SetImageList (UINT uiID, int cx, COLORREF clrTransp)
{
	...
	if (svg.Load(uiID))
	{
		...
		m_sizeImage = CSize (cx, cx);		// Assume square
...
#endif // BCGP_EXCLUDE_SVG
	{
		...
		if (!images.Load(uiID))
		{
			TRACE(_T("CBCGPBaseTabWnd::SetImageList Can't load bitmap: %x\n"), uiID);
			return FALSE;
		}

		images.SetSingleImage();
		images.SetImageSize(CSize (cx, images.GetImageSize().cy));
		...
		m_sizeImage = images.GetImageSize();
		images.ExportToImageList(m_Images);
	}
...
{
    if (m_bIsMDITab && m_bCloseMDITabOnMiddleClick)
    {
		int nTab = GetTabFromPoint(point);
		if (nTab >= 0)
		{
...
BOOL CBCGPBaseTabWnd::ToggleTabSelectState(int iTab, BOOL bRedraw)
{
	...
	else if (iTab == m_iActiveTab)
	{
		...
		for (int i = 0; i < m_iTabsNum; i ++)
		{
			CBCGPTabInfo* pTabCurr = (CBCGPTabInfo*) m_arTabs [i];
			ASSERT_VALID (pTabCurr);
			
			if (pTabCurr->m_bIsSelected && i != iTab)
			{
				SetActiveTab(i);
				break;
			}
...
//***************************************************************************************
BOOL CBCGPBaseTabWnd::DrawTabState(CDC* pDC, int iTab, CRect rectImage) const
{
	if (iTab < 0 || iTab >= m_iTabsNum)
	{
		TRACE(_T("CBCGPBaseTabWnd::DrawTabState: illegal tab number %d\n"), iTab);
		return FALSE;
	}
	...
	if (pTab->m_nStateAnimationStep < 0 || pTab->m_nStateAnimationStep >= GetTabStateAnimationFramesCount())
	{
		return FALSE;
	}

	return ((CBCGPBaseTabWnd*)this)->OnDrawTabStateFrame(pDC, iTab, rectImage, pTab->m_nStateAnimationStep);
}
//***************************************************************************************
BOOL CBCGPBaseTabWnd::OnDrawTabStateFrame(CDC* pDC, int /*iTab*/, CRect rectImage, int nStep)
{
	return m_StateAnimationImages.DrawEx(pDC, rectImage, nStep, CBCGPToolBarImages::ImageAlignHorzCenter, CBCGPToolBarImages::ImageAlignVertCenter);
}
