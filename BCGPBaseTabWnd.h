...
public:
	...
	static BOOL m_bAlwaysDrawTabIcon;

// Operations
public:
	void SetImageAutoScale(BOOL bSet = TRUE);
	...
	BOOL DrawTabState(CDC* pDC, int iTab, CRect rectImage) const;
...
public:
	...
	virtual BOOL IsUnderlineStyle() const	{return FALSE;}
	...
	BOOL IsCloseMDITabOnMiddleClick() const 
	{ 
		return m_bCloseMDITabOnMiddleClick; 
	}

	void SetCloseMDITabOnMiddleClick(BOOL bSet = TRUE)
	{ 
		m_bCloseMDITabOnMiddleClick = bSet;
	}

	BOOL IsPureFlatTab() const
	{
		return IsPointerStyle() || IsUnderlineStyle();
	}
...
// Overrides
	...
	virtual int GetTabStateAnimationFramesCount() const
	{
		return m_StateAnimationImages.GetCount();
	}
	
	virtual BOOL OnDrawTabStateFrame(CDC* pDC, int iTab, CRect rectImage, int nStep);
...
protected:
	int FindTabInfo (int nBarID, CBCGPTabInfo** ppTabInfo);
	...
	BOOL			m_bImageAutoScale;	// Is image list auto-scaled by DPI
