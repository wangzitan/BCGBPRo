								 ...
								 BOOL bDetachable)
{
	...
	if (pNewBar->IsKindOf (RUNTIME_CLASS (CBCGPBaseTabbedBar)))
	{
		...
		for (int i = 0; i < nTabsNum; i++)
		{
			...
			BOOL bTabVisible = pWndTab->IsTabVisible (i);
			BOOL bTabDetachable = pWndTab->IsTabDetachable (i);

			pWnd->EnableGripper (FALSE);

			if (!AddTab (pWnd, bTabVisible, bTabVisible, bTabDetachable))
			{
				ASSERT (FALSE);
			}
									  ...
									  BOOL bHide)
{
	...
	if (pBar->IsKindOf (RUNTIME_CLASS (CBCGPControlBar)))
	{
		...
		if (dockMethod == BCGP_DM_MOUSE && CBCGPDockManager::m_bKeepBarSizeOnFloating)
		{
			...
			CBCGPDockingControlBar* pDockingBarCurr = DYNAMIC_DOWNCAST (CBCGPDockingControlBar, pControlBar);
			if (pDockingBarCurr != NULL)
			{
				rectFloat.InflateRect(pDockingBarCurr->m_nBorderSize + 1, pDockingBarCurr->m_nBorderSize + 1);
			}
		}
