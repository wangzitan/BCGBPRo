...
BOOL CBCGPBreadcrumb::SelectPath (const CString& itemPath, TCHAR delimiter)
{
	...
	while (!s.IsEmpty ())
	{
		CString str = globalUtils.StringExtractItemQuoted (itemPath, tokens, iPos);
		if (str.IsEmpty ()) break;
		if (hItem == hRootItem && str == GetItemText (hRootItem))
		{
			SelectItem (hRootItem);
		}
		HBREADCRUMBITEM hSubItem = GetSubItemByName (hItem, str);
...
void CBCGPBreadcrumb::BeginInplaceEditing ()
{
	...
	if (pFont == NULL)
	{
		pFont = CFont::FromHandle (m_hFont == NULL ? (HFONT)::GetStockObject (DEFAULT_GUI_FONT) : m_hFont);
	}
...
BOOL CBCGPBreadcrumb::PreTranslateMessage (MSG* pMsg)
{
	...
	{
		if (globalUtils.ProcessCtrlEditAccelerators(m_pInplaceEdit, (UINT)pMsg->wParam, TRUE))
		{
			return TRUE;
		}
