...
protected:
	...
	bool MsgPaint (UINT code, WPARAM wParam, LPARAM lParam, LRESULT& rResult)
	{
		...
		if (code == WM_NCPAINT)
		{
			...
			{
#ifndef _BCGSUITE_
				if (m_pWnd->m_bVisualManagerStyle)
				{
					CBCGPVisualManager::GetInstance ()->OnDrawControlBorder (&dc, rectWindow, m_pWnd, m_pWnd->IsOnGlass());
				}
				else
				{
					CBCGPVisualManager::GetInstance ()->OnDrawControlBorderNoTheme(&dc, rectWindow, m_pWnd, m_pWnd->IsOnGlass());
				}
#else
				CBCGPVisualManager::GetInstance ()->OnDrawControlBorder (&dc, rectWindow, m_pWnd, m_pWnd->IsOnGlass());
#endif
		...
		if (code == WM_PRINT && (lParam & PRF_NONCLIENT) == PRF_NONCLIENT)
		{
			...
			{
				CBCGPVisualManager::GetInstance ()->OnDrawControlBorderNoTheme(pDC, rect, m_pWnd, FALSE);
			}
#else
			CBCGPVisualManager::GetInstance ()->OnDrawControlBorder(pDC, rect, m_pWnd, FALSE);
#endif
		...
		if (code == BCCM_DRAWMENUITEMICON && lParam != NULL && m_hImageList != NULL)
		{
			...
			if (nIndex >= 0 && nIndex < (int)m_arrCurrentMenuItems.GetSize())
			{
				...
				rResult = (LRESULT)ImageList_Draw(m_hImageList, iImage, pInfo->hdc, ptIcon.x, ptIcon.y, ILD_TRANSPARENT);
	...
	{
		HFONT hFont = m_hFont == NULL ? m_pWnd->GetCtrlFont() : m_hFont;
		if (hFont == NULL)
		{
