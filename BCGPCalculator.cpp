...
#ifndef _BCGSUITE_
			if (m_pParent->m_bIsDlgControl && !m_pParent->m_bVisualManagerStyle)
			{
				clrText = CBCGPVisualManager::GetInstance ()->CBCGPVisualManager::GetCalculatorButtonTextColor(m_bIsUserCommand, m_uiCmd);
			}
#endif
								...
								BOOL /*bDrawBorder*/, BOOL /*bGrayDisabledButtons*/)
{
	...
	if (m_pParent->OnDrawDisplay (pDC, rect, m_strText, m_bMem))
	{
		...
		if (m_pParent->m_bIsDlgControl && !m_pParent->m_bVisualManagerStyle)
		{
			clrText = CBCGPVisualManager::GetInstance ()->CBCGPVisualManager::GetCalculatorDisplayTextColor();
		}
...
//*********************************************************************************
CWnd* CBCGPCalculator::DoCreateOnPlaceHolder(CWnd* pParent, UINT nID, DWORD dwStyle, const CRect& rect, LPVOID /*lParam*/)
{
	if (!CreateControl(pParent, rect, nID))
	{
		return NULL;
	}
		...
		int cmd)
{
	...
	{
		return CBCGPVisualManager::GetInstance ()->CBCGPVisualManager::OnDrawCalculatorButton (pDC, rect, pButton, state, cmd, this);
	}
#endif
	return CBCGPVisualManager::GetInstance ()->OnDrawCalculatorButton (pDC, rect, pButton, state, cmd, this);
}
//***********************************************************************************
									 ...
									 const CString& strText, BOOL bMem)
{
	...
	if (m_bIsDlgControl && !m_bVisualManagerStyle)
	{
		return CBCGPVisualManager::GetInstance ()->CBCGPVisualManager::OnDrawCalculatorDisplay(pDC, rect, strText, bMem, this);
	}
#endif
	return CBCGPVisualManager::GetInstance ()->OnDrawCalculatorDisplay(pDC, rect, strText, bMem, this);
}
//***********************************************************************************
...
#ifndef _BCGSUITE_
	BOOL bVisualManagerStyle = m_bVisualManagerStyle || !m_bIsDlgControl;
#endif

	m_CalcImages.CopyTo (m_CalcImagesDigits);
#ifndef _BCGSUITE_
	m_CalcImagesDigits.AdaptColors(RGB (0, 0, 0), 
		bVisualManagerStyle ? CBCGPVisualManager::GetInstance ()->GetCalculatorButtonTextColor(FALSE, idCommandNone) : CBCGPVisualManager::GetInstance ()->CBCGPVisualManager::GetCalculatorButtonTextColor(FALSE, idCommandNone), 
		FALSE);
#endif

	m_CalcImages.CopyTo (m_CalcImagesCommands);
#ifndef _BCGSUITE_
	m_CalcImagesCommands.AdaptColors(RGB (0, 0, 0), 
		bVisualManagerStyle ? CBCGPVisualManager::GetInstance ()->GetCalculatorButtonTextColor(FALSE, idCommandClear) : CBCGPVisualManager::GetInstance ()->CBCGPVisualManager::GetCalculatorButtonTextColor(FALSE, idCommandClear), 
		FALSE);
#endif

	m_CalcImages.CopyTo (m_CalcImagesUser);
#ifndef _BCGSUITE_
	m_CalcImagesUser.AdaptColors(RGB (0, 0, 0), 
		bVisualManagerStyle ? CBCGPVisualManager::GetInstance ()->GetCalculatorButtonTextColor(TRUE, idCommandUser) : CBCGPVisualManager::GetInstance ()->CBCGPVisualManager::GetCalculatorButtonTextColor(TRUE, idCommandUser), 
		FALSE);
#endif
