...
BOOL CBCGPCalendar::m_bSelectNextPrevMonthDate = TRUE;
...
void CBCGPCalendar::OnDraw(CDC* pDCPaint) 
{
	if (!m_bCustomColors)
	{
		...
		{
			visualManager->CBCGPVisualManager::GetCalendarColors (this, m_Colors);
		}
	}
...
void CBCGPCalendar::RecalcLayout (BOOL bRedraw)
{
	...
	if (m_nMaxWeekDayCharacters > 2 || m_bIsPopup)
	{
		...
		const int nPadding = m_szCellPad.cx / 4;
...
void CBCGPCalendar::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	{
		BOOL bSelectNow = m_bSelectNextPrevMonthDate && !m_bMultipleSelection;
...
int CBCGPCalendar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_btnToday.m_bVisualManagerStyle = m_bVisualManagerStyle;
...
#ifndef _BCGSUITE_
		const int nDepth = globalUtils.ScaleByDPI(5);
		m_pWndShadow = new CBCGPShadowWnd(this, nDepth, nDepth, CSize(nDepth, nDepth));
		
		m_pWndShadow->Create();
		m_pWndShadow->Repos();
#endif		
		SetWindowPos (&wndTop, rectWindow.left, rectWindow.top, size.cx, size.cy, SWP_SHOWWINDOW);
		SetFocus ();
	}
...
void CBCGPCalendar::OnNcDestroy() 
{
#ifndef _BCGSUITE_
	if (m_pWndShadow->GetSafeHwnd () != NULL)
	{
...
void CBCGPCalendar::EnableVisualManagerStyle(BOOL bEnable)
{
	...
	m_btnToday.m_bVisualManagerStyle = bEnable;
...
//****************************************************************************************
CWnd* CBCGPCalendar::DoCreateOnPlaceHolder(CWnd* pParent, UINT nID, DWORD dwStyle, const CRect& rect, LPVOID /*lParam*/)
{
...
int CBCGPMonthPickerWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_nRowHeight = cy + globalUtils.ScaleByDPI(4);

	CSize sizeWindow (cx + sizeYear.cx + globalUtils.ScaleByDPI(20), m_nRowHeight * nVisbleMonths + globalUtils.ScaleByDPI(2));

	dc.SelectObject (pOldFont);

#ifndef _BCGSUITE_
	const int nDepth = globalUtils.ScaleByDPI(4);
	m_pWndShadow = new CBCGPShadowWnd(this, nDepth, nDepth, CSize(nDepth, nDepth));
...
void CBCGPMonthPickerWnd::OnPaint() 
{
	...
	const CBCGPCalendarColors& colors = m_pCalendarWnd->GetColors();
	...
	COLORREF clrFill = colors.clrBackground;
	if (clrFill == (COLORREF)-1)
	{
		clrFill = globalData.clrWindow;
	}

	COLORREF clrText = colors.clrText;
	if (clrText == (COLORREF)-1)
	{
		clrText = globalData.clrWindowText;
	}

	COLORREF clrFillSel = colors.clrSelected;
	if (clrFillSel == (COLORREF)-1)
	{
		clrFillSel = globalData.clrHilite;
	}
	
	COLORREF clrTextSel = colors.clrSelectedText;
	if (clrTextSel == (COLORREF)-1)
	{
		clrTextSel = globalData.clrTextHilite;
	}

	COLORREF clrBorder = colors.clrLine;
	if (clrBorder == (COLORREF)-1)
	{
		clrBorder = globalData.clrGrayedText;
	}
	...
	for (int i = 0; i < nVisbleMonths; i++)
	{
		...
		if (bDraw)
		{
			const BOOL bIsSelected = (i == m_nSelRow);

			if (bIsSelected)
			{
				CBrush br(clrFillSel);
				pDC->FillRect(rectMonth, &br);
			}

			CString str;
			str.Format (_T("%s %04d"), (LPCTSTR)m_strMonthName [nMonth - 1], nYear);

			pDC->SetTextColor(bIsSelected ? clrTextSel : clrText);
...
//****************************************************************************************
void CBCGPMonthPickerWnd::OnSize(UINT nType, int cx, int cy) 
{
	...
	{
		m_pWndShadow->Repos ();
		m_pWndShadow->SetWindowPos(this, -1, -1, -1, -1, SWP_NOACTIVATE | SWP_NOSIZE | SWP_NOMOVE);
	}
#endif
}
//****************************************************************************************
void CBCGPMonthPickerWnd::OnNcDestroy() 
{
#ifndef _BCGSUITE_
	if (m_pWndShadow->GetSafeHwnd () != NULL)
	{
