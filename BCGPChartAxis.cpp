...
double CBCGPChartAxis::GetLabelDistance() const
{
	...
	{
		double dblPadding = m_bThumbnailMode ? 0. : 6.;

		return (m_dblLabelDistance + dblPadding) * (IsVertical() ? szScaleRatio.cx : szScaleRatio.cy);
	}
...
double CBCGPChartAxis::ApproximateValue(double dblValue, double dblUnit, double dblApproximation)
{
	...
	if (log10(fabs(dblValue)) > 18 || log10(fabs(dblUnit)) > 18)
	{
...
double CBCGPChartAxis::CalcNormalValue(double dblValue) const
{
	...
	if (IsLogScale() && dblValue > 0)
	{
		...
		dblRange = CalcLog(dblMaxValue) - dblMinValue;
	}
...
BOOL CBCGPChartAxis::UpdateScaleBreaksInternal(const CBCGPChartValue& val, double dblThreshold, CBCGPChartSeries* pSeries)
{
	...
	for (POSITION pos = m_scaleParts.GetHeadPosition(); pos != NULL; m_scaleParts.GetNext(pos))
	{
		...
		if (dblVal < part.m_dblMinimum)
		{
			...
			{
				CBCGPChartAxisScalePart partNew;
				partNew.m_dblMinimum = partNew.m_dblMaximum = dblVal;
				m_scaleParts.InsertBefore(pos, partNew);
			}
			break;
		}
...
void CBCGPChartAxis::OnDrawAxisOnThickWall(CBCGPEngine3D* pEngine, CBCGPChartSide3D* pSide)
{
	...
	while (dblCurrValue <= dblMaxValue)
	{
			...
			dblCurrValue >= GetMinDisplayedValue())
		{
			...
			{
				dblVal = PointFromValue(dblMinorValue, TRUE, FALSE);
				ptStartTick = GetPointOnAxisPlane3D(dblVal, TRUE);
				ptEndTick = ptStartTick;
	...
	while (dblCurrValue <= dblMaxValue)
	{
		...
		if ((m_minorTickMarkType != CBCGPChartAxis::TMT_NO_TICKS) && dblCurrValue < GetMaxDisplayedValue() + GetDoubleCorrection())
		{
			...
			for (int i = 0; i < nMaxCount; i++)
			{
				...
				ptStartTick = ptAxisStart;
				ptEndTick = ptAxisStart;
...
void CBCGPChartAxis::OnDrawAxisLabels(CBCGPGraphicsManager* pGM, const CBCGPRect& rectDiagram)
{
	...
	double dblMajorUnitInterval = dblUnitsInInterval;
	if (IsLogScale() && dblMajorUnitInterval > 0)
	{
		double dblMinValue = GetMinDisplayedValue();
		if (dblMinValue == 0)
		{
		...
		dblMajorUnitInterval = CalcLog(CalcLog(dblUnitsInInterval) + CalcLog(dblMinValue), FALSE);
	}

	dblMajorUnitInterval = fabs(ScaleValue(dblMajorUnitInterval));
	...
	int nStep = 1;
	if (fabs(dblMajorUnitInterval) > DBL_EPSILON)
	{
		nStep = (int)(IsVertical() ? (m_szMaxLabelSize.cy / dblMajorUnitInterval) : (m_szMaxLabelSize.cx / dblMajorUnitInterval)) + 1;
	}
	...
	else
	{
		...
		for (int i = 0 ; i < m_pChart->GetSeriesCount(); i++)
		{
			...
			if (pSeries->IsShownOnAxis(this))
			{
				...
				{
					dblMin = min(dblMin, (double)valCurrMin);
					dblMax = max(dblMax, (double)valCurrMax);
				}
...
BOOL CBCGPChartAxisY::CalcMinMaxValues()
{
	...
	if (bMinMaxSet)
	{
		const double dblMax = 100.0 + FLT_EPSILON;

		if (80.0 < m_dblMaximum && m_dblMaximum < dblMax)
		{
			SetFixedMaximumDisplayValue(100., FALSE);
		}

		if (-dblMax < m_dblMinimum && m_dblMinimum < -80.)
		{
			SetFixedMinimumDisplayValue(-100., FALSE);
		}
		
		if (fabs(m_dblMinimum) < FLT_EPSILON)
		{
...
void CBCGPChartTernaryAxis::CommonInit()
{
	...
	m_titleMode = CBCGPChartTernaryAxis::TM_NORMAL;
}
//----------------------------------------------------------------------------//
...
void CBCGPChartTernaryAxis::CalcNameRect(CBCGPGraphicsManager* pGM, CBCGPRect& /*rectPlotArea*/, const CRect& /*rectChartArea*/, BOOL bReposOnly)
{
	...
	else
	{
		...
		const BOOL bTitleCenter = m_titleMode == TM_CENTER;
		CBCGPPoint ptCenter((m_ptAxisStart + m_ptAxisEnd) / 2);
		...
		case CBCGPChartAxis::ADP_RIGHT:
			...
			{
				m_rectAxisName.left = ptCenter.x + m_szMaxLabelSize.cx + GetLabelDistance();
				m_rectAxisName.top = ptCenter.y - szName.cy / 2;
			}
			break;
		...
		case CBCGPChartAxis::ADP_LEFT:
			...
			{
				m_rectAxisName.left = ptCenter.x - szName.cx - m_szMaxLabelSize.cx - GetLabelDistance();
				m_rectAxisName.top = ptCenter.y - szName.cy / 2;
			}
			break;
		...
		case CBCGPChartAxis::ADP_BOTTOM:
			...
			{
				m_rectAxisName.left = ptCenter.x - szName.cx / 2;
			}
...
void CBCGPChartTernaryAxis::CalcAxisPos(const CBCGPRect& rectDiagram, BOOL /*bStoreAxisPos*/)
{
	...
	rcDiagram.top += bTitleCenter ? 0 : pRightAxis->m_rectAxisName.Height();
...
//----------------------------------------------------------------------------//
void CBCGPChartTernaryAxis::SetTitleMode(CBCGPChartTernaryAxis::TitleMode tm, BOOL bRedraw)
{
	if (tm != m_titleMode)
	{
		m_titleMode = tm;
