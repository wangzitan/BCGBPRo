...
struct BCGCBPRODLLEXPORT BCGPChartFormatLine
{
	...
	double GetLineWidth(BOOL bScaled = FALSE) const 
	{
		...
		return bcgp_max(m_szScaleRatio.cx, m_szScaleRatio.cy) * m_dblWidth;
	}
...
struct BCGCBPRODLLEXPORT BCGPChartDataLabelOptions
{
	...
	{
		return m_dblDistanceFromMarker * bcgp_max(m_szScaleRatio.cx, m_szScaleRatio.cy);
	}

	double	GetKeyToLabelDistance() const
	{
		return m_nKeyToLabelDistance * bcgp_max(m_szScaleRatio.cx, m_szScaleRatio.cy);
	}
