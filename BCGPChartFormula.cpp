...
{
	int nActualStart = nIndex - m_nKPeriod + 1;
	if (nActualStart < 0)
	{
		return CBCGPChartValue();
	}
	...
	double dblMin = pSeriesLow->GetDataPointValue(nActualStart);
	double dblMax = pSeriesHigh->GetDataPointValue(nActualStart);

	int i = 0;

	for (i = nActualStart + 1; i <= nIndex; i++)
	{
		const double dblValLow = pSeriesLow->GetDataPointValue(i);
		const double dblValHigh = pSeriesHigh->GetDataPointValue(i);
	...
	double dblValue = dblMax != dblMin
						? (pSeriesClose->GetDataPointValue(nIndex) - dblMin) / (dblMax - dblMin)
						: 1.0;

	if (m_nSlowing > 1)
	{
		nActualStart = nIndex - m_nSlowing + 1;
		if (nActualStart < m_nKPeriod)
		{
			return CBCGPChartValue();
		}

		for (i = nActualStart; i < nIndex; i++)
		{
			dblMin = pLowBuffer->GetAt(i);
			dblMax = pHighBuffer->GetAt(i);

			dblValue += dblMax != dblMin
							? (pSeriesClose->GetDataPointValue(i) - dblMin) / (dblMax - dblMin)
							: 1.0;
		}

		dblValue /= (double)m_nSlowing;
	}

	return dblValue * 100.0;
}
//*******************************************************************************
...
{
	double dblVal = pInputSeries->GetDataPointValue(nIndex, CBCGPChartData::CI_Y);
														  ...
														  int nDataPointIndex)
{
	...
	{
		CBCGPChartData dataNewFromCallback = (*m_pfnTransitionCallback)(data, nDataPointIndex, m_pParentSeries, m_lParam);
		pDP->SetData(dataNewFromCallback);
		return pDP;
	}
	...
	for (i = 0; i < nCount; i++)
	{
		...
		if (!data.IsEmpty() || tn == CBCGPChartSeries::TN_VALUE)
		{
			...
			if (bCalculated)
			{
				if (errValue.IsEmpty())
				{
					dValue = arValues.GetSize() > 0 ? arValues[i] : (double)data.GetValue(CBCGPChartData::CI_Y);
				}
