											  ...
											  CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	BOOL bCenter = pFormatSeries->m_dataLabelFormat.m_options.m_position == BCGPChartDataLabelOptions::LP_CENTER || 
					...
					(category == BCGPChartColumn3D || category == BCGPChartBar3D || category == BCGPChartBar3DSmart || category == BCGPChartArea3D);
												...
												const BCGPChartCellParams& params, CBCGPChartSeries* pSeries)
{
	...
	else
	{
			...
			!m_pRelatedChart->IsChart3D())
		{
			...
			double dblMarkerSize = max(szMarker.cx, szMarker.cy);
			double dblMarkerLineWidth = params.m_markerLineStyle.GetLineWidth(TRUE);
			
			if (dblMarkerSize + dblMarkerLineWidth >= rectLegendKey.Height())
			{
				szMarker.SetSize(rectLegendKey.Height() - dblMarkerLineWidth, rectLegendKey.Height() - dblMarkerLineWidth);
			}
	...
	for (int j = 0; j < arStartIndexes.GetSize(); j++)
	{
		...
		for (int i = nMinDataPointIndex; i <= nMaxDataPointIndex; i++)
		{
				...
				curveType == BCGPChartFormatSeries::CCT_REVERSED_STEP) && i != nMaxDataPointIndex)
			{
				...
				CBCGPPoint ptNextPoint = pSeries->GetDataPointScreenPoint(k, 0);
				if (curveType == BCGPChartFormatSeries::CCT_STEP)
				{
						...
						CBCGPChartSurfaceTriangleID& triangleID, BOOL bIsGDI, BOOL bFill)
{
	...
	{
		for (i = 0; i < (int)triangle.m_arLevels.GetSize(); i++)
		{
									...
									CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	const double nAllBarWidth = dblBarWidth * nVisibleSeriesCount;

	if (pBarSeries->IsCustomOffset())
	{
		const double dblBarOffset = dblXUnitSize / 2 * pBarSeries->GetCustomOffsetPercent() / 100. - dblBarWidth / 2;
	...
	{
		const BOOL bReverseOrder = _IsSeriesReversedOrder(pSeries, nVisibleSeriesCount);
		const int nIndex = pXAxis->IsVertical() && (nVisibleSeriesCount > 1) && !bIsRange && 
					((!m_pRelatedChart->IsChart3D() && bReverseOrder) || (!bReverseOrder && m_pRelatedChart->IsChart3D()))
						? nVisibleSeriesCount - nSeriesIndex - 1
						: nSeriesIndex;

		const double dblBarOffset = dblBarWidth * (nIndex) - nAllBarWidth / 2;
...
//****************************************************************************************
BOOL CBCGPBarChartImpl::IsSeriesReversedOrder(const CBCGPChartSeries* pSeries) const
{
	return CBCGPBarChartImpl::_IsSeriesReversedOrder(pSeries, -1);
}
//****************************************************************************************
BOOL CBCGPBarChartImpl::_IsSeriesReversedOrder(const CBCGPChartSeries* pSeries, int nVisibleSeriesCount) const
{
	ASSERT_VALID(this);
	ASSERT_VALID(m_pRelatedChart);
	ASSERT_VALID(pSeries);
	...
	BCGPChartCategory category = m_pRelatedChart->GetChartCategory();
	if (category != BCGPChartBarSmart && category != BCGPChartBar3DSmart)
	{
		return FALSE;
	}
	...
	const BOOL bIsRange = pBarSeries->IsRangeSeries();
	const BOOL bIsGrouped = m_pRelatedChart->IsChart3DGrouped();

	if (!bIsGrouped ||
		((pBarSeries->IsStakedSeries() || bIsRange) && pBarSeries->GetColumnOverlapPercent() == 100))
	{
		return FALSE;
	}
	...
	if (nVisibleSeriesCount == -1)
	{
		nVisibleSeriesCount = pBarSeries->GetSeriesCountOnAxis();

		if (nVisibleSeriesCount == 0)
		{
	...
	return !pXAxis->m_bReverseOrder && (nVisibleSeriesCount > 1 || (pBarSeries->IsCustomOffset() && nVisibleSeriesCount == 1)) && !bIsRange;
}
...
void CBCGPPyramidChartImpl::OnDrawDiagram(CBCGPGraphicsManager* pGM, const CBCGPRect& rectDiagramArea, CBCGPChartSeries* pSeries)
{
	...
	const CBCGPChartPyramidSeries::DataPointsOrder order = pPyramidSeries->GetDataPointsOrder();

	if (pPyramidSeries->IsDataPointsSortingEnabled())
	{
		CArray<CBCGPChartDataPoint*, CBCGPChartDataPoint*> arSorted;
		pPyramidSeries->SortDataPoints(PYRAMID_SP_HEIGHT, arSorted);

		const int nCount = (int)arSorted.GetSize() - 1;

		for (int i = 0; i <= nCount; i++)
		{
			const int nIndex = order == CBCGPChartPyramidSeries::DPO_NORMAL ? nCount - i : i;

			OnDrawPyramidDataPoint(pGM, pPyramidSeries, pPyramidSeries->FindDataPointIndex(arSorted[nIndex]));
		}
	}
	else
	{
		const int nCount = pPyramidSeries->GetDataPointCount() - 1;

		for (int i = 0; i <= nCount; i++)
		{
			const int nIndex = order == CBCGPChartPyramidSeries::DPO_NORMAL ? nCount - i : i;

			if (pPyramidSeries->IsDataPointScreenPointsEmpty(nIndex))
			{
				continue;
			}

			OnDrawPyramidDataPoint(pGM, pPyramidSeries, nIndex);
		}
	}
}
//****************************************************************************************
																		...
																		CBCGPChartSeries* pSeries)
{
	...
	const CBCGPChartPyramidSeries::DataPointsOrder order = pFunnelSeries->GetDataPointsOrder();

	if (pFunnelSeries->IsDataPointsSortingEnabled())
	{
		CArray<CBCGPChartDataPoint*, CBCGPChartDataPoint*> arSorted;
		...
		for (int i = 0; i <= nCount; i++)
		{
			const int nIndex = order == CBCGPChartPyramidSeries::DPO_NORMAL ? i : nCount - i;

			OnDrawFunnelDataPoint(pGM, pFunnelSeries, pFunnelSeries->FindDataPointIndex(arSorted[nIndex]));
		}
	}
	else
	{
		const int nCount = pFunnelSeries->GetDataPointCount() - 1;

		for (int i = 0; i <= nCount; i++)
		{
			const int nIndex = order == CBCGPChartPyramidSeries::DPO_NORMAL ? i : nCount - i;

			if (pFunnelSeries->IsDataPointScreenPointsEmpty(nIndex))
			{
				continue;
			}

			OnDrawFunnelDataPoint(pGM, pFunnelSeries, nIndex);
		}
	}
}
//****************************************************************************************
		...
		CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	CBCGPChartStockSeries* pMainStockSeries = DYNAMIC_DOWNCAST(CBCGPChartStockSeries, pSeries);
	if (pMainStockSeries == NULL || !pMainStockSeries->IsMainStockSeries() ||
		pMainStockSeries->GetStockSeriesType() == CBCGPBaseChartStockSeries::SST_BAR || 
		pMainStockSeries->GetStockSeriesType() == CBCGPBaseChartStockSeries::SST_CANDLE)
	{
		return;
	}

	CBCGPLineChartImpl::OnCalcBoundingRect(pDataPoint, rectDiagramArea, pSeries, nDataPointIndex);
}
//****************************************************************************************
...
void CBCGPBoxPlotChartImpl::OnDrawDiagram(CBCGPGraphicsManager* pGM, const CBCGPRect& rectDiagramArea, CBCGPChartSeries* pSeries)
{
	...
	for (int i = pSeries->GetMinDataPointIndex(); i <= pSeries->GetMaxDataPointIndex(); i++)
	{
		...
		if (nWhiskersSizePercent > 0)
		{
			...
			for (int j = 0; j < 2; j++)
			{
				CBCGPPoint ptWhiskers1 = j == 0 ? ptWhiskersStart : ptWhiskersEnd;
				...
				m_pRelatedChart->OnDrawChartSeriesLine(pGM, ptWhiskers1, ptWhiskers2, pSeries, j);
			}
		}
