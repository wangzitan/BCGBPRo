...
void CBCGPChartLegendCell::OnDraw(CBCGPGraphicsManager* pGM, const CBCGPRect& rect)
{
	...
	switch (m_cellParams.m_textAlignment)
	{
	...
	case CBCGPTextFormat::BCGP_TEXT_ALIGNMENT_TRAILING:
		rectContent.left = rectContent.right - m_szContentSize.cx;
		break;
	}
...
//****************************************************************************************
void CBCGPChartLegendCell::SetTextAlignment(CBCGPTextFormat::BCGP_TEXT_ALIGNMENT textAlignment, BOOL bRedraw)
{
	m_cellParams.m_textAlignment = textAlignment;
...
//****************************************************************************************
CBCGPTextFormat::BCGP_TEXT_ALIGNMENT CBCGPChartLegendCell::GetTextAlignment() const
{
	return m_cellParams.m_textAlignment;
}
//****************************************************************************************
...
CBCGPSize CBCGPChartLegendVisualObject::CalcSize(CBCGPGraphicsManager* pGM)
{
	...
	if (pGM == NULL)
	{
		pGM = CBCGPGraphicsManager::CreateInstance();
	...
	if (IsVerticalLayout() && dblTotalWidth < m_szTitleSize.cx && m_arColumnWidths.GetSize() > 0)
	{
		...
		else if (dblTitleWidth > dblTotalWidth)
		{
...
//****************************************************************************************
HRESULT CBCGPChartLegendVisualObject::get_accName(VARIANT varChild, BSTR *pszName)
{
	if ((varChild.vt == VT_I4) && (varChild.lVal == CHILDID_SELF))
	{
		CString strName = m_strLegendTitle;
		if (strName.IsEmpty())
		{
			strName = _T("ChartLegend");
		}
