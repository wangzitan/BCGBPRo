...
void CBCGPChartLineObject::OnDrawShape(CBCGPGraphicsManager* pGM, const CBCGPRect& /*rectDiagram*/)
{
	...
	const double dblLineWidth = m_format.m_outlineFormat.GetLineWidth(TRUE);

	if (m_dblShadowDepth != 0.0 && pGM->IsSupported(BCGP_GRAPHICS_MANAGER_COLOR_OPACITY))
	{
		const double dblOffset = m_dblShadowDepth == DBL_MAX ? (dblLineWidth + 0.1) : m_dblShadowDepth;

		CBCGPRect rectShadow = m_rectScreen;
		rectShadow.OffsetRect(dblOffset, dblOffset);
		
		pGM->DrawLine(rectShadow.TopLeft(), rectShadow.BottomRight(), m_brShadow, dblLineWidth, &m_format.m_outlineFormat.m_strokeStyle);
	}

	pGM->DrawLine(m_rectScreen.TopLeft(), m_rectScreen.BottomRight(), brLine, dblLineWidth, &m_format.m_outlineFormat.m_strokeStyle);
}
//*******************************************************************************
...
void CBCGPChartInterLineColoringEffect::OnDraw(CBCGPGraphicsManager* pGM)
{
	...
	{
		BCGPSeriesColorsPtr colorsSeries;
		m_pSeries2->GetColors(colorsSeries, -1);
		...
		if (pDrawGeometry != NULL)
		{
			...
			if (pClipGeometry != NULL)
			{
				CBCGPBrush& brFillSeries = m_brBottomBrush.IsEmpty() ? *colorsSeries.m_pBrElementFillColor : m_brBottomBrush;
				DrawEffect(pGM, pDrawGeometry, pClipGeometry, rectBounds, brFillSeries);
