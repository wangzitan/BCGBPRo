...
BOOL CBCGPChartSeries::CanBeConvertedToCategory(BCGPChartCategory chartCategory) const
{
	if (m_chartCategory != chartCategory)
	{
		if (IsOptimizedLongDataMode() && 
			...
			chartCategory != BCGPChartBar && chartCategory != BCGPChartBarSmart && 
			chartCategory != BCGPChartHistogram && chartCategory != BCGPChartLine))
		{
			return FALSE;
		}
									...
									BOOL bAdjustLayout, BOOL bRedraw)
{
	...
	if (m_chartCategory == BCGPChartBar && chartCategory == BCGPChartColumn ||
		m_chartCategory == BCGPChartBarSmart && chartCategory == BCGPChartColumn ||
		m_chartCategory == BCGPChartColumn && chartCategory == BCGPChartBar ||
		m_chartCategory == BCGPChartColumn && chartCategory == BCGPChartBarSmart ||
		m_chartCategory == BCGPChartBar3D && chartCategory == BCGPChartColumn3D ||
		m_chartCategory == BCGPChartBar3DSmart && chartCategory == BCGPChartColumn3D ||
		m_chartCategory == BCGPChartColumn3D && chartCategory == BCGPChartBar3D ||
		m_chartCategory == BCGPChartColumn3D && chartCategory == BCGPChartBar3DSmart)
	{
		...
		if (m_pChart != NULL)
		{
			...
			if (pAxis != NULL && (pAxis->IsVertical() && (chartCategory != BCGPChartBar && chartCategory != BCGPChartBar3D && chartCategory != BCGPChartBarSmart && chartCategory != BCGPChartBar3DSmart) ||
				!pAxis->IsVertical() && (chartCategory == BCGPChartBar || chartCategory == BCGPChartBar3D || chartCategory == BCGPChartBarSmart || chartCategory == BCGPChartBar3DSmart)))
			{
	...
	if (chartCategory == BCGPChartBar || m_chartCategory == BCGPChartBar ||
		...
		chartCategory == BCGPChartBarSmart || m_chartCategory == BCGPChartBarSmart ||
		chartCategory == BCGPChartBar3DSmart || m_chartCategory == BCGPChartBar3DSmart)
	{
		if (m_pChart != NULL)
		{
			CBCGPChartAxis* pAxis = GetRelatedAxis(CBCGPChartSeries::AI_X);
			...
			if (pAxis != NULL)
			{
				BOOL bSetVert = chartCategory == BCGPChartBar || chartCategory == BCGPChartBar3D ||
								chartCategory == BCGPChartBarSmart || chartCategory == BCGPChartBar3DSmart;
...
void CBCGPChartSeries::SetMinValue(double dblValue, CBCGPChartData::ComponentIndex ci)
{
	...
	{
		dblValue = min(dblValue, (double)m_pErrorSeries->GetMinValue(ci));
	}
...
void CBCGPChartSeries::SetMaxValue(double dblValue, CBCGPChartData::ComponentIndex ci)
{
	...
	if (HasErrorBars() && ci == CBCGPChartData::CI_Y)
	{
		dblValue = max(dblValue, (double)m_pErrorSeries->GetMaxValue(ci));
	}
...
void CBCGPChartSeries::SetMinMaxValuesSimple(double dblValue, CBCGPChartData::ComponentIndex ci, int nDataPointIndex)
{
	...
	minValue.IsEmpty() ? SetMinValue(dblValue, ci) : SetMinValue(min ((double)minValue, dblValue), ci);
	maxValue.IsEmpty() ? SetMaxValue(dblValue, ci) : SetMaxValue(max ((double)maxValue, dblValue), ci);
...
void CBCGPChartSeries::OnCalcScreenPoints100Stacked(CBCGPGraphicsManager* pGM, const CBCGPRect& /*rectDiagramArea*/)
{
	...
	for (int i = 0; i < GetDataPointCount(); i++)
	{
		...
		dblCalculatedVal = bcg_clamp(dblCalculatedVal, -100., 100.0);
...
int CBCGPChartSeries::AddDataPoint(CBCGPChartDataPoint* pDataPoint)
{
	...
	if (m_bHistoryMode)
	{
		...
		if (bDeleteDP)
		{
			CBCGPChartDataPoint* pFirstDataPoint = m_arDataPoints.GetAt(0);
			if (pFirstDataPoint != NULL)
			{
				delete pFirstDataPoint;
			}
...
BOOL CBCGPChartSeries::OnGetDataPointTooltip(int nDataPointIndex, CString& strTooltip, CString& strTooltipDescr)
{
	...
	if (m_chartType == BCGP_CT_RANGE || m_chartCategory == BCGPChartBubble)
	{
		...
		strTooltipDescr.Format(strValueFormat, (LPCTSTR)strLabelX, (LPCTSTR)strLabelY, (LPCTSTR)strLabelY1);
	}
	else if (m_chartCategory == BCGPChartSurface3D)
	{
		CBCGPChartValue valZ = GetDataPointValue(nDataPointIndex, CBCGPChartData::CI_Z);
		if (valZ.IsEmpty())
		{
			return TRUE;
		}

		CBCGPChartAxis* pAxisZ = GetRelatedAxis(CBCGPChartSeries::AI_Z);
		ASSERT_VALID(pAxisZ);

		CString strLabelZ;
		pAxisZ->GetDisplayedLabel(valZ.GetValue(), strLabelZ);

		strTooltipDescr.Format(IDS_BCGBARRES_CHART_TOOLTIP_VALUE_FORMAT_XYZ, (LPCTSTR)strLabelX, (LPCTSTR)strLabelY, (LPCTSTR)strLabelZ);
	}
	else
	{
		strValueFormat.LoadString(IDS_BCGBARRES_CHART_TOOLTIP_VALUE_FORMAT);
		strTooltipDescr.Format(strValueFormat, (LPCTSTR)strLabelX, (LPCTSTR)strLabelY);
	}
...
void CBCGPChartSurfaceSeries::SetLevelRangeMode(LevelRangeMode rangeMode, double dblMinCustomRangeValue, double dblMaxCustomRangeValue)
{
	...
	{
		m_dblMinCustomLevelRangeVal = min ((double)GetMinValue(CBCGPChartData::CI_Y), dblMinCustomRangeValue);
		m_dblMaxCustomLevelRangeVal = max ((double)GetMaxValue(CBCGPChartData::CI_Y), dblMaxCustomRangeValue);
	}
}
//****************************************************************************************
...
void CBCGPChartSurfaceSeries::LevelIndexToString(int nLevelIndex, CString& strValues)
{
	...
	{
		CString strFormat;
		strFormat.Format(_T("%s - %s"), (LPCTSTR)m_strLegendLevelValueFormat, (LPCTSTR)m_strLegendLevelValueFormat);
...
//****************************************************************************************
BOOL CBCGPChartSurfaceSeries::HitTestDataPoint(const CBCGPPoint& pt, BCGPChartHitInfo* pHitInfo)
{
	if (m_pChart == NULL || m_pChartImpl == NULL || !m_bVisible)
	{
		return FALSE;
	}
	...
	CBCGPSurfaceChart3DImpl* pChartImpl = m_pChart->FindSurfaceChart3DImpl();
	if (pChartImpl == NULL || (int)pChartImpl->GetTriangleIDArray().GetSize() == 0)
	{
		return FALSE;
	}

	CArray<int, int> arIndexes;

	for (int i = 0; i < (int)m_arSurfaceTriangles.GetSize(); i++)
	{
		const CBCGPChartSurfaceTriangle& triangle = m_arSurfaceTriangles.GetData()[i];
		if (triangle.m_bIsEmpty)
		{
			continue;
		}

		if (bcg_pointInPolygon2((CBCGPPoint*)triangle.m_arPointsTransformed, 3, pt))
		{
			arIndexes.Add(i);
		}
	}

	const int nCountIndexes = (int)arIndexes.GetSize();
	if (nCountIndexes == 0)
	{
		return FALSE;
	}
	...
	if (nCountIndexes > 1)
	{
		for (int i = 0; i < nCountIndexes; i++)
		{
			for (int j = i + 1; j < nCountIndexes; j++)
			{
				const CBCGPChartSurfaceTriangle& triangle1 = m_arSurfaceTriangles.GetData()[arIndexes[i]];
				const CBCGPChartSurfaceTriangle& triangle2 = m_arSurfaceTriangles.GetData()[arIndexes[j]];

				if (triangle2.m_dblMinZRotated < triangle1.m_dblMinZRotated)
				{
					int tmp = arIndexes[j];
					arIndexes[j] = arIndexes[i];
					arIndexes[i] = tmp;
				}
			}
		}
	...
	pHitInfo->m_hitInfo = BCGPChartHitInfo::HIT_DATA_POINT;
	pHitInfo->m_nIndex2 = m_arSurfaceTriangles.GetData()[arIndexes[0]].m_nDataPointIndex;
	pHitInfo->m_nIndex3 = arIndexes[0];
...
void CBCGPChartLongSeries::OnCalcScreenPoints(CBCGPGraphicsManager* pGM, const CBCGPRect& rectDiagramArea)
{
	...
	for (int i = m_nMinDisplayedXIndex; i <= m_nMaxDisplayedXIndex; i++)
	{
		...
		else
		{
			...
			{
				valMaxPrev = bIsVertical ? max(pt.x, (double)valMaxPrev) : max(pt.y, (double)valMaxPrev);
				valMinPrev = bIsVertical ? min(pt.x, (double)valMinPrev) : min(pt.y, (double)valMinPrev);
			}
		}
...
void CBCGPChartBubbleSeries::OnCalcScreenPoints(CBCGPGraphicsManager* /*pGM*/, const CBCGPRect& /*rectDiagramArea*/)
{
	...
	double dblMaxSize = valMaxY1.IsEmpty() ? 1. : (double)valMaxY1;
...
//****************************************************************************************
BOOL CBCGPChartPercentSeries::OnGetDataPointTooltip(int nDataPointIndex, CString& strTooltip, CString& strTooltipDescr)
{
	CBCGPChartAxis* pAxisY = GetRelatedAxis(CBCGPChartSeries::AI_Y);
	CBCGPChartAxis* pAxisX = GetRelatedAxis(CBCGPChartSeries::AI_X);
	...
	strTooltipDescr.Format(_T("%d: %s (%3.1f%%)"), nDataPointIndex + 1, (LPCTSTR)strLabelY, (double)valX);
...
void CBCGPChartPieSeries::OnCalcScreenPoints(CBCGPGraphicsManager* pGM, const CBCGPRect& /*rectDiagramArea*/)
{
	...
	for (i = 0; i < GetDataPointCount(); i++)
	{
		...
		dblAngle = (fabs(dblPieArea) < DBL_EPSILON) ? 0. : (2.0 * M_PI * dblDPArea / dblPieArea);
		if (IsAnimated() && 
...
void CBCGPChartPyramidSeries::CommonInit()
{
	...
	m_bIsDataPointsSortEnabled = TRUE;
	m_DataPointsOrder = DPO_NORMAL;
...
//****************************************************************************************
void CBCGPChartPyramidSeries::EnableDataPointsSorting(BOOL bIsDataPointsSortEnabled)
{
	if (m_bIsDataPointsSortEnabled == bIsDataPointsSortEnabled)
	{
		return;
	}

	m_bIsDataPointsSortEnabled = bIsDataPointsSortEnabled;
}
//****************************************************************************************
void CBCGPChartPyramidSeries::SetDataPointsOrder(DataPointsOrder DPOrder)
{
	if (m_DataPointsOrder == DPOrder)
	{
		return;
	}

	m_DataPointsOrder = DPOrder;
}
//****************************************************************************************
...
void CBCGPChartPyramidSeries::OnCalcScreenPoints(CBCGPGraphicsManager* pGM, const CBCGPRect& /*rectDiagramArea*/)
{
	...
	const DataPointsOrder order = GetDataPointsOrder();
	double dblCurrOffset = 0;

	if (IsDataPointsSortingEnabled())
	{
		...
		const int nCount = (int)arDPSorted.GetSize() - 1;

		for (int nIndex = 0; nIndex <= nCount; nIndex++)
		{
			const int nDataPointIndex = FindDataPointIndex(arDPSorted[order == DPO_NORMAL ? nIndex : nCount - nIndex]);
			const double dblHeight = GetDataPointScreenPoint(nDataPointIndex, CBCGPPyramidChartImpl::PYRAMID_SP_HEIGHT).x;
	...
	{
		const int nCount = GetDataPointCount() - 1;

		for (int nIndex = 0; nIndex <= nCount; nIndex++)
		{
			const int nDataPointIndex = order == DPO_NORMAL ? nIndex : nCount - nIndex;
			if (IsDataPointScreenPointsEmpty(nDataPointIndex))
			{
				continue;
			}
...
void CBCGPChartFunnelSeries::OnCalcScreenPoints(CBCGPGraphicsManager* pGM, const CBCGPRect& /*rectDiagramArea*/)
{
	...
	if (IsDataPointsSortingEnabled())
	{
		...
		for (int nIndex = 0; nIndex <= nCount; nIndex++)
		{
			const int nDataPointIndex = FindDataPointIndex(arDPSorted[order == DPO_NORMAL ? nCount - nIndex : nIndex]);
			const double dblHeight = GetDataPointScreenPoint(nDataPointIndex, CBCGPPyramidChartImpl::PYRAMID_SP_HEIGHT).x;
			...
			{
				const double dblValue = fabs(GetDataPointValue(nDataPointIndex, CBCGPChartData::CI_Y).GetValue());
				if (dblValue <= m_dblNeckHeight)
				{
	...
	else
	{
		...
		for (int nIndex = 0; nIndex <= nCount; nIndex++)
		{
			const int nDataPointIndex = order == DPO_NORMAL ? nCount - nIndex : nIndex;
			if (IsDataPointScreenPointsEmpty(nDataPointIndex))
			{
				continue;
			}
...
BOOL CBCGPChartStockSeries::OnGetDataPointTooltip(int nDataPointIndex, CString& strTooltip, CString& strTooltipDescr)
{
	...
	strTooltipDescr.Format(strFormat, (LPCTSTR)strTime, dblOpen, dblHigh, dblLow, dblClose);
...
void CBCGPChartBarSeries::SetRelatedAxis(CBCGPChartAxis* pAxis, CBCGPChartSeries::AxisIndex axisIndex)
{
	...
	if (axisIndex == CBCGPChartSeries::AI_X)
	{
		pAxis->SetVertical(m_chartCategory == BCGPChartBar || m_chartCategory == BCGPChartBar3D || m_chartCategory == BCGPChartBarSmart || m_chartCategory == BCGPChartBar3DSmart);
	}
...
void CBCGPChartTernarySeries::SetMinMaxPercentage(double dblValue, CBCGPChartData::ComponentIndex ci)
{
	...
	minValue.IsEmpty() ? m_minPercentage.SetValue(dblValue, ci) : m_minPercentage.SetValue(min((double)minValue, dblValue), ci);
	maxValue.IsEmpty() ? m_maxPercentage.SetValue(dblValue, ci) : m_maxPercentage.SetValue(max((double)maxValue, dblValue), ci);
}
//****************************************************************************************
...
BOOL CBCGPChartTernarySeries::OnGetDataPointTooltip(int nDataPointIndex, CString& strTooltip, CString& strTooltipDescr)
{
	...
	strTooltipDescr.Format(strFormatDescr, (LPCTSTR)pAxisA->m_strAxisName, (LPCTSTR)strA,
		(LPCTSTR)pAxisB->m_strAxisName, (LPCTSTR)strB,
		(LPCTSTR)pAxisC->m_strAxisName, (LPCTSTR)strC);
...
//****************************************************************************************
CBCGPChartBoxPlotData CBCGPChartBoxPlotSeries::GetBoxPlotDataAt(int nDataPointIndex) const
{
	CBCGPChartBoxPlotData data;

	CBCGPChartValue value = GetDataPointValue(nDataPointIndex, CBCGPChartData::CI_Y);
	if (!value.IsEmpty())
	{
		data.m_bIsEmpty = FALSE;

		CBCGPChartValue value1;

		data.m_dblQ2 = value.GetValue();	

		value = GetChildSeries(CHART_BOXPLOT_SERIES_QUARTILE_IDX)->GetDataPointValue(nDataPointIndex, CBCGPChartData::CI_Y);
		value1 = GetChildSeries(CHART_BOXPLOT_SERIES_QUARTILE_IDX)->GetDataPointValue(nDataPointIndex, CBCGPChartData::CI_Y1);

		data.m_dblQ1 = value.GetValue();
		data.m_dblQ3 = data.m_dblQ1 + value1.GetValue();
		
		value = GetChildSeries(CHART_BOXPLOT_SERIES_WHISKERS_IDX)->GetDataPointValue(nDataPointIndex, CBCGPChartData::CI_Y);
		value1 = GetChildSeries(CHART_BOXPLOT_SERIES_WHISKERS_IDX)->GetDataPointValue(nDataPointIndex, CBCGPChartData::CI_Y1);

		data.m_dblMin = value.GetValue();
		data.m_dblMax = data.m_dblMin + value1.GetValue();

		value = GetChildSeries(CHART_BOXPLOT_SERIES_AVERAGE_IDX)->GetDataPointValue(nDataPointIndex, CBCGPChartData::CI_Y);
		data.m_dblAverage = value.IsEmpty() ? DBL_MAX : value.GetValue();

		value = GetChildSeries(CHART_BOXPLOT_SERIES_NOTCH_IDX)->GetDataPointValue(nDataPointIndex, CBCGPChartData::CI_Y);
		data.m_dblNotch = value.IsEmpty() ? DBL_MAX : data.m_dblQ2 - value.GetValue();
	}
...
void CBCGPChartErrorBarsSeries::CalculateScreenPoints(int nDataPointIndex, const CBCGPChartData& data, CBCGPChartDataPoint& dp) const
{
	...
	if (!valY1.IsEmpty() && (m_drawFlags & Draw_Plus))
	{
		...
		CBCGPChartData data1(valX, valY1);
		CBCGPPoint point(m_pChart->ScreenPointFromChartData(data1, nDataPointIndex, bIsEmpty, m_pOwnerSeries));
	...
	if (!valY2.IsEmpty() && (m_drawFlags & Draw_Minus))
	{
		...
		CBCGPChartData data1(valX, valY2);
		CBCGPPoint point(m_pChart->ScreenPointFromChartData(data1, nDataPointIndex, bIsEmpty, m_pOwnerSeries));
