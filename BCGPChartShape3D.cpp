...
void CBCGPChartShape3D::PrepareSidePoints(ShapeVertexType svt)
{
	...
	for (int i = 0; i < GetSideCount(); i++)
	{
		...
		for (int j = 0; j < pArThis->GetSize(); j++)
		{
			float x = (float)((*pArThis)[j].x * CBCGPChartSide3D::COLLISION_COEF);
			float y = (float)((*pArThis)[j].y * CBCGPChartSide3D::COLLISION_COEF);
			(*pArThis)[j] = CBCGPPoint(x, y);
		}
