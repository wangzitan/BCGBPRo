...
//****************************************************************************************
void CBCGPChartVisualObject::SetTernaryAxesLabelMode(CBCGPChartTernaryAxis::LabelMode lm, BOOL bRedraw)
{
...
//****************************************************************************************
void CBCGPChartVisualObject::SetTernaryAxesTitleMode(CBCGPChartTernaryAxis::TitleMode tm, BOOL bRedraw)
{
	for (int i = BCGP_CHART_A_TERNARY_AXIS; i <= BCGP_CHART_C_TERNARY_AXIS; i++)
	{
		CBCGPChartTernaryAxis* pAxis = DYNAMIC_DOWNCAST(CBCGPChartTernaryAxis, GetChartAxis(i));
...
{
	return (category == BCGPChartColumn3D || category == BCGPChartBar3D || category == BCGPChartBar3DSmart || 
			category == BCGPChartLine3D || category == BCGPChartArea3D || category == BCGPChartSurface3D);
}
//****************************************************************************************
...
void CBCGPChartVisualObject::AdjustLayout(CBCGPGraphicsManager* pGM)
{
	...
	if (!bCustomTitleRect)
	{
		...
		rectTitleArea.SetRect(rectChart.left + rectChart.Width() / 2 - szTitle.cx / 2, 
			rectChart.top + szTitleAreaPadding.cy, 
			rectChart.left + rectChart.Width() / 2 + szTitle.cx / 2,
			rectChart.top + szTitleAreaPadding.cy + szTitle.cy);
	...
	if (!bCustomPlotAreaRect)
	{
		...
		if (m_chartLayout.m_bShowChartTitle && !bCustomTitleRect && !m_chartLayout.m_bTitleOverlapsChart && 
			(!m_bIsThumbnailMode || (m_nThumbnailFlags & BCGP_CHART_THUMBNAIL_DRAW_TITLE) == BCGP_CHART_THUMBNAIL_DRAW_TITLE))
		{
	...
	if (pXAxis->m_axisLabelType != CBCGPChartAxis::ALT_NO_LABELS)
	{
		if (pXAxis->IsVertical())
		{
			...
			if (bHasDataTable)
			{
				const double dblMaxLabelWidth = max(pXAxis->m_szMaxLabelSize.cx, pXSecondaryAxis->m_szMaxLabelSize.cx) + pXAxis->m_dblLabelDistance + pXAxis->GetMajorTickMarkLen(FALSE);

				if (m_dblDataTableHeaderColumnWidth < dblMaxLabelWidth)
				{
					m_rectDiagramArea.left += dblMaxLabelWidth - m_dblDataTableHeaderColumnWidth;
					m_dblDataTableHeaderColumnWidth = dblMaxLabelWidth;
				}
			}
		}
	...
	if (pYAxis->m_axisLabelType != CBCGPChartAxis::ALT_NO_LABELS)
	{
		if (pYAxis->IsVertical())
		{
			...
			if (bHasDataTable)
			{
				const double dblMaxLabelWidth = max(pYAxis->m_szMaxLabelSize.cx, pYSecondaryAxis->m_szMaxLabelSize.cx) + pYAxis->m_dblLabelDistance + pYAxis->GetMajorTickMarkLen(FALSE);
...
{
	if (m_bIsThumbnailMode && (m_nThumbnailFlags & BCGP_CHART_THUMBNAIL_DRAW_TITLE) == 0)
	{
		return CBCGPSize();
	}
	...
	for (int i = 0; i < m_arAxes.GetSize(); i++)
	{
		...
		if ((rectBounds.PtInRect(pt) || bDiagram3D) && (m_bEnableMagnifier || m_bEnableZoom || m_bEnablePan))
		{
			...
			if ((hitInfo.m_hitInfo & BCGPChartHitInfo::HIT_AXIS_THUMB) != 0 && !bDiagram3D)
			{
				...
				{
					CBCGPChartAxis* pAxisCurr = GetChartAxis(hitInfo.m_nIndex1);

					if (pAxisCurr != NULL && pAxisCurr->IsThumbGripEnabled())
					{
						UINT idCursor = pAxisCurr->IsVertical() ? AFX_IDC_TRACKNS : AFX_IDC_TRACKWE;
		...
		else if (IsResizeAxesEnabled())
		{
			...
			if ((hitInfo.m_hitInfo & BCGPChartHitInfo::HIT_AXIS_RESIZE_BAND) != 0)
			{
				...
				if (pAxisCurr != NULL)
				{
					ASSERT_VALID(pAxisCurr);

					UINT nID = pAxisCurr->IsVertical() ? AFX_IDC_VSPLITBAR : AFX_IDC_HSPLITBAR;
...
BOOL CBCGPChartVisualObject::OnGetToolTip(const CBCGPPoint& pt, CString& strToolTip, CString& strDescr)
{
	...
	if (hi.m_hitInfo == BCGPChartHitInfo::HIT_AXIS)
	{
		...
		strToolTip.Format(strAxisNameFormat, (LPCTSTR)pAxis->m_strAxisName);
		...
		strDescr.Format(strAxisDescrFormat, (LPCTSTR)strLabel);
	...
	else if (hi.m_hitInfo == BCGPChartHitInfo::HIT_AXIS_SCALE_BREAK)
	{
		...
		strDescr.Format(_T("%s - %s"), (LPCTSTR)strLowValue, (LPCTSTR)strHighValue);
	}
...
//****************************************************************************************
CBCGPSurfaceChart3DImpl* CBCGPChartVisualObject::FindSurfaceChart3DImpl()
{
	CBCGPSurfaceChart3DImpl* pFirstImpl = NULL;
	CBCGPSurfaceChart3DImpl* pSurfaceImpl = NULL;
	...
	for (int i = 0; i < m_arData.GetSize(); i++)
	{
		...
		CBCGPSurfaceChart3DImpl* pImpl = DYNAMIC_DOWNCAST(CBCGPSurfaceChart3DImpl, pSeries->GetChartImpl());
		if (pImpl == NULL)
		{
			continue;
		}
		...
		if ((int)pImpl->GetTriangleIDArray().GetSize() > 0)
		{
			pSurfaceImpl = pImpl;
			break;
		}
	...
	if (pSurfaceImpl == NULL)
	{
		pSurfaceImpl = pFirstImpl;
	}
...
BOOL CBCGPChartVisualObject::HitTestInternal(const CBCGPPoint& pt, BCGPChartHitInfo* pHitInfo)
{
		 ...
		 (m_hitInfoFlags & BCGPChartHitInfo::HIT_DATA_LABEL) != 0))
	{
		...
		CArray<BCGPChartHitInfo, BCGPChartHitInfo> arDpSurface3D;
		...
		for (i = (int)m_arData.GetSize() - 1; i >= 0 ; i--)
		{
			...
			if ((m_hitInfoFlags & BCGPChartHitInfo::HIT_DATA_LABEL) != 0 && pSeries->GetChartCategory() != BCGPChartSurface3D)
			{
			...
			if ((m_hitInfoFlags & BCGPChartHitInfo::HIT_DATA_POINT) != 0)
			{
				if (pSeries->GetChartCategory() == BCGPChartSurface3D)
				{
					...
					{
						pHitInfo->m_nIndex1 = i;
						arDpSurface3D.Add(BCGPChartHitInfo(*pHitInfo));
					}
				}
				else if (!IsChart3D())
				{
		...
		const int nCountDpSurface3D = (int)arDpSurface3D.GetSize();
		if (nCountDpSurface3D > 0)
		{
			int index = 0;

			if (nCountDpSurface3D > 1)
			{
				CArray<int, int> arIndexes;
				arIndexes.SetSize(nCountDpSurface3D);
				for (i = 0; i < nCountDpSurface3D; i++)
				{
					arIndexes[i] = i;
				}
				...
				for (i = 0; i < nCountDpSurface3D; i++)
				{
					for (int j = i + 1; j < nCountDpSurface3D; j++)
					{
						const CBCGPChartSurfaceTriangleArray& arTriangles1 = ((CBCGPChartSurfaceSeries*)m_arData[arDpSurface3D[arIndexes[i]].m_nIndex1])->GetSurfaceTriangles();
						const CBCGPChartSurfaceTriangle& triangle1 = arTriangles1.GetData()[arDpSurface3D[arIndexes[i]].m_nIndex3];

						const CBCGPChartSurfaceTriangleArray& arTriangles2 = ((CBCGPChartSurfaceSeries*)m_arData[arDpSurface3D[arIndexes[j]].m_nIndex1])->GetSurfaceTriangles();
						const CBCGPChartSurfaceTriangle& triangle2 = arTriangles2.GetData()[arDpSurface3D[arIndexes[j]].m_nIndex3];
...
CBCGPSize CBCGPChartVisualObject::OnCalcAxisNameSize(CBCGPGraphicsManager* pGM, CBCGPChartAxis* pAxis)
{
	...
	if (pAxis == NULL || !pAxis->m_bDisplayAxisName || 
		(pAxis->IsThumbnailMode() && (m_nThumbnailFlags & BCGP_CHART_THUMBNAIL_DRAW_AXIS_NAME) == 0))
	{
		return sz;
	}
	...
	double dblWidth = 0.;
	if (pAxis->m_axisNameFormat.m_textFormat.IsWordWrap() && !pAxis->IsVertical())
	{
		dblWidth = m_rectPlotArea.Width();
	}

	sz = OnGetTextSize(pGM, pAxis->m_strAxisName, pAxis->m_axisNameFormat.m_textFormat, dblWidth);
...
//****************************************************************************************
BOOL CBCGPChartVisualObject::IsSeriesReversedOrder()
{
	ASSERT_VALID(this);

	if (m_Category != BCGPChartBarSmart && m_Category != BCGPChartBar3DSmart)
	{
		return FALSE;
	}

	const int nDataCount = (int)m_arData.GetSize();
	if (nDataCount < 2)
	{
		return FALSE;
	}
	...
	for (int i = 0; i < nDataCount; i++)
	{
		...
		CBCGPBarChartImpl* pImpl = DYNAMIC_DOWNCAST(CBCGPBarChartImpl, pSeries->GetChartImpl());
		if (pImpl != NULL)
		{
			bOrder = pImpl->IsSeriesReversedOrder(pSeries);
		}

		bReverseOrder = bReverseOrder && bOrder;
...
//****************************************************************************************
BOOL CBCGPChartVisualObject::IsLegendReversedOrder()
{
	BOOL bReverseOrder = IsSeriesReversedOrder();

	switch (m_LegendOrder)
	{
	case LO_REVERS:
		bReverseOrder = !bReverseOrder;
		break;
	case LO_CHARTTYPE:
		if ((m_Category == BCGPChartColumn || m_Category == BCGPChartColumn3D) && (m_Type == BCGP_CT_STACKED || m_Type == BCGP_CT_100STACKED))
		{
			CBCGPChartAxis* pAxis = GetChartAxis(BCGP_CHART_X_PRIMARY_AXIS);
...
CBCGPSize CBCGPChartVisualObject::CalcLegendSize(CBCGPGraphicsManager* pGM)
{
	...
	BOOL bReverseOrder = IsLegendReversedOrder();
	const int nDataCount = (int)m_arData.GetSize();

	for (i = 0; i < nDataCount; i++)
	{
		CBCGPChartSeries* pSeries = GetSeries(bReverseOrder ? nDataCount - i - 1 : i);
		if (pSeries == NULL || !pSeries->IsLegendEntryVisible())
		{
			continue;
		}
...
CBCGPSize CBCGPChartVisualObject::CalcTitleSize(CBCGPGraphicsManager* pGM)
{
	...
	if (!m_chartLayout.m_bShowChartTitle || pGM == NULL || 
		(m_bIsThumbnailMode && (m_nThumbnailFlags & BCGP_CHART_THUMBNAIL_DRAW_TITLE) == 0))
	{
		return szTitle;
	}
...
void CBCGPChartVisualObject::OnDraw(CBCGPGraphicsManager* pGMSrc, const CBCGPRect& /*rectClip*/, DWORD dwFlags)
{
	...
	if (!m_bIsThumbnailMode || (m_nThumbnailFlags & BCGP_CHART_THUMBNAIL_DRAW_TITLE) == BCGP_CHART_THUMBNAIL_DRAW_TITLE)
	{
...
void CBCGPChartVisualObject::ArrangeDataLabels(CBCGPGraphicsManager* pGM)
{
	...
	for (int iSeries = 0; iSeries < GetSeriesCount(); iSeries++)
	{
		...
		if ((category != BCGPChartColumn && category != BCGPChartBar && category != BCGPChartBarSmart && category != BCGPChartBoxPlot))
		{
			bOnlyColumns = FALSE;
			break;
		}
	...
	for (int nStep = 0; nStep < 2; nStep++)
	{
		for (int iSeries = 0; iSeries < GetSeriesCount(); iSeries++)
		{
			...
			if ((category == BCGPChartColumn || category == BCGPChartBar || category == BCGPChartBarSmart || category == BCGPChartBoxPlot) && nStep == 1)
			{
				continue;
			}
									  ...
									  BCGPChartFormatLabel& labelStyle)
{
	...
	if (!m_chartLayout.m_bShowChartTitle || 
		(m_bIsThumbnailMode && (m_nThumbnailFlags & BCGP_CHART_THUMBNAIL_DRAW_TITLE) == 0))
	{
		return;
	}
...
void CBCGPChartVisualObject::OnDrawAxisName(CBCGPGraphicsManager* pGM, CBCGPChartAxis* pAxis)
{
	...
	if (!NeedDisplayAxes() || !pAxis->IsAxisVisible() || !pAxis->m_bDisplayAxisName || 
		(pAxis->IsThumbnailMode() && (m_nThumbnailFlags & BCGP_CHART_THUMBNAIL_DRAW_AXIS_NAME) == 0))
	{
		return;
	}
...
void CBCGPChartVisualObject::OnDrawDiagram(CBCGPGraphicsManager* pGM, const CBCGPRect& rectDiagramArea)
{
	...
	const BOOL bReverseOrder = IsSeriesReversedOrder();
	const int nDataCount = (int)m_arData.GetSize();
	...
	if (IsChart3D())
	{
		for (i = 0; i < nDataCount; i++)
		{
			CBCGPChartSeries* pSeries = GetSeries(bReverseOrder ? nDataCount - i - 1 : i, FALSE);
			if (pSeries == NULL || !pSeries->m_bVisible)
			{
				continue;
			}
...
CBCGPChartSeries* CBCGPChartVisualObject::OnCreateChartSeries(BCGPChartCategory category, BCGPChartType type)
{
	...
	else if (category == BCGPChartBar || category == BCGPChartColumn || category == BCGPChartBarSmart || 
			category == BCGPChartColumn3D  || category == BCGPChartBar3D || category == BCGPChartBar3DSmart)
	{
...
void CBCGPChartVisualObject::ZoomByCurrentSelection(BOOL bZoomOut)
{
	...
	else
	{
		if (m_selectionFormat.m_selectionType == BCGPChartFormatSelection::ST_HORZ_AXIS_ONLY)
		{
			m_ptSelStart.y = m_rectDiagramArea.bottom;
			m_ptSelEnd.y = m_rectDiagramArea.top;
		}
		else if (m_selectionFormat.m_selectionType == BCGPChartFormatSelection::ST_VERT_AXIS_ONLY)
		{
			m_ptSelStart.x = m_rectDiagramArea.left;
			m_ptSelEnd.x = m_rectDiagramArea.right;
		}
	}
											...
											CBCGPChartSeries* pSeries, CBCGPChartData::ComponentIndex animationComponentIndex)
{
	...
	if (animationStyle == CBCGPChartSeries::BCGPChartAnimationStyle_None || IsImageCache())
	{
		return FALSE;
	}
