...
public:
	...
	virtual void CopyFrom(const CBCGPGaugeScaleObject& srcIn)
	{
		...
		m_bEquidistantLabels = src.m_bEquidistantLabels;
...
public:
	...
	void EnableEquidistantLabels(BOOL bEnable = TRUE, int nScale = 0);
	BOOL IsEquidistantLabels(int nScale = 0) const;
