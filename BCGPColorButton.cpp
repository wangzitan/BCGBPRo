...
void CBCGPColorButton::OnDraw (CDC* pDC, const CRect& rect, UINT uiState)
{
	...
	BOOL bIsReady = FALSE;
	BOOL bIsDisabled = (uiState & ODS_DISABLED) != 0;

	if (color != (COLORREF)-1 && !bIsDisabled)
	{
		...
		bIsReady = OnDrawPaletteColorBox(pDC, rectColor, color, m_Color == (COLORREF)-1, TRUE, FALSE);
	}

	if (!bIsReady)
	{
		COLORREF clrBorder = m_bVisualManagerStyle ? globalData.clrBarDkShadow : globalData.clrBtnDkShadow;
		if (bIsDisabled)
		{
...
//*****************************************************************************************
void CBCGPColorButton::SetWebColors()
{
	SetColors(CBCGPColor::GetRGBArray());
	SetColumnsNumber(BCGP_COLOR_COLUMNS);
}
//*****************************************************************************************
