...
class BCGCBPRODLLEXPORT CBCGPDropDownListBox :	public TBCGPInternalScrollBarWrapperWnd<CListBox>
{
	...
	friend class CBCGPScrollBarImpl;
...
class BCGCBPRODLLEXPORT CBCGPComboBox : public CComboBox
{
	...
	friend class CBCGPDropDownListBox;
...
public:
	...
	void SetItemGroupStart(int nIndex, BOOL bSet = TRUE);
	BOOL IsItemGroupStart(int nIndex) const;

	void SetVerticalPadding(int nPadding);
	int GetVerticalPadding() const;

	void SetHorizontalPadding(int nPadding);
...
// Overrides
	...
	virtual CSize GetItemSize(CDC* pDC, int nIndex);
	
	virtual CBCGPEdit& GetEditCtrl()
	{
		return m_wndEdit;
	}
...
protected:
	...
	void DoDrawGroupStart(CDC* pDC, CRect rect, int nIndex);
