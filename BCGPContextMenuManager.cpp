											 ...
											 CWnd* pWndOwner, BOOL bRightAlign)
{
	...
	HWND hWndOwner = m_bCheckForOwner ? pWndOwner->GetSafeHwnd() : NULL;
		...
		if (g_pWorkspace == NULL || !g_pWorkspace->OnBCGPIdle (pMenu))
		{
			...
			while (::IsWindow (hwndMenu))
			{
				...
				while (::PeekMessage (&msg, NULL, 0, 0, PM_REMOVE))
				{
					...
					if (m_bCancelTracking || (hWndOwner != NULL && !::IsWindow(hWndOwner)))
					{
						pMenu->DestroyWindow ();
...
//********************************************************************************
void CBCGPContextMenuManager::CancelTracking()
{
	ASSERT_VALID(this);	

	if (m_bIsTracking)
	{
		m_bCancelTracking = TRUE;
	}
}
//********************************************************************************
