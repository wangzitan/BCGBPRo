...
CBCGPControlRenderer::CBCGPControlRenderer()
	: m_bMirror     (FALSE)
	, m_bIsScaled   (FALSE)
	, m_dwFrameSides(BCGP_FRAME_SIDE_ALL)
{
}
//*********************************************************************************
CBCGPControlRenderer::~CBCGPControlRenderer()
...
BOOL CBCGPControlRenderer::Create (const CBCGPControlRendererParams& params, BOOL bFlipvert /*= FALSE*/)
{
	...
	if (lpszResID != NULL || hBitmap != NULL)
	{
		...
		{
			m_Bitmap.AdaptColors (m_Params.m_clrBase, m_Params.m_clrTarget);
		}
...
void CBCGPControlRenderer::DrawFrame (CDC* pDC, CRect rect, UINT index, BYTE alphaSrc/* = 255*/)
{
	...
	if (rt.Width () > 0 || rt.Height () > 0)
	{
		if (rt.Height () > 0)
		{
			if ((m_dwFrameSides & BCGP_FRAME_SIDE_LEFT) != 0 && rectSides.left > 0)
			{
				CRect r (rt);
				r.left  = rect.left;
			...
			if ((m_dwFrameSides & BCGP_FRAME_SIDE_RIGHT) != 0 && rectSides.right > 0)
			{
				CRect r (rt);
				r.right = rect.right;
		...
		if (rt.Width () > 0)
		{
			if ((m_dwFrameSides & BCGP_FRAME_SIDE_TOP) != 0 && rectSides.top > 0)
			{
				CRect r (rt);
				r.top    = rect.top;
			...
			if ((m_dwFrameSides & BCGP_FRAME_SIDE_BOTTOM) != 0 && rectSides.bottom > 0)
			{
				CRect r (rt);
		...
		if ((m_dwFrameSides & (BCGP_FRAME_SIDE_LEFT | BCGP_FRAME_SIDE_TOP)) != 0 && rectCorners.left > 0 && rectCorners.top > 0)
		{
			CRect rectPart;
			if (m_bMirror)
			{
		...
		if ((m_dwFrameSides & (BCGP_FRAME_SIDE_RIGHT | BCGP_FRAME_SIDE_TOP)) != 0 && rectCorners.right > 0 && rectCorners.top > 0)
		{
			CRect rectPart;
			if (m_bMirror)
			{
		...
		if ((m_dwFrameSides & (BCGP_FRAME_SIDE_LEFT | BCGP_FRAME_SIDE_BOTTOM)) != 0 && rectCorners.left > 0 && rectCorners.bottom > 0)
		{
			CRect rectPart;
			if (m_bMirror)
			{
		...
		if ((m_dwFrameSides & (BCGP_FRAME_SIDE_RIGHT | BCGP_FRAME_SIDE_BOTTOM)) != 0 && rectCorners.right > 0 && rectCorners.bottom > 0)
		{
			CRect rectPart;
			if (m_bMirror)
			{
...
BOOL CBCGPControlRenderer::CopyTo (CBCGPControlRenderer& dest)
{
	...
	if (m_Bitmap.CopyTo (dest.m_Bitmap))
	{
		...
		dest.m_dwFrameSides = m_dwFrameSides;
...
{
	SetFrameSides(BCGP_FRAME_SIDE_RIGHT | BCGP_FRAME_SIDE_BOTTOM);
}
//*********************************************************************************
CBCGPShadowRenderer::~CBCGPShadowRenderer()
{
...
void CBCGPShadowRenderer::Draw (CDC* pDC, CRect rect, UINT index/* = 0*/, BYTE alphaSrc/* = 255*/)
{
	if (128 <= alphaSrc)
	{
		const DWORD dwFrameSides = GetFrameSides();
		CRect rectCorners (m_Params.m_rectCorners);
		...
		if ((dwFrameSides & (BCGP_FRAME_SIDE_LEFT | BCGP_FRAME_SIDE_TOP)) == (BCGP_FRAME_SIDE_LEFT | BCGP_FRAME_SIDE_TOP))
		{
			CRect rectInter (rect);
			rectInter.top += m_Params.m_rectSides.top;
			rectInter.bottom = rectInter.top + m_Params.m_rectSides.top;

			rectInter.left += m_Params.m_rectSides.left;
			rectInter.right = rectInter.left + m_Params.m_rectSides.right;

			FillInterior (pDC, rectInter, ind, alphaSrc);
		}

		if ((dwFrameSides & (BCGP_FRAME_SIDE_RIGHT | BCGP_FRAME_SIDE_TOP)) == (BCGP_FRAME_SIDE_RIGHT | BCGP_FRAME_SIDE_TOP))
		{
			CRect rectInter (rect);
			rectInter.top += m_Params.m_rectSides.top;
			rectInter.bottom = rectInter.top + m_Params.m_rectSides.top;
		...
		if ((dwFrameSides & (BCGP_FRAME_SIDE_LEFT | BCGP_FRAME_SIDE_BOTTOM)) == (BCGP_FRAME_SIDE_LEFT | BCGP_FRAME_SIDE_BOTTOM))
		{
			CRect rectInter (rect);
			rectInter.bottom -= m_Params.m_rectSides.bottom;
		...
		if ((dwFrameSides & (BCGP_FRAME_SIDE_RIGHT | BCGP_FRAME_SIDE_BOTTOM)) == (BCGP_FRAME_SIDE_RIGHT | BCGP_FRAME_SIDE_BOTTOM))
		{
			CRect rectInter (rect);
			rectInter.bottom -= m_Params.m_rectSides.bottom;
			rectInter.top = rectInter.bottom - m_Params.m_rectSides.bottom;
...
{
	if (!CBCGPToolBarImages::IsRTL () || m_bMirror)
	{
		CBCGPControlRenderer::DrawFrame(pDC, rect, index, alphaSrc);
		return;
	}
	...
	if (rt.Width () > 0 || rt.Height () > 0)
	{
		if (rt.Height () > 0)
		{
			if ((dwFrameSides & BCGP_FRAME_SIDE_RIGHT) != 0 && rectSides.left > 0)
			{
				CRect r (rt);
				r.left  = rect.left;
				r.right = r.left + rectSides.left;
			...
			if ((dwFrameSides & BCGP_FRAME_SIDE_LEFT) != 0 && rectSides.right > 0)
			{
				CRect r (rt);
				r.right = rect.right;
				r.left  = r.right - rectSides.right;
		...
		if (rt.Width () > 0)
		{
			if ((dwFrameSides & BCGP_FRAME_SIDE_TOP) != 0 && rectSides.bottom > 0)
			{
				CRect r (rt);
				r.top    = rect.top;
				r.bottom = r.top + rectSides.top;

				CRect rectPart = CRect (rectImage.left + rectCorners.left, 
						rectImage.top, rectImage.right - rectCorners.right, rectImage.top + rectSides.top);

				m_Bitmap.DrawEx (pDC, r, ind, sides[2].horz, sides[2].vert, rectPart, alphaSrc);
			}

			if ((dwFrameSides & BCGP_FRAME_SIDE_BOTTOM) != 0 && rectSides.bottom > 0)
			{
				CRect r (rt);
				r.bottom = rect.bottom;
				r.top    = r.bottom - rectSides.bottom;
		...
		if ((dwFrameSides & (BCGP_FRAME_SIDE_LEFT | BCGP_FRAME_SIDE_TOP)) != 0 && rectCorners.right > 0 && rectCorners.top > 0)
		{
		...
		if ((dwFrameSides & (BCGP_FRAME_SIDE_RIGHT | BCGP_FRAME_SIDE_TOP)) != 0 && rectCorners.left > 0 && rectCorners.top > 0)
		{
		...
		if ((dwFrameSides & (BCGP_FRAME_SIDE_LEFT | BCGP_FRAME_SIDE_BOTTOM)) != 0 && rectCorners.right > 0 && rectCorners.bottom > 0)
		{
		...
		if ((dwFrameSides & (BCGP_FRAME_SIDE_RIGHT | BCGP_FRAME_SIDE_BOTTOM)) != 0 && rectCorners.left > 0 && rectCorners.bottom > 0)
		{
