...
BOOL CBCGPCustomizeMenuButton::OnClickMenuItem()
{
	...
	CBCGPPopupMenuBar* pMenuBar = DYNAMIC_DOWNCAST(CBCGPPopupMenuBar, m_pWndParent);
	if (pMenuBar == NULL)
	{
		return FALSE;
	}
	...
	{	
		...
		UINT nIndexNew = m_pWndToolBar->InsertButton(*pButton, m_uiIndex);
		
		if (nIndexNew == -1)
		{
			nIndexNew = m_pWndToolBar->InsertButton(*pButton);
		}
		else
		{
		...
		if (m_bAddSpr) 
		{
			if (nIndexNew < (UINT)m_pWndToolBar->GetCount ())
			{
				CBCGPToolbarButton* pBtn = m_pWndToolBar->GetButton (nIndexNew + 1);
