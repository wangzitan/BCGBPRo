...
CBCGPDateTimeCtrl::CBCGPDateTimeCtrl()
{
	...
	m_wndSpin.m_bVisualManagerStyle = FALSE;

	m_bDontChangeLocale = FALSE;

	m_bIsCalendarTodayButton = TRUE;
	m_bIsReadOnly = FALSE;

	m_rectSpin.SetRectEmpty();
...
{
	EnableVisualManagerStyle((BOOL)wp);
	return 0L;
}
//*****************************************************************************************
void CBCGPDateTimeCtrl::DrawItem(LPDRAWITEMSTRUCT lpDIS) 
{
	...
	BOOL bDrawDateTimeOnly = m_bDrawDateTimeOnly || m_bIsReadOnly;
	...
	if (!bDrawDateTimeOnly)
	{
		// Draw check box:
		DrawCheckBox (pDC, lpDIS->itemState);
	}

	if (!bDrawDateTimeOnly && m_bCheckBoxIsAvailable && !m_bIsChecked && !m_strEmptyDate.IsEmpty())
	{
	...
	else
	{
		...
		for (int i = iStart; i < m_iPartsNumber; i ++)
		{
			...
			if (m_bIsChecked && m_bShowSelection && !m_bIsReadOnly && i == m_iPartNum)	// Selected part
			{
				CRect rectFill = rect;
...
{
	if (!m_dropCalendar || m_bIsReadOnly)
		return;
	...
	m_pPopup->EnableTodayButton (m_bIsCalendarTodayButton);
...
void CBCGPDateTimeCtrl::OnLButtonDown(UINT /*nFlags*/, CPoint point) 
{
	...
	if (m_rectDropButton.PtInRect (point) && IsWindowEnabled() && !m_bIsReadOnly)
	{
...
//****************************************************************************************
DATE CBCGPDateTimeCtrl::GetDate() const
{
	if (m_bIsChecked)
	{
...
//****************************************************************************************
DATE CBCGPDateTimeCtrl::GetMaxDate() const
{
	return (DATE) m_MaxDate;
}
//****************************************************************************************
DATE CBCGPDateTimeCtrl::GetMinDate()  const
{
...
{
	if (m_monthFormat == -1)	// System, based on the short date format
	{
		m_monthFormat = 2;	// Numeric

		TCHAR szLocaleData [100];
		if (GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_SSHORTDATE, szLocaleData, 100) > 0)
		{
			CString strData = szLocaleData;
			strData.MakeUpper();
			
			if (strData.Find(_T("MMMM")) >= 0)
			{
				m_monthFormat = 1;
			}
			else if (strData.Find(_T("MMM")) >= 0)
			{
				m_monthFormat = 0;
			}
		}
	...
	{
		m_rectText.left += globalUtils.ScaleByDPI(1);
	}
	...
	if (m_dropCalendar && m_showDate && !m_bIsReadOnly)
	{
		if (m_bAutoResize)
		{
	...
	if (m_spinButton && !m_bIsReadOnly)
	{
		...
		m_rectSpin = CRect(CPoint(rectClient.left + m_iControlWidth - nSpinWidthCurr - nDelta, rectClient.top), CSize(nSpinWidthCurr, m_iControlHeight - nDelta));

		m_wndSpin.SetWindowPos(NULL, m_rectSpin.left, m_rectSpin.top, m_rectSpin.Width(), m_rectSpin.Height(), SWP_NOACTIVATE | SWP_NOZORDER);
		m_wndSpin.ShowWindow(SW_SHOW);

		m_wndSpin.EnableWindow (IsWindowEnabled () && m_bIsChecked && !IsReadOnly());
	}
...
void CBCGPDateTimeCtrl::DrawCheckBox (CDC* pDC, UINT uiState)
{
	...
	BOOL bIsHighlighted = (m_bShowSelection && !m_bIsReadOnly && m_CurrPartType == CHECK_BOX) || m_bMouseOnCheckBox;
	BOOL bIsDisabled = (uiState & ODS_DISABLED);
	
#ifndef _BCGSUITE_
	if (m_bVisualManagerStyle)
	{
		visualManagerMFC->OnDrawCheckBox(pDC, m_rectCheck, bIsHighlighted, m_bIsChecked, !bIsDisabled);
	}
	else
	{
		visualManagerMFC->CBCGPVisualManager::OnDrawCheckBox(pDC, m_rectCheck, bIsHighlighted, m_bIsChecked, !bIsDisabled);
	}
#else
	visualManagerMFC->OnDrawCheckBox(pDC, m_rectCheck, bIsHighlighted, m_bIsChecked, !bIsDisabled);
#endif
...
void CBCGPDateTimeCtrl::DrawDateDropButton (CDC* pDC, UINT uiState)
{
	...
	if (m_bVisualManagerStyle)
	{
		visualManagerMFC->OnDrawDateTimeDropButton (pDC, m_rectDropButton, bDisabled, m_bDropButtonIsPressed && !bDisabled, m_bMouseOnDropButton && !bDisabled, this);
	}
	else
	{
		visualManagerMFC->CBCGPVisualManager::OnDrawDateTimeDropButton(pDC, m_rectDropButton, bDisabled, m_bDropButtonIsPressed && !bDisabled, m_bMouseOnDropButton && !bDisabled, this);
	}
...
void CBCGPDateTimeCtrl::SetPartsOrder ()
{
	...
	if (m_checkButton && !m_bIsReadOnly)
	{
...
void CBCGPDateTimeCtrl::PushDigit (int iDigit)
{
	...
	case MONTH:
		if (m_monthFormat == 2 && iNumber >= 1 && iNumber <= 12)
		{
			iMonth = iNumber;
			...
			// Check for the max. day in the new month:
			iDay = min(iDay, GetDaysInMonth (iMonth, iYear));
		}
		break;
...
//****************************************************************************************
HRESULT CBCGPDateTimeCtrl::get_accRole(VARIANT varChild, VARIANT *pvarRole)
{
...
{
	if (m_bIsChecked && !m_bIsReadOnly)
	{
		CPoint ptCursor;
		::GetCursorPos (&ptCursor);
		ScreenToClient (&ptCursor);

		if (m_rectText.PtInRect (ptCursor) && !m_rectDropButton.PtInRect (ptCursor) && !m_rectSpin.PtInRect(ptCursor))
		{
...
void CBCGPDateTimeCtrl::EnableVisualManagerStyle(BOOL bEnable)
{
	...
	m_wndSpin.m_bVisualManagerStyle = bEnable;
...
LRESULT CBCGPDateTimeCtrl::OnPrint(WPARAM wp, LPARAM lp)
{
	...
	if ((dwFlags & PRF_NONCLIENT) == PRF_NONCLIENT && (GetStyle () & WS_BORDER))
	{
		...
		{
			visualManager->OnDrawControlBorderNoTheme(pDC, rect, this, FALSE);
		}
#else
		visualManager->OnDrawControlBorder(pDC, rect, this, FALSE);
#endif
