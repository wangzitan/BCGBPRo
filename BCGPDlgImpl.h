...
struct BCGCBPRODLLEXPORT CBCGPControlInfoTip
{
	enum BCGPControlInfoTipStyle
	{
		...
		BCGPINFOTIP_Ok,
		BCGPINFOTIP_Warning,
		BCGPINFOTIP_InfoBlue,
		BCGPINFOTIP_Question,
		BCGPINFOTIP_Bulb,
...
public:
	...
	static CSize LoadInfoTipIcons(CBCGPToolBarImages& infotipIcons);

	static BOOL m_bKeepBackstageLeftPanelHighlight;
...
protected:
	...
	void SetUnderlineKeyboardShortcuts(BOOL bSet, BOOL bRedraw);
	...
	void EnableGesturePan(BOOL bEnable = TRUE);
	
	void SetEditBoxesVerticalAlignment(int nAlignment);

#if (!defined _BCGSUITE_) && (!defined _BCGSUITE_INC_)
	void SetEditBoxesSimplifiedBrowseIcon(BOOL bSet);
#endif
	
	BOOL ProcessFocusEditAccelerators(UINT nChar);

	CWnd&					m_Dlg;
	BOOL					m_bDisableShadows;
	BOOL					m_bGroupBoxesDrawByParent;
	...
	BOOL					m_bAutoResizeCtrls;
	CList<UINT,UINT>		m_lstNonAutoResizedItems;
	...
	int						m_nEditBoxesVertAlignment;
	BOOL					m_bEditBoxesSimplifiedIcon;
...
extern BCGCBPRODLLEXPORT UINT BCGM_ONCHANGEUNDERLINE;
extern BCGCBPRODLLEXPORT UINT BCGM_ONGETGESTURECONFIG;
