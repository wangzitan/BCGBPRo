    ...
    m_pSDManager (NULL)
{
	...
	m_bFloatingBarsMenuButton = FALSE;
	...
	if (bRebuildCaptionButtons)
	{
		for (POSITION pos = m_lstMiniFrames.GetHeadPosition (); pos != NULL;)
		{
			...
			if (pMiniFrame != NULL)
			{
				...
				pMiniFrame->UpdateTooltips();
			}
		}
	}
}
//------------------------------------------------------------------------//
void CBCGPDockManager::EnableFloatingBarsMenuButton(BOOL bEnable)
{
	ASSERT_VALID(this);
	
	bEnable = bEnable ? TRUE : FALSE;
	BOOL bRebuildCaptionButtons = bEnable != m_bFloatingBarsMenuButton;
	
	m_bFloatingBarsMenuButton = bEnable;
...
CBCGPBaseControlBar* CBCGPDockManager::FindBarByID (UINT uBarID, BOOL bSearchMiniFrames)
{
	...
	for (pos = m_lstControlBars.GetHeadPosition (); pos != NULL;)
	{
		...
		else if (pBar->IsKindOf (RUNTIME_CLASS (CBCGPDockBar)))
		{
			...
			CBCGPControlBar* pBarCurr = pDockBar->FindBarByID (uBarID);
			if (pBarCurr != NULL)
			{
				return DYNAMIC_DOWNCAST (CBCGPBaseControlBar, pBarCurr);
			}
		}
		else if (pBar->IsKindOf (RUNTIME_CLASS (CBCGPReBar)))
		{
			...
			CBCGPBaseControlBar* pBarCurr = DYNAMIC_DOWNCAST(CBCGPBaseControlBar,
			...
			*ppTargetBar = NULL;
			...
			if (pWnd->IsKindOf (RUNTIME_CLASS (CBCGPMiniFrameWnd)))
			{
				CBCGPMiniFrameWnd* pMiniFrame = 
					DYNAMIC_DOWNCAST (CBCGPMiniFrameWnd, pWnd);
				ASSERT_VALID (pMiniFrame);
				CBCGPControlBar* pBarCurr =
					DYNAMIC_DOWNCAST (CBCGPControlBar, pMiniFrame->GetControlBar ());
				if (pBarCurr != NULL && (pBarCurr->GetEnabledAlignment () & dwAlignment) == 0)
				{
					return;
				}
			}
			...
			{
				CBCGPDockingControlBar* pBarCurr = DYNAMIC_DOWNCAST (CBCGPDockingControlBar, pWnd);
				if ((pBarCurr->GetEnabledAlignment () & dwAlignment) == 0)
				{
					return;
				}
			}
...
BOOL CBCGPDockManager::SaveState (LPCTSTR lpszProfileName, UINT uiID)
{
	...
	CString strSection;
	strSection.Format (REG_SECTION_FMT, (LPCTSTR)strProfileName, uiID);
		...
		if (pNextBar->IsKindOf (RUNTIME_CLASS (CBCGPDockingControlBar)))
		{
			...
			if (pDockingBar->IsKindOf (RUNTIME_CLASS (CBCGPBaseTabbedBar)))
			{
				...
				if (pDefaultSlider != NULL)
				{
					...
					if (pListBarIDs != NULL)
					{
						for (POSITION posListBarIDs = pListBarIDs->GetHeadPosition (); posListBarIDs != NULL;)
						{
							UINT nIDNext = pListBarIDs->GetNext (posListBarIDs);
										  ...
										  BOOL bIncludeTabs)
{
	for (POSITION pos = m_lstControlBars.GetHeadPosition (); pos != NULL;)	
	{
		...
		if (pBar->IsKindOf (RUNTIME_CLASS (CBCGPDockBar)))
		{
			...
			for (POSITION posDockBars = lstDockedBars.GetHeadPosition (); posDockBars != NULL;)
			{
				CBCGPBaseControlBar* pDockedBar = DYNAMIC_DOWNCAST (CBCGPBaseControlBar,
																		lstDockedBars.GetNext (posDockBars));
...
{
	if (!m_bControlBarsContextMenu || m_bIsPrintPreviewMode)
	{
		return;
	}
