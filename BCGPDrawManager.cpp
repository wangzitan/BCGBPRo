...
{
	if (nDepth <= 0 || rect.IsRectEmpty () || rect.Width() <= nDepth || rect.Height() <= nDepth)
	{
		return TRUE;
	}
...
BYTE CBCGPDrawManager::GetColorBrightness(COLORREF clr)
{
...
{
	return GetColorBrightness(clr) < 143;
}

BOOL CBCGPDrawManager::IsLightColor(COLORREF clr)
{
	return GetColorBrightness(clr) > 178;
}

BOOL CBCGPDrawManager::IsPaleColor(COLORREF clr)
{
	return GetColorBrightness(clr) >= 230;
}
...
COLORREF CBCGPDrawManager::ColorInvert(COLORREF clr)
{
	return RGB(
		255 - GetRValue(clr),
		255 - GetGValue(clr),
...
COLORREF CBCGPDrawManager::GetContrastColor(COLORREF clrBackground, COLORREF clrText)
{
	BOOL bNeedToChange = FALSE;
	BOOL bBackgroundIsDark = CBCGPDrawManager::IsDarkColor(clrBackground);
	...
	{
		BOOL bTextIsDark = CBCGPDrawManager::IsDarkColor(clrText);

		if (bBackgroundIsDark == bTextIsDark)
		{
			bNeedToChange = TRUE;
		}
	}
	...
	return bBackgroundIsDark ? RGB(255, 255, 255) : RGB(0, 0, 0);
}
