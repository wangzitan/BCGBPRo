...
{
	int nSecondsPrev = m_lSeconds;
	int nMinutesPrev = m_lMinutes;
	int nHoursPrev = m_lHours;
	int nDaysPrev = m_lDays;

	const int nMaxDay = 1000;

	switch (m_iSelectedPart)
	{
	case DAYS:
		if (m_lDays + iDelta < 0 || m_lDays + iDelta >= nMaxDay)
		{
			return;
		}
	...
	if (iDelta > 0)
	{
		if (m_lSeconds > 59)
		{
			...
			if (!m_bCyclicScroll && m_bShowHoursMinutes)
			{
				m_lMinutes++;
			}
		}
		...
		if (m_lHours > 23)
		{
			...
			if (m_bShowDays && m_lDays < nMaxDay && !m_bCyclicScroll)
			{
				m_lDays++;
			}
		}
	...
	else
	{
		if (m_lSeconds < 0)
		{
			...
			{
				if ((m_lMinutes > 0 || m_lHours > 0 || (m_lDays > 0 && m_bShowDays)) && m_bShowHoursMinutes)
				{
					m_lSeconds = 59;
					m_lMinutes--;
				}
		...
		if (m_lMinutes < 0)
		{
			...
			{
				if ((m_lHours > 0 || (m_lDays > 0 && m_bShowDays)) && m_bShowHoursMinutes)
				{
					m_lMinutes = 59;
					m_lHours--;
				}
		...
		if (m_lHours < 0)
		{
			...
			{
				if ((!m_bCyclicScroll && m_lDays > 0 && m_bShowDays) && m_bShowHoursMinutes)
				{
					m_lHours = 23;
					m_lDays--;
				}
	...
	if (GetSafeHwnd () != NULL)
	{
		if (nSecondsPrev != m_lSeconds)
		{
			InvalidateRect(m_rectParts[SECONDS]);
		}

		if (nMinutesPrev != m_lMinutes)
		{
			InvalidateRect(m_rectParts[MINUTES]);
		}

		if (nHoursPrev != m_lHours)
		{
			InvalidateRect(m_rectParts[HOURS]);
		}

		if (nDaysPrev != m_lDays)
		{
			InvalidateRect(m_rectParts[DAYS]);
		}
...
//****************************************************************************
void CBCGPDurationCtrl::OnDestroy() 
{
	m_hFont = NULL;
	CButton::OnDestroy();
}
