...
CBCGPEditCtrl::CBCGPEditCtrl() :
	...
	m_bIntelliSenseCloseOnShift (TRUE),
	m_bKeepTabs (TRUE), 
	m_nTabLogicalSize (0),
...
void CBCGPEditCtrl::Initialize ()
{
	...
	BOOL bIsBorder = (GetStyle() & WS_BORDER) || (GetExStyle() & WS_EX_CLIENTEDGE);
...
//********************************************************************************
void CBCGPEditCtrl::InitGestures()
{
#ifndef _BCGPEDIT_STANDALONE
...
void CBCGPEditCtrl::OnDraw(CDC* pDC, LPCRECT lpRectClip)
{
	...
	CRect rectClip = lpRectClip == NULL ? rectClient : CRect(lpRectClip);
										...
										CPoint ptTopLeft)
{
	...
	BOOL bResult = pIntelliSenseWnd->Create (lstIntelliSenseData, 
		WS_POPUP | WS_VISIBLE | MFS_SYNCACTIVE | MFS_4THICKFRAME, 
		ptTopLeft, this, m_pIntelliSenseLBFont, m_pIntelliSenseImgList);

	CBCGPBaseIntelliSenseLB* pListBox = pIntelliSenseWnd->GetListBox();
	if (pListBox != NULL)
	{
		pListBox->m_bCloseOnShift = m_bIntelliSenseCloseOnShift;
	}
...
//***************************************************************************************
BOOL CBCGPEditCtrl::ScrollToCaret (BOOL bVCenter, BOOL bRedrawOnScroll)
{
	if (m_bDisableSetCaret)
	{
		return FALSE;
	...
	if (TRUE)
	{
		...
		if (bVCenter)
		{
			int nVCenter = m_rectText.top + m_rectText.Height() / 2 - m_nLineHeight;

 			if (m_ptCaret.y < nVCenter + m_nLineHeight)
			{
				nDeltaY = ((m_ptCaret.y - nVCenter - m_nLineHeight) / m_nLineHeight);
			}
			else if (m_ptCaret.y >= nVCenter)
			{
				nDeltaY = ((m_ptCaret.y - nVCenter) / m_nLineHeight);
			}
		}
		else
		{
...
BOOL CBCGPEditCtrl::OffsetToPoint (int nOffset, CPoint& pt, LPPOINT ptRowColumn, LPINT pnScrollOffset)
{
	...
	{
		pHiddenText = FindCollapsedBlockInRange (i, nOffset, TRUE);
		if (pHiddenText != NULL)
		{
			ASSERT_VALID (pHiddenText);
...
void CBCGPEditCtrl::OnDrawLineNumbersBar (CDC* pDC, CRect rect)
{
	...
	{
		CRect rectLineNum (rect);
		rectLineNum.top = (nVirtualRow - m_nScrollOffsetVert) * m_nLineHeight;
		rectLineNum.bottom = rectLineNum.top + m_nLineHeight;
		OnDrawLineNumber (pDC, rectLineNum, nRow);
	...
	for (i = nStartOffset; i <= nEndOffset; i++)
	{
		...
		if (chNext != g_chEOL && !bDrawSymbol && pHiddenText == NULL)
		{
			if (nNextDelimiter == -1) 
			{
				...
				for (int iIdxCurr = i; iIdxCurr <= nEndOffset; iIdxCurr++)
				{
					TCHAR ch = m_strBuffer [iIdxCurr];
					if (m_strWordDelimeters.Find (ch) != -1)
					{
						nNextDelimiter = iIdxCurr;
						break;
					}
				}
...
BOOL CBCGPEditCtrl::OnGestureEventPan(const CPoint& ptFrom, const CPoint& ptTo, CSize& sizeOverPan)
{
	if (m_nLineHeight == 0 || m_nMaxCharWidth == 0)
	{
		return TRUE;
	}
	...
	int dx = ptFrom.x - ptTo.x;
	int dy = ptFrom.y - ptTo.y;

	int nVertOffset = (int)(0.5 + (double)abs(dy) / m_nLineHeight);
	if (dy < 0)
	{
		nVertOffset = -nVertOffset;
	}

	int nHorzOffset = (int)(0.5 + (double)abs(dx) / m_nMaxCharWidth);
	if (dx < 0)
	{
		nHorzOffset = -nHorzOffset;
	}
	...
	if (abs(nVertOffset) > abs(nHorzOffset))
	{
		...
		int nVertScrollOffset = m_nScrollOffsetVert + nVertOffset;
		const int nVertMax = m_szTotalScroll.cy;
		...
		if (nVertOffset != m_nScrollOffsetVert)
		{
	...
	else
	{
		...
		int nHorzScrollOffset = m_nScrollOffsetHorz + nHorzOffset;
		const int nHorzMax = m_szTotalScroll.cx;
		...
		if (nHorzOffset != m_nScrollOffsetHorz)
		{
...
//***************************************************************************************
CWnd* CBCGPEditCtrl::DoCreateOnPlaceHolder(CWnd* pParent, UINT nID, DWORD dwStyle, const CRect& rect, LPVOID /*lParam*/)
{
...
BOOL CBCGPEditCtrl::OnScroll (int fnBar, UINT nSBCode, int nPos)
{
	...
	if (fnBar == SB_VERT && !m_bScrollVertEmptyPage && scrollInfo.nMax < (int)scrollInfo.nPage)
	{
		nScrollOffset = 0;
	}
	...
	if (fnBar == SB_VERT)
	{
		...
		for (int iIdxRow = 0; iIdxRow < abs (nActualScroll); )
		{
			if (bDirection)
			{
				...
				nNextOffset = m_strBuffer.Find (g_chEOL, nNextOffset + 1);
			}
			else
			{
...
BOOL CBCGPEditCtrl::IndentSelection (BOOL bForward)
{
						...
						pData->m_arInsDelStrings, FALSE))
	{
		...
		iEndSel = m_iEndSel; 
		int nEndRowOffset2 = GetRowStartByOffset (iEndSel, TRUE);

		if (nEndRowOffset2 == iEndSel)
		{
			iEndSel--;
		}
...
void CBCGPEditCtrl::AddUndoAction (BCGP_EDIT_UNDO_ACTION* pUndoAction)
{
	...
	if (!m_bEOL)
	{
		...
		{
			POSITION posSave = pos;	
			BCGP_EDIT_UNDO_ACTION* pUndoActionCurr = 
				(BCGP_EDIT_UNDO_ACTION*) m_lstUndoActions.GetPrev (pos);
			m_lstUndoActions.RemoveAt (posSave);
			OnRemoveUndoAction (pUndoActionCurr);
			delete pUndoActionCurr;
		}
	}
...
BOOL CBCGPEditCtrl::OnUndo (BOOL bSetCaret, BOOL bRedraw)
{
	...
	{
		BCGP_EDIT_UNDO_ACTION* pUndoActionSaved = 
		(BCGP_EDIT_UNDO_ACTION*) m_lstUndoActions.GetAt (m_posUndoBuffer);
		if ((pUndoActionSaved->m_dwFlags & g_dwUAFSaved))
		{
			bSaved = TRUE;
		}
	}
	...
	if (bResult)
	{
		...
		if (m_posUndoBuffer != NULL)
		{
			BCGP_EDIT_UNDO_ACTION* pUndoActionForceNext = 
			(BCGP_EDIT_UNDO_ACTION*) m_lstUndoActions.GetAt (m_posUndoBuffer);
			if (pUndoActionForceNext->m_bForceNextUndo)
			{
...
int CBCGPEditCtrl::RowFromOffset (int nOffset, BOOL bCalcAll, BOOL bSkipHidden) const
{
	...
	for (;  ; nRow++)
	{
		iIdx = m_strBuffer.Find (g_chEOL, iIdx);
		while (iIdx >= 0 && bSkipHidden && FindCollapsedBlock (iIdx) != NULL)
		{
			iIdx = m_strBuffer.Find (g_chEOL, iIdx + 1);
		}
...
BOOL CBCGPEditCtrl::DeleteMarker (int nRow, DWORD dwMarkerType, BOOL bRedraw)
{
	...
	for (POSITION pos = m_lstMarkers.GetHeadPosition (); pos != NULL;)
	{
		...
		{
			DWORD dwMarkerTypeCurr = pMarkerNext->m_dwMarkerType;
			...
			if (OnRemoveMarker (pMarkerNext, posSave))
			{
				...
				if (bRedraw)
				{
					...
					if (dwMarkerTypeCurr & g_dwBCGPEdit_LineColorMarker)
					{
						RedrawText (nRow, 0);
					}
				}
...
BOOL CBCGPEditCtrl::LoadXMLSettings (const CString& strFileName)
{
	...
	try
	{
		...
		{
			TRACE(_T("File not found: %s"), (LPCTSTR)strFileName);
			return FALSE;
		}
...
COLORREF CBCGPEditCtrl::GetColorFromBuffer (const CString strBuffer)
{
	...
	CString str;
	str.Format (_T("0x%s%s%s"), (LPCTSTR)strColor.Right (2), (LPCTSTR)strColor.Mid (2, 2), (LPCTSTR)strColor.Left (2));
...
BOOL CBCGPEditCtrl::SaveFile (const CString& strFileName)
{
	...
	{
		TRACE(_T("Cannot open file: %s"), (LPCTSTR)strFileName);
		return FALSE;
	}
...
int CBCGPOutlineNode::Print (CString &str, int nLevel, int nCount) const
{
	...
	for (POSITION pos = m_lstNodes.GetHeadPosition (); pos != NULL;)
	{
		...
		CString strSubNode;
		nCount = pNode->Print (strSubNode, nLevel + 1, nCount);
...
void CBCGPEditCtrl::UpdateOffsetRelatedData (int nStartOffset, int nEndOffset)
{
	...
	const int nNumChars = nEndOffset - nStartOffset;

	for (POSITION pos = m_lstMarkers.GetHeadPosition(); pos != NULL;)
	{
		CBCGPHiliteMarker* pMarkerNext = DYNAMIC_DOWNCAST (CBCGPHiliteMarker, m_lstMarkers.GetNext(pos));
		if (pMarkerNext != NULL)
		{
			ASSERT_VALID (pMarkerNext);
			
			if (pMarkerNext->m_nStart >= nStartOffset)
			{
				if (CanUpdateMarker (pMarkerNext))
				{
					pMarkerNext->m_nStart += nNumChars;
					pMarkerNext->m_nEnd += nNumChars;
				}
			}
			else if (pMarkerNext->m_nEnd > nStartOffset)
			{
				if (CanUpdateMarker (pMarkerNext))
				{
					pMarkerNext->m_nEnd += nNumChars;
...
void CBCGPEditCtrl::RemoveOffsetDataInRange (int nStartOffset, int nEndOffset)
{
	...
	for (POSITION pos = m_lstMarkers.GetHeadPosition(); pos != NULL;)
	{
		...
		if (pMarkerNext != NULL)
		{
			...
			CBCGPEditMarkerRange range = pMarkerNext->IsInRangeByOffset(nStartOffset, nEndOffset);
				...
				CanRemoveMarker (pMarkerNext))
			{
				...
				CanUpdateMarker (pMarkerNext))
			{
				pMarkerNext->UpdateMarkerForDeletedRangeByOffset(nStartOffset, nEndOffset);
			}
			else if (pMarkerNext->m_nStart >= nEndOffset)
			{
				if (CanUpdateMarker(pMarkerNext))
				{
					const int nNumChars = nEndOffset - nStartOffset;
...
void CBCGPEditCtrl::UpdateHiddenLines (BCGP_EDIT_OUTLINE_CHANGES& changes)
{
	...
	for (pos = changes.m_lstRemoved.GetHeadPosition (); pos != NULL; )
	{
		CBCGPOutlineNode* pNodeRemoved = (CBCGPOutlineNode*) changes.m_lstRemoved.GetNext (pos);
		ASSERT_VALID (pNodeRemoved);

		CObList lst;
		if (pNodeRemoved->m_bCollapsed)
		{
			lst.AddTail (pNodeRemoved);
		}
		else
		{
			pNodeRemoved->GetBlocksByState (lst, TRUE);
		}
	...
	for (pos = changes.m_lstInserted.GetHeadPosition (); pos != NULL; )
	{
		CBCGPOutlineNode* pNodeInserted = (CBCGPOutlineNode*) changes.m_lstInserted.GetNext (pos);
		ASSERT_VALID (pNodeInserted);

		CObList lst;
		if (pNodeInserted->m_bCollapsed)
		{
			lst.AddTail (pNodeInserted);
		}
		else
		{
			pNodeInserted->GetBlocksByState (lst, TRUE);
		}
...
BOOL CBCGPEditCtrl::GetHyperlinkToolTip (CString& strToolTip)
{
	if (m_bEnableHyperlinkSupport)
	{
		...
		{
			BCGP_EDIT_HYPERLINK_DEF hyperlink = m_arrHyperlinks [m_nCurrHyperlinkHot];
			strToolTip.Format (_T("%s\nCTRL + click to follow link"), (LPCTSTR)hyperlink.m_strHyperlink);
...
BOOL CBCGPEditCtrl::GetCurLine (CString& strLine) const
{
	...
	if (nLineBeginOffset >= 0 && nLineBeginOffset == nLineEndOffset)
	{
		strLine.Empty();
		return TRUE;
	}
	else if (nLineBeginOffset >= 0 && nLineEndOffset > nLineBeginOffset && 
...
LRESULT CBCGPEditCtrl::OnStyleChanging (WPARAM wp, LPARAM lp)
{
	...
	{
		m_bIsBorder = (lpStyleStruct->styleNew & WS_BORDER) || (GetExStyle() & WS_EX_CLIENTEDGE);
	}
	else if (GWL_EXSTYLE == nStyleType)
	{
		m_bIsBorder = (lpStyleStruct->styleNew & WS_EX_CLIENTEDGE) || (GetStyle() & WS_BORDER);
	}
...
LRESULT CBCGPEditCtrl::OnGestureEvent(WPARAM wp, LPARAM lp)
{
...
//******************************************************************************************************************
LRESULT CBCGPEditCtrl::OnTabletQuerySystemGestureStatus(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	return (LRESULT)TABLET_DISABLE_PRESSANDHOLD;
}
//*****************************************************************************************************************
LRESULT CBCGPEditCtrl::OnGetGestureConfig(WPARAM, LPARAM)
{
	return 1L;	
}
