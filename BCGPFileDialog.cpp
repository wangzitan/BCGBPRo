...
void CBCGPFileDialog::OnInitDone()
{
...

	{
		...
		m_wndRecentList.InsertColumn (0, strFile, LVCFMT_LEFT, globalUtils.ScaleByDPI(100));
...
//******************************************************************************************
void CBCGPFileDialog::OnTabSelchange() 
{
	if (m_wndDummy.GetSafeHwnd() != NULL)
	{
...
//**************************************************************************************
void CBCGPFileDialog::AddNewItem (LPCTSTR lpszName, int iIconIndex)
{
	m_lstNewItems.AddTail (new CNewItemInfo (lpszName, iIconIndex));
}
//**************************************************************************************
void CBCGPFileDialog::SetDlgIcon (HICON hIconBig, HICON hIconSmall/* = NULL*/)
{
	m_hIconBig = hIconBig;
	m_hIconSmall = (hIconSmall == NULL) ? m_hIconBig : hIconSmall;
}

BEGIN_MESSAGE_MAP(CBCGPFDTabWnd, CBCGPTabWnd)
	ON_WM_ERASEBKGND()
END_MESSAGE_MAP()

BOOL CBCGPFDTabWnd::OnEraseBkgnd(CDC* pDC)
{
	CRect rectClient;
...
void CBCGPFDTabWnd::FireChangeActiveTab (int /*nNewTab*/)
{
	if (m_pParent != NULL)
	{
