...
CBCGPFrameImpl::CBCGPFrameImpl(CFrameWnd* pFrame) :
	...
	m_rectMonitor(0, 0, 0, 0),
	m_bInWindowPosChanging(FALSE),
	m_bInChangeVisualManager(FALSE),
	m_bHandleStyleChanging(FALSE),
	m_dwStyleSaved(0),
	m_pLightBoxDlg(NULL)
{
...
void CBCGPFrameImpl::OnCloseFrame()
{
	...
	if (m_pLightBoxDlg->GetSafeHwnd() != NULL)
	{
		m_pLightBoxDlg->DestroyWindow();
		m_pLightBoxDlg = NULL;
	}
...
#ifndef BCGP_EXCLUDE_RIBBON
		if (m_pRibbonBar->GetSafeHwnd () != NULL && m_pRibbonBar->IsInAutoHideMode())
		{
...
const CBCGPToolBar* CBCGPFrameImpl::CreateNewToolBar (LPCTSTR lpszName)
{
	...
	if (GetMonitorInfo(MonitorFromWindow (m_pFrame->GetSafeHwnd(), MONITOR_DEFAULTTONEAREST), &mi))
	{
		rectDesktop = mi.rcWork;
	}
	else
	...
	CRect rectFloat(rectDesktop.CenterPoint(), rectBar.Size());
...
BOOL CBCGPFrameImpl::ProcessKeyboard (int nKey, BOOL* pbProcessAccel)
{
	...
	if (pActivePopupMenu != NULL)
	{
		...
		if (pActivePopupMenu->IsFloaty ())
		{
			...
			if (bIsFloatyActive || nKey == VK_CONTROL || nKey == VK_SHIFT)
			{
				return FALSE;
			}
	...
	{
		m_pRibbonBar->ToggleMinimizeState ();
		return TRUE;
	}
...
BOOL CBCGPFrameImpl::ProcessMouseClick (UINT uiMsg, POINT pt, HWND hwnd)
{
	...
	if (m_pRibbonBar->GetSafeHwnd() != NULL && m_pRibbonBar->IsWindowVisible ())
	{
		...
		if (m_pLightBoxDlg != NULL && m_pLightBoxDlg->ShouldCloseOnClick(m_pRibbonBar, pt))
		{
			if (!CloseLightBox())
			{
				return TRUE;
	...
	if (!CBCGPToolBar::IsCustomizeMode () && pActivePopupMenu != NULL)
	{
		...
		if (clickArea == CBCGPPopupMenu::OUTSIDE)
		{
			...
			if (pActivePopupMenu != NULL && !pActivePopupMenu->InCommand ())
			{
				...
				{
					CBCGPPopupMenuBar* pClickedMenuBar = DYNAMIC_DOWNCAST(CBCGPPopupMenuBar, CWnd::FromHandlePermanent (hwnd));
					if (pClickedMenuBar != NULL && pClickedMenuBar->IsStopMouseClickAfterCloseActiveMenu())
					{
...
BOOL CBCGPFrameImpl::ProcessMouseMove (POINT pt)
{
	...
	if (!CBCGPToolBar::IsCustomizeMode () && pActivePopupMenu != NULL)
	{
		...
		if (rectMenu.PtInRect (pt) || pActivePopupMenu->GetMenuBar ()->FindDestBar (pt) != NULL)
		{
			return FALSE;	// Default processing
		}
		...
		if (pActivePopupMenu->IsFloaty())
		{
			CBCGPRibbonFloaty* pFloaty = DYNAMIC_DOWNCAST(CBCGPRibbonFloaty, pActivePopupMenu);
			if (pFloaty != NULL && !pFloaty->IsMouseCapture())
			{
				return FALSE;
			}
		}
...
void CBCGPFrameImpl::ShowQuickCustomizePane(CBCGPPopupMenu* pMenuPopup)
{
	...
	else
	{
		...
		const CObList& lstAllToolbars = CBCGPToolBar::GetAllToolbars ();
	
		CBCGPToolBar* pRealToolBar = NULL;
		for (POSITION pos = lstAllToolbars.GetHeadPosition (); pos != NULL;)
		{
			pRealToolBar = (CBCGPToolBar*)lstAllToolbars.GetNext (pos);
	...
	for (UINT i=0; i< uiCount; i++)
	{
		...
		if (pWndParentToolbar->IsHideEmptyMenus() && pBtn->IsEmptyDropDownMenu())
		{
			continue;
		}
...
//********************************************************************************
BOOL CBCGPFrameImpl::OnStyleChanging(int nStyleType, LPSTYLESTRUCT lpStyleStruct)
{
	if (!m_bHandleStyleChanging)
	{
		return FALSE;
	}
...
#ifndef BCGP_EXCLUDE_RIBBON
	if (m_pRibbonBar->GetSafeHwnd () != NULL &&
		((m_pRibbonBar->IsWindowVisible () || IsFullScreen ())|| !m_pFrame->IsWindowVisible ()) &&
	...
	if (bIsRibbonCaption || m_bInChangeVisualManager || m_bDisableThemeCaption || 
		m_bIsMDIChildFrame)
	{
		return FALSE;
	}
	...
	{
		if ((lpStyleStruct->styleNew & WS_CAPTION) != 0 &&
			(lpStyleStruct->styleOld & WS_CAPTION) == 0)
		{
			m_bHasCaption = TRUE;
			bChanged = TRUE;
		}
		else if ((lpStyleStruct->styleNew & WS_CAPTION) == 0 &&
				 (lpStyleStruct->styleOld & WS_CAPTION) != 0)
		{
			m_bHasCaption = FALSE;
			bChanged = TRUE;
		...
		if ((lpStyleStruct->styleNew & WS_BORDER) != 0 &&
			!m_bHasBorder)
		{
			m_bHasBorder = TRUE;
			bChanged = TRUE;
		}
		else if ((lpStyleStruct->styleNew & WS_BORDER) == 0 &&
				m_bHasBorder)
		{
			m_bHasBorder = FALSE;
			bChanged = TRUE;
...
void CBCGPFrameImpl::OnGetMinMaxInfo (MINMAXINFO FAR* lpMMI)
{
		...
		bIsRibbonCaption)
	{
		...
		if (m_rectMonitor != rect)
		{
			OnMonitorChanged(rect);
		}
...
BOOL CBCGPFrameImpl::OnNcCalcSize(BOOL /*bCalcValidRects*/, NCCALCSIZE_PARAMS FAR* lpncsp)
{
		...
		m_pRibbonBar->IsReplaceFrameCaption ())
	{
		...
		{
			if (((m_pFrame->GetStyle () & WS_MAXIMIZE) == 0 && !IsFullScreen ()) || globalData.GetShellAutohideBars () == 0 ||
...
void CBCGPFrameImpl::UpdateCaption ()
{
		...
		!CBCGPVisualManager::GetInstance ()->OnUpdateNcButtons(m_pFrame, m_lstCaptionSysButtons, rectCaption))
	{
		...
		if (m_pFrame->IsIconic() && m_pFrame->IsKindOf(RUNTIME_CLASS(CMDIChildWnd)))
		{
			sizeButton.cx = min(sizeButton.cx, sizeButton.cy);
		}

		CSize szOffset = IsRibbonCaption() ? CSize(0, 0) : CBCGPVisualManager::GetInstance()->GetFrameCaptionButtonOffset(m_pFrame);

		int x = rectCaption.right - sizeButton.cx + szOffset.cx;
		int y = rectCaption.top + szOffset.cy;
...
void CBCGPFrameImpl::RemoveFrame (CFrameWnd* pFrame)
{
	...
	CBCGPVisualManager::GetInstance()->RemoveActivateFlag(pFrame);
}
//*************************************************************************
...
#ifndef BCGP_EXCLUDE_RIBBON
	if (IsFullScreen() && m_pRibbonBar != NULL && m_pRibbonBar->IsReplaceFrameCaption())
	{
		return HTNOWHERE;
	}
#endif
...
void CBCGPFrameImpl::OnLButtonDown(CPoint point)
{
	...
	if (!m_bCaptured)
	{
		if (m_pFrame->IsKindOf(RUNTIME_CLASS(CMDIChildWnd)))
		{
			...
			if (WindowFromPoint(point) != m_pFrame->GetSafeHwnd())
			{
...
void CBCGPFrameImpl::OnTrackCaptionButtons (CPoint point)
{
	...
	{
		const BOOL bMDIChild = m_pFrame->IsKindOf(RUNTIME_CLASS(CMDIChildWnd));
		if (!bMDIChild || (bMDIChild && WindowFromPoint(point) == m_pFrame->GetSafeHwnd()))
		{
...
void CBCGPFrameImpl::OnChangeVisualManager ()
{
	...
	{
		bIsRibbonCaption = (m_pRibbonBar->IsWindowVisible () || !m_pFrame->IsWindowVisible ()) && m_pRibbonBar->IsReplaceFrameCaption ();
	}
#endif

	const BOOL bIsFullScreen = IsFullScreen() && !bIsRibbonCaption;

	globalData.EnableWindowAero(m_pFrame->GetSafeHwnd(), CBCGPVisualManager::GetInstance()->IsDWMCaptionSupported());

	if (CBCGPVisualManager::GetInstance ()->CanShowFrameShadows() && !m_bIsMDIChildFrame && 
		(m_pFrame->GetStyle() & WS_CHILD) == 0 && !m_bDisableThemeCaption && !m_bIsOleInPlaceActive &&
		!bIsFullScreen &&
		(((m_bHasBorder || (m_pFrame->GetStyle() & WS_THICKFRAME) != 0) && m_bHandleStyleChanging) || !m_bHandleStyleChanging))
	{
	...
	BOOL bRepositionWindow = bIsFullScreen;
	...
	if (!bIsFullScreen)
	{
		...
		else
		{
			...
			{
				bRepositionWindow = TRUE;
			}
		}
	}

	if (bRepositionWindow)
	{
...
BOOL CBCGPFrameImpl::ProcessSysKeyUp(WPARAM wParam, LPARAM lParam)
{
		...
		(wParam == VK_MENU || (wParam == VK_F10 && !bIsCtrlPressed  && !bIsShiftPressed)))
	{
		...
		else if (m_pMenuBar->IsPopupMode() && bKeyIsNotReleased && !IsFullScreen())
		{
...
void CBCGPFrameImpl::CancelToolbarMode()
{
	...
	if (pWndUnderCursor->GetSafeHwnd() != NULL && CWnd::FromHandlePermanent(pWndUnderCursor->GetSafeHwnd()) != NULL)
	{
		if ((GetAsyncKeyState(VK_LBUTTON) & 0x8000) == 0)
		{
			pWndUnderCursor->SendMessage(WM_CANCELMODE);
		}
	}
}
//************************************************************************************
...
void CBCGPFrameImpl::GetMonitorRect(CRect& rect)
{
	...
	if (GetMonitorInfo (MonitorFromWindow (m_pFrame->GetSafeHwnd(), MONITOR_DEFAULTTONEAREST),
		&mi))
	{
...
//********************************************************************************
BOOL CBCGPFrameImpl::AdjustMaximizedSize(int& cx, int& cy)
{
	BOOL bRes = FALSE;
	BOOL bIsRibbonCaption = FALSE;
...
//************************************************************************************
void CBCGPFrameImpl::OnMonitorChanged(const CRect& rectNew)
{
	m_rectMonitor = rectNew;
...
//************************************************************************************
BOOL CBCGPFrameImpl::SetLightBoxDialog(CBCGPLightBoxDialog* pDlg)
{
	if (pDlg != NULL)
	{
		if (m_pLightBoxDlg != NULL && m_pLightBoxDlg->m_bIsModalLoop)
		{
			TRACE0("You cannot create a second light box dialog when another modal light box is active\n");
			ASSERT(FALSE);
			return FALSE;
		}

		CloseLightBox();	// Close current light box
	}
...
//************************************************************************************
BOOL CBCGPFrameImpl::CloseLightBox(BOOL bNotify/* = TRUE*/)
{
	if (m_pLightBoxDlg != NULL)
	{
		ASSERT_VALID(m_pLightBoxDlg);

		if (!m_pLightBoxDlg->CloseMe(bNotify))
		{
			return FALSE;
		}
...
//************************************************************************************
BOOL CBCGPFrameImpl::IsRibbonCaption() const
{
#ifndef BCGP_EXCLUDE_RIBBON
	return (m_pRibbonBar->GetSafeHwnd() != NULL && m_pRibbonBar->IsWindowVisible () && m_pRibbonBar->IsReplaceFrameCaption ());
#else
	return FALSE;
#endif
}
//************************************************************************************
void CBCGPFrameImpl::ShowNCArea(BOOL bShow, UINT nFlags)
{
	ASSERT_VALID(m_pFrame);
	...
	if (bShow)
	{
		if (m_dwStyleSaved != 0)
		{
			m_bHandleStyleChanging = TRUE;
			m_pFrame->ModifyStyle(0, m_dwStyleSaved, SWP_SHOWWINDOW | SWP_FRAMECHANGED);
	...
	if (nFlags & BCGP_NC_AREA_FRAME)
	{
		if ((dwStyle & WS_BORDER) == WS_BORDER || m_bHasBorder)
		{
			m_dwStyleSaved |= WS_BORDER;
		}

		if ((dwStyle & WS_THICKFRAME) == WS_THICKFRAME)
		{
			m_dwStyleSaved |= WS_THICKFRAME;
		}

		if ((dwStyle & WS_SIZEBOX) == WS_SIZEBOX)
		{
			m_dwStyleSaved |= WS_SIZEBOX;
		}
	}

	if ((nFlags & BCGP_NC_AREA_CAPTION))
	{
		if ((dwStyle & WS_CAPTION) == WS_CAPTION || m_bHasCaption)
		{
			m_dwStyleSaved |= WS_CAPTION;
		}

		if ((dwStyle & WS_SYSMENU) == WS_SYSMENU)
		{
			m_dwStyleSaved |= WS_SYSMENU;
		}
	}
	...
	{
		m_bHandleStyleChanging = TRUE;
		m_pFrame->ModifyStyle(m_dwStyleSaved, 0, SWP_SHOWWINDOW | SWP_FRAMECHANGED);
	}
}
