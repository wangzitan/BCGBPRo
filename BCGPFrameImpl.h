...
static const DWORD BCGP_NC_AREA_CAPTION = 0x01;
static const DWORD BCGP_NC_AREA_FRAME = 0x02;
...
protected:
	...
	CRect				m_rectMonitor;
	BOOL				m_bInWindowPosChanging;
	BOOL				m_bInChangeVisualManager;
	BOOL				m_bHandleStyleChanging;
	DWORD				m_dwStyleSaved;

	CBCGPLightBoxDialog*	m_pLightBoxDlg;
...
protected:
	...
	void EnableFullScreenCloseToolBar(BOOL bShow = TRUE)
	{
		m_FullScreenMgr.EnableCloseToolBar(bShow);
	}
	...
	BOOL OnStyleChanging(int nStyleType, LPSTYLESTRUCT lpStyleStruct);
	BOOL OnNcPaint();
	...
	BOOL AdjustMaximizedSize(int& cx, int& cy);
	...
	void OnMonitorChanged(const CRect& rectOld);

	BOOL SetLightBoxDialog(CBCGPLightBoxDialog* pDlg);
	
	CBCGPLightBoxDialog* GetLightBoxDialog() const
	{
		return m_pLightBoxDlg;
	}
	
	BOOL CloseLightBox(BOOL bNotify = TRUE);
	
	BOOL IsRibbonCaption() const;

	void ShowNCArea(BOOL bShow, UINT nFlags);
};

#define UserToobars		UserToolbars
#define GetFullScreeen	GetFullScreen
#define IsFullScreeen	IsFullScreen
