...
void CBCGPFullScreenImpl::ShowFullScreen(CFrameWnd* pFrame)
{
	...
	BOOL bNormalizeFrame = pFrame->IsZoomed();
	...
	{
		pbcgMDIFrame->ShowNCArea(FALSE);
		rectFrame.SetRect(rcScreen.left, rcScreen.top, rcScreen.right, rcScreen.bottom);
	}
	else // Maybe SDI
	{
		CBCGPFrameWnd* pbcgFrame = DYNAMIC_DOWNCAST (CBCGPFrameWnd, pFrame);
	...
	if (m_bShowCloseToolBar)
	{
		...
		const int nToolBarSize = globalUtils.ScaleByDPI(100);
		...
		m_pwndFullScreenBar->SetWindowPos (0, nToolBarSize, nToolBarSize, nToolBarSize, nToolBarSize, SWP_NOZORDER | SWP_NOACTIVATE | SWP_SHOWWINDOW);
		m_pwndFullScreenBar->SetWindowText (strCaption);

		CRect rectBar;
		rectBar.left = rcScreen.left + nToolBarSize;
		rectBar.top = rcScreen.top + nToolBarSize;
		rectBar.right = rectBar.left + 2 * nToolBarSize;
		rectBar.bottom = rectBar.top + 2 * nToolBarSize;
		m_pwndFullScreenBar->FloatControlBar (rectBar);
	}
	...
	if (m_bShowMenu)
	{
		if (pbcgMDIFrame != NULL)
		{
			const CBCGPMenuBar* pMDIMenuBar = pbcgMDIFrame->GetMenuBar ();

			if (pMDIMenuBar != NULL)
			{
				if (m_bTabsArea)
				{
					if (pMDIMenuBar->CanFloat ())
					{
						pbcgMDIFrame->DockControlBar ((CBCGPBaseControlBar*)pMDIMenuBar);
					}
					pbcgMDIFrame->ShowControlBar ((CBCGPBaseControlBar*)pMDIMenuBar, TRUE,FALSE, FALSE);
				}
				else
				{
					if (pMDIMenuBar->CanFloat ())
					{
						pbcgMDIFrame->DockControlBar ((CBCGPBaseControlBar*)pMDIMenuBar);
						((CBCGPBaseControlBar*)pMDIMenuBar)->FloatControlBar (CRect(300, 200, 500, 500));
					}
					pbcgMDIFrame->ShowControlBar ((CBCGPBaseControlBar*)pMDIMenuBar, TRUE,FALSE, FALSE);
				}
			}	
		...
		else
		{
			...
			if (pbcgFrame != NULL)
			{
				const CBCGPMenuBar* pSDIMenuBar = pbcgFrame->GetMenuBar ();

				if (pSDIMenuBar != NULL)
				{
					if (pSDIMenuBar->CanFloat ())
					{
						pbcgFrame->DockControlBar ((CBCGPBaseControlBar*)pSDIMenuBar);
					}
					pbcgFrame->ShowControlBar ((CBCGPBaseControlBar*)pSDIMenuBar, TRUE,FALSE, FALSE);
				}	
			}
		}
	}
...
void CBCGPFullScreenImpl::RestoreState(CFrameWnd* pFrame) 
{
	...
	// Destroy the toolbar
	if (m_pwndFullScreenBar->GetSafeHwnd() != NULL)
	{
	...
	g_pWorkspace->m_bExitingFullScreenMode = TRUE;

	if (pbcgMDIFrame != NULL)
	{
		pbcgMDIFrame->ShowNCArea(TRUE);
	...
	g_pWorkspace->m_bExitingFullScreenMode = FALSE;
}
//*************************************************************************************
