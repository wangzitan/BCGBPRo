...
LRESULT CBCGPGanttChart::OnGetGestureConfig(WPARAM, LPARAM)
{
	return 1L;
}
...
void CBCGPGanttChart::OnVScroll (UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
	...
	m_wndScrollVert.GetScrollInfo (&sci);
	int minY = sci.nMin;
	int maxY = bcgp_max(0, sci.nMax - sci.nPage + 1);
	int y = sci.nPos;
	int yOld = y;
...
BOOL CBCGPGanttChart::OnGestureEventPan(const CPoint& ptFrom, const CPoint& ptTo, CSize& sizeOverPan)
{
	...
	const int nOverPan = globalData.GetTextHeight();
	
	BOOL bIsVert = abs(nVertOffset) > abs(nHorzOffset);

	CBCGPScrollBar& wndScrollBar = bIsVert ? m_wndScrollVert : m_wndScrollHorz;
	if (wndScrollBar.DoGesturePan(ptFromScreen, ptToScreen))
	{
		return FALSE;
	}

	const int nOffset = bIsVert ? nVertOffset : nHorzOffset;
	...
	const int nPos = wndScrollBar.GetScrollPos() + nOffset;
	const BOOL bIsOverPan = (scrollInfo.nMax == scrollInfo.nMin) || (nPos < scrollInfo.nMin) || (nPos >= (int)(scrollInfo.nMax - scrollInfo.nPage));
	...
	else
	{
		...
		else
		{
			...
			COleDateTime dtNewPos = dtMin + COleDateTimeSpan((dtMax - dtMin) * (nPos - scrollInfo.nMin) / (scrollInfo.nMax - scrollInfo.nMin));
