...
class BCGCBPRODLLEXPORT CBCGPGaugeLevelBarObject :	public CObject,
													public CBCGPAnimationManager
{
	...
	DECLARE_DYNCREATE(CBCGPGaugeLevelBarObject)
		
public:
	enum BCGP_GAUGE_LEVEL_OBJECT_STYLE
	{
		BCGP_GAUGE_LEVEL_OBJECT_STYLE_FIRST		= 0,
		BCGP_GAUGE_LEVEL_OBJECT_RECT			= BCGP_GAUGE_LEVEL_OBJECT_STYLE_FIRST,
		BCGP_GAUGE_LEVEL_OBJECT_ROUNDED_RECT	= 1,
		BCGP_GAUGE_LEVEL_OBJECT_THERMOMETER		= 2,
		BCGP_GAUGE_LEVEL_OBJECT_STYLE_LAST		= BCGP_GAUGE_LEVEL_OBJECT_THERMOMETER
	};
...
public:
	CBCGPGaugeLevelBarObject()
	{
		...
		m_dblWidth = 0.;
		m_dblOffsetFromFrame = 0.;
		m_Style = BCGP_GAUGE_LEVEL_OBJECT_RECT;
		m_pParentGauge = NULL;
	}

	CBCGPGaugeLevelBarObject(const CBCGPGaugeLevelBarObject& src)
	{
		CopyFrom(src);
	}

	CBCGPGaugeLevelBarObject(
		const CBCGPBrush& brFill, const CBCGPBrush& brFillValue, const CBCGPBrush& brFrame,
		int nScale = 0, double dblWidth = 0., double dblOffsetFromFrame = 0.,
		BCGP_GAUGE_LEVEL_OBJECT_STYLE style = BCGP_GAUGE_LEVEL_OBJECT_RECT)
	{
		m_dblValue = 0.;
		m_nScale = nScale;
		m_brFill = brFill;
		m_brFillValue = brFillValue;
		m_brOutline = brFrame;
		m_dblWidth = dblWidth;
		m_dblOffsetFromFrame = dblOffsetFromFrame;
		m_Style = style;
	...
	CBCGPGaugeLevelBarObject& operator = (const CBCGPGaugeLevelBarObject& src)
	{
		CopyFrom(src);
		return *this;
	}

	virtual CBCGPGaugeLevelBarObject* CreateCopy() const
	{
		return new CBCGPGaugeLevelBarObject(*this);
	}
	
	virtual void CopyFrom(const CBCGPGaugeLevelBarObject& src)
	{
		m_nScale = src.m_nScale;
		m_dblValue = src.m_dblValue;
		m_brFill = src.m_brFill;
		m_brFillValue = src.m_brFillValue;
		m_brOutline = src.m_brOutline;
	...
	void SetValue(double dblValue, double dblAnimationTime = 0.0 /* seconds, 0 - no animation */)
	{
		if (IsAnimated())
		{
			StopAnimation(FALSE);
		...
		if (dblAnimationTime > 0)
		{
			StartAnimation(m_dblValue, dblValue, dblAnimationTime, BCGPANIMATION_AccelerateDecelerate);
		}
	...
	void SetFillValueBrush(const CBCGPBrush& brFill)
	{
		m_brFillValue = brFill;
	}

	const CBCGPBrush& GetFillValueBrush() const
	{
		return m_brFillValue;
	}
	...
	BCGP_GAUGE_LEVEL_OBJECT_STYLE GetStyle() const
	{
		return m_Style;
	}

	void SetStyle(BCGP_GAUGE_LEVEL_OBJECT_STYLE style)
	{
		m_Style = style;
	}
...
protected:
	...
	double							m_dblValue;
	CBCGPBrush						m_brFill;
	CBCGPBrush						m_brFillValue;
	CBCGPBrush						m_brOutline;
	CBCGPComplexGeometry			m_Shape;
	double							m_dblWidth;
	double							m_dblOffsetFromFrame;
	BCGP_GAUGE_LEVEL_OBJECT_STYLE	m_Style;
	CBCGPGaugeImpl*					m_pParentGauge;
};
...
public:
	...
	virtual BOOL SetLevelBarValue(double dblVal, int nIndex = 0, UINT uiAnimationTime = 100 /* Ms, 0 - no animation */, BOOL bRedraw = FALSE);
...
public:
	...
	// Level bars:
	int AddLevelBar(const CBCGPBrush& brFill, const CBCGPBrush& brFrame, const CBCGPBrush& brFillValue,
		int nScale = 0, double dblWidth = 0., double dblOffsetFromFrame = 0., CBCGPGaugeLevelBarObject::BCGP_GAUGE_LEVEL_OBJECT_STYLE style = CBCGPGaugeLevelBarObject::BCGP_GAUGE_LEVEL_OBJECT_RECT, BOOL bRedraw = FALSE);
	
	int AddLevelBar(CBCGPGaugeLevelBarObject* pLevelBar, BOOL bRedraw = FALSE);
	
	BOOL ModifyLevelBar(int nIndex, const CBCGPGaugeLevelBarObject& levelBar, BOOL bRedraw = FALSE);
	
	CBCGPGaugeLevelBarObject* GetLevelBar(int nIndex)
	{
		...
		return m_arLevelBars[nIndex];
	}
	
	BOOL RemoveLevelBar(int nIndex, BOOL bRedraw = FALSE);
	
	int GetLevelBarsCount() const
	{
		return (int)m_arLevelBars.GetSize();
	}
	
	void RemoveAllLevelBars(BOOL bRedraw = FALSE);
...
protected:
	...
	CArray<CBCGPGaugeLevelBarObject*, CBCGPGaugeLevelBarObject*>			m_arLevelBars;
};
