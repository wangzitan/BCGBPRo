...
public:
	...
	void RemoveSingleAmp(CString& strText, BOOL bReplace2AmpWith1 = TRUE);
	...
	BOOL GetParentDialogFont(HWND hWndCtrl, HFONT& hFont, BOOL bCheckParent = FALSE);
	...
	BOOL IsPlacedOnActiveMenu(const CWnd* pWnd);
	...
	int DrawTextWithHighlightedArea(CDC* pDC, const CString& str, LPRECT lpRect, UINT nFormat, int nHightLightStart, int nHightlightLen, COLORREF clrHighlight, COLORREF clrHighlightText);
	
	BOOL ProcessCtrlEditAccelerators(CWnd* pCtrl, UINT nChar, BOOL bIsEditable, BOOL* pbChanged = NULL);
	...
	BOOL	m_bRemoveDoubleAmpersands;
