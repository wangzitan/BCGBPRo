...
IMPLEMENT_DYNAMIC(CBCGPGeometryGroup, CBCGPGeometry)
...
CBCGPGraphicsManager* CBCGPGraphicsManager::CreateInstance(BCGP_GRAPHICS_MANAGER manager, BOOL bFallback, CBCGPGraphicsManagerParams* pParams)
{
	...
	case BCGP_GRAPHICS_MANAGER_GDI_PLUS:
		pGM = new CBCGPGraphicsManagerGdiPlus(NULL, TRUE, pParams);
		if (pGM->IsValid())
		{
			return pGM;
		}
	...
	int nDrawingFlags)
{
	...
	double dblGradientStep = IsSupported(BCGP_GRAPHICS_MANAGER_ANTIALIAS) ? 1.5 : 20;
	...
	if (bSmouthFill)
	{
		if (IsSupported(BCGP_GRAPHICS_MANAGER_ANTIALIAS))
		{
			...
			{
				double dblAngle2 = min(dblAngle + 2.0 * dblGradientStep, dblFinishAngle);
...
{
	DrawArrowEx3(pt1, pt2, length, brFill, brLine, arrowAngle, dwStyle, 1.0, NULL);
}

void CBCGPGraphicsManager::DrawArrowEx3(const CBCGPPoint& pt1, const CBCGPPoint& pt2, double length, 
	const CBCGPBrush& brFill, const CBCGPBrush& brLine, double arrowAngle, DWORD dwStyle,
	double lineWidth, const CBCGPStrokeStyle* pStrokeStyle)
{
	...
	DrawGeometry(shapeArrow, brLine, lineWidth, pStrokeStyle);
}

void CBCGPGraphicsManager::DrawArrow4(const CBCGPPoint& pt1, const CBCGPPoint& pt2, double length, 
	const CBCGPBrush& brFill, const CBCGPBrush& brLine, double arrowAngle, DWORD dwStyle)
{
	DrawArrowEx4(pt1, pt2, length, brFill, brLine, arrowAngle, dwStyle, 1.0, NULL);
}

void CBCGPGraphicsManager::DrawArrowEx4(const CBCGPPoint& pt1, const CBCGPPoint& pt2, double length, 
	const CBCGPBrush& brFill, const CBCGPBrush& brLine, double arrowAngle, DWORD dwStyle,
	double lineWidth, const CBCGPStrokeStyle* pStrokeStyle)
{
    const double angle = bcg_angle(CBCGPPoint(pt1), CBCGPPoint(pt2));
...
void CBCGPColor::Mix(const CBCGPColor& rOther, double ratio /* 0 - 1 */)
{
	ratio = bcg_clamp(ratio, 0., 1.);

	r = r * ratio + rOther.r * (1. - ratio);
	g = g * ratio + rOther.g * (1. - ratio);
	b = b * ratio + rOther.b * (1. - ratio);
	a = a * ratio + rOther.a * (1. - ratio);
}
...
BOOL CBCGPColor::AdaptColors(COLORREF clrBase, COLORREF clrTone, BOOL bClampHue /*= TRUE*/)
{
	if (IsNull())
	{
		return FALSE;
	...
	COLORREF clrOrig = (COLORREF)*this;
	
	double H,S,L;
	CBCGPDrawManager::RGBtoHSL (clrOrig, &H, &S, &L);

#ifndef _BCGSUITE_
	const double dblTolerance = CBCGPToolBarImages::m_dblColorTolerance;
#else
	const double dblTolerance = 0.2;
#endif

	if (fabs (dSrcH - H) >= dblTolerance)
	{
		return FALSE;
	}
	...
	COLORREF colorNew = CBCGPDrawManager::HLStoRGB_ONE(HNew, bClampHue ? L : LNew, SNew);
		
	*this = CBCGPColor(colorNew, a);
	return TRUE;
}

void CBCGPColor::InvertColors()
{
	if (IsNull())
	{
		return;
	...
	COLORREF clr = CBCGPDrawManager::ColorInvert((COLORREF)*this);
	*this = CBCGPColor(clr, a);
}

void CBCGPColor::ConvertToGrayScale(double dblLumRatio /*= 1.0*/)
{
	if (IsNull())
	{
		return;
	...
	*this = CBCGPColor(RGB(gray, gray, gray), a);
}

void CBCGPColor::Simplify()
{
	if (IsNull() || a == 0)
	{
		return;
	...
	if (a != 1 && (r > a || g > a || b > a))
	{
		const double coeff = a / max(r, max(g, b));

		clr = RGB(
			(BYTE)(min(r * coeff, a) * 255),
			(BYTE)(min(g * coeff, a) * 255),
			(BYTE)(min(b * coeff, a) * 255));
	}
...
////////////////////////////////////////////////////////////////////////////////////
// CBCGPTransformMatrix

void CBCGPTransformMatrix::CreateTranslate(double x, double y)
{
	m_arMatrix.RemoveAll();
	m_arMatrix.Add(1);
	m_arMatrix.Add(0);
	m_arMatrix.Add(0);
	m_arMatrix.Add(1);
	...
	m_bCheckRelativeTranslate = (fabs(x) > 0.0 && fabs(x) <= 1.0) || (fabs(y) > 0.0 && fabs(y) <= 1.0);
}

void CBCGPTransformMatrix::CreateScale(double x, double y)
{
	m_arMatrix.RemoveAll();
	m_arMatrix.Add(x);
	m_arMatrix.Add(0);
	m_arMatrix.Add(0);
	m_arMatrix.Add(y == 0.0 ? x : y);
	m_arMatrix.Add(0);
	m_arMatrix.Add(0);
}

void CBCGPTransformMatrix::CreateRotate(double a, double x, double y)
{
	const double dblAngle = bcg_deg2rad(a);
	const double dblSin = sin(dblAngle);
	const double dblCos = cos(dblAngle);
	
	m_arMatrix.RemoveAll();
	m_arMatrix.Add(dblCos);
	m_arMatrix.Add(dblSin); 
	m_arMatrix.Add(-dblSin);
	m_arMatrix.Add(dblCos);
	m_arMatrix.Add(0); 
	m_arMatrix.Add(0);
	
	m_ptRotateCenter = CBCGPPoint(x, y);
}

void CBCGPTransformMatrix::CreateSkewX(double a)
{
	const double dblAngle = bcg_deg2rad(a);
	...
	m_arMatrix.Add(1);
	m_arMatrix.Add(0);
	m_arMatrix.Add(tan(dblAngle));
	m_arMatrix.Add(1);
}

void CBCGPTransformMatrix::CreateSkewY(double a)
{
	const double dblAngle = bcg_deg2rad(a);
...
CBCGPBrush::CBCGPBrush(const CBCGPBrush& brush)
{
	...
	m_extendMode		= brush.m_extendMode;

	m_TextureImage.Destroy();

	m_GradientStops.Append(brush.m_GradientStops);
	m_ptGradient1 = brush.m_ptGradient1;
	m_ptGradient2 = brush.m_ptGradient2;
	m_szGradientRadius = brush.m_szGradientRadius;
	m_arTransforms.RemoveAll();
	m_arTransforms.Append(brush.m_arTransforms);
}
...
void CBCGPBrush::CommonInit()
{
	...
	m_extendMode = BCGP_GRADIENT_EXTEND_MODE_CLAMP;
}
...
void CBCGPBrush::MakeLighter(double dblRatio)
{
	...
	for (int i = 0; i < (int)m_GradientStops.GetSize(); i++)
	{
		m_GradientStops[i].m_Color.MakeLighter(dblRatio);
	}
}
...
void CBCGPBrush::MakeDarker(double dblRatio)
{
	...
	for (int i = 0; i < (int)m_GradientStops.GetSize(); i++)
	{
		m_GradientStops[i].m_Color.MakeDarker(dblRatio);
	}
}
...
void CBCGPBrush::MakePale(double dblLum)
{
	...
	for (int i = 0; i < (int)m_GradientStops.GetSize(); i++)
	{
		m_GradientStops[i].m_Color.MakePale(dblLum);
	}
}

void CBCGPBrush::AdaptColors(COLORREF clrBase, COLORREF clrTone, BOOL bClampHue /*= TRUE*/)
{
	BOOL bNeedToDestroy = FALSE;

	if (m_color.AdaptColors(clrBase, clrTone, bClampHue))
	{
		bNeedToDestroy = TRUE;
	}
		
	if (m_colorGradient.AdaptColors(clrBase, clrTone, bClampHue))
	{
		bNeedToDestroy = TRUE;
	}
	...
	for (int i = 0; i < (int)m_GradientStops.GetSize(); i++)
	{
		if (m_GradientStops[i].m_Color.AdaptColors(clrBase, clrTone, bClampHue))
		{
			bNeedToDestroy = TRUE;
		}
	}
...
void CBCGPBrush::InvertColors()
{
	BOOL bNeedToDestroy = FALSE;
	
	if (!m_color.IsNull())
	{
		m_color.InvertColors();
		bNeedToDestroy = TRUE;
	}
	
	if (!m_colorGradient.IsNull())
	{
		m_colorGradient.InvertColors();
		bNeedToDestroy = TRUE;
	}

	for (int i = 0; i < (int)m_GradientStops.GetSize(); i++)
	{
		if (!m_GradientStops[i].m_Color.IsNull())
		{
			m_GradientStops[i].m_Color.InvertColors();
			bNeedToDestroy = TRUE;
		}
	}
...
void CBCGPBrush::ConvertToGrayScale(double dblLumRatio /*= 1.0*/)
{
	BOOL bNeedToDestroy = FALSE;
	
	if (!m_color.IsNull())
	{
		m_color.ConvertToGrayScale(dblLumRatio);
		bNeedToDestroy = TRUE;
	}
	
	if (!m_colorGradient.IsNull())
	{
		m_colorGradient.ConvertToGrayScale(dblLumRatio);
		bNeedToDestroy = TRUE;
	}
	...
	for (int i = 0; i < (int)m_GradientStops.GetSize(); i++)
	{
		if (!m_GradientStops[i].m_Color.IsNull())
		{
			m_GradientStops[i].m_Color.ConvertToGrayScale(dblLumRatio);
			bNeedToDestroy = TRUE;
		}
	}
...
void CBCGPBrush::Simplify()
{
	...
	for (int i = 0; i < (int)m_GradientStops.GetSize(); i++)
	{
		if (!m_GradientStops[i].m_Color.IsNull())
		{
			m_GradientStops[i].m_Color.Simplify();
			bNeedToDestroy = TRUE;
		}
	}
...
void CBCGPBrush::Mix(const CBCGPBrush& brOther, double ratio /* 0 - 1 */)
{
	ratio = bcg_clamp(ratio, 0., 1.);

	m_color.Mix(brOther.m_color, ratio);
	m_colorGradient.Mix(brOther.m_colorGradient, ratio);

	m_opacity = m_opacity * ratio + brOther.m_opacity * (1. - ratio);
}
...
void CBCGPBrush::CopyFrom(const CBCGPBrush& src)
{
	...
	m_GradientStops.RemoveAll();
	m_GradientStops.Append(src.m_GradientStops);

	m_ptGradient1 = src.m_ptGradient1;
	m_ptGradient2 = src.m_ptGradient2;

	m_szGradientRadius = src.m_szGradientRadius;
	m_arTransforms.RemoveAll();
...
{
	const int nStops = (int)m_GradientStops.GetSize();
	if (nStops > 0)
	{
		if (src.m_GradientStops.GetSize() != nStops)
		{
			return FALSE;
		}
		...
		{
			if (!m_GradientStops[i].CompareWith(src.m_GradientStops[i]))
			{
				return FALSE;
			}
		}
...
/*	TODO
		if (m_ptGradient1 != src.m_ptGradient1 || m_ptGradient2 != m_ptGradient2)
		{
			return FALSE;
		}
*/
...
void CBCGPBrush::SetLinearGradientStops(const CBCGPBrushGradientStops& stops, const CBCGPGradientPoint& pt1, const CBCGPGradientPoint& pt2, const CBCGPTransformsArray& arTransforms, BCGP_GRADIENT_EXTEND_MODE extendMode)
{
	if (m_pGM != NULL)
	{
		ASSERT_VALID(m_pGM);
	...
	m_gradientType = BCGP_GRADIENT_VERTICAL;

	m_GradientStops.RemoveAll();
	m_GradientStops.Append(stops);

	m_ptGradient1 = pt1;
	m_ptGradient2 = pt2;

	m_arTransforms.RemoveAll();
	m_arTransforms.Append(arTransforms);
...
void CBCGPBrush::SetRadialGradientStops(const CBCGPBrushGradientStops& stops, const CBCGPGradientPoint& ptCenter, const CBCGPGradientPoint& radius, const CBCGPGradientPoint& ptOffset, const CBCGPTransformsArray& arTransforms, BCGP_GRADIENT_EXTEND_MODE extendMode)
{
	if (m_pGM != NULL)
	{
		ASSERT_VALID(m_pGM);
	...
	m_gradientType = BCGP_GRADIENT_RADIAL_CENTER;
	
	m_GradientStops.RemoveAll();
	m_GradientStops.Append(stops);

	m_ptGradient1 = ptCenter;
	m_ptGradient2 = ptOffset;

	m_szGradientRadius = radius;
...
void CBCGPBrush::PrepareAlphaMode(double dblOpacity)
{
	if (dblOpacity < 0.0)
	{
		dblOpacity = m_opacity;
	}

	if (m_GradientStops.GetSize() == 0)
	{
		double dblColorOpacity = GetColor().a;
		if (dblColorOpacity >= 0.0 && dblColorOpacity < 1.0)
		{
			dblOpacity *= dblColorOpacity;
		}
		
		SetColor(CBCGPColor(1.0, 1.0, 1.0), dblOpacity);
	}
	else
	{
		for (int i = 0; i < (int)m_GradientStops.GetSize(); i++)
		{
			double dblOpacityCurr = dblOpacity;

			double dblColorOpacity = m_GradientStops[i].m_Color.a;
			if (dblColorOpacity >= 0.0 && dblColorOpacity < 1.0)
			{
				dblOpacityCurr *= dblColorOpacity;
			}

			m_GradientStops[i].m_Color = CBCGPColor(1.0, 1.0, 1.0, dblOpacityCurr);
		}
	}
}
...
void CBCGPGeometry::GeometryToPolygon(CArray<POINT, POINT>& arPointsGDI, CArray<INT, INT>& arPointsCount, BOOL bExtendedMappingMode, CDC* pDCPaint)
{
	UNREFERENCED_PARAMETER(bExtendedMappingMode);
	UNREFERENCED_PARAMETER(pDCPaint);

	arPointsGDI.RemoveAll();
	arPointsCount.RemoveAll();
}
...
{
	const double aRot = bcg_normalize_deg(m_dblRotationAngle);
	const double aRotR = bcg_deg2rad(aRot);

	CBCGPPoint pt2(m_Point);

	if (aRot != 0.0)
	{
		double ac = bcg_angle(m_Point.x - ptFrom.x, ptFrom.y - m_Point.y) + aRotR;
		double ad = bcg_distance(ptFrom, m_Point);
	...
	const double y2 = fabs(pt2.y) < (double)FLT_EPSILON ? 0.0 : (pt2.y);
	...
	if (aRot != 0.0)
	{
		const double ac = bcg_angle(ptCenter.x, -ptCenter.y) - aRotR;
		const double ad = bcg_distance(CBCGPPoint(), ptCenter);

		ptCenter.x = ad * cos(ac);
		ptCenter.y = -ad * sin(ac);
	}
...
void CBCGPStrokeStyle::AdjustDashCap(double dblWidth)
{
	if (dblWidth <= 2.5 && m_dashStyle == BCGP_DASH_STYLE_DOT && m_dashCap == BCGP_CAP_STYLE_FLAT)
	{
...
void CBCGPStrokeStyle::SetDashOffset(FLOAT dashOffset)
{
	if (dashOffset == m_dashOffset)
	{
		return;
	}
...
void CBCGPStrokeStyle::SetDashes(const CArray<FLOAT, FLOAT>& arDashes)
{
	Destroy();
...
void CBCGPStrokeStyle::SetStartCap(BCGP_CAP_STYLE style)
{
	if (m_startCap == style)
	{
		return;
...
void CBCGPStrokeStyle::SetEndCap(BCGP_CAP_STYLE style)
{
	if (m_endCap == style)
	{
		return;
...
void CBCGPStrokeStyle::SetDashCap(BCGP_CAP_STYLE style)
{
	if (m_dashCap == style)
	{
		return;
...
void CBCGPStrokeStyle::SetLineJoin(BCGP_LINE_JOIN lineJoin)
{
...
void CBCGPTextFormat::CopyFrom(const CBCGPTextFormat& from)
{
	...
	m_bAdjustLineSpacing =		from.m_bAdjustLineSpacing;
...
BOOL CBCGPTextFormat::CompareWith(const CBCGPTextFormat& src) const
{
	return 
		...
		m_bAdjustLineSpacing ==		src.m_bAdjustLineSpacing &&
...
void CBCGPTextFormat::SetAdjustLineSpacing(BOOL bSet)
{
	if (m_bAdjustLineSpacing != bSet)
	{
		Destroy();
		m_bAdjustLineSpacing = bSet;
	}
}
...
void CBCGPSplineGeometry::GeometryToPolygon(CArray<POINT, POINT>& arPointsGDI, CArray<INT, INT>& arPointsCount, BOOL bExtendedMappingMode, CDC* pDCPaint)
{
	CBCGPGeometry::GeometryToPolygon(arPointsGDI, arPointsCount, bExtendedMappingMode, pDCPaint);

	const CBCGPPointsArray& arPoints = GetPoints();
	int nCount = (int)arPoints.GetSize();
	if (nCount > 1)
	{
		nCount = (nCount - 1) / 3;
	...
	if (IsClosed() && nCount > 0)
	{
		...
		if (bExtendedMappingMode && pDCPaint != NULL)
		{
			pDCPaint->LPtoDP((LPPOINT)arPointsGDI.GetData(), (int)arPointsGDI.GetSize());
		}
		
		arPointsCount.Add((INT)arPointsGDI.GetSize());
	}
}
...
void CBCGPComplexGeometry::CopyFrom(const CBCGPComplexGeometry& src)
{
	Clear();	
	...
	const CObArray& arSegments = src.GetSegments();
	...
	for (int i = 0; i < arSegments.GetSize(); i++)
	{
		...
		CObject* pNewSegment = NULL;
		
		if (pSegment->IsKindOf(RUNTIME_CLASS(CBCGPLineSegment)))
		{
			pNewSegment = ((CBCGPLineSegment*)pSegment)->CreateCopy();
		}
		else if (pSegment->IsKindOf(RUNTIME_CLASS(CBCGPBezierSegment)))
		{
			pNewSegment = ((CBCGPBezierSegment*)pSegment)->CreateCopy();
		}
		else if (pSegment->IsKindOf(RUNTIME_CLASS(CBCGPArcSegment)))
		{
			pNewSegment = ((CBCGPArcSegment*)pSegment)->CreateCopy();
		}

		if (pNewSegment != NULL)
		{
			m_arSegments.Add(pNewSegment);
		}
	}
}

void CBCGPComplexGeometry::GeometryToPolygon(CArray<POINT, POINT>& arPointsGDI, CArray<INT, INT>& arPointsCount, BOOL bExtendedMappingMode, CDC* pDCPaint)
{
	CBCGPGeometry::GeometryToPolygon(arPointsGDI, arPointsCount, bExtendedMappingMode, pDCPaint);

	const CObArray& arSegments = GetSegments();
	if (arSegments.GetSize () == 0)
	{
		return;
	}

	CBCGPPoint ptCurr = GetStartPoint();
	arPointsGDI.Add(ptCurr);
	...
	for (int i = 0; i < arSegments.GetSize(); i++)
	{
		...
		if (pSegment->IsKindOf(RUNTIME_CLASS(CBCGPLineSegment)))
		{
			...
			arPointsGDI.Add(pLineSegment->m_Point);
		...
		else if (pSegment->IsKindOf(RUNTIME_CLASS(CBCGPArcSegment)))
		{
			...
			for (int a = 1; a < D - 1; a++)
			{
				...
				if (pt != arPointsGDI[arPointsGDI.GetSize () - 1])
				{
					arPointsGDI.Add(pt);
				}
			}
			...
			if (pt != arPointsGDI[arPointsGDI.GetSize () - 1])
			{
				arPointsGDI.Add(pArcSegment->m_Point);
			}
...
void CBCGPGeometryGroup::AddGeometries(const CArray<CBCGPGeometry*, CBCGPGeometry*>& arGeometries)
{
	m_arGeometries.Append(arGeometries);
}

void CBCGPGeometryGroup::AddGeometry(CBCGPGeometry* pGeometry)
{
	m_arGeometries.Add(pGeometry);	
}

void CBCGPGeometryGroup::Clear()
{
	m_arGeometries.RemoveAll();
...
void CBCGPGeometryGroup::GeometryToPolygon(CArray<POINT, POINT>& arPointsGDI, CArray<INT, INT>& arPointsCount, BOOL bExtendedMappingMode, CDC* pDCPaint)
{
	CBCGPGeometry::GeometryToPolygon(arPointsGDI, arPointsCount, bExtendedMappingMode, pDCPaint);

	for (int i = 0; i < (int)m_arGeometries.GetSize(); i++)
	{
		CBCGPGeometry* pGeometry = m_arGeometries[i];
		if (pGeometry == NULL)
		{
			continue;
		}
		...
		CArray<INT, INT> arCount;

		pGeometry->GeometryToPolygon(arPoints, arCount, bExtendedMappingMode, pDCPaint);
		
		if (arPoints.GetSize() > 0)
		{
			arPointsGDI.Append(arPoints);
			arPointsCount.Append(arCount);
		}
	}
}
...
void CBCGPPolygonGeometry::GeometryToPolygon(CArray<POINT, POINT>& arPointsGDI, CArray<INT, INT>& arPointsCount, BOOL bExtendedMappingMode, CDC* pDCPaint)
{
	CBCGPGeometry::GeometryToPolygon(arPointsGDI, arPointsCount, bExtendedMappingMode, pDCPaint);
