...
public:
	...
	void Reflection(const CBCGPPoint& pt)
	{
		x = 2.0 * pt.x - x;
		y = 2.0 * pt.y - y;
		z = 2.0 * pt.z - z;
...
public:
	...
	{
		CBCGPRect tmp;
		tmp.left   = bcgp_max(left  , rt.left  );
		tmp.right  = bcgp_min(right , rt.right );
		tmp.top    = bcgp_max(top   , rt.top   );
		tmp.bottom = bcgp_min(bottom, rt.bottom);
	...
	CBCGPRect operator | (const CBCGPRect& rt) const
	{
		if (IsValid())
		{
			...
			{
				CBCGPRect tmp;
				tmp.left   = bcgp_min(left  , rt.left  );
				tmp.right  = bcgp_max(right , rt.right );
				tmp.top    = bcgp_min(top   , rt.top   );
				tmp.bottom = bcgp_max(bottom, rt.bottom);
				return tmp;
			}
			else
			{
...
struct BCGCBPRODLLEXPORT CBCGPTransformMatrix
{
	...
	CBCGPTransformMatrix()
	{
		m_bCheckRelativeTranslate = FALSE;
		m_ptRotateCenter - CBCGPPoint(-DBL_MAX, -DBL_MAX);
	}
	
	CBCGPTransformMatrix& operator = (const CBCGPTransformMatrix& src)
	{
		m_arMatrix.RemoveAll();
		m_arMatrix.Append(src.m_arMatrix);
		
		m_ptRotateCenter = src.m_ptRotateCenter;
		m_bCheckRelativeTranslate = src.m_bCheckRelativeTranslate;
		return *this;
	}
	
	BOOL IsEmpty() const
	{
		return m_arMatrix.GetSize() == 0;
	}

	BOOL CompareWith(const CBCGPTransformMatrix& other)
	{
		if (m_arMatrix.GetSize() != other.m_arMatrix.GetSize() ||
			m_ptRotateCenter != other.m_ptRotateCenter ||
			m_bCheckRelativeTranslate != other.m_bCheckRelativeTranslate)
		{
			return FALSE;
		}

		const int nSize = (int)m_arMatrix.GetSize();
		if (nSize == 0)
		{
			return TRUE;
		}

		return	memcmp(m_arMatrix.GetData(), other.m_arMatrix.GetData(), nSize * sizeof(double));
	}
	
	CArray<double, double>	m_arMatrix;
	CBCGPPoint				m_ptRotateCenter;
	BOOL					m_bCheckRelativeTranslate;
};

typedef CArray<CBCGPTransformMatrix, const CBCGPTransformMatrix&> CBCGPTransformsArray;
...
public:
	...
	void SetTransform(const CBCGPTransformMatrix& transform)
	{
		if (!m_Transform.CompareWith(transform))
		{
			Destroy();
	...
	const CBCGPTransformMatrix& GetTransform() const
	{
		return m_Transform;
	}
	...
	virtual void GeometryToPolygon(CArray<POINT, POINT>& arPointsGDI, CArray<INT, INT>& arPointsCount, BOOL bExtendedMappingMode, CDC* pDCPaint);

protected:
	BCGP_FILL_MODE			m_FillMode;
	CBCGPTransformMatrix	m_Transform;
};
...
public:
	...
	void CopyFrom(const CBCGPPolygonGeometry& src)
	{
		Clear();
		
		m_arPoints.Copy(src.m_arPoints);
...
public:
	...
	CBCGPLineSegment* CreateCopy() const
	{
		return new CBCGPLineSegment(m_Point);
	}
...
public:
	...
	CBCGPBezierSegment* CreateCopy() const
	{
		return new CBCGPBezierSegment(m_Point1, m_Point2, m_Point3);
	}
...
public:
	...
	CBCGPArcSegment* CreateCopy() const
	{
		return new CBCGPArcSegment(m_Point, m_Radius, m_bIsClockwise, m_bIsLargeArc, m_dblRotationAngle);
	}
...
public:
	...
	void Clear();
	void CopyFrom(const CBCGPComplexGeometry& src);
...
class BCGCBPRODLLEXPORT CBCGPGeometryGroup : public CBCGPGeometry
{
	DECLARE_DYNAMIC(CBCGPGeometryGroup)
...
public:
	...
	CBCGPGeometryGroup(const CArray<CBCGPGeometry*, CBCGPGeometry*>& arGeometries)
	{
		m_arGeometries.Append(arGeometries);
	}

	virtual ~CBCGPGeometryGroup()
	{
		Clear();
	}
	
	void AddGeometries(const CArray<CBCGPGeometry*, CBCGPGeometry*>& arGeometries);
	void AddGeometry(CBCGPGeometry* pGeometry);
	void Clear();

	const CArray<CBCGPGeometry*, CBCGPGeometry*>& GetGeometries() const
	{
		return m_arGeometries;
	}
	
	virtual void GeometryToPolygon(CArray<POINT, POINT>& arPointsGDI, CArray<INT, INT>& arPointsCount, BOOL bExtendedMappingMode, CDC* pDCPaint);

protected:
	CArray<CBCGPGeometry*, CBCGPGeometry*> m_arGeometries;
};
...
public:
	...
	BOOL AdaptColors(COLORREF clrBase, COLORREF clrTone, BOOL bClampHue = TRUE);
	void InvertColors();
	...
	void Mix(const CBCGPColor& rOther, double ratio /* 0 - 1 */);
...
////////////////////////////////////////////////////////////////////////////////////
// CBCGPBrushGradientStop

struct BCGCBPRODLLEXPORT CBCGPBrushGradientStop
{
	CBCGPBrushGradientStop(const CBCGPColor& color = CBCGPColor::Black, double dblOffset = 0.0)
	{
		m_Color = color;
		m_dblOffset = dblOffset;
	}
	
	BOOL CompareWith(const CBCGPBrushGradientStop& src) const
	{
		return (m_Color == src.m_Color) && (m_dblOffset == src.m_dblOffset);
	}
...
typedef CArray<CBCGPBrushGradientStop, const CBCGPBrushGradientStop&>	CBCGPBrushGradientStops;

////////////////////////////////////////////////////////////////////////////////////
// CBCGPGradientPoint

struct BCGCBPRODLLEXPORT CBCGPGradientPoint
{
	CBCGPGradientPoint(double xIn = 0.0, double yIn = 0.0, BOOL bIsAbsoluteIn = FALSE)
	{
		x = xIn;
		y = yIn;
		bIsAbsolute = bIsAbsoluteIn;
	}
...
public:
	...
	{
		BCGP_GRADIENT_EXTEND_MODE_CLAMP = 0,
...
public:
	...
	{
		return m_color.IsNull() && m_colorGradient.IsNull() && m_TextureImage.IsEmpty() && m_GradientStops.GetSize() == 0;
	}
	...
	void AdaptColors(COLORREF clrBase, COLORREF clrTone, BOOL bClampHue = TRUE);
	void InvertColors();
	void ConvertToGrayScale(double dblLumRatio = 1.0);
	void Simplify();

	void Mix(const CBCGPBrush& brOther, double ratio /* 0 - 1 */);
	...
	void SetLinearGradientStops(const CBCGPBrushGradientStops& stops, const CBCGPGradientPoint& pt1, const CBCGPGradientPoint& pt2, 
		const CBCGPTransformsArray& arTransforms, BCGP_GRADIENT_EXTEND_MODE extendMode = BCGP_GRADIENT_EXTEND_MODE_CLAMP);

	void SetRadialGradientStops(const CBCGPBrushGradientStops& stops, const CBCGPGradientPoint& ptCenter, const CBCGPGradientPoint& radius, const CBCGPGradientPoint& ptOffset, 
		const CBCGPTransformsArray& arTransforms, BCGP_GRADIENT_EXTEND_MODE extendMode = BCGP_GRADIENT_EXTEND_MODE_CLAMP);
	
	const CBCGPBrushGradientStops& GetGradientStops() const
	{
		return m_GradientStops;
	}
	
	void PrepareAlphaMode(double dblDefaultOpacity);

	const CBCGPGradientPoint& GetGradientPoint1() const { return m_ptGradient1; };
	const CBCGPGradientPoint& GetGradientPoint2() const { return m_ptGradient2; };
	const CBCGPGradientPoint& GetGradientRadius() const { return m_szGradientRadius; }

	const CBCGPTransformsArray& GetGradientTransforms() const 
	{ 
		return m_arTransforms; 
	}

	BOOL IsTransformed() const { return m_bIsTransformed; }
	void SetTransformed(BOOL bSet = TRUE) { m_bIsTransformed = bSet; }

	BCGP_GRADIENT_EXTEND_MODE GetExtendMode() const { return m_extendMode; }
...
protected:
	...
	CBCGPBrushGradientStops		m_GradientStops;
	CBCGPGradientPoint			m_ptGradient1;
	CBCGPGradientPoint			m_ptGradient2;
	CBCGPGradientPoint			m_szGradientRadius;
	CBCGPTransformsArray		m_arTransforms;
	BOOL						m_bIsTransformed;
	BCGP_GRADIENT_EXTEND_MODE	m_extendMode;
};
...
public:
	...
	void SetDashes(const CArray<FLOAT, FLOAT>& arDashes);
	...
	void SetStartCap(BCGP_CAP_STYLE style);
	...
	void SetEndCap(BCGP_CAP_STYLE style);
	...
	void SetDashCap(BCGP_CAP_STYLE style);
	...
	void SetLineJoin(BCGP_LINE_JOIN lineJoin);
...
// Operations:
	...
	void AdjustDashCap(double dblWidth = 1.0);
...
public:
	...
	void SetAdjustLineSpacing(BOOL bSet = TRUE);

	BOOL IsAdjustLineSpacing() const
	{
		return m_bAdjustLineSpacing;
	}
...
public:
	enum BCGP_GRAPHICS_MANAGER
	{
		...
		BCGP_GRAPHICS_MANAGER_GDI_PLUS,
	};
...
public:
	...
	virtual void SetMask(const CBCGPGeometry& geometry, const CBCGPBrush& opacityBrush, const CBCGPTransformMatrix& transform = CBCGPTransformMatrix()) = 0;
	virtual void SetMask(const CBCGPGeometry& geometry, double opacity, const CBCGPTransformMatrix& transform = CBCGPTransformMatrix()) = 0;
	...
	virtual BOOL PopTransform() {	return FALSE; }
