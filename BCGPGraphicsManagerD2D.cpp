...
static inline double GetMatrixValue(const CArray<double, double>& arValues, int nIndex, double size = 0.0)
{
...
static inline FLOAT GetMatrixValueF(const CArray<double, double>& arValues, int nIndex, double size = 0.0)
{
	return (FLOAT)GetMatrixValue(arValues, nIndex, size);
}

static D2D1::Matrix3x2F CreateMatrix(const CArray<double, double>& arMatrix, const CBCGPSize& size = CBCGPSize())
{
	return D2D1::Matrix3x2F(
		GetMatrixValueF(arMatrix, 0), 
		GetMatrixValueF(arMatrix, 1),
		GetMatrixValueF(arMatrix, 2),
		GetMatrixValueF(arMatrix, 3),
		GetMatrixValueF(arMatrix, 4, size.cx),
		GetMatrixValueF(arMatrix, 5, size.cy));
}
...
BOOL CBCGPGraphicsManagerD2D::m_bD2DInitialized = FALSE;
BOOL CBCGPGraphicsManagerD2D::m_bD2DFailed = FALSE;
...
BOOL CBCGPGraphicsManagerD2D::m_bRectOffsets = TRUE;
...
BOOL CBCGPGraphicsManagerD2D::InitD2D(BCGP_D2D1_FACTORY_TYPE d2dFactoryType, BCGP_DWRITE_FACTORY_TYPE writeFactoryType)
{
	...
	{
		DWRITECREATEFACTORY pfDWriteCreateFactory = (DWRITECREATEFACTORY)::GetProcAddress(m_hinstDWriteDLL, "DWriteCreateFactory");
		if (pfDWriteCreateFactory != NULL)
		{
			hr = (*pfDWriteCreateFactory)(writeFactoryType, __uuidof(IDWriteFactory), (IUnknown**)&m_pWriteFactory);
		}
	}
	...
	CBCGPGraphicsManagerD2D* pGM = new CBCGPGraphicsManagerD2D();

	pGM->BindDC(&dc);

	if (pGM->m_bIsBindError)
	{
		m_bD2DFailed = TRUE;
	}
	else
...
static void SetupBrushWithGradientStops(CBCGPGraphicsManagerD2D* pGM, CBCGPBrush& brush, ID2D1Brush* pD2DBrush, const CBCGPRect& rect)
{
	BOOL bIsRadial = brush.GetGradientType() == CBCGPBrush::BCGP_GRADIENT_RADIAL_CENTER;
	...
	{
		ptFirst.x = rect.left + rect.Width() * brush.GetGradientPoint1().x / 100;
		ptFirst.y = rect.top + rect.Height() * brush.GetGradientPoint1().y / 100;
	}
	...
	{
		ptSecond.x = rect.left + rect.Width() * brush.GetGradientPoint2().x / 100;
		ptSecond.y = rect.top + rect.Height() * brush.GetGradientPoint2().y / 100;
	}
	...
	if (bIsRadial)
	{
		...
		radialGradientBrush->SetCenter(D2DPoint(pGM, ptFirst));
		
		CBCGPPoint ptRadius(brush.GetGradientRadius().x, brush.GetGradientRadius().y);
		...
		radialGradientBrush->SetRadiusX((float)ptRadius.x);
		radialGradientBrush->SetRadiusY((float)ptRadius.y);

		radialGradientBrush->SetGradientOriginOffset(D2DPoint(pGM, ptSecond - ptFirst));
	}
	else
	{
		ID2D1LinearGradientBrush* linearGradientBrush = (ID2D1LinearGradientBrush*)pD2DBrush;
		
		linearGradientBrush->SetStartPoint(D2DPoint(pGM, ptFirst));
		linearGradientBrush->SetEndPoint(D2DPoint(pGM, ptSecond));
	}

	if (!brush.IsTransformed())
	{
		...
		if (arTransforms.GetSize() > 0)
		{
			for (int i = 0; i < (int)arTransforms.GetSize(); i++)
			{
				...
				else if (fabs(ptRotateCenter.x) > 0 && fabs(ptRotateCenter.x) < 1. && 
					fabs(ptRotateCenter.y) > 0 && fabs(ptRotateCenter.y) < 1)
				{
				...
				{
					D2D1::Matrix3x2F m0;
					pD2DBrush->GetTransform(&m0);

					translate.CreateTranslate(ptRotateCenter.x, ptRotateCenter.y);
					pD2DBrush->SetTransform(CreateMatrix(translate.m_arMatrix) * m0);
				}
				
				D2D1::Matrix3x2F m1;
				pD2DBrush->GetTransform(&m1);
				
				pD2DBrush->SetTransform(CreateMatrix(t.m_arMatrix, t.m_bCheckRelativeTranslate ? rect.Size() : CBCGPSize()) * m1);
				brush.SetTransformed();
													...
													CBCGPBrush::BCGP_GRADIENT_TYPE gradientType)
{
	...
	{
		SetupBrushWithGradientStops(this, brush, pD2DBrush, CBCGPRect(ptFrom, ptTo));
		return pD2DBrush;
	}
...
ID2D1Brush* CBCGPGraphicsManagerD2D::PrepareBrush(CBCGPBrush& brush, const CBCGPRect& rectIn, CBCGPBrush::BCGP_GRADIENT_TYPE gradientType)
{
	...
	if (brush.GetGradientStops().GetSize() > 0)
	{
		SetupBrushWithGradientStops(this, brush, pD2DBrush, rect);
	}
	else if (brush.HasTextureImage())
	{
		if (!brush.IsWaterMarkImage())
		{
...
CBCGPGraphicsManagerD2D::~CBCGPGraphicsManagerD2D()
{
	...
	{
		D2D1_MATRIX_3X2_F* pMatrix = (D2D1_MATRIX_3X2_F*)m_lstTransforms.RemoveTail();
		if (pMatrix != NULL)
		{
			delete pMatrix;
		}
...
ID2D1RenderTarget* CBCGPGraphicsManagerD2D::GetCurrentRenderTarget()
{
...
CBCGPGraphicsManager* CBCGPGraphicsManagerD2D::CreateOffScreenManager(const CBCGPRect& rect, CBCGPImage* pImageDest)
{
	...
	if (!pGM->BeginDraw())
	{
		...
		TRACE0("CBCGPGraphicsManagerD2D::CreateOffScreenManager failed.\r\n");
	}
	...
	double lineWidth, const CBCGPStrokeStyle* pStrokeStyle)
{
	...
	m_pRenderTarget->DrawRectangle(D2DRect(this, rect, m_bRectOffsets), pD2DBrush, (float) lineWidth,
	...
	double lineWidth, const CBCGPStrokeStyle* pStrokeStyle)
{
	...
	{
		D2D1_RECT_F rect;
		pGeometry->GetBounds(D2D1::Matrix3x2F::Identity(), &rect);
...
void CBCGPGraphicsManagerD2D::ResetTransform()
{
	if (m_pRenderTarget == NULL)
	{
		return;
...
void CBCGPGraphicsManagerD2D::PushTransform()
{
	D2D1_MATRIX_3X2_F* pMatrix = new D2D1_MATRIX_3X2_F;

	m_pRenderTarget->GetTransform(pMatrix);

	m_lstTransforms.AddTail((LPVOID)pMatrix);
}

BOOL CBCGPGraphicsManagerD2D::PopTransform()
{
	if (m_lstTransforms.IsEmpty())
	{
		TRACE0("CBCGPGraphicsManagerD2D::PopTransform: transform is not pushed - call CBCGPGraphicsManagerD2D::PushTransform first\n");
		ASSERT(FALSE);
	...
	m_pRenderTarget->SetTransform(*pMatrix);
...
void CBCGPGraphicsManagerD2D::SetRotateTransform(double dblAngle, const CBCGPPoint& ptCenterIn)
{
	if (m_pRenderTarget == NULL)
	{
		return;
	...
	D2D1::Matrix3x2F m1;
	m_pRenderTarget->GetTransform(&m1);

	DWRITE_MATRIX transform = MakeRotateTransform(
		float(bcg_deg2rad(-dblAngle)), float(ptCenter.x), float(ptCenter.y));

	D2D1::Matrix3x2F m2;
	memcpy(&m2, &transform, sizeof(m2));

	m_pRenderTarget->SetTransform(m2 * m1);
}

void CBCGPGraphicsManagerD2D::SetScaleTransform(const CBCGPSize& size, const CBCGPPoint& ptCenter /*= CBCGPPoint(-1., -1.)*/)
{
	if (m_pRenderTarget == NULL)
	{
		return;
	...
	D2D1::Matrix3x2F m2 = ptCenter == CBCGPPoint(-1., -1.) ?
		D2D1::Matrix3x2F::Scale((float)size.cx, (float)size.cy) :
		D2D1::Matrix3x2F::Scale((float)size.cx, (float)size.cy, D2DPoint(this, ptCenter));
...
void CBCGPGraphicsManagerD2D::SetTranslateTransform(const CBCGPSize& size)
{
	if (m_pRenderTarget == NULL)
	{
		return;
	...
	D2D1::Matrix3x2F m2 = D2D1::Matrix3x2F::Translation((float)size.cx, (float)size.cy);
	
	m_pRenderTarget->SetTransform(m2 * m1);
}

void CBCGPGraphicsManagerD2D::SetMatrixTransform(const CArray<double, double>& arValues)
{
	if (m_pRenderTarget == NULL || arValues.GetSize() == 0)
	{
		return;
	}
	...
	D2D1::Matrix3x2F m2 = CreateMatrix(arValues);
	...
	const CBCGPBrush& foregroundBrush)
{
	...
	FLOAT lineSpacing = 0;
	FLOAT baseline = 0;
	BOOL bLineSpacingWasChanged = FALSE;

	if (textFormat.GetTextVerticalAlignment() == CBCGPTextFormat::BCGP_TEXT_ALIGNMENT_TRAILING)
	{
		if (!textFormat.IsWordWrap() && textFormat.GetDrawingAngle() == 0. && textFormat.IsAdjustLineSpacing())
		{
			pWriteTextFormat->GetLineSpacing(&lineSpacingMethod, &lineSpacing, &baseline);
			pWriteTextFormat->SetLineSpacing(DWRITE_LINE_SPACING_METHOD_UNIFORM, 0.0, baseline);
			bLineSpacingWasChanged = TRUE;
		}
	...
	if (bLineSpacingWasChanged)
	{
		pWriteTextFormat->SetLineSpacing(lineSpacingMethod, lineSpacing, baseline);
	}
}
...
LPVOID CBCGPGraphicsManagerD2D::CreateGeometry(CBCGPGeometry& geometry)
{
	...
	else if (geometry.IsKindOf(RUNTIME_CLASS(CBCGPPolygonGeometry)))
	{
		...
		geometrySink->BeginFigure(
			arPoints.GetSize() > 0 ? D2DPoint(this, arPoints[0]) : D2DPoint(this, CBCGPPoint(0., 0.)), 
			D2D1_FIGURE_BEGIN_FILLED);
	...
	{
		CBCGPGeometryGroup& geometryGroup = (CBCGPGeometryGroup&)geometry;
		
		const CArray<CBCGPGeometry*, CBCGPGeometry*>& arGeometries = geometryGroup.GetGeometries();
		const int nSize = (UINT)arGeometries.GetSize();

		if (nSize > 0)
		{
			CArray<ID2D1Geometry*, ID2D1Geometry*> arD2DGeometries;
			arD2DGeometries.SetSize(nSize);

			for (int i = 0; i < nSize; i++)
			{
				arD2DGeometries[i] = (ID2D1Geometry*)CreateGeometry(*arGeometries[i]);
			}

			ID2D1GeometryGroup* pGeometryGroup = NULL;

			m_pDirect2dFactory->CreateGeometryGroup(
				(geometryGroup.GetFillMode() == CBCGPGeometry::BCGP_FILL_MODE_ALTERNATE)? D2D1_FILL_MODE_ALTERNATE : D2D1_FILL_MODE_WINDING, 
				arD2DGeometries.GetData(), (UINT)nSize, &pGeometryGroup);
		
			pIGeometry = pGeometryGroup;
		}
	}

	if (pIGeometry != NULL)
	{
		if (!geometry.GetTransform().IsEmpty())
		{
			ID2D1TransformedGeometry* pTransformedGeometry = NULL;
			
			m_pDirect2dFactory->CreateTransformedGeometry(
				pIGeometry, 
				CreateMatrix(geometry.GetTransform().m_arMatrix),
				&pTransformedGeometry);

			if (pTransformedGeometry != NULL)
			{
				pIGeometry->Release();
				pIGeometry = pTransformedGeometry;
			}
		}
...
LPVOID CBCGPGraphicsManagerD2D::CreateBrush(CBCGPBrush& brushIn)
{
	...
	ID2D1RenderTarget* pRenderTarget = GetCurrentRenderTarget();
	...
	case CBCGPBrush::BCGP_GRADIENT_PIPE_VERTICAL:
		{
			...
			int nStops = (int) stops.GetSize();

			D2D1_GRADIENT_STOP* pD2DStops = NULL;

			if (nStops > 0)
			{
				pD2DStops = new D2D1_GRADIENT_STOP[nStops];

				for (int i = 0; i < nStops; i++)
				{
					pD2DStops[i].position = (FLOAT)stops[i].m_dblOffset;
					pD2DStops[i].color = D2DColor(stops[i].m_Color);
				}
			}
			else
			{
				...
				nStops = (bIsCenter || bIsPipe) ? 3 : 2;

				pD2DStops = new D2D1_GRADIENT_STOP[nStops];

				if (bIsCenter)
				{
					pD2DStops[0].position = 0.;
					pD2DStops[0].color = D2DColor(brush.GetGradientColor());

					pD2DStops[1].position = .5;
					pD2DStops[1].color = D2DColor(brush.GetColor());

					pD2DStops[2].position = 1.;
					pD2DStops[2].color = D2DColor(brush.GetGradientColor());
				}
				else if (bIsPipe)
				{
					pD2DStops[0].position = 0.;
					pD2DStops[0].color = D2DColor(brush.GetColor());
					...
					pD2DStops[1].position = brush.GetGradientType() == CBCGPBrush::BCGP_GRADIENT_PIPE_VERTICAL ?
						0.7f : 0.3f;
					pD2DStops[1].color = D2DColor(clrLight);

					pD2DStops[2].position = 1.;
					pD2DStops[2].color = D2DColor(brush.GetColor());
				}
				else
				{
					pD2DStops[0].position = 0.;
			...
			hr = pRenderTarget->CreateGradientStopCollection(
				...
				nStops,
				D2D1_GAMMA_2_2,
				(D2D1_EXTEND_MODE)brush.GetExtendMode(),
...
void CBCGPGraphicsManagerD2D::SetMask(const CBCGPGeometry& geometry, const CBCGPBrush& opacityBrushIn, const CBCGPTransformMatrix& transform)
{
	HRESULT hr = S_OK;
	...
	CBCGPBrush opacityBrush = opacityBrushIn;
	if (opacityBrush.GetGradientStops().GetSize() == 0)
	{
		double H,S,L;
		CBCGPDrawManager::RGBtoHSL(opacityBrush.GetColor(), &H, &S, &L);

		opacity = (L * opacityBrush.GetOpacity());
	}
	else
	{
		CBCGPRect bounds;
		GetGeometryBoundingRect(geometry, bounds);

		pD2DBrush = PrepareBrush(opacityBrush, bounds, opacityBrush.GetGradientType());
		if (pD2DBrush == NULL)
		{
			return;
		}
	...
	hr = m_pRenderTarget->CreateLayer(&m_pCurrMask);
	if (FAILED(hr))
	{
		TRACE1("CBCGPGraphicsManagerD2D::SetMask: CreateLayer Failed. %x\r\n", hr);
		return;
	}
	...
	params.contentBounds = D2DRect(this, CBCGPRect(-D2D1::FloatMax(), -D2D1::FloatMax(), D2D1::FloatMax(),  D2D1::FloatMax()));
	params.geometricMask = (ID2D1Geometry*)CreateGeometry((CBCGPGeometry&)geometry);
	params.maskAntialiasMode = D2D1_ANTIALIAS_MODE_PER_PRIMITIVE;
	params.maskTransform = transform.IsEmpty() ? D2D1::Matrix3x2F::Identity() : CreateMatrix(transform.m_arMatrix);
	params.opacity = (FLOAT)opacity;
	params.opacityBrush = pD2DBrush;
	params.layerOptions = D2D1_LAYER_OPTIONS_NONE;
	
	m_pRenderTarget->PushLayer(&params, m_pCurrMask);
}

void CBCGPGraphicsManagerD2D::SetMask(const CBCGPGeometry& geometry, double opacity, const CBCGPTransformMatrix& transform /*= CBCGPTransformMatrix()*/)
{
	HRESULT hr = S_OK;
...
void CBCGPGraphicsManagerD2D::ReleaseMask()
{
	if (m_pCurrMask != NULL)
	{
		if (m_pRenderTarget != NULL)
...
ID2D1Bitmap* CBCGPGraphicsManagerD2D::CreateImageFromHBITMAP(HBITMAP hBitmap, BOOL bIsIgnoreAlphaBitmap)
{
	...
	ID2D1RenderTarget* pRenderTarget = GetCurrentRenderTarget();
	if (pRenderTarget == NULL || m_pWicFactory == NULL)
	{
		ASSERT(FALSE);
		return NULL;
	}
	...
	pRenderTarget->CreateBitmapFromWicBitmap (pWICBitmap, 0, &pBitmap);
...
ID2D1StrokeStyle* CBCGPGraphicsManagerD2D::CreateStrokeStyle(const CBCGPStrokeStyle* pStyle, double lineWidth)
{
	...
	lineWidth *= min(m_sizeScaleTransform.cx, m_sizeScaleTransform.cy);
...
void CBCGPGraphicsManagerD2D::OnFillGeometryBevel(const CBCGPBrush& brFill, const CBCGPGeometry& shape, const CBCGPGeometry& shapeInternal)
{
	...
	pGeometryInternal->GetBounds(D2D1::Matrix3x2F::Identity(), &rect);
