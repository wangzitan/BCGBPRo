...
void CBCGPGraphicsManagerGDI::Clear(const CBCGPColor& color)
{
	...
	if (pWnd->GetSafeHwnd() != NULL)
	{
		...
		m_pDM->DrawRect(rectWindow, 
			color.IsNull() ? globalData.clrWindow : (COLORREF)color, (COLORREF)-1);
	}
...
void CBCGPGraphicsManagerGDI::FillRoundedRectangle(
	const CBCGPRoundedRect& rect, const CBCGPBrush& brush)
{
	if (m_pDM == NULL || brush.IsEmpty() || brush.GetOpacity() == 0.0)
	{
		return;
	}
...
void CBCGPGraphicsManagerGDI::FillEllipse(
	const CBCGPEllipse& ellipse, const CBCGPBrush& brush)
{
	if (m_pDM == NULL || m_pDCPaint == NULL || brush.IsEmpty() || brush.GetOpacity() == 0.0)
	{
		return;
	}
	...
	double lineWidth, const CBCGPStrokeStyle* pStrokeStyle)
{
	...
	else if (geometry.IsKindOf(RUNTIME_CLASS(CBCGPGeometryGroup)))
	{
		...
		{
			DrawGeometry(*arGeometries[i], brush, lineWidth, pStrokeStyle);
		}
	...
	const CBCGPBrush& foregroundBrush)
{
	...
	if (!textFormat.IsWordWrap() && dAngle != 0. && strText.Find(_T('\n')) < 0)
	{
		...
		{
			TEXTMETRIC tm;
			m_pDCPaint->GetTextMetrics(&tm);
			
			point.y += tm.tmDescent;
		}
	...
	else
	{
		...
		if (textFormat.IsAdjustLineSpacing())
		{
			...
			rect.OffsetRect(0, tm.tmDescent);
		}
...
LPVOID CBCGPGraphicsManagerGDI::CreateGeometry(CBCGPGeometry& geometry)
{
	...
	else if (geometry.IsKindOf(RUNTIME_CLASS(CBCGPRoundedRectangleGeometry)))
	{
		...
		hrgn = ::CreateRoundRectRgn(pt1.x, pt1.y, pt2.x, pt2.y, ptCorners.x, ptCorners.y);
			...
			geometry.IsKindOf(RUNTIME_CLASS(CBCGPSplineGeometry)) ||
			geometry.IsKindOf(RUNTIME_CLASS(CBCGPComplexGeometry)))
	{
		CArray<POINT, POINT> arPointsGDI;
		CArray<INT, INT> arPointsCount;
		geometry.GeometryToPolygon(arPointsGDI, arPointsCount, m_bExtendedMappingMode, m_pDCPaint);

		if (arPointsGDI.GetSize() > 0)
		{
			hrgn = ::CreatePolygonRgn((LPPOINT)arPointsGDI.GetData(), (int)arPointsGDI.GetSize(), 
	...
	else if (geometry.IsKindOf(RUNTIME_CLASS(CBCGPGeometryGroup)))
	{
		...
		if (arPointsGDI.GetSize() > 0)
		{
			hrgn = ::CreatePolyPolygonRgn((LPPOINT)arPointsGDI.GetData(), (LPINT)arPointsCount.GetData(), 
										(int)arPointsCount.GetSize(),
										(geometry.GetFillMode() == CBCGPGeometry::BCGP_FILL_MODE_ALTERNATE)
											? ALTERNATE
											: WINDING);
		}
...
LPVOID CBCGPGraphicsManagerGDI::CreateImage(CBCGPImage& image)
{
	...
	if (image.GetResourceID() != 0)
	{
		...
		if (svg.Load(image.GetResourceID()))
		{
...
void CBCGPGraphicsManagerGDI::CreateBrushImage(CBCGPBrush& brush)
{
	...
	const BOOL bIsGradientStops = stops.GetSize() > 0;
	...
	else if (bIsGradientStops)
	{
		gradient_count = (int)stops.GetSize();
		gradient = new XGradientStop[gradient_count];

		for (int i = 0; i < gradient_count; i++)
		{
			gradient[i] = XGradientStop((float)stops[i].m_dblOffset, stops[i].m_Color);
		}
	}
	else
	{
...
void CBCGPGraphicsManagerGDI::FillGradientByType(const CRect& rect, const CBCGPBrush& brush)
{
	if (rect.Width () == 0 || rect.Height () == 0 || 
		m_pDM == NULL || m_pDCPaint == NULL || brush.IsEmpty() || brush.GetOpacity() == 0.0)
	{
		return;
	}
