...
/////////////////////////////////////////////////////////////////////////////
// A helper class appears like CWaitCursor when a boolean flag is TRUE

class CConditionalWaitCursor
{
	// Construction/Destruction
public:
	CConditionalWaitCursor(BOOL bEnabled)
	{
		m_pWaitCursor = bEnabled ? new CWaitCursor() : NULL;
	}
	
	~CConditionalWaitCursor()
	{
...
CBCGPGridItem* CBCGPGridItem::HitTest (CPoint point, CBCGPGridRow::ClickArea* pnArea)
{
	...
	CRect rect = (GetMergedCells() != NULL) ? GetMergedRect() : GetRect();
	rect.right++;
...
void CBCGPGridItem::OnDrawValue (CDC* pDC, CRect rect)
{
	...
	if (!m_bInPlaceEdit)
	{
		...
		if (pWndList->m_clrCurrColorScale != (COLORREF)-1)
		{
			COLORREF clrContrastText = CBCGPDrawManager::GetContrastColor(pWndList->m_clrCurrColorScale, pDC->GetTextColor());
			if (clrContrastText != (COLORREF)-1)
			{
				COLORREF clrTextOld1 = pDC->SetTextColor(clrContrastText);
				if (clrTextOld == (COLORREF)-1)
				{
					clrTextOld = clrTextOld1;
				}
			}

			pWndList->m_clrCurrColorScale = (COLORREF)-1;
		}
	}
	...
	CBCGPRAII<DWORD> raiiMarkedIndexes(pWndList->m_nMarkedIndexes, m_nMarkedIndexes);
...
void CBCGPGridItem::OnDrawButton (CDC* pDC, CRect rect)
{
	...
	pDC->FillRect (rect, pWndList->IsVisualManagerStyle() ? &globalData.brBarFace : &globalData.brBtnFace);
	...
	if (m_dwFlags & BCGP_GRID_ITEM_HAS_LIST)
	{
		...
		{
			visualManagerMFC->OnDrawComboDropButton(pDC, rect, !m_bEnabled, m_bButtonIsDown, m_bButtonIsDown, &button);
		}
		else
		{
			if (!CBCGPVisualManager::GetInstance ()->DrawComboDropButtonWinXP(pDC, rect, !m_bEnabled, m_bButtonIsDown, FALSE))
			{
				rect.DeflateRect(1, 1);
				pDC->FillRect(rect, &globalData.brBtnFace);
				
				CBCGPMenuImages::Draw (pDC, CBCGPMenuImages::IdArowDownLarge, rect, m_bEnabled ? CBCGPMenuImages::ImageBlack : CBCGPMenuImages::ImageGray);
	...
	COLORREF clrTextOld = pDC->SetTextColor(pWndList->IsVisualManagerStyle() ? globalData.clrBarText : globalData.clrBtnText);
	...
	#else
		COLORREF colorBorder = globalData.IsHighContastMode () ? globalData.clrBtnDkShadow : globalData.clrHilite;
...
BOOL CBCGPGridItem::GetLabelBoundsRect (CDC* pDC, CRect& rectItem, BOOL bIncludeImage)
{
	...
	rect.DeflateRect (GRID_TEXT_MARGIN, GRID_TEXT_VMARGIN);
	rect.right = bcgp_min(rect.left + szText.cx, rect.right);
	...
	if (bIncludeImage)
	{
		rectItem.right = bcgp_min(rect.right + GRID_TEXT_VMARGIN, rectItem.right);
	}
	else
	{
...
BOOL CBCGPGridItem::OnEdit (LPPOINT)
{
	...
	if (m_pWndInPlace != NULL)
	{
		...
		bcgpGestureManager.SetGestureConfig(m_pWndInPlace->GetSafeHwnd(), gestureConfig);
		...
		if (m_dwFlags & BCGP_GRID_ITEM_HAS_LIST)
		{
			...
			if (m_pWndCombo != NULL)
			{
				...
				{
					OnAddComboBoxItem(nOptionIndex, m_lstOptions.GetNext(pos), GetOptionData(nOptionIndex));
				}
			}
		}
...
//****************************************************************************************
int CBCGPGridItem::OnAddComboBoxItem(int nIndex, const CString& strItem, DWORD_PTR dwItemData)
{
	UNREFERENCED_PARAMETER(nIndex);
	
	ASSERT_VALID(m_pWndCombo);
	
	int nComboIndex = m_pWndCombo->AddString(strItem);
	if (nComboIndex >= 0 && dwItemData != 0)
	{
		m_pWndCombo->SetItemData(nComboIndex, dwItemData);
	}
...
void CBCGPGridItem::AdjustInPlaceEditRect (CRect& rectEdit, CRect& rectSpin)
{
	...
	if (GetMergedCells () != NULL)
	{
		...
		{
			rectEdit.top = bcgp_min (rectEdit.bottom, pWndList->m_rectList.top);
		}
		if (rectEdit.bottom > pWndList->m_rectList.bottom)
		{
			rectEdit.bottom = bcgp_max (rectEdit.top, pWndList->m_rectList.bottom);
		}
	}
	else
	{
	...
	{
		rectEdit.right = bcgp_max (rectEdit.left, pWndList->m_rectList.right);
	}
...
CComboBox* CBCGPGridItem::CreateCombo (CWnd* pWndParent, CRect rect)
{
	...
	rect.right = bcgp_max (rect.left + m_sizeCombo.cx, rect.right);
...
BOOL CBCGPGridItem::OnDblClick (CPoint)
{
	...
	{
		CBCGPGridCtrl* pGrid = m_pGridRow->GetOwnerList();
		CConditionalWaitCursor wait(pGrid != NULL ? pGrid->IsWaitCursorEnabled() : TRUE);
...
CBCGPGridItem* CBCGPGridItem::GetMergedMainItem () const
{
	if (m_pMerged != NULL)
	{
		...
		const CBCGPGridItemID id = m_pMerged->GetMainItemID ();
		if (GetGridItemID() == id)
		{
			return (CBCGPGridItem*)this;
		}
...
void CBCGPGridItem::OnDrawDataColorScale (CDC* pDC, CRect rect)
{
	...
	CBCGPGridCtrl* pWndList = GetOwnerList ();
	ASSERT_VALID (pWndList);
	pWndList->m_clrCurrColorScale = clr;
}
//*******************************************************************************************
void CBCGPGridItem::OnDrawDataBar (CDC* pDC, CRect rect)
{
	...
	rectBar.right = rectBar.left + bcgp_max (1, rectBar.Width() * m_nDataBarPerc / 100);
...
BOOL CBCGPGridItem::GetDataColorScale(COLORREF& clrBackground, BOOL bSelection /*= FALSE*/)
{
	ASSERT_VALID (this);
...
void CBCGPGridColorItem::OnDrawValue (CDC* pDC, CRect rect)
{
	...
	const COLORREF color = m_Color == (COLORREF)-1 ? m_ColorAutomatic : m_Color;

	if (!OnDrawPaletteColorBox(pDC, rectColor, color, m_Color == (COLORREF)-1, TRUE, FALSE))
	{
		CBrush br(color);
		...
		const COLORREF clrBorder = pWndList->IsVisualManagerStyle() ? globalData.clrBarDkShadow : globalData.clrBtnDkShadow;
...
void CBCGPGridColorItem::AdjustInPlaceEditRect (CRect& rectEdit, CRect& rectSpin)
{
	...
	{
		rectEdit.right = bcgp_max (rectEdit.left, pWndList->GetListRect ().right);
	}
...
//********************************************************************************
void CBCGPGridColorItem::SetColors(const CArray<COLORREF, COLORREF>& colors)
{
	m_Colors.RemoveAll();
	m_Colors.Append(colors);
}
//********************************************************************************
void CBCGPGridColorItem::SetWebColors()
{
	SetColors(CBCGPColor::GetRGBArray());
	m_nColumnsNumber = BCGP_COLOR_COLUMNS;
}
//********************************************************************************
...
//*****************************************************************************************
void CBCGPGridCheckItem::Init()
{
	CBCGPGridItem::Init();
	m_bAllowEdit = FALSE;
}
//*****************************************************************************************
void CBCGPGridCheckItem::SetFlags()
{
	CBCGPGridItem::SetFlags();
	m_dwFlags = m_dwFlags & ~BCGP_GRID_ITEM_HAS_LIST;
}
//*****************************************************************************************
...
//******************************************************************************************
int CBCGPGridRow::InsertItem (int nPos, CBCGPGridItem *pNewItem)
{
	ASSERT_VALID(this);
	...
	const int nCount = (int) m_arrRowItems.GetSize();
	...
	{
		for (int i = nPos; i < (int) m_arrRowItems.GetSize(); i++)
		{
			CBCGPGridItem *pItem = m_arrRowItems [i];
			ASSERT_VALID (pItem);
			pItem->m_nIdColumn = i + 1;
		}
		
		pNewItem->SetOwnerRow(this);
		m_arrRowItems.InsertAt(nPos, pNewItem);
		pNewItem->m_nIdColumn = nPos;
...
//******************************************************************************************
BOOL CBCGPGridRow::RemoveItem (int nPos)
{
	ASSERT_VALID(this);
	...
	if (nPos >= 0 && nPos < nCount)
	{
		if (m_arrRowItems[nPos]->GetMergedCells() != NULL)
		{
			ASSERT(FALSE); // unmerge first
			return FALSE;
		}

		for (int i = nPos + 1; i < nCount; i++)
		{
			CBCGPGridItem *pItem = m_arrRowItems [i];
			ASSERT_VALID (pItem);
			pItem->m_nIdColumn = i - 1;
		...
		delete m_arrRowItems [nPos];
		m_arrRowItems.RemoveAt (nPos);
...
{
	return DoInsertSubItem(pItem, NULL, bRedraw, FALSE, TRUE);
}
//*******************************************************************************************
BOOL CBCGPGridRow::InsertSubItem (CBCGPGridRow* pItem, CBCGPGridRow* pInsertAfterItem, BOOL bRedraw)
{
	// If pInsertAfterItem is NULL insert as the last subitem
	
	return DoInsertSubItem(pItem, pInsertAfterItem, bRedraw, FALSE, FALSE);
}
//*******************************************************************************************
BOOL CBCGPGridRow::InsertFirstSubItem (CBCGPGridRow* pItem, BOOL bRedraw)
{
	return DoInsertSubItem(pItem, NULL, bRedraw, TRUE, FALSE);
}
//*******************************************************************************************
BOOL CBCGPGridRow::InsertLastSubItem (CBCGPGridRow* pItem, BOOL bRedraw)
{
	return DoInsertSubItem(pItem, NULL, bRedraw, FALSE, FALSE);
}
//*******************************************************************************************
BOOL CBCGPGridRow::DoInsertSubItem(CBCGPGridRow* pItem, CBCGPGridRow* pInsertAfterItem, BOOL bRedraw, BOOL bInsertFirst, BOOL bCheckNewItems)
{
	ASSERT_VALID (this);
	ASSERT_VALID (pItem);
	...
	if (pInsertAfterItem == NULL)
	{
		nPosInsertAfter = bInsertFirst ? nPosParent : nPosParent + nSubItemsCount;
	}
	else
	{
		...
		for (int i = 0; i < nSubItemsCount; i++)
		{
			...
			{
				nPosInsertAfter = pSubRow->GetRowId() + pSubRow->GetSubItemsCount(TRUE);
				nSubItemIndex = i;
				break;
			}
		}
...
//*******************************************************************************************
BOOL CBCGPGridRow::HasNextSibling(BOOL bVisibleOnly) const
{
	...
	const CBCGPGridRow* pRow = this;
	do
	{
		CBCGPGridRow* pNextSiblingRow = GetOwnerList()->GetRow(pRow->GetRowId() + pRow->GetSubItemsCount(TRUE) + 1);
		if (pNextSiblingRow != NULL)
		{
			if (bVisibleOnly && !pNextSiblingRow->IsItemVisible())
			{
				pRow = pNextSiblingRow;
				continue;
			}
...
CRect CBCGPGridRow::GetNameRect(int dx) const
{
	...
	int nLeftMargin = bcgp_max (m_pWndList->GetExtraHierarchyOffset(), HasExpandButton() ? m_pWndList->GetButtonWidth() : 0);
...
CRect CBCGPGridRow::GetExpandBoxRect(int dx) const
{
	...
	const int nExpandBoxWidth = bcgp_max(m_pWndList->GetExtraHierarchyOffset(), m_pWndList->GetButtonWidth());

	rectExpand.DeflateRect (dx, 0, 0, 0);
	rectExpand.right = bcgp_min (rectExpand.left + nExpandBoxWidth, rectExpand.right);
...
CRect CBCGPGridRow::GetCheckBoxRect(int dx) const
{
	...
	const int nExpandBoxWidth = bcgp_max(m_pWndList->GetExtraHierarchyOffset(), HasExpandButton() ? m_pWndList->GetButtonWidth() : 0);
	
	rectCheckBox.DeflateRect(dx + nExpandBoxWidth, 0, 0, 0);
	rectCheckBox.right = bcgp_min(rectCheckBox.left + m_pWndList->GetButtonWidth(), rectCheckBox.right);
	...
	if (m_Rect.PtInRect (point))
	{
		...
		else
		{
			if (HasValueField ())
			{
				...
				{
					CBCGPGridItem* pItem = GetMergedMainItem(m_arrRowItems [i]);
					ASSERT_VALID (pItem);
...
void CBCGPGridRow::Expand (BOOL bExpand)
{
	...
	CConditionalWaitCursor wait(m_pWndList != NULL ? m_pWndList->IsWaitCursorEnabled() : TRUE);
...
//*******************************************************************************************
void CBCGPGridRow::SetBoldFont(BOOL bSet /*= TRUE*/, BOOL bRedraw /*=FALSE*/)
{
	ASSERT_VALID (this);
	...
	if (m_bBold != bSet)
	{
		...
		if (bRedraw && m_pWndList->GetSafeHwnd () != NULL)
		{
			ASSERT_VALID (m_pWndList);
			m_pWndList->InvalidateRect (m_Rect);
		}
...
void CBCGPGridRow::Repos (int& y)
{
	...
	if (bShowItem)
	{
		...
		nRowHeight += (m_nLines - 1) * (nRowHeight - 2 * (m_pWndList->m_bIsPrinting ? ::MulDiv(TEXT_VMARGIN, nXMul, nXDiv) : GRID_TEXT_VMARGIN));
		...
		while (nPos != m_pWndList->GetColumnsInfo ().End ())
		{
			...
			if (bShowItem && bShowColumn)
			{
				...
				CRect rectItem (
					...
					y, 
					bcgp_min (nLeft + nWidth - 1, m_Rect.right),
					y + (IsGroup () ? nLargeRowHeight : nRowHeight));
...
void CBCGPGridRow::OnFillGroupBackground (CDC* pDC, CRect rectFill, BOOL bGroupUnderline)
{
	...
	{
		if (IsSelected() ? m_pWndList->m_ColorData.m_GroupSelColors.Draw (pDC, rectFill) : m_pWndList->m_ColorData.m_GroupColors.Draw (pDC, rectFill))
		{
			bCustomColors = TRUE;
		}
...
void CBCGPGridRow::OnDrawName (CDC* pDC, CRect rect)
{
	...
	if (!HasValueField () || bIsAutoGroup)
	{
		...
		rect.left = bcgp_min(rect.left, rect.right);
		...
		if (m_pWndList->m_bHighlightGroups && IsSelected ())
		{
			...
			rectFocus.right = 
				bcgp_min (rect.right, rectFocus.left + pDC->GetTextExtent (strName).cx);
...
void CBCGPGridRow::OnDrawItems (CDC* pDC, CRect rect)
{
	...
	HFONT hFontOld = m_bBold ? m_pWndList->SetBoldFont(pDC) : NULL;
	...
	for (int i = 0; i < m_arrRowItems.GetSize (); i++)
	{
		...
		if (!rectValue.IsRectEmpty ())
		{
			...
			rectClipItem.left = bcgp_max (m_pWndList->m_rectRowHeader.right, rectClipItem.left);
			
			// filterbar:
			if (m_pWndList->IsFilterBarEnabled())
			{
				rectClipItem.top = bcgp_max (m_pWndList->m_rectFilterBar.bottom, rectClipItem.top);
			}
				...
				!m_pWndList->GetColumnsInfo ().IsColumnFrozen (i))
			{
				...
				rectClipItem.left = bcgp_max (nFreezeOffset, rectClipItem.left);
...
void CBCGPGridRow::OnDrawExpandBox (CDC* pDC, CRect rect)
{
	...
	if (m_pWndList->m_bIsPrinting)
	{
		...
		int nMinSize = (szOne.cx != 0 && szOne.cy != 0) ?
			bcgp_min (rect.Width () / szOne.cx, rect.Height () / szOne.cy) : 0;
		int nBoxSize = bcgp_min (9, nMinSize); // in logical units
	...
	else
	{
		...
		else
		{
			...
			{
				clrText = m_pWndList->m_ColorData.m_clrText;
			}
		}
...
void CBCGPGridRow::OnDrawRowMarker (CDC* pDC, CRect rect)
{
	...
	int nBoxSize = bcgp_min (globalUtils.ScaleByDPI(9), rect.Width ());
	...
	if (m_pWndList->IsRowMarkerOnRowHeader())
	{
		if (m_pWndList->m_ColorData.m_HeaderSelColors.m_clrBackground != (COLORREF)-1)
		{
			clr = m_pWndList->m_ColorData.m_HeaderSelColors.m_clrBackground;
		}
	}
	else
	{
...
BOOL CBCGPGridRow::OnEdit (LPPOINT lptClick)
{
	...
	for (int i = 0; i < m_arrRowItems.GetSize (); i++)
	{
		...
		if (GetItemRect(pItem).PtInRect(*lptClick) && pItem->m_bEnabled)
		{
			m_bInPlaceEdit = pItem->OnEdit (lptClick);
...
BOOL CBCGPGridRow::OnClickValue (UINT uiMsg, CPoint point)
{
	...
	for (int i = 0; i < m_arrRowItems.GetSize (); i++)
	{
		...
		if (GetItemRect(pItem).PtInRect (point))
		{
...
//*******************************************************************************************
CRect CBCGPGridRow::GetItemRect(CBCGPGridItem* pItem) const
{
	ASSERT_VALID(pItem);
	return (pItem->GetMergedCells() != NULL) ? pItem->GetMergedRect() : pItem->GetRect();
}
//*******************************************************************************************
CBCGPGridItem* CBCGPGridRow::GetMergedMainItem(CBCGPGridItem* pItem) const
{
	if (pItem != NULL && pItem->GetMergedCells() != NULL)
	{
		CBCGPGridItem* pMergedItem = pItem->GetMergedMainItem();
		if (pMergedItem != NULL)
		{
									   ...
									   BOOL bIsGroupBox)
{
		...
		m_pWndList->m_bDragFromChooser) ) ) 
	{
		...
		{			
			...
			if (rect.Width() >= cx + nImageMargin + nArrow)
			{
				...
				{
					pt.x += bcgp_max(0, (rect.Width() - cxText - nArrow - cx) / 2);
				}
				else if (nAlign & HDF_RIGHT)
				{
					pt.x += bcgp_max(0, rect.Width() - cxText - nArrow - cx - 1);
				}

				if (nAlign & HDF_BITMAP_ON_RIGHT)
				{
 					pt.x = bcgp_min(pt.x + cxText, bcgp_max(pt.x, rect.right - nArrow - cx - nImageMargin));
				}
	...
	{
		BOOL bAllowSelectedHeader = 
			!m_pWndList->IsWholeRowSel() && (!m_pWndList->IsDisableWholeSelHeaderHighlighting() || !m_pWndList->IsWholeRowSelected()) ||
			m_pWndList->IsRowHeaderEnabled() && m_pWndList->IsAllSelected();
...
void CBCGPGridColumnsInfo::PrintColumn (CDC* pDC, int nColumn, CRect rectItem, CRect rectClipItem)
{
	...
	if (nColState != 0)
	{
		...
		rectArrow.bottom = bcgp_min (rectArrow.bottom, rectArrow.top + m_pWndList->GetPrintParams ().m_nRowHeight);
		int nMargin = ::MulDiv(5, nXMul, nXDiv);
		rectArrow.DeflateRect (nMargin, nMargin);
		rectArrow.left = bcgp_max (rectArrow.right - rectArrow.Height (), rectArrow.left);
...
void CBCGPGridColumnsInfo::OnColumnsOrderChanged ()
{
	...
	if (m_pWndList->GetSafeHwnd () != NULL)
	{
		m_pWndList->OnColumnsOrderChanged (); // Sends BCGM_GRID_HEADER_CHANGED
	}
}
//*******************************************************************************************
...
int CBCGPGridColumnsInfo::GetColumnWidth (int nColumn) const
{
	...
	if (nColumn >= 0 && nColumn < m_arrColumns.GetSize ())
	{
		...
		CalculateColumnWidth(nColumn, nSize, bAutoSize);
		if (IsAutoSize() && bAutoSize)
		{
			return nSize;
		}
...
void CBCGPGridColumnsInfo::Resize (int nTotalWidth, int nStartColumn)
{
	...
	int nPosColumn = Begin ();
	while (nPosColumn != End ())
	{
		int nColumn = Next (nPosColumn);
		if (nColumn == -1)
		{
			break; // no more visible columns
		}
		...
		if (pColumn->m_bVisible)
		{
			...
			if (CanChangeWidth (nColumn) && nCount > nPosStartColumn && !bAutoSize)
			{
				// Can sizing
			...
			while (nPos != End ())
			{
				...
				if (pColumn->m_bVisible)
				{
					...
					if (CanChangeWidth (nColumn) && nCount > nPosStartColumn && !bAutoSize)
					{
						...
						nSize = bcgp_max (nSize, nMin);
						
						pColumn->m_nAutoSize = (int) (k * nSize);
						pColumn->m_nAutoSize = bcgp_max (GetMinWidth (nColumn), pColumn->m_nAutoSize);
					}
					else
					{
						// Locked size
	...
	else
	{
		...
		const double dblScale = m_pWndList == NULL ? 1.0 : m_pWndList->GetScale();
		m_nTotalWidth = bcg_scale(nDefaultWidth, dblScale) + nHierarchyOffset;
	}
	...
	// returns FALSE, if width is not changing
{
	...
	if (m_bAutoSize)
	{
		...
		nWidth = bcgp_max (nMin, bcgp_min (nMax, nWidth));
	}
	else
	{
		int nMin = GetMinWidth (nColumn);
		nWidth = bcgp_max (nMin, nWidth);
	}
...
//*******************************************************************************************
void CBCGPGridColumnsInfo::CalculateColumnWidth(int nColumn, int& nWidth, BOOL &bAutoSize) const
{
	if (m_pWndList == NULL || nColumn < 0 || nColumn >= m_arrColumns.GetSize())
	{
		ASSERT(FALSE);
		return;
	}
	...
	if (!pColumn->m_bFixedSize && !pColumn->m_bLocked)
	{
		// Best fit column
...
void CBCGPGridColumnsInfoEx::ReposHeaderItems ()
{
	...
	const int nFreezeOffset = pWndList->m_rectList.left + GetFreezeOffset();
	...
	{
		CRect rectColumnHeader (0, 0, 0, 0);
		...
		for (int nLine = 0; nLine < m_nMultiLineCount; nLine++)
		{
			...
			{
				nMinLeft = bcgp_min (rectColumnHeader.left, nMinLeft);
				nMaxRight = bcgp_max (rectColumnHeader.right, nMaxRight);
			}
			...
			nMinTop = bcgp_min (nTop, nMinTop);
			nMaxBottom = bcgp_max (nBottom, nMaxBottom);
			...
			if (nIndex >= 0)
			{
				...
				if (IsFreezeColumnsEnabled() && nMinLeft < nFreezeOffset && !IsColumnFrozen(nColumn))
				{
					BOOL bFoundFrozenColumns = FALSE;

					CBCGPMergedHeaderItem* pMergedHeaderItem = DYNAMIC_DOWNCAST(CBCGPMergedHeaderItem, data.m_pItem);
					if (pMergedHeaderItem != NULL)
					{
						for (int j = 0; j < pMergedHeaderItem->m_arrColumns.GetSize(); j++)
						{
							if (IsColumnFrozen(pMergedHeaderItem->m_arrColumns[j]))
							{
								bFoundFrozenColumns = TRUE;
								break;
							}
						}
					}

					if (bFoundFrozenColumns)
					{
						data.m_bPartiallyFrozen = TRUE;
						if (data.m_rect.IsRectEmpty())
						{
							nMinLeft = nFreezeOffset;
							
							if (nMaxRight < nFreezeOffset)
							{
								nMaxRight = nFreezeOffset;
							}
						}
						else
						{
				...
				if (!data.m_rect.IsRectEmpty ())
				{
					nMinLeft = bcgp_min (data.m_rect.left, nMinLeft);
					nMaxRight = bcgp_max (data.m_rect.right, nMaxRight);
					nMinTop = bcgp_min (data.m_rect.top, nMinTop);
					nMaxBottom = bcgp_max (data.m_rect.bottom, nMaxBottom);
				}

				CRect rectItem = rectHeader;
				if (nMinLeft <= nMaxRight)
				{
					rectItem.left = bSaveLeft ? nMinLeft : data.m_rect.left;
...
void CBCGPGridColumnsInfoEx::GetHeaderTooltip (int nColumn, CPoint pt, CString& strText) const
{
		...
		m_layout.GetHeaderItem (nColumn, nLine, rectDummy, pItem))
	{
		...
		{
			strText = (pItem != NULL) ? pItem->m_strName : CString(GetColumnName (nColumn));
		}
	}
}
//******************************************************************************************
						 ...
						 BOOL bIsGroupBox)
{
	...
	for (int nLine = 0; nLine < m_nMultiLineCount; nLine++)
	{
		...
		//-----------------------------------------------
		// Special processing for the merged header items
		//-----------------------------------------------
		BOOL bIsPartiallyFrozenMergedItem = (IsFreezeColumnsEnabled() && m_layout.IsPartiallyFrozen(nColumn, nLine) && !IsColumnFrozen(nColumn) && pWndList->m_nHorzScrollOffset > 0);
		if (bIsPartiallyFrozenMergedItem)
		{
			const int nFreezeOffset = pWndList->m_rectList.left + GetFreezeOffset();

			CRect rectColumnClip = rect;
			if (nFreezeOffset > rectColumnClip.left)
			{
				rectColumnClip.left = bcgp_min (rectColumnClip.right, nFreezeOffset);
			}

			CRgn rgnClip;
			rgnClip.CreateRectRgnIndirect (&rectColumnClip);
			pDC->SelectClipRgn (&rgnClip);

			pWndList->OnFillHeaderBackground(pDC, pWndList->m_rectHeader);

			if (pWndList->m_rgnClipHeader.GetSafeHandle() != NULL)
			{
				pDC->SelectClipRgn(&pWndList->m_rgnClipHeader);
			}
			else
			{
...
//******************************************************************************************
BOOL CBCGPGridColumnsInfoEx::CHeaderLayout::IsPartiallyFrozen(int nCol, int nLine) const
{
	ASSERT(m_arrLayout.GetSize () == m_nLines * m_nColumns);
	...
	return m_arrLayoutData [nIndex].m_bPartiallyFrozen;
}
...
/////////////////////////////////////////////////////////////////////////////
// CBCGPFilterClearButton

CBCGPFilterClearButton::CBCGPFilterClearButton()
{
}

void CBCGPFilterClearButton::DoDrawItem(CDC* pDC, CRect rect, UINT /*uiState*/)
{
	...
	CBCGPEdit dummy;
	if (!CBCGPVisualManager::GetInstance ()->OnDrawBrowseButton (pDC, rect, &dummy, state, clrText))
	{
		return;
	}

#ifndef _BCGSUITE_
	if (CBCGPDrawManager::IsLightColor(clrText) && m_ImageClearLight.IsValid())
	{
		m_ImageClearLight.DrawEx(pDC, rect, 0, CBCGPToolBarImages::ImageAlignHorzCenter, CBCGPToolBarImages::ImageAlignVertCenter);
	}
	else
#endif
	{
		m_ImageClear.DrawEx(pDC, rect, 0, CBCGPToolBarImages::ImageAlignHorzCenter, CBCGPToolBarImages::ImageAlignVertCenter);
	}
}

BEGIN_MESSAGE_MAP(CBCGPFilterClearButton, CBCGPButton)
	ON_WM_CREATE()
END_MESSAGE_MAP()

int CBCGPFilterClearButton::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	if (CBCGPButton::OnCreate(lpCreateStruct) == -1)
		return -1;
	
	CBCGPLocalResource localRes;

	m_ImageClear.Load(globalData.Is32BitIcons() ? IDB_BCGBARRES_CLEAR32 : IDB_BCGBARRES_CLEAR);
...
#ifndef _BCGSUITE_
	m_ImageClear.CopyTo(m_ImageClearLight);
	m_ImageClearLight.AdaptColors(RGB(82, 11, 153), RGB(192, 192, 192), FALSE);
#endif
...
BOOL	CBCGPGridCtrl::m_bUseSystemFont = FALSE;
...
UINT BCGM_GRID_HEADER_CHANGED = ::RegisterWindowMessage (_T("BCGM_GRID_HEADER_CHANGED"));
...
UINT BCGM_GRID_FILTERBAR_CHANGED = ::RegisterWindowMessage (_T("BCGM_GRID_FILTERBAR_CHANGED"));
...
void CBCGPGridCtrl::InitConstructor ()
{
	...
	m_bDisableWholeSelHeaderHighlighting = FALSE;
	...
	m_nRecentSelectAction = TVC_UNKNOWN;
	...
	m_clrCurrColorScale = (COLORREF)-1;
}

CWnd* CBCGPGridCtrl::DoCreateOnPlaceHolder(CWnd* pParent, UINT nID, DWORD dwStyle, const CRect& rect, LPVOID /*lParam*/)
{
...
void CBCGPGridCtrl::SetScale(double dblScale, BOOL bNotifyParent, CPoint ptCenter)
{
	...
	if (bNotifyParent && GetSafeHwnd () != NULL)
	{
		...
		{
			pOwnerWnd->SendMessage(BCGM_GRID_SCALE_CHANGED, GetDlgCtrlID(), (LPARAM)this);
		}
	}
}
//******************************************************************************************
...
int CBCGPGridCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if (m_pToolTip->GetSafeHwnd () != NULL)
	{
		...
		if (m_bCellCustomToolTips)
		{
			m_pToolTip->AddTool (this, LPSTR_TEXTCALLBACK, &rectDummy, nIdToolTipItems);
		}
	}
...
LRESULT CBCGPGridCtrl::OnChangeVisualManager(WPARAM, LPARAM)
{
	...
	if (m_DefaultHeaderBtnImages.IsValid())
	{
		LoadHeaderBtnImages();
	}
...
//*****************************************************************************************
LRESULT CBCGPGridCtrl::OnGetGestureConfig(WPARAM, LPARAM)
{
	return 1L;	
}
//*****************************************************************************************
void CBCGPGridCtrl::AdjustLayout ()
{
	...
	if (m_nHorzScrollTotal > 0 || m_bScrollHorzShowAlways)
	{
		...
		szScroll.cx = bcgp_max (0, m_rectRowHeader.Width () + m_rectList.Width ());
		szScroll.cy = bcgp_max (0, szScroll.cy - cyScroll);
	}
	...
	if (m_pToolTip->GetSafeHwnd () != NULL)
	{
		...
		if (m_bCellCustomToolTips)
		{
			m_pToolTip->SetToolRect (this, nIdToolTipItems, m_rectList);
		}
	}
...
//******************************************************************************************
void CBCGPGridCtrl::OnAfterTrackHeader (int nColumn, int nNewWidth)
{
	UNREFERENCED_PARAMETER(nColumn);
	UNREFERENCED_PARAMETER(nNewWidth);
}
//******************************************************************************************
void CBCGPGridCtrl::SetRowHeight ()
{
	if (m_bIsPrinting)
	{
		...
		ASSERT(m_pPrintDC->m_hAttribDC != NULL);
		...
		HFONT hfontOld = SetCurrFont(m_pPrintDC);
		HFONT hfontOldAttrib = SetCurrFont(CDC::FromHandle(m_pPrintDC->m_hAttribDC));
		...
		::SelectObject(m_pPrintDC->GetSafeHdc(), hfontOld);
		::SelectObject(m_pPrintDC->m_hAttribDC, hfontOldAttrib);
	}
	else
	{
		CClientDC dc (this);
		...
		m_nBaseHeight = tm.tmHeight + 2 * GRID_TEXT_VMARGIN;
	...
	else
	{
		...
		for (i = 0; i < arrSort.GetSize (); i++)
		{
			...
			else
			{
				...
				for (int j = bcgp_max (0, nParentLevel + 1); j < nGroupCount; j++)
				{
					arrPositions [j] = posInserted;
				}
...
void CBCGPGridCtrl::CleanUpAutoGroups (CBCGPGridCtrl::AUTOGROUP_CLEANUP_MODE nMode)
{
	...
	if (nMode == AG_COPY_AUTOGROUPS)
	{
		...
		for (POSITION pos = m_lstAutoGroups.GetHeadPosition (); pos != NULL; )
		{
			...
			for (POSITION posChild = pRow->m_lstSubItems.GetHeadPosition (); posChild != NULL; )
			{
				CBCGPGridRow* pChildRow = pRow->m_lstSubItems.GetNext (posChild);
...
void CBCGPGridCtrl::CalcEditMargin ()
{
	...
	m_nEditTopMargin = bcgp_max(0, (int)(.5 + 0.5 * (m_nRowHeight - rectEdit.Height() + GRID_TEXT_VMARGIN)));
...
HFONT CBCGPGridCtrl::GetDefaultFont ()
{
	return CBCGPGridCtrl::m_bUseSystemFont ? 
		(HFONT) ::GetStockObject (SYSTEM_FIXED_FONT) : (HFONT) globalData.fontRegular.GetSafeHandle ();
}
//******************************************************************************************
...
void CBCGPGridCtrl::OnDraw(CDC* pDCPaint)
{
	...
	m_rectClip.top = bcgp_max (m_rectClip.top, m_rectList.top);
	...
	rectGroupByBox.bottom = bcgp_min (rectGroupByBox.top + m_nGroupByBoxHeight, rectClient.bottom);
...
void CBCGPGridCtrl::OnDrawList (CDC* pDC)
{
	...
	if (m_bVirtualMode)
	{
		CList<int,int> lstSelected;

		for (int nIndex = m_nVertScrollOffset / m_nRowHeight; nIndex < m_nVirtualRows; nIndex++)
		{
			//-----------------
			// Get virtual row:
			//-----------------
			...
			if (IsRowSelected(pRow->GetRowId()))
			{
				lstSelected.AddTail(nIndex);
			}
			else
			{
		...
		{
			CBCGPGridRow* pRow = GetVirtualRow(lstSelected.GetNext(pos));
			if (pRow == NULL)
			{
				break;
			}
	...
	{
		const CList<CBCGPGridRow*, CBCGPGridRow*>& lst = (!IsSortingMode () && !IsGrouping ()) ? m_lstItems : m_lstTerminalItems;
		POSITION pos = m_nFirstVisibleItem == -1 ? lst.GetHeadPosition () : lst.FindIndex (m_nFirstVisibleItem);

		CList<CBCGPGridRow*, CBCGPGridRow*> lstSelected;
		...
		while (pos != NULL)
		{
			...
			if (IsRowSelected(pItem->GetRowId()))
			{
		...
		for (pos = lstSelected.GetHeadPosition(); pos != NULL;)
		{
			OnDrawRowHeaderItem(pDC, lst.GetNext(pos));
		}
	}
...
void CBCGPGridCtrl::OnDrawGroupByBox (CDC* pDC, CRect rect)
{
	...
	if (GetColumnsInfo ().GetGroupColumnCount () > 0)
	{
		...
		for (int i = 0; i < GetColumnsInfo ().GetGroupColumnCount (); i++)
		{
			...
			if (nCol != -1)
			{
				...
				rectItem.bottom = bcgp_min (rectItem.top + nItemHeight, rect.bottom);
				rectItem.right = bcgp_min (rectItem.left + nItemWidth, rect.right);

				if (m_bIsPrinting)
				{
					pDC->FillRect (rectItem, &brHeaderBG);
				}
	...
	else if (!m_bIsPrinting)
	{
		...
		rect.right = bcgp_min (rect.right, rect.left + nTextWidth + 2 * CALCULATED_TEXT_MARGIN);
...
void CBCGPGridCtrl::OnDrawHeader (CDC* pDC)
{
	...
	for (int i = 0; i <= GetColumnsInfo ().GetColumnCount(); i++)
	{
		...
		if (m_nHorzScrollOffset > 0)
		{
			...
			m_rgnClipHeader.CreateRectRgnIndirect (&rectColumnClip);
			pDC->SelectClipRgn (&m_rgnClipHeader);

			GetColumnsInfo ().DrawColumn (pDC, i, rectColumn, nMargin, nMargin);

			pDC->SelectClipRgn (NULL);
			m_rgnClipHeader.DeleteObject();
		}
		else
		{
			GetColumnsInfo ().DrawColumn (pDC, i, rectColumn, nMargin, nMargin);
...
BOOL CBCGPGridCtrl::DoDrawText (CDC* pDC, CString strText, CRect rect, UINT uiDrawTextFlags, LPRECT lpRectClip, BOOL bNoCalcExtent)
{
	...
	{
		HFONT hFontCur = NULL;
		if (pDC->IsPrinting() && pDC->m_hAttribDC != pDC->GetSafeHdc()) // print preview
		{
			hFontCur = (HFONT) ::GetCurrentObject(pDC->GetSafeHdc(), OBJ_FONT);
		}
		...
		if (hFontCur != NULL)
		{
			::SelectObject(pDC->GetSafeHdc(), hFontCur);
		}
	}
	...
	if ((uiFlags & DT_SINGLELINE) == 0 && !bNoCalcExtent) // multiline
	{
		...
		if (rect.Height () > rectTextExtent.Height ())
		{
			...
			if (uiFlags & DT_VCENTER)
			{
				...
				nDecreseBottom = bcgp_max (0, nDecreseBottom - nOffset);
			}
	...
	if (m_nMarkedIndexes != 0 && (uiFlags & DT_SINGLELINE) != 0 && m_paramsFilterBar.IsMarkText())
	{
		globalUtils.DrawTextWithHighlightedArea(pDC, strText, rectText, uiFlags, LOWORD(m_nMarkedIndexes), HIWORD(m_nMarkedIndexes), 
			m_paramsFilterBar.clrMarkBackground, m_paramsFilterBar.clrMarkText);
	}
	else
	{
		HFONT hFontC = (HFONT) ::GetCurrentObject(pDC->GetSafeHdc(), OBJ_FONT);
		CFont *pFt = CFont::FromHandle(hFontC);
		LOGFONT lf;
		if (pFt)
			pFt->GetLogFont(&lf);
...
void CBCGPGridCtrl::OnDrawHeaderItemBorder (CDC* pDC, CRect rect, int nCol)
{
	...
	BOOL bAllowSelectedHeader = !IsWholeRowSel() && (!IsDisableWholeSelHeaderHighlighting() || !IsWholeRowSelected()) || IsRowHeaderEnabled() && IsAllSelected();

	CBCGPGridHeaderParams params;
	params.m_nHeaderPart = CBCGPGridHeaderParams::HeaderTop;

	CRect rectInnerBorders (0, 0, 0, 0);
	CRect rectOuterBorders (0, 0, 0, 0);
...
void CBCGPGridCtrl::OnDrawRowHeaderItem (CDC* pDC, CBCGPGridRow* pItem)
{
	...
	// ----------------------------------
	{
		...
		const BOOL bIsSelected = IsRowSelected(pItem->GetRowId());
		if (bIsSelected && m_ColorData.m_HeaderSelColors.m_clrBackground != (COLORREF)-1)
		{
			rectItem.bottom++;
		}
		...
		params.m_rectInnerBorders = rectInnerBorders;
		params.m_rectOuterBorders = rectOuterBorders;
		params.m_nItemSelected = bIsSelected ? CBCGPGridHeaderParams::Selected : CBCGPGridHeaderParams::NotSelected;
...
void CBCGPGridCtrl::OnDrawTreeLines (CDC* pDC, CBCGPGridRow* pRow, CRect rect)
{
	...
	if (pRow->GetParent() != NULL)
	{
		...
		if (pRow->HasExpandButton())
		{
			...
			int nBoxSize = bcgp_min(globalUtils.ScaleByDPI(9), rectBox.Width());
		...
		BOOL bFilter = IsFilterEnabled() || IsFilterBarEnabled();
		...
		if (pRow->CBCGPGridRow::GetHierarchyLevel() <= nLevels)
		{
			...
			if (pRow->HasNextSibling(bFilter))
			{
				pDC->MoveTo (x, y2);
		...
		while (pParentRow != NULL && pParentRow->GetParent() != NULL)
		{
			...
			{
				if (pParentRow->HasNextSibling(bFilter))
				{
...
void CBCGPGridCtrl::DrawHeaderItem (CDC* pDC, CRect rect, CBCGPHeaderItem* pHeaderItem)
{
	...
	if (bDrawText)
	{
		...
		DrawHeaderItemText(pDC, rect, pHeaderItem);
...
void CBCGPGridCtrl::PrintHeaderItem (CDC* pDC, CRect rectItem, CRect rectClipItem, CBCGPHeaderItem* pHeaderItem)
{
	...
	PrintHeaderItemText(pDC, rectItem, rectClipItem, pHeaderItem);
	...
	{
		CRect rectArrow = rectItem;
		rectArrow.bottom = bcgp_min (rectArrow.bottom, rectArrow.top + GetPrintParams ().m_nRowHeight);
		int nMargin = ::MulDiv(5, nXMul, nXDiv);
		rectArrow.DeflateRect (nMargin, nMargin);
		rectArrow.left = bcgp_max (rectArrow.right - rectArrow.Height (), rectArrow.left);
...
//****************************************************************************************
void CBCGPGridCtrl::DrawHeaderItemText(CDC* pDC, CRect rect, CBCGPHeaderItem* pHeaderItem)
{
	ASSERT_VALID(this);
...
//****************************************************************************************
void CBCGPGridCtrl::PrintHeaderItemText(CDC* pDC, CRect rectItem, CRect rectClipItem, CBCGPHeaderItem* pHeaderItem)
{
	ASSERT_VALID(this);
	...
	case CBCGPGridHeaderParams::HeaderTopLeft:
		...
		//----------------------
		{
			...
			if (m_bHeaderSelectAllMarker)
			{
				...
				int nMin = bcgp_min (rectMarker.Width (), rectMarker.Height ());
				nMin = bcgp_min (nMin, GetButtonWidth () - nPadding);
...
void CBCGPGridCtrl::OnDrawCheckBox (CDC* pDC, CRect& rect, int nState, BOOL bHighlighted, BOOL bPressed, BOOL bEnabled, UINT nHorzAlign, UINT nVertAlign)
{
	...
	const int dx = bcgp_max(0, (int)(.5 + .5 * (rect.Width() - sizeCheckBox.cx)));
	const int dy = bcgp_max(0, (int)(.5 + .5 * (rect.Height() - sizeCheckBox.cy)));
...
void CBCGPGridCtrl::TrackHeader (int nOffset)
{
	...
	else
	{
		...
		if (bShowDragContext)
		{
			...
			nNewWidth = bcgp_max (m_nBaseHeight, nNewWidth);
...
{
	CConditionalWaitCursor wait(IsWaitCursorEnabled());
...
//****************************************************************************************
BOOL CBCGPGridCtrl::LoadHeaderBtnImages()
{
	CBCGPLocalResource locaRes;
	
	m_DefaultHeaderBtnImages.Clear();
	m_DefaultHeaderBtnImages.SetImageSize(CSize(12, 12));

	if (globalData.Is32BitIcons())
	{
		if (!m_DefaultHeaderBtnImages.Load(IDB_BCGBARRES_GRID_FILTER_ICONS32))
		{
			return FALSE;
		}

#ifndef _BCGSUITE_
		m_DefaultHeaderBtnImages.AdaptColors(RGB(92, 92, 92), IsVisualManagerStyle() ? globalData.clrBarText : globalData.clrBtnText);
#endif
		globalUtils.ScaleByDPI(m_DefaultHeaderBtnImages);
	}
	else
	{
		if (!m_DefaultHeaderBtnImages.Load(IDB_BCGBARRES_GRID_FILTER_ICONS))
		{
			return FALSE;
		}
	}

	RedrawWindow(GetHeaderRect());
	return TRUE;
}
//****************************************************************************************
...
{
	bNoCase = FALSE;
	clrMarkBackground = CLR_NONE;
	clrMarkText = CLR_NONE;
...
UINT CBCGPGridFilterEdit::OnGetDlgCode()
{
	return DLGC_WANTALLKEYS | DLGC_HASSETSEL;
}

BOOL CBCGPGridFilterEdit::PreTranslateMessage(MSG* pMsg)
{
	if (pMsg->message == WM_KEYDOWN)
	{
		if (globalUtils.ProcessCtrlEditAccelerators(this, (UINT)pMsg->wParam, TRUE))
		{
			return TRUE;
		}
...
void CBCGPGridCtrl::EnableFilterBar (BOOL bEnable, LPCTSTR lpszPrompt, BOOL bCaseSensitive,
									 const CList<int, int>* pListOfColumnIndexes, BOOL bUpdate,
									 COLORREF clrMarkBackground, COLORREF clrMarkText)
{
	if (bEnable)
	{
	...
	m_paramsFilterBar.clrMarkBackground = clrMarkBackground;
	m_paramsFilterBar.clrMarkText = clrMarkText;
	...
	if (bEnable)
	{
		...
		{
			m_btnFilterClear.Create(_T(""), WS_CHILD | WS_VISIBLE, CRect(0, 0, 0, 0), this, ID_FILTERBAR_BUTTON);
		}
	}
	else
	{
...
CWnd* CBCGPGridCtrl::OnCreateFilterBarCtrl (int nColumn, LPCTSTR lpszPrompt)
{
	...
	pEdit->SetVerticalAlignment(TA_CENTER);
...
BOOL CBCGPGridCtrl::IsItemFilteredByFilterBar (CBCGPGridItem* pItem, int /*nColumn*/, const CString& strFilter)
{
	...
	if (m_paramsFilterBar.bNoCase)
	{
		...
		nIndex = strItem.Find (strFilter);
	}
	else
	{
		nIndex = pItem->GetLabel ().Find(strFilter);
	}
	...
	if (m_paramsFilterBar.IsMarkText())
	{
		pItem->UpdateFilterMarkedIndexes(nIndex, strFilter.GetLength());
	}
...
void CBCGPGridCtrl::OnFilterBarChanged (int nColumn, const CString& strNewSearch)
{
	...
	if (bChanged)
	{
		...
		if (pOwnerWnd->GetSafeHwnd() != NULL)
		{
			pOwnerWnd->SendMessage(BCGM_GRID_FILTERBAR_CHANGED, nColumn, (LPARAM)this);
		}
	}
}
//****************************************************************************************
...
void CBCGPGridCtrl::OnFilterBarUpdate (int nColumn)
{
	...
	if (m_paramsFilterBar.IsMarkText() && !IsVirtualMode())
	{
		const CList<CBCGPGridRow*, CBCGPGridRow*>& lst = (!IsSortingMode () && !IsGrouping ()) ? m_lstItems : m_lstTerminalItems;
								   ...
								   CPoint pt)
{
	...
	{
		CRect rectHeaderColumn (0, 0, 0, 0);
...
void CBCGPGridCtrl::AdjustFilterBarCtrls ()
{
	...
	if (m_btnFilterClear.GetSafeHwnd () != NULL)
	{
		...
		rectLeft.right = bcgp_min (rectLeft.left + m_rectRowHeader.Width (), rectWorkArea.right);
		...
		else
		{
			...
			m_btnFilterClear.SetWindowPos (NULL, rectLeft.left, rectLeft.top, rectLeft.Width () - 1, rectLeft.Height () - 1,
	...
	for (int iColumn = 0; iColumn < GetColumnsInfo ().GetColumnCount (); iColumn++)
	{
		...
		if (pCtrl->GetSafeHwnd () != NULL)
		{
			...
			int nFreezeOffset = (GetColumnsInfo ().IsFreezeColumnsEnabled () &&
 			...
 			rect.left = bcgp_max (rect.left, rectWorkArea.left + nFreezeOffset);
 			rect.right = bcgp_min (rect.right - 1, rectWorkArea.right);
...
void CBCGPGridCtrl::OnDrawHeaderMenuButton (CDC* pDC, CRect rect, int nColumn, BOOL bSortArrow)
{
	...
	{			
		...
		CPoint pt = rect.TopLeft ();
		pt.x ++;
		pt.y += bcgp_max (0, (rect.Height () - sizeImage.cy) / 2);
...
CRect CBCGPGridCtrl::GetHeaderMenuButtonRect (CRect rectItem, int nColumn) const
{
	...
	{
		rectBtn = rectItem;
		rectBtn.left = bcgp_max (rectBtn.right - GetButtonWidth (), rectBtn.left);
	}
...
{ 
	...
	if (var1.vt == var2.vt)
	{
		...
		case VT_UI2:
#if _MSC_VER >= 1500
			return DoCompareItems ((unsigned short) var1, (unsigned short) var2);
#endif
		case VT_I2:
...
CString CBCGPGridCtrl::GetGroupName (int nGroupCol, CBCGPGridItem* pItem)
{
	...
	CString strGroup;
	strGroup.Format (_T("%s: %s"), (LPCTSTR)strColumn, (LPCTSTR)strValue);
...
int CBCGPGridCtrl::GetGroupColumnRect (int nPos, CRect& rect, CDC* pDC)
{
	...
	for (int i = 0; i < GetColumnsInfo ().GetGroupColumnCount (); i++)
	{
		...
		if (nCol != -1)
		{
			...
			rectItem.bottom = bcgp_min (rectItem.top + nItemHeight, rectGroupByBox.bottom);
			rectItem.right = bcgp_min (rectItem.left + nItemWidth, rectGroupByBox.right);
...
BOOL CBCGPGridCtrl::IsColumnSelected (int nColumn, BOOL bAllItemsSelected) const
{
	...
	for (POSITION pos = m_lstSel.GetHeadPosition (); pos != NULL;)
	{
		...
		if (nPos != -1 && nPosLeft != -1 && nPosRight != -1 &&
			bcgp_min (nPosLeft, nPosRight) <= nPos && bcgp_max (nPosLeft, nPosRight) >= nPos)
		{
			if (!bAllItemsSelected)
			{
				return TRUE;
...
//******************************************************************************************
BOOL CBCGPGridCtrl::IsWholeRowSelected () const
{
	const int nFirstColumn = GetColumnsInfo ().GetFirstVisibleColumn ();
	const int nLastColumn = GetColumnsInfo ().GetLastVisibleColumn ();
...
void CBCGPGridCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	case CBCGPGridRow::ClickExpandBox:
		ExpandRow(pCurSel, !pCurSel->IsExpanded());
		break;
	...
	case CBCGPGridRow::ClickValue:
		if (pItem == NULL)
		{
			// Translate a click to the row
			pCurSel->OnClickValue(WM_LBUTTONDOWN, point);
			return;
		}
	...
	case WM_MOUSEMOVE:
		if (pMsg->wParam == 0)	// No buttons pressed
		{
			...
			if (m_bCellCustomToolTips)
			{
				...
				if (HitTest(ptCursor, id, pItem) != NULL)
				{
					GetItemCustomTooltip(id, strToolTip, strDescr);
					bIsCustomToolTip = !strToolTip.IsEmpty();

					if (m_idLastToolTip != id)
					{
						m_pToolTip->Pop();
...
void CBCGPGridCtrl::OnMeasureListRect (CRect& rect)
{
	...
	int nMultiLineHeightDelta = 0;
	if (!IsReportCtrl())
	{
		nMultiLineHeightDelta = m_nMultiLineExtraRowsCount * (m_nRowHeight - 2 * GRID_TEXT_VMARGIN);
	}
	...
	rect.bottom = rect.top + 
		...
		nItemsCount * m_nRowHeight + nPreviewHeightTotal + nMultiLineHeightDelta;
}
//******************************************************************************************
void CBCGPGridCtrl::SetScrollSizes ()
{
	...
	else
	{
		...
		m_nVertScrollOffset = bcgp_max (0, bcgp_min (m_nVertScrollOffset, 
	...
	m_nHorzScrollOffset = bcgp_min (bcgp_max (0, m_nHorzScrollOffset), 
...
int CBCGPGridCtrl::CalcExtraHeightTotal ()
{
	...
	if (!IsSortingMode () && !IsGrouping ())
	{
			...
			nIndex++)
		{
			...
			{
				const int nHeight = pItem->m_Rect.Height();
				const int nDefaultHeight = pItem->IsGroup() ? m_nLargeRowHeight : m_nRowHeight;
				if (nHeight > nDefaultHeight)
				{
					nPreviewHeightTotal += nHeight - nDefaultHeight;
				}
			}
		}
	}
...
//******************************************************************************************
BOOL CBCGPGridCtrl::OnGestureEventPan(const CPoint& ptFrom, const CPoint& ptTo, CSize& sizeOverPan)
{
	CBCGPPopupMenu* pMenuActive = CBCGPPopupMenu::GetActiveMenu ();
	if (pMenuActive != NULL && !IsPlacedOnActiveMenu())
	{
		pMenuActive->SendMessage(WM_CLOSE);
		return TRUE;
	}
	...
	const int nVertOffset = ptFrom.y - ptTo.y;
	const int nHorzOffset = ptFrom.x - ptTo.x;

	if (abs(nVertOffset) > abs(nHorzOffset))
	{
		if (m_wndScrollVert.DoGesturePan(ptFromScreen, ptToScreen))
		{
			return FALSE;
		}

		int nVertScrollOffset = m_nVertScrollOffset + nVertOffset;
		const int nVertMax = m_nVertScrollTotal - m_nVertScrollPage + 1;

		if (nVertScrollOffset < 0)
		{
			sizeOverPan.cy = -nVertScrollOffset;
			nVertScrollOffset = 0;
		}
		else if (nVertScrollOffset > nVertMax)
		{
			sizeOverPan.cy = nVertMax - nVertScrollOffset;
			nVertScrollOffset = nVertMax;
		}

		if (nVertOffset != m_nVertScrollOffset)
		{
			SetScrollPos(SB_VERT, nVertScrollOffset, FALSE);
			OnVScroll(SB_THUMBPOSITION, nVertScrollOffset, NULL);
		}
	}
	else
	{
		if (m_wndScrollHorz.DoGesturePan(ptFromScreen, ptToScreen))
		{
			return FALSE;
		}

		int nHorzScrollOffset = m_nHorzScrollOffset + nHorzOffset;
		const int nHorzMax = m_nHorzScrollTotal - m_nHorzScrollPage + 1;
		
		if (nHorzScrollOffset < 0)
		{
			sizeOverPan.cx = -nHorzScrollOffset;
			nHorzScrollOffset = 0;
		}
		else if (nHorzScrollOffset > nHorzMax)
		{
			sizeOverPan.cx = nHorzMax - nHorzScrollOffset;
			nHorzScrollOffset = nHorzMax;
		}

		if (nHorzOffset != m_nHorzScrollOffset)
		{
			SetScrollPos(SB_HORZ, nHorzScrollOffset, FALSE);
			OnHScroll(SB_THUMBPOSITION, nHorzScrollOffset, NULL);
		}
	}

	return sizeOverPan != CSize(0, 0);
}
//******************************************************************************************
BOOL CBCGPGridCtrl::OnGestureEventZoom(const CPoint& ptCenterIn, double dblZoomFactor)
{
	CBCGPPopupMenu* pMenuActive = CBCGPPopupMenu::GetActiveMenu ();
	if (pMenuActive != NULL && !IsPlacedOnActiveMenu())
	{
	...
	CPoint ptCenter = ptCenterIn;
	ptCenter.x -= m_rectList.left;
	ptCenter.y -= m_rectList.top;
	
	SetScale (m_dblScale * dblZoomFactor, TRUE, ptCenter);
	return TRUE;
}
//******************************************************************************************
								  ...
								  BOOL bCalcVisibleOnly) const
{
	...
	if (m_bVirtualMode)
	{
		nCountFrom = bcgp_min (m_nVirtualRows, nCountFrom);
		nCountTo = bcgp_min (m_nVirtualRows, nCountTo);
...
int CBCGPGridCtrl::OffsetVisibleRow (int nStartFrom, int nOffsetCount, BOOL bDirForward) const
{
	...
	if (m_bVirtualMode)
	{
		...
		nResult = bcgp_max (0, nResult);
		return bcgp_min (m_nVirtualRows, nResult);
	}
...
void CBCGPGridCtrl::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	...
	if (nSBCode == SB_THUMBTRACK || nSBCode == SB_THUMBPOSITION)
	{
		...
		if (pMsg == NULL || pMsg->message != WM_MOUSEWHEEL)
		{
	...
	if (m_bVirtualMode)
	{
		int nItemsCount = bcgp_max (1, m_nVertScrollPage / m_nRowHeight);
	...
	CRect rectListScrollClip = m_rectList;
	rectListScrollClip.top += 1;

	ScrollWindow (0, dy, m_rectList, rectListScrollClip);
	...
	InvalidateRect(CRect(m_rectList.left, m_rectList.top, m_rectList.right, rectListScrollClip.top));
	UpdateWindow ();
}
//*******************************************************************************************
...
{
	if (m_nDraggedColumn >= 0 && !m_nDragHeaderScroll || m_bTracking || m_bSelecting && !m_bSelectingScroll)
	{
		return;
	}
	...
	{
	case SB_LINEUP:
		m_nHorzScrollOffset -= bcgp_max (1, m_nBaseHeight);
		break;
	...
	{
		rectClip.left = bcgp_min (m_rectRowHeader.right, rectClip.right);
	}
	if (GetColumnsInfo ().IsFreezeColumnsEnabled ())
	{
		rectClip.left = bcgp_min (rectClip.left + GetColumnsInfo().GetFreezeOffset(), rectClip.right);
	}
	...
	if (GetColumnsInfo().IsFreezeColumnsEnabled() && !m_rectTrackSel.IsRectEmpty())
	{
...
{
	return globalUtils.IsPlacedOnActiveMenu(this);
}
//******************************************************************************************
...
void CBCGPGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	...
	if (pSel->IsGroup ())
	{
		...
		{
			ExpandRow(pSel, !pSel->IsExpanded ());
		}
	}
...
{
	CBCGPRAII<UINT> raiiRecentAction(m_nRecentSelectAction, TVC_BYKEYBOARD);
				...
				if (m_bVirtualMode)
				{
					...
					if (pItemCurSel->GetMergedCells () != NULL)
					{
						...
						if (pItemCurSel->GetMergedRange (range))
						{
							...
							{
								nPos = bcgp_max (nPosLeft, nPosRight);
							}
							else
							{
								nPos = -1;
			...
			if (m_nVertScrollPage != 0)
			{
				...
				if (pRowCurSel != NULL)
				{
					...
					CPoint point (rect.right - 1,
								  bcgp_max (rect.top + m_nVertScrollPage - m_nRowHeight,
									   rect.bottom + 1));
					...
					else
					{
						...
						id = CBCGPGridItemID (bcgp_min (nNextRow, nLastRow), idCurSel.m_nColumn);
					}
	...
	case VK_PRIOR:
		{
			...
			if (m_nVertScrollPage != 0)
			{
				...
				if (pRowCurSel != NULL)
				{
					...
					CRect rect = pRowCurSel->GetRect ();
					CPoint point (rect.right - 1,
								  bcgp_min (rect.bottom - m_nVertScrollPage + m_nRowHeight,
									   rect.top - 1));
					...
					else
					{
						...
						id = CBCGPGridItemID (bcgp_max (nPrevRow, nFirstRow), idCurSel.m_nColumn);
					}
...
//******************************************************************************************
void CBCGPGridCtrl::ExpandRow(CBCGPGridRow* pRow, BOOL bExpand)
{
	ASSERT_VALID(this);
	ASSERT_VALID(pRow);
...
//****************************************************************************************
BOOL CBCGPGridCtrl::OnDrawPaletteColorBox(CDC* pDC, const CRect rectColor, COLORREF color, BOOL bIsAutomatic, BOOL bIsSelected, BOOL bOnPopupMenu)
{
	ASSERT_VALID (this);
	...
	return pColorItem->OnDrawPaletteColorBox(pDC, rectColor, color, bIsAutomatic, bIsSelected, bOnPopupMenu);
}
//****************************************************************************************
...
void CBCGPGridCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	else if (m_bTracking)
	{
		...
		if (bUpdate)
		{
			...
			OnAfterTrackHeader (m_nTrackColumn, nNewWidth);
		}
	}
...
{
	if (m_bCellCustomToolTips == bEnable)
	{
		return;
	}

	m_bCellCustomToolTips = bEnable;
	...
	if (bEnable)
	{
		m_pToolTip->AddTool(this, LPSTR_TEXTCALLBACK, &m_rectList, nIdToolTipItems);
	}
	else
	{
		m_pToolTip->DelTool(this, nIdToolTipItems);
	}
}
//********************************************************************************
...
BOOL CBCGPGridCtrl::ProcessClipboardAccelerators (UINT nChar)
{
	...
	if (pSelItem == NULL || pSelItem->m_pWndInPlace->GetSafeHwnd () == NULL)
	{
		return FALSE;
	}
	...
	if (m_bIgnoreCtrlBtn && (::GetAsyncKeyState (VK_CONTROL) & 0x8000) && !bIsRAlt)
	{
		if (nChar == _T('A') || nChar == _T('C') || nChar == VK_INSERT || nChar == _T('V') || nChar == _T('X') || nChar == _T('Z') || nChar == _T('Y'))
		{
			return FALSE;
		}
	}

	if (m_bIgnoreShiftBtn && (::GetAsyncKeyState (VK_SHIFT) & 0x8000))
	{
	...
	return globalUtils.ProcessCtrlEditAccelerators(pSelItem->m_pWndInPlace, nChar, pSelItem->IsAllowEdit() && pSelItem->IsEnabled());
}
//*******************************************************************************************
...
BOOL CBCGPGridCtrl::IndexToOrder (CBCGPGridRange &rangeIndex) const
{
		...
		rangeIndex.m_nRight >= 0 && rangeIndex.m_nRight < nColumnCount)
	{
		...
		if (nPosLeft != -1 && nPosRight != -1)
		{
			rangeIndex.Set (bcgp_min (nPosLeft, nPosRight), rangeIndex.m_nTop, 
							bcgp_max (nPosLeft, nPosRight), rangeIndex.m_nBottom);
			return TRUE;
		}
	}
...
void CBCGPGridCtrl::SelectRange (const CBCGPGridRange &range, BOOL bSelect, BOOL bRedraw)
{
	...
	CBCGPGridRange rangeNormalized (
			bcgp_min (range.m_nLeft, range.m_nRight),
			bcgp_min (range.m_nBottom, range.m_nTop),
			bcgp_max (range.m_nLeft, range.m_nRight),
			bcgp_max (range.m_nBottom, range.m_nTop));
	...
	if (m_bVirtualMode)
	{
		...
		for (POSITION pos = m_CachedItems.m_lstCache.GetHeadPosition (); pos != NULL; )
		{
			...
			int nOffset = bcgp_max (0, rangeNormalized.m_nTop - cpi.nFirst);
							   ...
							   DWORD dwSelMode, BOOL bRedraw)
{
	...
	if (m_bWholeRowSel && m_bSingleSel)
	{
		CBCGPGridRow* pSelNew = GetCurSel ();
		OnChangeSelection (pSelNew, pOldSelectedItem);

		if (pOldSelectedItem != NULL)
		{
			ASSERT_VALID (pOldSelectedItem);

			pOldSelectedItem->OnKillSelection (pSelNew);
		...
		if (pSelNew != NULL)
		{
			ASSERT_VALID (pSelNew);

			pSelNew->OnSetSelection (pOldSelectedItem);

			if (bRedraw)
			{
				CRect rect = pSelNew->m_Rect;
				InvalidateRect (rect);
			}
		}
	}
									...
									const int nLastColumn, const int nLastRow)
{
    ...
    else
    {
		...
		{
			nLeftColumn = bcgp_min (m_idActive.m_nColumn, idItem.m_nColumn);
			nRightColumn = bcgp_max (m_idActive.m_nColumn, idItem.m_nColumn);
		}

		pRangeLast->Set (
			bRow ? nFirstColumn : nLeftColumn,
			bColumn ? nFirstRow : bcgp_min (m_idActive.m_nRow, idItem.m_nRow),
			bRow ? nLastColumn : nRightColumn,
			bColumn ? nLastRow : bcgp_max (m_idActive.m_nRow, idItem.m_nRow) );
...
//		int nPosRight2 = GetColumnsInfo ().IndexToOrder (pRange2->m_nRight);
		...
		{
			if (nPosLeft1 != -1 && bcgp_min (nPosLeft2, nPosRight2) < nPosLeft1)
			{
				pRange->m_nLeft = pRange2->m_nLeft;
			}

			if (nPosRight1 != -1 && bcgp_max (nPosLeft2, nPosRight2) > nPosRight1)
			{
				pRange->m_nRight = pRange2->m_nRight;
			}
		}

		int nTop = bcgp_min (pRange2->m_nTop, pRange2->m_nBottom);
		int nBottom = bcgp_max (pRange2->m_nTop, pRange2->m_nBottom);
										 ...
										 int nFirstItem, int nLastItem)
{
	...
	m_PrintParams.m_pageInfo.Init (		// set up printing range
		bcgp_min (nFirstItem, nLastItem), bcgp_max (nFirstItem, nLastItem),
...
void CBCGPGridCtrl::OnPrint(CDC* pDC, CPrintInfo* pInfo)
{
	...
	HFONT hfontOld = SetCurrFont (pDC);
	HFONT hfontOldAttrib = SetCurrFont (CDC::FromHandle (pDC->m_hAttribDC));
	pDC->SetTextColor (m_clrPrintText);
	pDC->SetBkMode (TRANSPARENT);
	...
	rectGroupByBox.bottom = bcgp_min (rectGroupByBox.bottom, pInfo->m_rectDraw.bottom);
	OnDrawGroupByBox (pDC, rectGroupByBox);
	...
	::SelectObject (pDC->GetSafeHdc (), hfontOld);
	::SelectObject (pDC->m_hAttribDC, hfontOldAttrib);
}
//*******************************************************************************************
void CBCGPGridCtrl::OnPrintPageHeader(CDC* pDC, CPrintInfo* pInfo)
{
	...
	if (OnGetPageHeaderRect (pDC, pInfo, rectHeader))
	{
		...
		{
			pInfo->m_rectDraw.top = bcgp_min (rectHeader.bottom, pInfo->m_rectDraw.bottom);
		}
	}
}
//*******************************************************************************************
void CBCGPGridCtrl::OnPrintPageFooter(CDC* pDC, CPrintInfo* pInfo)
{
	...
	if (OnGetPageFooterRect (pDC, pInfo, rectFooter))
	{
		...
		{
			pInfo->m_rectDraw.bottom = bcgp_max (rectFooter.top, pInfo->m_rectDraw.top);
		}
	}
}
//*******************************************************************************************
...
CRect CBCGPGridCtrl::OnGetFilterBarRect (CDC*, const CRect& rectDraw)
{
	...
	if (m_bFilterBar && !m_bIsPrinting)
	{
		...
		rect.bottom = rect.top + m_nBaseHeight + globalUtils.ScaleByDPI(2);
...
CRect CBCGPGridCtrl::OnGetRowHeaderRect (CDC*, const CRect& rectDraw)
{
 	...
 	if (m_bRowHeader)
	{
		...
		else
		{
			...
			{
				rect.bottom = 
					bcgp_max (rect.top + m_nRowHeight, rect.bottom - m_nGridFooterHeight);
			}
...
CRect CBCGPGridCtrl::OnGetGridRect (CDC*, const CRect& rectDraw)
{
	...
	else
	{
		...
		{
			rectList.bottom = 
				bcgp_max (rectList.top + m_nRowHeight, rectList.bottom - m_nGridFooterHeight);
		}
...
CRect CBCGPGridCtrl::GetGridHeaderRect () const
{
	...
	rectHeader.bottom = bcgp_min (rectHeader.top + m_nGridHeaderHeight, rectHeader.bottom);
	return rectHeader;
}
//****************************************************************************************
CRect CBCGPGridCtrl::GetGridFooterRect () const
{
	...
	rectFooter.bottom = bcgp_max (rectFooter.top + m_nGridFooterHeight, rectFooter.bottom);
...
int CBCGPGridCtrl::CalcPages (CDC* pDC, CPrintInfo* /*pInfo*/)
{
	...
	if (m_bVirtualMode)
	{
		...
		int nFirstInPage = bcgp_min (nLastVirtualRow, m_PrintParams.m_pageInfo.m_nFirstItem);
		const int nLast = bcgp_min (nLastVirtualRow, m_PrintParams.m_pageInfo.m_nLastItem);
		...
		do
		{
			...
			{
				int nPageItems = bcgp_min (nItems, nLast - nFirstInPage + 1);
	...
	while (pos != NULL && nFirstInPage + nTotal <= m_PrintParams.m_pageInfo.m_nLastItem)
	{
		...
		if (!pItem->m_Rect.IsRectEmpty ())
		{
			...
			if (pItem->m_Rect.bottom >= rectPage.bottom)
			{
				...
				rectPage.OffsetRect (0, pItem->m_Rect.top - rectPage.top);
...
int CBCGPGridCtrl::OnAddGridPageRowForPrint (CBCGPGridPage& page, CRect rectPage)
{
	...
	do
	{
		...
		if (bNextPage)
		{
			...
			{
				nActualPageWidth = bcgp_min (nHorzPage, nXLeft);
			}
...
//*******************************************************************************************
void CBCGPGridCtrl::OnMeasurePrintPageItems(CRect& rectItems)
{
	if (IsPrinting() && GetPrintDC()->GetSafeHdc() != NULL)
	{
		int nFirst = GetPrintParams().m_pageInfo.m_nFirstInPage;
		int nCount = GetPrintParams().m_pageInfo.m_nTotalInPage;
		
		int nPrintGridHeight = 0;
		
		if (IsVirtualMode())
		{
			nPrintGridHeight = 
				GetPrintParams().m_pageInfo.m_nGroupsInPage * GetPrintParams().m_nLargeRowHeight +
				GetPrintParams().m_pageInfo.m_nItemsInPage * GetPrintParams().m_nRowHeight;
		}
		else
		{
			const CList<CBCGPGridRow*, CBCGPGridRow*>& lst =
			...
			// Calculate the height of items on a page
			...
			for (int j = nFirst; pos != NULL && j < nFirst + nCount; j++)
			{
				...
				nPrintGridHeight += pItem->GetRect().Height();
			}
		}
		
		rectItems = GetPrintParams().m_rectList;
		rectItems.bottom = bcgp_min(rectItems.top + nPrintGridHeight, rectItems.bottom);
	}
	else
	{
		rectItems.SetRectEmpty();
	}
}
//*******************************************************************************************
void CBCGPGridCtrl::OnPrintHeader(CDC* pDC, CPrintInfo* /*pInfo*/)
{
	...
	while (nPos != GetColumnsInfo ().End ())
	{
		...
		if (nWidth > 0) // if column visible
		{
			...
			{
				rectClipItem.left = bcgp_max (rectHeader.left, rectClipItem.left);
			}
			if (rectItem.right > rectHeader.right)
			{
				rectClipItem.right = bcgp_min (rectHeader.right, rectClipItem.right);
			}
			rectClipItem.right = bcgp_min (m_PrintParams.m_rectList.left + m_PrintParams.m_pageInfo.m_nPageWidth, rectClipItem.right);
	...
	rectItems.left = bcgp_max (m_PrintParams.m_rectList.left - nHorzScrollOffset, rectHeader.left);
	rectItems.right = bcgp_min (nXLeft, rectHeader.right);
	rectItems.right = bcgp_min (m_PrintParams.m_rectList.left + m_PrintParams.m_pageInfo.m_nPageWidth, rectItems.right);
...
void CBCGPGridCtrl::OnPrintList(CDC* pDC, CPrintInfo* pInfo)
{
	...
	OnMeasurePrintPageItems(rectItems);
	...
	rectClipItems.right = bcgp_min (nXLeft, nPageRight);
	...
	if (!pItem->m_Rect.IsRectEmpty ())
	{
		...
		if (pItem->m_Rect.bottom >= m_PrintParams.m_rectList.top)
		{
			...
			if (GetColumnsInfo ().GetColumnCount (TRUE) > 0)
			{
				...
				if (nCol0Idx != -1)
				{
					...
					if (nCol0Width > 0)
					{
 						...
 						rectClipLeft.right = bcgp_min (rectClipLeft.right, nCol0Right);
					}
				}
			}
			...
			if (rectName.right > rectName.left)
			{
				...
				{
					pOldFont = pDC->SelectObject(CFont::FromHandle(m_hPrinterBoldFont));
				}
...
BOOL CBCGPGridCtrl::DragColumn (CPoint ptScreen)
{
	...
	else
	{
		...
		else if (bDragOverHeader)
		{
			m_nDragHeaderScroll = FALSE;
			UINT nScrollCode = 0;
			if (m_rectList.right - point.x < m_nRowHeight) 
			{
				// scroll right
				nScrollCode = SB_LINEDOWN;
				m_nDragHeaderScroll = TRUE;
			}
			else if (point.x - (m_rectList.left + 1) < m_nRowHeight)
			{
				// scroll left
			...
			if (m_nDragHeaderScroll && !IsColumnAutoSizeEnabled())
			{
				OnBeforeDragScroll();
				OnHScroll(nScrollCode, 0, NULL);
			}
			...
			else if (!CanDropColumn (nPosDrop, nPosDraggedColumn) || m_nDragHeaderScroll)
			{
...
BOOL CBCGPGridCtrl::StopDragColumn (CPoint point, BOOL bUpdate)
{
	...
	else
	{
		...
		if (bDragOverHeader)
		{
			if (m_nDragHeaderScroll)
			{
				// Auto scrolling happens - do nothing
				return FALSE;
			}
...
{
	CRect rectList = GetListRect();
	if (rectList.Height() > m_nRowHeight && rectList.Width() > m_nRowHeight)
	{
		int nOffset = 1;

		if (m_bSelecting && !m_bHeaderRowSelecting && !m_bHeaderColSelecting && !rectList.PtInRect(ptClient))
		{
		...
		else if (m_bHeaderRowSelecting && !m_rectRowHeader.PtInRect(ptClient))
		{
			ptClient.x = bcg_clamp(ptClient.x, m_rectRowHeader.left + nOffset, m_rectRowHeader.right - nOffset);
			ptClient.y = bcg_clamp(ptClient.y, m_rectRowHeader.top + nOffset, m_rectRowHeader.bottom - nOffset);
		}
		else if (m_bHeaderColSelecting && !m_rectHeader.PtInRect(ptClient))
		{
			ptClient.x = bcg_clamp(ptClient.x, m_rectHeader.left + nOffset, m_rectHeader.right - nOffset);
			ptClient.y = bcg_clamp(ptClient.y, m_rectHeader.top + nOffset, m_rectHeader.bottom - nOffset);
		}
	}
...
//*****************************************************************************
void CBCGPGridCtrl::OnColumnsOrderChanged ()
{
	ASSERT_VALID (this);
	...
	if (pOwner != NULL)
	{
		pOwner->SendMessage(BCGM_GRID_HEADER_CHANGED, GetDlgCtrlID(), 0);
	}
}
//*****************************************************************************
...
{
	return -1;	// the default value is CBCGPGridColumnsItem::m_nDefaultSize
}
//*****************************************************************************
	...
	while (nPos != GetColumnsInfo ().End ())
	{
		...
		if (bIsRangeBound || bInRange)
		{
			...
			if (pItem != NULL)
			{
				...
				CBCGPGridRange rangeCurr;
				if (pItem->GetMergedRange (rangeCurr))
				{
					// add once:
...
void CBCGPGridCtrl::RedrawMergedItems (CDC* pDC)
{
	...
	while (!m_lstMergedItemsToRedraw.IsEmpty())
	{
		...
		{
			CBCGPGridRow* pRow1 = pItem->GetParentRow();
			if (pRow1)
			{
				hFontOld =(HFONT) (pRow1->m_bBold ? ::SelectObject(pDC->GetSafeHdc(), GetBoldFont(TRUE).GetSafeHandle()) : NULL);
			}
			...
			{
				rectClipItem.top = bcgp_max (m_rectFilterBar.bottom + 1, rectClipItem.top);
			}
			...
			pMerged->SetNeedRedraw (FALSE);
			(HFONT) ::SelectObject(pDC->GetSafeHdc(), hFontOld);
			hFontOld = NULL;
		}
	}
}
...
void CBCGPGridCtrl::pfnCallbackExportItem (CBCGPGridItem* pItem, const CBCGPGridRange&, LPARAM lParam)
{
	...
	if (params != NULL)
	{
		...
		COLORREF clrText = pItem->m_clrText;
		COLORREF clrBackground = pItem->m_clrBackground;
		COLORREF clrColorScale = (COLORREF)-1;

		if (pItem->GetDataColorScale(clrColorScale) && clrColorScale != (COLORREF)-1)
		{
			COLORREF clrContrastText = CBCGPDrawManager::GetContrastColor(clrColorScale, clrText);
			if (clrContrastText != (COLORREF)-1)
			{
				clrText = clrContrastText;
			}

			clrBackground = clrColorScale;
		}

		CString strColors;
		if (clrText != (COLORREF)-1 && (params->dwFlags & EF_IncludeTextColor) != 0)
		{
			CString strColor;
		...
		if (clrBackground != (COLORREF)-1 && (params->dwFlags & EF_IncludeBackgroundColor) != 0)
		{
			CString strBgColor;
			strBgColor.Format(_T("background-color:#%02x%02x%02x;"), 
				GetRValue(clrBackground), GetGValue(clrBackground), GetBValue(clrBackground));
		...
		{
			strAttr.Format(_T(" style=\"%s\""), (LPCTSTR)strColors);
			strTD += strAttr;
		}
		strTD += _T(">");
...
BOOL CBCGPGridCtrl::OnQueryClearNonEmptyItem (const CBCGPGridItem* /*pItem*/) const
{
	...
	return (BCGPMessageBox(strPrompt, MB_OKCANCEL) == IDOK);
}
//*****************************************************************************
...
//****************************************************************************
LRESULT CBCGPGridCtrl::OnPrint(WPARAM wp, LPARAM lp)
{
...
LRESULT CBCGPGridCtrl::OnBCGUpdateToolTips (WPARAM wp, LPARAM)
{
	...
	if (nTypes & BCGP_TOOLTIP_TYPE_GRID)
	{
		...
		if (m_pToolTip->GetSafeHwnd () != NULL)
		{
			...
			m_pToolTip->AddTool (this, LPSTR_TEXTCALLBACK, &m_rectHeader, nIdToolTipHeader);
...
BOOL CBCGPGridCtrl::OnNeedTipText(UINT /*id*/, NMHDR* pNMH, LRESULT* /*pResult*/)
{
	...
	else if (pNMH->idFrom == nIdToolTipItems && m_bCellCustomToolTips)
	{
		CBCGPGridItem* pItem = NULL;
		...
		{
			CString strDescr;
			GetItemCustomTooltip(id, strTipText, strDescr);

			if (m_bToolTipCleared && id == m_idLastToolTip)
			{
				return FALSE;
			}
...
//****************************************************************************************
void CBCGPGridCtrl::GetItemCustomTooltip(const CBCGPGridItemID& id, CString& strText, CString& strDescription) const
{
...
BOOL CBCGPHeaderItemDragWnd::Create (CBCGPGridCtrl* pGrid, int nItem)
{
	...
	if (m_pWndGrid->m_bDragGroupItem || m_pWndGrid->m_bDragFromChooser)
	{
		...
		else
		{
			...
			rect.right = rect.left + bcgp_max (sizeText.cx + globalUtils.ScaleByDPI(20), nWidth);
		}
...
int CBCGPGridColumnListBox::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	int nHeight = bcgp_min (10, bcgp_max(m_Columns.GetColumnCount(), 5)) * nRowHeight;
	...
	for (int i = 0; i < m_Columns.GetColumnCount (); i++)
	{
		...
		nMaxColumnWidth = bcgp_max (nMaxColumnWidth,
			dc.GetTextExtent (strColumn).cx);

		if (!m_Columns.GetColumnVisible(i) && m_Columns.m_pWndList->CanShowColumnInColumnChooser(i))
		{
...
void CBCGPGridColumnChooser::UpdateList ()
{
	...
	{
		if (!m_wndList.m_Columns.GetColumnVisible(i) && m_pOwnerGrid->CanShowColumnInColumnChooser(i))
		{
...
void CBCGPGridColumnChooser::OnPaint() 
{
	...
	rect.top = bcgp_max (rect.top, rect.CenterPoint ().y - globalData.GetTextHeight () / 2);
...
BOOL CBCGPGridCache::SetCachedRow (int nId, CBCGPGridRow* pItem)
{
	...
	int nNewPageFirst = bcgp_max (0, nId - nNewPageSize);
	...
	for (POSITION pos = m_lstCache.GetHeadPosition (); pos != NULL; )
	{
		...
		{
			nNewPageFirst = bcgp_min (cpi.nFirst + cpi.nSize, nId);
