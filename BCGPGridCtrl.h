...
public:
	...
	void Set (const CBCGPGridItemID &id1, const CBCGPGridItemID &id2)
	{
		...
		m_nLeft = bcgp_min (id1.m_nColumn, id2.m_nColumn);
		m_nTop = bcgp_min (id1.m_nRow, id2.m_nRow);
		m_nRight = bcgp_max (id1.m_nColumn, id2.m_nColumn);
		m_nBottom = bcgp_max (id1.m_nRow, id2.m_nRow);
	}
	...
	void Normalize ()
	{
		CBCGPGridRange rangeNormalized (
			bcgp_min (m_nLeft, m_nRight),
			bcgp_min (m_nBottom, m_nTop),
			bcgp_max (m_nLeft, m_nRight),
			bcgp_max (m_nBottom, m_nTop));
...
public:
	...
	int  InsertItem (int nIndex, CBCGPGridItem *pItem);
	...
	BOOL InsertFirstSubItem (CBCGPGridRow* pItem, BOOL bRedraw = TRUE);
	BOOL InsertLastSubItem (CBCGPGridRow* pItem, BOOL bRedraw = TRUE);
	BOOL HasNonFilteredSubItems() const;
	BOOL HasNextSibling(BOOL bVisibleOnly = FALSE) const;
...
protected:
	...
	BOOL DoInsertSubItem(CBCGPGridRow* pItem, CBCGPGridRow* pInsertAfterItem, BOOL bRedraw, BOOL bInsertFirst, BOOL bCheckNewItems);

	CRect GetItemRect(CBCGPGridItem* pItem) const;
	CBCGPGridItem* GetMergedMainItem(CBCGPGridItem* pItem) const;
...
public:
	...
	void SetBoldFont(BOOL bSet = TRUE, BOOL bRedraw = FALSE);
...
public:
	...
	void UpdateFilterMarkedIndexes(int nIndex, int nLen)
	{
		m_nMarkedIndexes = MAKELONG(nIndex, nLen);
	}
...
public:
	...
	virtual BOOL GetDataColorScale(COLORREF& clrBackground, BOOL bSelection = FALSE);
...
protected:
	...
	virtual int OnAddComboBoxItem(int nIndex, const CString& strItem, DWORD_PTR dwItemData);
...
protected:
	...
	int				m_nDataIconPerc;
	DWORD			m_nMarkedIndexes;	// low word: start marked text, high word: marked text length
};
...
public:
	...
	{ 
		...
		UNREFERENCED_PARAMETER(rectColor);
		UNREFERENCED_PARAMETER(color);
		UNREFERENCED_PARAMETER(bIsAutomatic);
		UNREFERENCED_PARAMETER(bIsSelected);
		UNREFERENCED_PARAMETER(bOnPopupMenu);
...
{
	// Construction
	BCGP_GRID_FILTERBAR_PARAM (BOOL bCaseSensitive = FALSE) : 
		bNoCase (!bCaseSensitive),
	...
	BOOL IsMarkText() const
	{
		return clrMarkBackground != CLR_NONE;
	}
	
	// Attributes
	BOOL		bNoCase;
	COLORREF	clrMarkBackground;
	COLORREF	clrMarkText;
...
public:
	...
	virtual BOOL ChangeColumnOrder (int nNewPos, int nColumn);
...
protected:
	void CalculateColumnWidth(int nColumn, int& nWidth, BOOL &bAutoSize) const;
...
protected:
	...
	public:
		...
		void SetRectsEmpty ();
		BOOL IsPartiallyFrozen(int nCol, int nLine) const;
	...
	public:
		...
		struct HeaderLayoutData
		{
			...
			HeaderLayoutData& operator=(const HeaderLayoutData& rhs)
			{
				...
				m_rect = rhs.m_rect;
				m_pItem = rhs.m_pItem;
				m_bPartiallyFrozen = rhs.m_bPartiallyFrozen;
				return *this;
			}
...
class CBCGPFilterClearButton : public CBCGPButton
{
public:
	CBCGPFilterClearButton();
	virtual void DoDrawItem(CDC* pDC, CRect rect, UINT itemState);

protected:
	CBCGPToolBarImages	m_ImageClear;

#if (!defined _BCGSUITE_) && (!defined _BCGSUITE_INC_)
	CBCGPToolBarImages	m_ImageClearLight;
#endif
...
BCGCBPRODLLEXPORT extern UINT BCGM_GRID_HEADER_CHANGED;
...
BCGCBPRODLLEXPORT extern UINT BCGM_GRID_FILTERBAR_CHANGED;
...
public:
	...
	void EnableFilterBar (BOOL bEnable, LPCTSTR lpszPrompt, BOOL bCaseSensitive = FALSE,
		...
		COLORREF clrMarkBackground = CLR_NONE, COLORREF clrMarkText = CLR_NONE);
	...
	void SetDisableWholeSelHeaderHighlighting (BOOL bHighlight = TRUE)
	{
		m_bDisableWholeSelHeaderHighlighting = bHighlight;
	}

	BOOL IsDisableWholeSelHeaderHighlighting () const
	{
		return m_bDisableWholeSelHeaderHighlighting;
	}
	...
	void EnableCustomToolTips(BOOL bEnable = TRUE);

	BOOL IsCustomToolTipEnabled() const
	{
		return m_bCellCustomToolTips;
	}
	...
	BOOL EnableWaitCursor(BOOL bEnable)
	{
		BOOL bOld = m_bWaitCursor;
...
protected:
	...
	CBCGPInplaceToolTipCtrl	m_ToolTip;	// Cell inplace tooltip control
	CToolTipCtrl*			m_pToolTip; // Common tooltip used for header control and cells
	BOOL					m_bCellCustomToolTips;
	BOOL					m_bToolTipCleared;
	CBCGPGridItemID			m_idLastToolTip;
	...
	DWORD			m_nMarkedIndexes;		// option for DoDrawText method. Low word: start marked text, high word: marked text length
	...
	BOOL		m_bDisableWholeSelHeaderHighlighting; // If TRUE, do not highlight header when all items in row(column) are selected
	...
	UINT		m_nRecentSelectAction;	// TVC_UNKNOWN, TVC_BYMOUSE or TVC_BYKEYBOARD
	...
	CBCGPFilterClearButton		m_btnFilterClear;
...
public:
	...
	virtual void ExpandRow(CBCGPGridRow* pRow, BOOL bExpand = TRUE);
...
public:
	virtual BOOL IsReportCtrl() const { return FALSE; }
	...
	virtual void OnAfterTrackHeader (int nColumn, int nNewWidth);
	...
	virtual void OnMeasurePrintPageItems (CRect& rect);
	...
	virtual BOOL CanShowColumnInColumnChooser (int nColumn) const
	{
		UNREFERENCED_PARAMETER(nColumn);
		return TRUE;
	}	
...
protected:
	...
	virtual void DrawHeaderItemText(CDC* pDC, CRect rect, CBCGPHeaderItem* pHeaderItem);
	virtual void PrintHeaderItemText(CDC* pDC, CRect rect, CRect rectClip, CBCGPHeaderItem* pHeaderItem);
	...
	virtual void GetItemCustomTooltip(const CBCGPGridItemID& id, CString& strText, CString& strDescription) const;
...
protected:
	...
	afx_msg LRESULT OnGetGestureConfig(WPARAM, LPARAM);
	DECLARE_MESSAGE_MAP()
