...
//****************************************************************************************
BOOL CBCGPGridSerializeManager::IsSkipHiddenItemsOnCopyPaste() const
{
	ASSERT (m_pOwnerGrid != NULL);
	return m_pOwnerGrid->IsFilterEnabled() || m_pOwnerGrid->IsFilterBarEnabled();
}
//****************************************************************************************
BOOL CBCGPGridSerializeManager::PrepareDataFromSelection ()
{
	...
	for (i = 0; i < m_pOwnerGrid->GetSelectionCount (); i++)
	{
		...
		if (m_pOwnerGrid->GetSelection (i, rangeIndex) &&
			IndexToOrder (rangeIndex, rangeOrder))
		{
			CList<CBCGPGridRange, CBCGPGridRange&>	lstSelectionByRows;

			// When filtered - use list of visible rows
			if (IsSkipHiddenItemsOnCopyPaste())
			{
				BOOL bChanged = FALSE;
				const int nRowOffset = (rangeOrder.m_nTop != -1) ? rangeOrder.m_nTop - m_idRangesOffset.m_nRow: 0;
				for (int nRow = m_idRangesOffset.m_nRow + nRowOffset; nRow != -1 && nRow <= rangeOrder.m_nBottom; )
				{
					CBCGPGridRange rangeNextVisibleRow(rangeOrder.m_nLeft, nRow, rangeOrder.m_nRight, nRow);
					lstSelectionByRows.AddTail(rangeNextVisibleRow);
					
					int nNextVisibleRow = m_pOwnerGrid->OffsetVisibleRow(nRow + 1, 1, TRUE);
					if (nNextVisibleRow <= nRow)
					{
						bChanged = TRUE;
						break;
					}

					if (nNextVisibleRow != nRow + 1)
					{
						bChanged = TRUE;
					}

					nRow = nNextVisibleRow;
				}

				if (!bChanged)
				{
					lstSelectionByRows.RemoveAll(); // use original continuous one range
				}
			}

			if (lstSelectionByRows.IsEmpty())
			{
				lstSelectionByRows.AddTail(rangeOrder);
			}
			...
			for (POSITION posRange = lstSelectionByRows.GetHeadPosition(); posRange != NULL; nCount++)
			{
				rangeOrder = lstSelectionByRows.GetNext(posRange);

				CBCGPGridRange rangeRelative (
					nColOffset, 
					nRowOffset + nCount, 
					nColOffset + (rangeOrder.m_nRight - rangeOrder.m_nLeft), 
					nRowOffset + nCount + (rangeOrder.m_nBottom - rangeOrder.m_nTop));
										...
										BOOL bMove)
{
 	...
 	if (!m_bSkipData)
	{
		...
		for (i = 0; i < GetRangeCount (); i++)
		{
			...
			// When filtered - skip hidden rows
			if (IsSkipHiddenItemsOnCopyPaste())
			{
				rangeIndex.m_nBottom = m_pOwnerGrid->OffsetVisibleRow(rangeIndex.m_nBottom, 1, FALSE);
			}
