...
CBCGPBaseIntelliSenseLB::CBCGPBaseIntelliSenseLB () : m_pImageList (NULL), m_nLBHeight (0), 
											 m_pFont(NULL), 
											 m_bCloseOnCtrl(TRUE), m_bCloseOnShift(TRUE), m_bCloseOnAlt(TRUE)
{
...
BOOL CBCGPBaseIntelliSenseLB::PreTranslateMessage(MSG* pMsg) 
{
	...
	case WM_KEYDOWN:
		if (GetParent()->GetSafeHwnd() != NULL)
		{
			...
			if (bIsCtrlPressed && m_bCloseOnCtrl || bIsShiftPressed && m_bCloseOnShift || bIsAltPressed && m_bCloseOnAlt)
			{
