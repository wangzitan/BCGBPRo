...
BOOL CBCGPKeyMapDlg::OnInitDialog() 
{
...

	{
		...
		if (m_bEnablePrint)
		{
			...
			m_ButtonPrint.SetImage (globalData.Is32BitIcons () ? IDB_BCGBARRES_PRINT32 : IDB_BCGBARRES_PRINT, NULL);
		...
		m_ButtonCopy.SetImage (globalData.Is32BitIcons () ? IDB_BCGBARRES_COPY32 : IDB_BCGBARRES_COPY, NULL);
...
{	
	...
	strItem.Format (_T("%-30s\t%-20s\t%s"),
		(LPCTSTR)m_KeymapList.GetItemText (nItem, iColumnCommand),
		(LPCTSTR)strKeys,
		(LPCTSTR)m_KeymapList.GetItemText (nItem, iColumnDescr));
...
void CBCGPKeyMapDlg::OnInsertItem (CBCGPBaseRibbonElement* pElem, int nItem)
{
	...
	globalUtils.RemoveSingleAmp(strLabel);
