...
{
	if (m_bVisualManagerStyle && !m_bImageListStateChanged && (GetExtendedStyle() & LVS_EX_CHECKBOXES) == LVS_EX_CHECKBOXES)
	{
...
LRESULT CBCGPListCtrl::OnStyleChanged(WPARAM wp, LPARAM lp)
{
	...
	if (((lpStyleStruct->styleNew & LVS_TYPEMASK) == LVS_REPORT) &&
		((lpStyleStruct->styleOld & LVS_TYPEMASK) != LVS_REPORT))
	{
...
//************************************************************************
void CBCGPListCtrl::OnEnable(BOOL bEnable)
{
...
//************************************************************************
LRESULT CBCGPListCtrl::OnSetImageList(WPARAM wp, LPARAM lp)
{
	LRESULT lRes = Default();
...
//************************************************************************
LRESULT CBCGPListCtrl::OnThemeChanged(WPARAM, LPARAM)
{
	LRESULT lRes = Default();

	if (m_bVisualManagerStyle && !m_bImageListStateChanged && (GetExtendedStyle() & LVS_EX_CHECKBOXES) == LVS_EX_CHECKBOXES &&
		m_ilState.GetSafeHandle() != NULL && m_ilState.GetSafeHandle() != GetImageList(LVSIL_STATE)->GetSafeHandle())
	{
		// recreate our icons when SetWindowTheme is called
...
//************************************************************************
void CBCGPListCtrl::UpdateImageListState()
{
	if ((GetExtendedStyle() & LVS_EX_CHECKBOXES) == 0)
	{
		return;
	}

	if (m_ilState.GetSafeHandle() != NULL && 
		m_ilState.GetSafeHandle() == GetImageList(LVSIL_STATE)->GetSafeHandle())
	{
		const BOOL bIsDisabled = !IsWindowEnabled();
		COLORREF clrBkgnd = GetDefaultBkColor();
