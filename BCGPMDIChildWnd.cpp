...
{
	const BOOL bIsMaximized = IsZoomed();
	BOOL bRedraw = FALSE;
	...
	if (IsWindowVisible())
	{
		if (bIsMaximized)
		{
			...
			if (m_pMDIFrame != NULL)
			{
				...
				if (!m_pMDIFrame->IsMDITabbedGroup())
				{
					pRibbonBar = m_pMDIFrame->GetRibbonBar();
				
					if (pRibbonBar->GetSafeHwnd() != NULL && pRibbonBar->IsWindowVisible() && pRibbonBar->IsReplaceFrameCaption())
					{
						bRedraw = TRUE;
					}
				}
		...
		{
			bRedraw = m_Impl.IsOwnerDrawCaption();
		}
	}
...
//****************************************************************************************
void CBCGPMDIChildWnd::EnableFloatingBarsMenuButton(BOOL bEnable)
{
	m_dockManager.EnableFloatingBarsMenuButton(bEnable);
}
//****************************************************************************************
BOOL CBCGPMDIChildWnd::AreFloatingBarsHaveMenuButton() const
{
...
void CBCGPMDIChildWnd::RecalcLayout (BOOL bNotify)
{
	...
	if (!m_bIsMinimized)
	{
		...
		if (m_dockManager.IsPrintPreviewValid () ||
			m_dockManager.IsOLEContainerMode ())
		{
			if (pActiveView != NULL && pActiveView->IsKindOf (RUNTIME_CLASS (CBCGPPrintPreviewView)))
			{
				m_dockManager.RecalcLayout (bNotify);
				CRect rectClient = m_dockManager.GetClientAreaBounds ();
				pActiveView->SetWindowPos (NULL, rectClient.left, rectClient.top,
			...
			else
			{
				...
				if (pActiveView != NULL && GetParentFrame ()->GetActiveFrame () == this &&
					m_bActivating)
				{
					ASSERT_VALID (pActiveView);

					COleDocument* pDoc = DYNAMIC_DOWNCAST (COleDocument, pActiveView->GetDocument ());
					if (pDoc != NULL)
					{
						ASSERT_VALID (pDoc);
						pActiveItem = pDoc->GetInPlaceActiveItem (pActiveView);
					...
					if (pFrame->GetSafeHwnd() != NULL)
					{
						...
						{
							pActiveView->SetWindowPos (NULL, 0, 0, rectClient.Width (), rectClient.Height (),
													SWP_NOZORDER  | SWP_NOACTIVATE);
						}
					}
				}
