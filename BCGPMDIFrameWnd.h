...
protected:
	...
	CString					m_strMoreWindowsMenuItem;
	...
	CRuntimeClass*			m_pMDITearOffFrameRTI;
...
public:
	...
	void EnableFloatingBarsMenuButton(BOOL bEnable = TRUE);
	BOOL AreFloatingBarsHaveMenuButton() const;
	...
	void EnableTearOffMDIChildren(BOOL bEnable = TRUE, CRuntimeClass* pMDITearOffFrameRTI = NULL)
	{
		if (!bEnable)
		{
		...
		if (pMDITearOffFrameRTI != NULL)
		{
			ASSERT(pMDITearOffFrameRTI->IsDerivedFrom(RUNTIME_CLASS(CBCGPMDIFrameWnd)));
		}

		m_wndClientArea.m_bEnableTearOffMDIChildren = bEnable;
		m_pMDITearOffFrameRTI = pMDITearOffFrameRTI;
	}
	...
	CRuntimeClass* GetMDITearOffFrameRTI()
	{
		return m_pMDITearOffFrameRTI;
	}
	...
	void EnableFullScreenCloseToolBar(BOOL bEnableToolBar)
	{
		m_Impl.m_FullScreenMgr.EnableCloseToolBar(bEnableToolBar);
	}
	...
	CBCGPRibbonBackstageView* GetRibbonBackstageView() const
	{
		return m_Impl.m_pBackstageView;
	}
	...
	void ShowNCArea(BOOL bShow, UINT nFlags = BCGP_NC_AREA_CAPTION | BCGP_NC_AREA_FRAME)
	{
		m_Impl.ShowNCArea(bShow, nFlags);
	}
	...
	BOOL IsLightBoxDialogActive() const
	{
		return m_Impl.GetLightBoxDialog() != NULL;
	}
...
protected:
	...
	void PrepareWindowsMenu(CBCGPPopupMenu& menuPopup);
...
// Implementation:
	...
	BOOL SetLightBoxDialog(CBCGPLightBoxDialog* pDlg)
	{
		return m_Impl.SetLightBoxDialog(pDlg);
	}

	CBCGPLightBoxDialog* GetLightBoxDialog() const
	{
		return m_Impl.GetLightBoxDialog();
	}
	
	void CloseLightBox()
	{
		m_Impl.CloseLightBox();
	}
