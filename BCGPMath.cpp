...
bool bcg_pointInPolygon2 (CBCGPPoint* pts, int nPoints, const CBCGPPoint& pt)
{
    if (nPoints < 3)
    {
        return false;
    ...
    while(next < nPoints)
    {
        bool b = (pts[next].y <= pt.y);

        if (pts[current].y <= pt.y)
        {
            b = !b;
        }
        ...
        if (b)
        {
            if (((pts[next].x - pts[current].x) * (pt.y - pts[current].y) / 
                (pts[next].y - pts[current].y)) <= (pt.x - pts[current].x))
            {
                cnt++;
            }
        }
