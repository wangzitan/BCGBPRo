			...
			case HTMENU_BCG:
				if (!m_bHostsToolbar)
				{
					...
					if (pBtn != NULL)
					{
						...
						CRect rectButton = pBtn->GetRect();

						CPoint ptMenu(rectButton.left + rectWnd.left + rectBorderSize.left, rectButton.bottom + rectWnd.top + rectBorderSize.top);
						OnContextMenu(this, ptMenu);
						break;
					}
				}
...
void CBCGPMiniFrameWnd::OnBarRecalcLayout ()
{
	...
	if (pBar != NULL)
	{
		if (CBCGPControlBar::m_bHandleMinSize && !CBCGPBarContainer::m_bRetainInternalSliderPosition)
		{
			CSize size;
...
void CBCGPMiniFrameWnd::SetCaptionButtons (DWORD dwButtons)
{
	...
	if (!m_bHostsToolbar)
	{
		if (pDockManager != NULL && pDockManager->AreFloatingBarsHaveMenuButton())
		{
			dwButtons |= BCGP_CAPTION_BTN_CUSTOMIZE;
		}
		else
		{
			dwButtons &= ~BCGP_CAPTION_BTN_CUSTOMIZE;
	...
	if (pNMH->idFrom > 0 && (int)pNMH->idFrom <= m_lstCaptionButtons.GetCount())
	{
		...
		if (pos != NULL)
		{
			...
			case HTMENU_BCG:
				...
				{
					strTipText.LoadString(IDS_BCGBARRES_MENU);
				}
...
void CBCGPMiniFrameWnd::SetCaptionButtonsToolTips (BOOL bForceRebuild)
{
	...
	for (i = 0; i < m_lstCaptionButtons.GetCount(); i++)
	{
