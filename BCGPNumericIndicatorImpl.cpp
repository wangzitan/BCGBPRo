...
IMPLEMENT_DYNAMIC(CBCGPNumericIndicatorCtrl, CBCGPVisualCtrl)
	...
	, m_Digit       (NULL)
{
	...
	m_fOriginalTextSize		= 0.0;
	m_dblFinalValue			= 0.0;
	m_dblAnimPerc			= 0.0;
	m_dblDrawCharOpacity	= 1.0;
...
void CBCGPNumericIndicatorImpl::CreateResources()
{
	...
	m_fOriginalTextSize = m_textFormat.GetFontSize();
...
FLOAT CBCGPNumericIndicatorImpl::GetTextFormatAutoSize()
{
	if (m_nCells == 0)
	{
		return 0.0;
	...
	const double nCellWidth = (m_rect.Width() - 2 * m_sizeMargin.cx) / m_nCells;
	return (FLOAT)(2.0 * min(nCellWidth, m_rect.Height()) / 3.0);
}

void CBCGPNumericIndicatorImpl::SetDirty(BOOL bSet /*= TRUE*/, BOOL bRedraw /*= FALSE*/)
{
	if (bSet)
	{
		...
		if (pData->IsAnimated())
		{
			pData->StopAnimation(FALSE);

			m_dblAnimPerc =  0.0;
			m_dblDrawCharOpacity = 1.0;
			SetValue(m_dblFinalValue, FALSE);

			bRedraw = TRUE;
		}
	}

	CBCGPGaugeImpl::SetDirty(bSet, bRedraw);
}

void CBCGPNumericIndicatorImpl::OnAnimation(CBCGPVisualDataObject* pDataObject)
{
	CBCGPGaugeImpl::OnAnimation(pDataObject);
	m_dblAnimPerc = pDataObject->GetAnimatedValue();

	Redraw();
}

void CBCGPNumericIndicatorImpl::OnAnimationFinished(CBCGPVisualDataObject* pDataObject)
{
	CBCGPGaugeImpl::OnAnimationFinished(pDataObject);
	m_dblAnimPerc =  0.0;
	m_dblDrawCharOpacity = 1.0;

	SetValue(m_dblFinalValue, TRUE);
}

void CBCGPNumericIndicatorImpl::SetTextFormatAutoSize(BOOL bSet /*= TRUE*/)
{
	m_bTextFormatAutoSize = bSet;
	m_textFormat.SetFontSize(bSet ? GetTextFormatAutoSize() : m_fOriginalTextSize);
...
void CBCGPNumericIndicatorImpl::OnDraw(CBCGPGraphicsManager* pGM, const CBCGPRect& rectClip, DWORD dwFlags)
{
	...
	if (m_rect.IsRectEmpty() || !m_bIsVisible || m_nCells == 0)
	{
		return;
	}
	...
	if (m_nDecimals == 0)
	{
		...
		if (m_dblFinalValue != dblValue && m_dblAnimPerc > 0.0)
		{
			strLabelNew.Format(strFormat, (int)m_dblFinalValue);
		}
	}
	else
	{
		...
		if (m_dblFinalValue != dblValue && m_dblAnimPerc > 0.0)
		{
			strLabelNew.Format(strFormat, m_dblFinalValue);
		}
	}
	...
	strLabelNew.TrimLeft();
	strLabelNew.TrimRight();
	...
	BOOL bForwardAnim = m_dblFinalValue > dblValue;
	...
	for (int i = 0; i < m_nCells; i++)
	{
		...
		if (nIndex >= 0)
		{
			...
			if (ch >= _T('0') && ch <= _T('9') && nIndex < strLabelNew.GetLength())
			{
				chNew = strLabelNew[nIndex];
			}
		...
		if (ch != 0)
		{
			...
			{
				if (ch != chNew && chNew != 0 && chNew >= _T('0') && chNew <= _T('9'))
				{
					if (m_Style != BCGP_NUMERIC_INDICATOR_CLASSIC)
					{
						m_dblDrawCharOpacity = 1.0 - m_dblAnimPerc;
						OnDrawChar(pGM, m_textFormat, rectCell, ch, i, typeCell);

						m_dblDrawCharOpacity = m_dblAnimPerc;
						OnDrawChar(pGM, m_textFormat, rectCell, chNew, i, typeCell);

						m_dblDrawCharOpacity = 1.0;
					}
					else
					{
						double dy = rectCell.Height() * m_dblAnimPerc;

						CBCGPRect rectOld = rectCell;
						rectOld.OffsetRect(0, bForwardAnim ? -dy : dy);

						OnDrawChar(pGM, m_textFormat, rectOld, ch, i, typeCell);

						CBCGPRect rectNew = rectCell;
						rectNew.OffsetRect(0, bForwardAnim ? rectCell.Height() - dy : -rectCell.Height() + dy);

						OnDrawChar(pGM, m_textFormat, rectNew, chNew, i, typeCell);
					}
				}
				else
				{
...
{
	CBCGPBrush& br = type == BCGP_NUMERIC_INDICATOR_CELL_DIGIT ? m_Colors.m_brDigit :
	...
	{
		if (m_dblDrawCharOpacity < 1.0)
		{
			const double opacity = br.GetOpacity();

			br.SetOpacity(m_dblDrawCharOpacity * opacity);
...
void CBCGPNumericIndicatorImpl::SetTextFormat(const CBCGPTextFormat& textFormat)
{
	...
	m_fOriginalTextSize = textFormat.GetFontSize();

	if (m_bTextFormatAutoSize)
	{
		m_textFormat.SetFontSize(GetTextFormatAutoSize());
	}
...
void CBCGPNumericIndicatorImpl::SetStyle(BCGPNumericIndicatorStyle style)
{
	if (m_Style != style)
	{
		...
		double cx = globalUtils.ScaleByDPI(2.0);
		double cy = globalUtils.ScaleByDPI(2.0);
		...
		case BCGP_NUMERIC_INDICATOR_DIGITAL:
			cx = globalUtils.ScaleByDPI(1.0);
			break;
		}

		m_sizeMargin = CBCGPSize(cx, cy);

		SetDirty();
	}
}
//*******************************************************************************
BOOL CBCGPNumericIndicatorImpl::SetValue(double dblVal, BOOL bRedraw, UINT uiAnimationTime, CBCGPAnimationManager::BCGPAnimationType animationType)
{
	CBCGPGaugeDataObject* pData = DYNAMIC_DOWNCAST(CBCGPGaugeDataObject, m_arData[0]);
	ASSERT_VALID(pData);

	m_dblFinalValue = m_dblAnimPerc = 0.0;
	...
	{
		pData->StartAnimation(0, 1, 0.001 * uiAnimationTime, animationType);
		m_dblFinalValue = dblVal;
	}
...
//*******************************************************************************
void CBCGPNumericIndicatorImpl::OnRectChanged(const CBCGPRect& rectOld)
{
	CBCGPGaugeImpl::OnRectChanged(rectOld);
...
void CBCGPNumericIndicatorImpl::CopyFrom(const CBCGPBaseVisualObject& srcObj)
{
	...
	m_bTextFormatAutoSize = src.m_bTextFormatAutoSize;
	m_fOriginalTextSize = src.m_fOriginalTextSize;
}
