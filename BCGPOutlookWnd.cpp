...
void CBCGPOutlookPopupWnd::OnNcDestroy() 
{
	...
	CBCGPOutlookWnd* pOutlookWnd = DYNAMIC_DOWNCAST(CBCGPOutlookWnd, m_pParent);
	if (pOutlookWnd == NULL)
	{
		CBCGPOutlookBar* pBar = DYNAMIC_DOWNCAST(CBCGPOutlookBar, m_pParent);
		if (pBar != NULL)
		{
			pOutlookWnd = DYNAMIC_DOWNCAST(CBCGPOutlookWnd, pBar->GetUnderlinedWindow());
		}
	}
...
class CBCGPOutlookCustomizeButton : public CCustomizeButton
{
	...
	BOOL IsMode2013()
	{
		CBCGPOutlookBarToolBar* pToolBar = DYNAMIC_DOWNCAST(CBCGPOutlookBarToolBar, GetParentToolbar());
		if (pToolBar != NULL)
		{
			ASSERT_VALID(pToolBar);
			return pToolBar->IsMode2013();
		}
			...
			BOOL /*bDrawBorder*/, BOOL /*bGrayDisabledButtons*/)
{
	...
	if (IsMode2013())
	{
		...
		int size = globalUtils.ScaleByDPI(4);
		int size2 = globalUtils.ScaleByDPI(2);
		int dx = 2 * size;
		int y = rect.CenterPoint().y - size2;
		int x1 = rect.CenterPoint().x - dx;
		int x2 = x1 + 2 * dx;
		
		for (int x = x1; x <= x2; x += dx)
		{
			CRect rectBox(CPoint(x - size2, y), CSize(size, size));
			dm.DrawEllipse(rectBox, clrText, (COLORREF)-1);
		}
	}
	else
	{
...
BOOL CBCGPOutlookBarToolBar::OnSendCommand (const CBCGPToolbarButton* pButton)
{
	...
	if (nIndex >= 0)
	{
		...
		if (m_TabButtons.Lookup (nIndex, iTab))
		{
			...
			{
				m_pParentBar->FireChangeActiveTab(iTab);
			}
...
CSize CBCGPOutlookBarToolBar::GetButtonSize(CSize sizeImage)
{
	BOOL bIsMode2013 = IsMode2013();
	...
	int cxPadding = globalUtils.ScaleByDPI(bIsMode2013 ? 20 : 10);
	int cyPadding = globalUtils.ScaleByDPI(bIsMode2013 ? 12 : 6);

	CSize sizeButton = sizeImage + CSize(cxPadding, cyPadding + 2 * nToolbarMarginHeight);

	return globalUtils.ScaleByDPI(sizeButton);
}

BOOL CBCGPOutlookBarToolBar::IsMode2013() const
{
	ASSERT_VALID(m_pParentBar);
	return m_pParentBar->IsMode2013();
}

void CBCGPOutlookBarToolBar::AdjustLocations ()
{
	CSize sizeButton = GetButtonSize(GetImageSize());

	CSize sizeCustomizeButton (0, 0);
	if (m_pCustomizeBtn != NULL)
	{
		sizeCustomizeButton = sizeButton;
		sizeCustomizeButton.cx = max (sizeCustomizeButton.cx, CBCGPMenuImages::Size ().cx + globalUtils.ScaleByDPI(10));
	}
	...
	if (IsMode2013())
	{
		...
		for (POSITION pos = m_Buttons.GetHeadPosition();  pos != NULL;)
		{
			...
			if (x + sizeButton.cx + sizeCustomizeButton.cx >= rectToolbar.right && pCustomizeBtn == NULL && m_pCustomizeBtn != NULL)
			{
	...
	{
		const int cxPadding = globalUtils.ScaleByDPI(2);
...
void CBCGPOutlookWnd::RecalcLayout ()
{
	...
	const BOOL bIsMode2013 = IsMode2013();
	...
	if (bIsMode2003)
	{
		...
		nToolBarHeight = m_wndToolBar.GetButtonSize(sizeImage).cy;
	}
	...
	int nVisibleTabsNum = bIsMode2013 ? 0 : GetVisibleTabsNum ();
	...
	if (bIsMode2003)
	{
		...
		{
			m_rectSplitter.SetRectEmpty();
			m_rectWndArea.bottom = rectClient.bottom - nToolBarHeight - 1;
		}
		else
		{
	...
	if (nVisibleTabsNum > 1 || !IsHideSingleTab ())
	{
		...
		for (i = 0; i < m_iTabsNum; i ++)
		{
			...
			if (i == m_iActiveTab && !bIsMode2003)
			{
				...
				{
					CBCGPTabInfo* pTabCurr = (CBCGPTabInfo*) m_arTabs [j];
					if (pTabCurr->m_bVisible)
					{
						nVisibleAfter++;
					}
				}
...
BOOL CBCGPOutlookWnd::SetActiveTab (int iTab)
{
	...
	if (bAnimate)
	{
		...
		for (int i = 0; i < nSteps; i++)
		{
			...
			{
				CBCGPTabInfo* pTabInfo = (CBCGPTabInfo*) m_arTabs [j];
...
//**********************************************************************************
void CBCGPOutlookWnd::SetTabsHeight()
{
	const int nPadding = globalUtils.ScaleByDPI(9);

	const int nImageHeight = m_sizeImage.cy <= 0 ? 0 : m_sizeImage.cy + nPadding;
	
	int nTextMargin = 0;
	int nTextHeight = GetTextHeight(nTextMargin);
	
	m_nTabsHeight = (max (nImageHeight, nTextHeight - nTextMargin + nPadding));
}
//**********************************************************************************
...
//********************************************************************************
BOOL CBCGPOutlookWnd::IsMode2013 () const
{
	ASSERT_VALID (this);

	CBCGPOutlookBar* pOutlookBar = DYNAMIC_DOWNCAST(CBCGPOutlookBar, GetParent());

	return pOutlookBar != NULL && pOutlookBar->IsMode2013 ();
}
//*******************************************************************************
					...
					if (m_bIsCollapsed && m_pAutoHideWindow->GetSafeHwnd() != NULL && !m_bPopupIsDisplayed)
					{
						...
						if (pOutlookBar->GetSafeHwnd() != NULL)
						{
							...
							if (pDefaultSlider->GetSafeHwnd() != NULL)
							{
								...
								if (pDefaultSlider->GetCurrentAlignment () == CBRS_ALIGN_RIGHT)
								{
									bIsLeftAlign = FALSE;
								}
							}
						...
						if (!bIsLeftAlign)
						{
							rectPopup.OffsetRect(-m_cxParent, 0);
						}
						else
						{
...
void CBCGPOutlookWnd::RebuildToolBar ()
{
	...
	CSize sizeButton = sizeImage + m_wndToolBar.GetButtonSize(sizeImage);
	...
	for (int i = 0; i < m_iTabsNum; i ++)
	{
		...
		if (pTab->m_bVisible && (pTab->m_rect.IsRectEmpty () || bIsMode2013))
		{
...
BOOL CBCGPOutlookWnd::SetToolbarImageList (UINT uiID, int cx, COLORREF clrTransp)
{
	...
	if (svg.Load(uiID))
	{
		m_sizeToolbarImage = CSize (cx, cx);		// Assume square
		
		CBCGPToolBarImages images;
		images.SetImageSize(m_sizeToolbarImage);
		
		images.AddSVGList(svg);
		images.ExportToImageList(m_imagesToolbar);
		
		UseAlphaBlendIcons(TRUE, FALSE);
	}
	else
#endif
	{
