...
(
	...
	, m_bIsScheduleTimeDeltaDay(FALSE)
...
void CBCGPPlannerManagerCtrl::OnDraw (CDC* pDC)
{
	...
	rectClient.bottom = pView->m_rectApps.bottom;
...
void CBCGPPlannerManagerCtrl::OnPreparePrinting(CPrintInfo* pInfo, CRuntimeClass* pPrinterClass)
{
	...
	if (pPrinterClass == NULL)
	{
		...
		case BCGP_PLANNER_TYPE_SCHEDULE:
		...
		case BCGP_PLANNER_TYPE_WEEK_DAYS:
			pPrinterClass = RUNTIME_CLASS (CBCGPPlannerPrintWeek);
			break;
...
void CBCGPPlannerManagerCtrl::OnBeginPrinting(CDC* pDC, CPrintInfo* pInfo, CRuntimeClass* pPrinterClass /*= NULL*/)
{
	...
	if (m_pPrinter != NULL)
	{
		...
		CBCGPAppointmentBaseMultiStorage* pStorage = NULL;
		if (IsMultiResourceStorage())
		{
			pStorage = DYNAMIC_DOWNCAST(CBCGPAppointmentBaseMultiStorage, GetStorage());
		}

		UINT nOldResourceID = CBCGPAppointmentBaseMultiStorage::e_UnknownResourceID;

		if (pStorage != NULL)
		{
			nOldResourceID = pStorage->GetCurrentResourceID();
			pStorage->SetCurrentResourceID (GetCurrentResourceID());
		}
		...
		if (pStorage != NULL)
		{
			pStorage->SetCurrentResourceID (nOldResourceID);
		}
	}	
}
...
void CBCGPPlannerManagerCtrl::DoPaint(CDC* pDCPaint) 
{
	...
	CRect rectClientStored(rectClient);
	...
	if (m_wndScrollVert.GetSafeHwnd() != NULL && m_wndScrollVert.IsWindowVisible() && 
		m_wndScrollHorz.GetSafeHwnd() != NULL && m_wndScrollHorz.IsWindowVisible())
	{
		CRect rectGripper(rectClientStored);
		rectGripper.left = pView->m_rectApps.right;
		rectGripper.top = pView->m_rectApps.bottom;
											   ...
											   BOOL bRedraw /*= TRUE*/)
{
	...
	const double nTotalDays = span.GetTotalDays ();

	if (nTotalDays == 6 && 
		date1 == CBCGPPlannerView::GetFirstWeekDay (date1, GetFirstDayOfWeek () + 1) &&
		m_Type != BCGP_PLANNER_TYPE_SCHEDULE)
	{
		SetType (m_bUseDayViewInsteadWeekView ? BCGP_PLANNER_TYPE_WEEK_DAYS : BCGP_PLANNER_TYPE_WEEK, FALSE);
	...
	else if ((nTotalDays > 6 && m_Type != BCGP_PLANNER_TYPE_SCHEDULE) || nTotalDays >= 14)
	{
	...
	else
	{
		...
		else if (m_Type == BCGP_PLANNER_TYPE_SCHEDULE)
		{
			SetType (BCGP_PLANNER_TYPE_SCHEDULE, FALSE);
		}
		else
		{
		...
		if (bUpdate)
		{
			...
			span = COleDateTimeSpan(bAdd ? dtSel2 - dtSel1 : dtSel1 - dtSel2);
...
void CBCGPPlannerManagerCtrl::SetDate (const COleDateTime& date, BOOL bRedraw /*= TRUE*/)
{
	...
	if ((m_Type == BCGP_PLANNER_TYPE_WEEK || m_Type == BCGP_PLANNER_TYPE_MONTH))
	{
	...
	else
	{
		...
		else
		{
			...
			if (m_Type == BCGP_PLANNER_TYPE_WORK_WEEK || m_Type == BCGP_PLANNER_TYPE_WEEK_DAYS)
			{
...
void CBCGPPlannerManagerCtrl::SetType (BCGP_PLANNER_TYPE type, BOOL bRedraw /*= TRUE*/)
{
	...
	m_pCurrentView->m_pPlanner = this;
	...
	if (m_Type == BCGP_PLANNER_TYPE_DAY ||
		...
		m_Type == BCGP_PLANNER_TYPE_MULTI ||
		m_Type == BCGP_PLANNER_TYPE_SCHEDULE ||
		m_Type == BCGP_PLANNER_TYPE_WEEK_DAYS)
	{
		...
		if (((m_Type == BCGP_PLANNER_TYPE_DAY || m_Type == BCGP_PLANNER_TYPE_SCHEDULE) && oldType == BCGP_PLANNER_TYPE_MULTI) ||
			(m_Type == BCGP_PLANNER_TYPE_MULTI && (oldType == BCGP_PLANNER_TYPE_DAY || oldType == BCGP_PLANNER_TYPE_SCHEDULE)))
		{
...
BOOL CBCGPPlannerManagerCtrl::AddAppointment (CBCGPAppointment* pApp, BOOL bQuery, BOOL bRedraw)
{
	...
	const BOOL bAllOrMulti = pApp->IsAllDay() || pApp->IsMultiDay();
												 ...
												 BOOL bQuery, BOOL bRedraw)
{
	...
	{
		SortAppointments (m_arQueryApps, (int)m_arQueryApps.GetSize (), GetCurrentView()->GetSortAppointmentsType());
		GetCurrentView ()->AdjustAppointments ();
	}
...
BOOL CBCGPPlannerManagerCtrl::RemoveAppointment (CBCGPAppointment* pApp, BOOL bQuery, BOOL bRedraw)
{
	...
	if (ID != 0)
	{
		for (int i = 0; i < (int)m_arQueryApps.GetSize (); i++)
		{
...
CBCGPPlannerView* CBCGPPlannerManagerCtrl::OnCreateView(BCGP_PLANNER_TYPE type)
{
	...
	case BCGP_PLANNER_TYPE_MULTI:
		...
		break;
	case BCGP_PLANNER_TYPE_SCHEDULE:
		pView = new CBCGPPlannerViewSchedule;
		break;
	case BCGP_PLANNER_TYPE_WEEK_DAYS:
		pView = new CBCGPPlannerViewWeekDays;
		break;
	default:
		ASSERT (FALSE);
...
BOOL CBCGPPlannerManagerCtrl::PreCreateWindow (CREATESTRUCT& cs)
{
	CRuntimeClass* viewClass[] =
	{
		...
		RUNTIME_CLASS(CBCGPPlannerViewSchedule),
		RUNTIME_CLASS(CBCGPPlannerViewWeekDays)
	};
...
int CBCGPPlannerManagerCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_wndScrollHorz.Create (WS_CHILD | WS_VISIBLE | SBS_HORZ, CRect (0, 0, 0, 0), this, 
		BCGP_PLANNER_ID_SCROLL_HORZ);
...
CScrollBar* CBCGPPlannerManagerCtrl::GetScrollBarCtrl(int nBar) const
{
	...
	{
		pScrollBar = (CScrollBar*)&m_wndScrollVert;
	}
	else if (nBar == SB_HORZ && m_wndScrollHorz.GetSafeHwnd () != NULL)
	{
		pScrollBar = (CScrollBar*)&m_wndScrollHorz;
	}
...
void CBCGPPlannerManagerCtrl::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	GetCurrentView ()->OnHScroll (nSBCode, nPos, pScrollBar);
}
...
CBCGPPlannerView::BCGP_PLANNER_TIME_DELTA CBCGPPlannerManagerCtrl::GetTimeDelta() const
{
	if (GetType() == BCGP_PLANNER_TYPE_SCHEDULE && m_bIsScheduleTimeDeltaDay)
	{
		return CBCGPPlannerView::BCGP_PLANNER_TIME_DELTA_60;
	}

	return m_TimeDelta;
}

void CBCGPPlannerManagerCtrl::SetScheduleTimeDeltaDay(BOOL bSet, BOOL bRedraw/* = TRUE*/)
{
	if (m_bIsScheduleTimeDeltaDay != bSet)
	{
		m_bIsScheduleTimeDeltaDay = bSet;
												 ...
												 const COleDateTime& date2) const
{
	...
	BCGP_SORTAPPOINTMENTS_TYPE option = GetCurrentView()->GetSortAppointmentsType();
	if (option != BCGP_SORTAPPOINTMENTS_BY_TYPE_AND_DATE)
	{
		SortAppointments(ar, (int)ar.GetSize(), option);
	}
}
	...
	XBCGPAppointmentArray& ar, BCGP_PLANNER_TYPE type, const COleDateTime& dtTo)
{
	try
	{
		...
		if (dtTo != COleDateTime ())
		{
			...
			if (type == BCGP_PLANNER_TYPE_DAY || 
				type == BCGP_PLANNER_TYPE_WORK_WEEK ||
				type == BCGP_PLANNER_TYPE_MULTI ||
				type == BCGP_PLANNER_TYPE_SCHEDULE ||
				type == BCGP_PLANNER_TYPE_WEEK_DAYS)
			{
...
void CBCGPPlannerManagerCtrl::StartDragDrop ()
{
	...
	if (!IsDragDrop() && CanStartDragDrop ())
	{
...
BOOL CBCGPPlannerManagerCtrl::CanStartDragDrop () const
{
	if (!IsDragDrop () &&
		 ...
		 GetSelectedAppointmentsCount () > 0)
	{
		CSize czScroll (::GetSystemMetrics (SM_CXDRAG), ::GetSystemMetrics (SM_CYDRAG));
...
BOOL CBCGPPlannerManagerCtrl::OnDrop(COleDataObject* pDataObject, DROPEFFECT dropEffect, CPoint point)
{
	...
	if (pView->IsCaptureMatched () && dropEffect != DROPEFFECT_COPY)
	{
		return FALSE;
	}
	...
	m_dtDrop = pView->m_dtCaptureCurrent;
	...
	try
	{
		...
		bRes = CBCGPPlannerManagerCtrl::SerializeFrom (*pFile, ar, m_Type, 
			IsDragDrop()
				? COleDateTime ()
				: pView->GetDateFromPoint (point));

		UINT nResourceID  = pView->GetResourceFromPoint (point);
		UINT nResourceOld = pView->GetCurrentResourceID ();
		...
		{
			BOOL bAllDay = FALSE;
			const BOOL bSheduleView = m_Type == BCGP_PLANNER_TYPE_SCHEDULE;
			const BOOL bCanMakeAllDay = m_Type == BCGP_PLANNER_TYPE_DAY || 
										m_Type == BCGP_PLANNER_TYPE_WORK_WEEK ||
										m_Type == BCGP_PLANNER_TYPE_MULTI;

			const CBCGPPlannerView::BCGP_PLANNER_HITTEST hit = pView->HitTestArea (point);
			...
			if (bSheduleView && hit != CBCGPPlannerView::BCGP_PLANNER_HITTEST_HEADER_RESOURCE)
			{
				for (i = 0; i < (int)ar.GetSize (); i++)
				{
					...
					if (pApp->IsAllDay () && 
						(m_dtDrop.GetHour() != 0 || m_dtDrop.GetMinute() != 0 || m_dtDrop.GetSecond() != 0))
					{
			...
			if (IsDragDrop() && (!bSheduleView || hit != CBCGPPlannerView::BCGP_PLANNER_HITTEST_HEADER_RESOURCE))
			{
				COleDateTime dtS (pView->m_dtCaptureStart);
			...
			if (IsDragDrop() && dropEffect != DROPEFFECT_COPY)
			{
			...
			const int c_Count = (int)ar.GetSize ();

			if (c_Count > 0)
			{
				BOOL bRemoveRecurrence = !IsDragDrop() || dropEffect == DROPEFFECT_COPY;

				if (IsDragDrop())
				{
					for (i = 0; i < (int)m_arQueryApps.GetSize (); i++)
					{
						CBCGPAppointment* pApp = m_arQueryApps[i];
				...
				if (!pView->SetCurrentResourceID (nResourceID, FALSE, FALSE))
				{
					nResourceID = nResourceOld;
				}

				pView->SetSelection (dtSel1, dtSel2, FALSE);
			}

			if (!IsDragDrop() ||
				(IsDragDrop() && (dropEffect == DROPEFFECT_MOVE || dropEffect == DROPEFFECT_COPY)))
			{
				ClearAppointmentSelection (FALSE);
				QueryAppointments ();
...
BOOL CBCGPPlannerManagerCtrl::PreTranslateMessage(MSG* pMsg)
{
	...
	if (pMsg->message == WM_KEYDOWN && GetSelectedAppointmentsCount() == 1)
	{
		const CBCGPAppointment* pApp = GetSelectedAppointments ().GetHead ();
		if (pApp != NULL && pApp->IsEditExists())
		{
			if (globalUtils.ProcessCtrlEditAccelerators(pApp->m_pWndInPlace, (UINT) pMsg->wParam, TRUE))
			{
				return TRUE;
			}
		}
...
void CBCGPPlannerManagerCtrl::UpdateCalendarsSelection (CBCGPCalendar* pWndCalendar /*= NULL*/)
{
	...
	if (date1 != dt || date2 != (dt + COleDateTimeSpan (nDuration - 1, 0, 0 ,0)))
	{
		...
		CList<DATE, DATE&> lstDatesRes;

		for (int i = 0; i < nDuration; i++)
		{
			lstDatesRes.AddTail (dt.m_dt);
			dt += span;
		}

		pWndCalendar->SetSelectedDates (lstDatesRes, FALSE);
	}
}
												 ...
												 DROPEFFECT dropEffect, COleDateTime dateTo)
{
	...
	try
	{
		...
		bRes = CBCGPPlannerManagerCtrl::SerializeFrom(*pFile, ar, BCGP_PLANNER_TYPE_WEEK, m_dtDrop);
...
void CBCGPPlannerManagerCtrl::SetToolTipWholeArea (BOOL bToolTipWholeArea)
{
	if (m_bToolTipWholeArea != bToolTipWholeArea)
	{
		m_bToolTipWholeArea = bToolTipWholeArea;
...
void CBCGPPlannerManagerCtrl::AddToolTipInfo (const CBCGPAppointment* pApp)
{
	...
	if (pApp != NULL)
	{
		...
		if (pApp->GetDSDraw ().IsEmpty ())
		{
			...
			{
				CRect rt;
				if (!m_bToolTipWholeArea)
				{
					rt = pApp->GetRectEditHitTest ();
...
//**************************************************************************
BOOL CBCGPPlannerManagerCtrl::EnsureVisibleTime(const COleDateTime& rTime, BOOL bPartialOK)
{
	CBCGPPlannerView* pView = GetCurrentView ();
	if (pView == NULL)
	{
	...
	return pView->EnsureVisibleTime(rTime, bPartialOK);
}
//**************************************************************************
BOOL CBCGPPlannerManagerCtrl::EnsureVisibleTime(CBCGPPlannerView::BCGP_PLANNER_EVT type, BOOL bPartialOK)
{
	CBCGPPlannerView* pView = GetCurrentView ();
	if (pView == NULL)
	{
	...
	return pView->EnsureVisibleTime(type, bPartialOK);
}
//**************************************************************************
