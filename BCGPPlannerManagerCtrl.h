...
public:
	...
	enum BCGP_PLANNER_TYPE
	{
		...
		BCGP_PLANNER_TYPE_SCHEDULE  = 5,
		BCGP_PLANNER_TYPE_WEEK_DAYS = 6,
		BCGP_PLANNER_TYPE_LAST      = BCGP_PLANNER_TYPE_WEEK_DAYS
	};
	...
	void SetTimeDelta (CBCGPPlannerView::BCGP_PLANNER_TIME_DELTA delta, BOOL bRedraw = TRUE);

	CBCGPPlannerView::BCGP_PLANNER_TIME_DELTA GetTimeDelta () const;

	void SetScheduleTimeDeltaDay(BOOL bSet, BOOL bRedraw = TRUE);
	BOOL IsScheduleTimeDeltaDay() const
	{
		return m_bIsScheduleTimeDeltaDay;
	}
	...
	BOOL IsDefaultDrawFlags() const
	{
		return m_bDefaultDrawFlags;
	}
...
public:
	...
	BOOL IsToolTipWholeArea () const
	{
		return m_bToolTipWholeArea;
	}
	void SetToolTipWholeArea (BOOL bToolTipWholeArea);
...
public:
	...
	BOOL EnsureVisibleTime(const COleDateTime& rTime, BOOL bPartialOK);
	BOOL EnsureVisibleTime(CBCGPPlannerView::BCGP_PLANNER_EVT type, BOOL bPartialOK);
...
protected:
	...
	CBCGPPlannerView::BCGP_PLANNER_TIME_DELTA
								m_TimeDelta;
	BOOL						m_bIsScheduleTimeDeltaDay;

	CImageList					m_ilUpDown;
	CSize						m_szUpDown;

	CBCGPPlannerView*			m_pViews[BCGP_PLANNER_TYPE_LAST + 1];
