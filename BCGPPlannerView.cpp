...
CBCGPPlannerView::CBCGPPlannerView()
	...
	, m_dwDrawTimeFlags (BCGP_PLANNER_DRAW_TIME_ALL_FLAGS)
{
...
HFONT CBCGPPlannerView::SetCurrFont (CDC* pDC, BOOL bBold/* = FALSE*/, BOOL bExtended/* = FALSE*/)
{
	ASSERT_VALID (pDC);
	
	HFONT hFont = GetFont (bBold, bExtended);
...
int CBCGPPlannerView::GetCaptionHeight () const
{
	int nHeight = m_nRowHeight;
	if ((GetDrawFlags() & BCGP_PLANNER_DRAW_VIEW_CAPTION_DAY_EXTENDED) == BCGP_PLANNER_DRAW_VIEW_CAPTION_DAY_EXTENDED)
	{
		nHeight = (nHeight * 5) / 4;
	}
...
DWORD CBCGPPlannerView::GetDrawFlags() const
{
	return GetPlanner()->GetDrawFlags();
}
...
void CBCGPPlannerView::AdjustCaptionFormat (CDC* pDC)
{
	...
	for (int n = 0; n < (int)sa.GetSize (); n++)
	{
...
void CBCGPPlannerView::AdjustScrollSizes ()
{
	...
	{
		pwndScroll = m_pPlanner->GetHeaderScrollBarCtrl (SB_VERT);
	...
	if (pwndScroll->GetSafeHwnd () != NULL && CanUseScrollBar(SB_VERT))
	{
	...
	pwndScroll = m_pPlanner->GetScrollBarCtrl (SB_HORZ);
	if (pwndScroll->GetSafeHwnd () != NULL && CanUseScrollBar(SB_HORZ))
	{
		si.nMax  = m_nScrollHorzTotal;
		si.nPage = m_nScrollHorzPage;
		si.nPos  = m_nScrollHorzOffset;
...
void CBCGPPlannerView::AdjustLayout (BOOL bRedraw)
{
	...
	const int cyScroll = ::GetSystemMetrics (SM_CXVSCROLL);

	CScrollBar* pHeaderScrollBar = m_pPlanner->GetHeaderScrollBarCtrl (SB_VERT);
	CScrollBar* pScrollBarVert = m_pPlanner->GetScrollBarCtrl (SB_VERT);
	CScrollBar* pScrollBarHorz = m_pPlanner->GetScrollBarCtrl (SB_HORZ);
	if (m_pPlanner->IsScrollBarVisible ())
	{
		CRect rectAppsOld(m_rectApps);

		if ((m_pPlanner->IsHeaderScrollingEnabled() && CanUseHeaderScrolling()) ||
			CanUseScrollBar(SB_VERT))
		{
		...
		if (CanUseScrollBar(SB_HORZ))
		{
			pScrollBarHorz->ShowScrollBar (TRUE);

			pScrollBarHorz->SetWindowPos (NULL, rectClient.left, 
				m_rectApps.bottom - cyScroll, m_rectApps.right - rectClient.left, cyScroll, SWP_NOZORDER | SWP_NOACTIVATE);

			m_rectApps.bottom -= cyScroll;
			nScrollHeight -= cyScroll;
		}
		else
		{
			pScrollBarHorz->ShowScrollBar (FALSE);
		}
		...
		if (CanUseScrollBar(SB_VERT))
		{
			pScrollBarVert->ShowScrollBar (TRUE);
			pScrollBarVert->SetWindowPos (NULL, m_rectApps.right, 
				nScrollTop, cxScroll, nScrollHeight, SWP_NOZORDER | SWP_NOACTIVATE);
		}
		else
		{
			pScrollBarVert->ShowScrollBar (FALSE);
		}
	}
	else
	{
...
void CBCGPPlannerView::ConstructCaptionText (const COleDateTime& day, CString& strText, const CString& strFormat)
{
	...
	if ((GetDrawFlags() & BCGP_PLANNER_DRAW_VIEW_CAPTION_DAY_UPPER) == BCGP_PLANNER_DRAW_VIEW_CAPTION_DAY_UPPER)
	{
		strText.MakeUpper();
	}
}
...
void CBCGPPlannerView::DrawAppointment (CDC* pDC, CBCGPAppointment* pApp, CBCGPAppointmentDrawStructEx* pDS)
{
	...
	const BOOL bDrawDuration = (dwDrawFlags & BCGP_PLANNER_DRAW_APP_NO_DURATION) == 0 &&
		(dwDrawFlags & BCGP_PLANNER_DRAW_APP_DURATION_WEEK_MONTH) == BCGP_PLANNER_DRAW_APP_DURATION_WEEK_MONTH;
	...
	const BOOL bDrawShadow  = IsDrawAppsShadow () && !pApp->IsSelected () && (dwDrawFlags & BCGP_PLANNER_DRAW_APP_NO_SHADOW) == 0;
	...
	const int nSpacingLeft = bDrawDuration ? 0 : 3;
	const int nSpacingRight = 3;
...
HFONT CBCGPPlannerView::SetFont (HFONT hFont)
{
	...
	if (hFont != NULL)
	{
		...
		if (m_FontEx.GetSafeHandle () != NULL)
		{
			m_FontEx.DeleteObject ();
		}

		if (m_FontBoldEx.GetSafeHandle () != NULL)
		{
			m_FontBoldEx.DeleteObject ();
		}
		...
		m_FontEx.CreateFontIndirect (&lf);

		lf.lfWeight = FW_BOLD;
		m_FontBoldEx.CreateFontIndirect (&lf);
	}
...
HFONT CBCGPPlannerView::GetFont (BOOL bBold/* = FALSE*/, BOOL bExtended/* = FALSE*/)
{
	if (bBold)
	{
		bBold = (GetDrawFlags () & (BCGP_PLANNER_DRAW_VIEW_CAPTION_DAY_BOLD | BCGP_PLANNER_DRAW_VIEW_CAPTION_DAY_BOLD_FC |
			BCGP_PLANNER_DRAW_VIEW_HEADER_BOLD | BCGP_PLANNER_DRAW_VIEW_HEADER_BOLD_CUR)) != 0;
	}
	...
	if (bBold)
	{
		if (bExtended)
		{
			return (HFONT) (m_FontBoldEx.GetSafeHandle () != NULL 
								? m_FontBoldEx.GetSafeHandle () 
#ifndef _BCGSUITE_
								: globalData.fontGroupBold);
#else
								: globalData.fontBold);
#endif
		}
	...
	if (bExtended)
	{
		return (HFONT) (m_FontEx.GetSafeHandle () != NULL 
							? m_FontEx.GetSafeHandle () 
#ifndef _BCGSUITE_
							: globalData.fontGroup);
#else
...
BOOL CBCGPPlannerView::OnMouseWheel(UINT nFlags, short zDelta, CPoint /*pt*/)
{
	...
	if ((nFlags & MK_SHIFT) == MK_SHIFT && CanUseScrollBar(SB_HORZ) && m_nScrollHorzTotal > 0)
	{
	...
	else if (CanUseScrollBar(SB_VERT) && m_nScrollTotal > 0)
	{
		for (int i = 0; i < nSteps; i++)
		{
...
BOOL CBCGPPlannerView::OnScroll(UINT nScrollCode, UINT nPos, BOOL bDoScroll)
{
	...
	int nBar = LOBYTE(nScrollCode);
	UINT nSBCode = HIBYTE(nScrollCode);

	if (!bDoScroll)
	{
		return nBar == SB_VERT ? m_nScrollTotal != 0 : m_nScrollHorzTotal != 0;
	}
...
BOOL CBCGPPlannerView::OnHScroll(UINT /*nSBCode*/, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
	ASSERT_VALID (m_pPlanner);

	StopEditAppointment ();

	m_pPlanner->SetScrollPos (SB_HORZ, m_nScrollHorzOffset, TRUE);
/*
	if (m_arDragAppointments.GetSize () > 0)
	{
		CPoint pt;
...
void CBCGPPlannerView::StartCapture ()
{
	...
	if (CanCaptureAppointment (hitCapturedAppointment))
	{
		...
		if (m_pCapturedAppointment != NULL)
		{
			...
			m_CapturedProperties.Add
				(
					BCGP_APPOINTMENT_PROPERTY_ALLDAY, 
					new CBCGPAppointmentProperty (m_pCapturedAppointment->IsAllDay ())
				);
		}
...
void CBCGPPlannerView::StopCapture ()
{
	...
	if (m_pCapturedAppointment != NULL && m_bCapturedAppointmentChanged)
	{
...

		{
			...
			if (m_CapturedProperties.PropertyExists (BCGP_APPOINTMENT_PROPERTY_ALLDAY))
			{
				bAllDay = *((CBCGPAppointmentProperty*)m_CapturedProperties.Get (BCGP_APPOINTMENT_PROPERTY_ALLDAY));
			}

			bCapturedAppointmentChanged = 
				m_pCapturedAppointment->GetStart () != dtStart ||
				m_pCapturedAppointment->GetFinish () != dtFinish ||
				m_pCapturedAppointment->IsAllDay() != bAllDay;
		}
...
void CBCGPPlannerView::RestoreCapturedAppointment ()
{
	if (m_pCapturedAppointment != NULL && m_bCapturedAppointmentChanged)
	{
		...
		m_pCapturedAppointment->SetAllDay(bAllDay);
	}
}
...
void CBCGPPlannerView::GetWeekBarText(const COleDateTime& day1, const COleDateTime& day2, CString& strText) const
{
	ASSERT_VALID(m_pPlanner);

	m_pPlanner->GetWeekBarText (day1, day2, strText);
}
