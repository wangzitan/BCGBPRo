...
void CBCGPPlannerViewDay::OnDrawAppointmentsDuration (CDC* pDC)
{
	...
	for (int nApp = 0; nApp < 2; nApp++)
	{
		...
		for (int i = 0; i < (int)arApps.GetSize (); i++)
		{
			...
			for(int nDay = nStart; nDay < nEnd; nDay++)
			{
				CRect rt (m_ViewRects[nDay]);
				...
				if (!pApp->IsAllDay ())
				{
					...
					rt.IntersectRect (rt, m_ViewRects[nDay]);
				}
...
void CBCGPPlannerViewDay::OnDrawUpDownIcons (CDC* pDC)
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		for (int j = 0; j < (int)ar.GetSize (); j++)
		{
...
void CBCGPPlannerViewDay::AdjustLayout (CDC* /*pDC*/, const CRect& rectClient)
{
	...
	m_rectTimeBar.right = m_rectTimeBar.left + (long)(m_nRowHeight * ((nMinuts == 60 || bTimeBarSimple) ? 2.5 : 3.0)) + globalUtils.ScaleByDPI(5);
			...
			for (i = 0; i < (int)arApps.GetSize (); i++)
			{
				...
				if (CBCGPPlannerView::IsAppointmentInDate (*pApp, date))
				{
					...
					if (pApp->IsAllDay () || pApp->IsMultiDay ())
					{
						...
						for (int j = 0; j < (int)arAllDays.GetSize (); j++)
						{
							if (arAllDays[j] == pApp)
							{
								bAdd = FALSE;
					...
					{
						rtApp.right -= 2 * nAppointmentSpace;
					...
					for (i = 0; i < nCount; i++)
					{
						...
						for (int j = 0; j < (int)ar->GetSize (); j++)
						{
							...
							for (int k = i + 1; k < nCount; k++)
							{
								...
								for (int m = 0; m < (int)arNext->GetSize (); m++)
								{
									...
									if (rtInter.IntersectRect (rtApp, pAppNext->GetRectDraw (date)))
									{
										rtApp.right = rectFill.left + nWidth * k - 
											(bDrawShadow ? nAppointmentSpace : 0);
										pApp->SetRectDraw (rtApp, date);
										break;
									}
								}
...
void CBCGPPlannerViewDay::OnDrawClient (CDC* pDC, const CRect& rect)
{
	...
	const int nFirstWorkingHour   = GetFirstWorkingHour (FALSE);
	const int nFirstWorkingMinute = GetFirstWorkingMinute (FALSE);
	const int nLastWorkingHour    = GetLastWorkingHour (FALSE);
	const int nLastWorkingMinute  = GetLastWorkingMinute (FALSE);
	...
	for (nDay = 0; nDay < nDays; nDay++)
	{
		...
		BCGP_PLANNER_WORKING_STATUS AllPeriodWorkingStatus = 
			GetWorkingPeriodParameters (nResourceID, dtStart + COleDateTimeSpan (0, 0, iStart * nMinuts, 0), dtStart + COleDateTimeSpan (0, 0, (iEnd * nMinuts) - 1, 59), WorkingParameters); 
		...
		for (int iStep = iStart; iStep < iEnd; iStep++)
		{
			...
			{ // We don't know for the day -> we should see for the period
				...
				SpecificPeriodWorkingStatus = GetWorkingPeriodParameters (nResourceID, CurrentPeriodStart, CurrentPeriodEnd, WorkingParameters); 
			}
										 ...
										 BOOL bDrawTimeLine)
{
	...
	int nMinDelta = max (pDC->GetTextExtent (_T("000")).cx, globalUtils.ScaleByDPI(18));
		...
		if (rectHour.top >= nHeaderHeight || rectHour.bottom > nHeaderHeight)
		{
			...
			if (nCount == 1 && !bTimeBarSimple)
			{
				...
				if (!b24Hours)
				{
					if (nHour == 12)
					{
						str.Format (_T("12 %s"), bAM ? (LPCTSTR)strAM : (LPCTSTR)strPM);
					}
				}
	...
	if (bDrawTimeLine && bTimeLineFullWidth)
	{
		rectTimeLine.left = rectBar.right - 1;
		OnDrawTimeLine (pDC, rectTimeLine);
	}
}
...
CBCGPPlannerViewDay::HitTest (const CPoint& point) const
{
	...
	if (hit == BCGP_PLANNER_HITTEST_CLIENT || hit == BCGP_PLANNER_HITTEST_HEADER_ALLDAY)
	{
...
BOOL CBCGPPlannerViewDay::OnKeyDown(UINT nChar, UINT /*nRepCnt*/, UINT /*nFlags*/)
{
	...
	else if (nChar == VK_HOME || nChar == VK_END)
	{
		...
		int nPos = (60 * (sel2.GetHour () - GetFirstViewHour()) + sel2.GetMinute ()) / nMinuts + (nChar == VK_HOME ? 0 : 1);
		if (nPos < m_nScrollOffset || (m_nScrollOffset + m_nScrollPage) < nPos)
		{
			OnVScroll (SB_THUMBPOSITION, 
				nChar == VK_HOME ? nPos : nPos - m_nScrollPage, NULL);
		}
...
CRect CBCGPPlannerViewDay::GetRectFromDate(const COleDateTime& date) const
{
	...
	rect.top += (((date.GetHour() - GetFirstViewHour()) * 60 + date.GetMinute()) / CBCGPPlannerView::GetTimeDeltaInMinuts (GetTimeDelta ()) - m_nScrollOffset) * m_nRowHeight;
...
BOOL CBCGPPlannerViewDay::OnLButtonDown(UINT nFlags, CPoint point)
{
		...
		hit == BCGP_PLANNER_HITTEST_HEADER_ALLDAY)
	{
		...
		if (dt != COleDateTime () && (dt != m_Date || 
			...
			nResourceID != GetPlanner()->GetCurrentResourceID ()))
		{
			m_Date = dt;
...
BOOL CBCGPPlannerViewDay::EnsureVisible(const CBCGPAppointment* pApp, BOOL bPartialOK)
{
	...
	rect.OffsetRect(0, -nViewRectTop);

	int nPos = rect.top / nRowHeight;

	if (rect.top >= 0 && rect.Height () <= nViewRectHeight)
	{
		nPos = rect.bottom / nRowHeight - nScrollPage;
	}
...
BOOL CBCGPPlannerViewDay::EnsureVisibleTime(const COleDateTime& rTime, BOOL bPartialOK)
{
	if (rTime == COleDateTime())
	{
		return FALSE;
	}

	COleDateTime date(GetDateStart());
	date.SetDateTime(date.GetYear(), date.GetMonth(), date.GetDay(), min(max(rTime.GetHour(), GetFirstViewHour()), GetLastViewHour()), rTime.GetMinute(), 0);

	CRect rect(GetRectFromDate(date));
	if (rect.IsRectEmpty())
	{
		return FALSE;
	}
	...
	if (bPartialOK && rectInter.IntersectRect(rect, m_rectApps))
	{
		return TRUE;
	}
	...
	int nPos = rect.top / m_nRowHeight;

	if (rect.top >= 0 && rect.Height () <= nViewRectHeight)
	{
		nPos = rect.bottom / m_nRowHeight - m_nScrollPage;
	}
...
BOOL CBCGPPlannerViewDay::EnsureVisibleTime(const BCGP_PLANNER_EVT type, BOOL bPartialOK)
{
	if (type == BCGP_PLANNER_EVT_APPOINTMENT_FIRST || type == BCGP_PLANNER_EVT_APPOINTMENT_LAST)
	{
		CBCGPAppointment* pApp = NULL;

		const XBCGPAppointmentArray& ar = GetQueryedAppointments();

		if (type == BCGP_PLANNER_EVT_APPOINTMENT_FIRST)
		{
			for (int i = 0; i < (int)ar.GetSize(); i++)
			{
				CBCGPAppointment* p = ar[i];
				if (p->IsAllDay () || p->IsMultiDay ())
				{
					continue;
				}
		...
		{
			for (int i = (int)ar.GetSize() - 1; i >= 0; i--)
			{
				CBCGPAppointment* p = ar[i];
				if (p->IsAllDay () || p->IsMultiDay ())
				{
		...
		return EnsureVisible(pApp, bPartialOK);
	}
	
	int nHour = 0;
	int nMinute = 0;
	
	if (type == BCGP_PLANNER_EVT_APPOINTMENT_MIN || type == BCGP_PLANNER_EVT_APPOINTMENT_MAX)
	{
		BOOL bFound = FALSE;
		int minHour = 23;
		int minMinute = 59;
		...
		for (int i = 0; i < (int)ar.GetSize(); i++)
		{
			...
			COleDateTime minTime(max(GetDateStart(), pApp->GetStart()));
			COleDateTime maxTime(min(GetDateEnd(), pApp->GetFinish()));

			if (minTime.GetDay() == maxTime.GetDay())
			{
				if (minTime.GetHour() < minHour || (minTime.GetHour() == minHour && minTime.GetMinute() < minMinute))
				{
					minHour = minTime.GetHour();
					minMinute = minTime.GetMinute();
				}
				if (maxTime.GetHour() < minHour || (maxTime.GetHour() == minHour && maxTime.GetMinute() < minMinute))
				{
					minHour = maxTime.GetHour();
					minMinute = maxTime.GetMinute();
				}

				if (maxHour < minTime.GetHour() || (minTime.GetHour() == maxHour && maxMinute < minTime.GetMinute()))
				{
					maxHour = minTime.GetHour();
					maxMinute = minTime.GetMinute();
				}
				if (maxHour < maxTime.GetHour() || (maxTime.GetHour() == maxHour && maxMinute < maxTime.GetMinute()))
				{
					maxHour = maxTime.GetHour();
					maxMinute = maxTime.GetMinute();
				}
			}
			else
			{
			...
			if (minHour == 0 && minMinute == 0 && maxHour == 23 && maxMinute == 59)
			{
				break;
			}
		}
		...
		nHour = type == BCGP_PLANNER_EVT_APPOINTMENT_MIN ? minHour : maxHour;
		nMinute = type == BCGP_PLANNER_EVT_APPOINTMENT_MIN ? minMinute : maxMinute;
	}
	else
	{
		switch (type)
		{
		case BCGP_PLANNER_EVT_VIEW_FIRST:
			nHour = GetFirstViewHour();
			break;
		case BCGP_PLANNER_EVT_VIEW_LAST:
			nHour = GetLastViewHour();
			break;
		case BCGP_PLANNER_EVT_WORK_FIRST:
			nHour = GetFirstWorkingHour();
			nMinute = GetFirstWorkingMinute();
			break;
		case BCGP_PLANNER_EVT_WORK_LAST:
			nHour = GetLastWorkingHour();
			nMinute = GetLastWorkingMinute();
			break;
		case BCGP_PLANNER_EVT_CURRENT:
			{
	...
	COleDateTime time(GetDateStart());
	time.SetDateTime(time.GetYear(), time.GetMonth(), time.GetDay(), nHour, nMinute, 0);

	return EnsureVisibleTime(time, bPartialOK);
}
