...
void CBCGPPlannerViewWorkWeek::OnActivate(CBCGPPlannerManagerCtrl* pPlanner, const CBCGPPlannerView* pOldView)
{
	...
	m_DateEnd   = m_DateStart + COleDateTimeSpan ((IsFullWeek() ? 7 : pPlanner->GetWorkWeekInterval ()) - 1, 23, 59, 59);
...
void CBCGPPlannerViewWorkWeek::SetDate (const COleDateTime& date)
{
	...
	m_DateEnd   = m_DateStart + COleDateTimeSpan ((IsFullWeek() ? 7 : GetWorkWeekInterval ()) - 1, 23, 59, 59);
...
{
	return CBCGPPlannerView::GetFirstWeekDay (date, IsFullWeek() ? (CBCGPPlannerManagerCtrl::GetFirstDayOfWeek () + 1) : 2);
}
...
IMPLEMENT_DYNCREATE(CBCGPPlannerViewWeekDays, CBCGPPlannerViewWorkWeek)

CBCGPPlannerViewWeekDays::CBCGPPlannerViewWeekDays()
...
#ifdef _DEBUG
void CBCGPPlannerViewWeekDays::AssertValid() const
{
	CBCGPPlannerViewWorkWeek::AssertValid();
}

void CBCGPPlannerViewWeekDays::Dump(CDumpContext& dc) const
{
	CBCGPPlannerViewWorkWeek::Dump(dc);
}
#endif

CString CBCGPPlannerViewWeekDays::GetAccName() const
{
	return _T("Full Week View");
}
