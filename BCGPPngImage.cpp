...
#if (_MSC_VER >= 1300) && defined (BCGP_USE_CIMAGE_FOR_PNG)
...
static void PNGAPI _png_write_data(png_structp png_ptr, png_bytep data, png_size_t length)
{
	CFile* pFile = (CFile*)(png_ptr->io_ptr);
	ASSERT_VALID (pFile);
...
static void PNGAPI _png_write_flush(png_structp png_ptr)
{
	CFile* pFile = (CFile*)(png_ptr->io_ptr);
	ASSERT_VALID (pFile);
...
static HBITMAP PngToBitmap(BYTE *pbImage, int cxImgSize, int cyImgSize, int cImgChannels)
{
    ...
    int cImgRowBytes = cImgChannels * cxImgSize;
    int cDIRowBytes = cImgChannels == 4
                                ? (cImgChannels * cxImgSize)
                                : (((3 * cxImgSize + 3L) >> 2) << 2);

    for (int yImg = 0; yImg < cyImgSize; yImg++)
    {
        BYTE* src = pbImage + yImg * cImgRowBytes;
        BYTE* dst = pBits + (cyImgSize - yImg - 1) * cDIRowBytes;
                   ...
                   int *piWidth, int *piHeight, int *piChannels, png_color *pBkgColor)
{
    ...
    try
    {
		...
		if (CBCGPPngImage::m_bUseBackground)
		{
...
BOOL CBCGPPngImage::m_bUseBackground = FALSE;
