...
int CBCGPShadowWnd::OnMouseActivate(CWnd* /*pDesktopWnd*/, UINT /*nHitTest*/, UINT /*message*/) 
{
...
BOOL CBCGPPopupMenu::m_bUseAppMainWndInNotifyParentDlg = FALSE;
...
void CBCGPPopupMenu::Initialize ()
{
	...
	m_bNormalizeLocationInScreen = TRUE;
}
//****************************************************************************************
CBCGPPopupMenu::~CBCGPPopupMenu()
{
...
void CBCGPPopupMenu::RecalcLayout (BOOL /*bNotify*/) 
{
	...
	if (m_bNormalizeLocationInScreen)
	{
		//---------------------------------------
	...
	if (m_pMenuCustomizationPage != NULL)
	{
		...
		CSize sizeBorders = globalUtils.GetSystemBorders(this);
		size.cy += 2 * sizeBorders.cy + globalUtils.ScaleByDPI(5); // Add some extra space for new item dropping.
		size.cx += 2 * sizeBorders.cx;
	}
	else if (bIsTearOff)
	{
	...
	if (pParentMenuBar != NULL && pParentMenuBar->IsFloating ())
	{
		...
		case DROP_DIRECTION_LEFT:
			...
			if (bRTL)
			{
				...
				if (m_bNormalizeLocationInScreen && m_ptLocation.x > rectScreen.right)
				{
			...
			else
			{
				...
				if (m_bNormalizeLocationInScreen && m_ptLocation.x < rectScreen.left)
				{
	...
	//---------------------------------------------
	if (m_bNormalizeLocationInScreen &&
		((bRTL && (m_ptLocation.x - size.cx < rectScreen.left)) || 
	...
	if (m_bNormalizeLocationInScreen && m_ptLocation.y + size.cy > rectScreen.bottom)
	{
			...
			m_DropDirection != DROP_DIRECTION_LEFT && m_DropDirection != DROP_DIRECTION_RIGHT)
		{
            ...
            if (ptTop.y < 0)
            {
				...
				if (m_bScrollable)
				{
					CBCGPToolBar* pParentToolBar = m_pParentBtn == NULL ? NULL : DYNAMIC_DOWNCAST(CBCGPToolBar, m_pParentBtn->m_pWndParent);
	...
	if (m_bNormalizeLocationInScreen && m_ptLocation.y < rectScreen.top)
	{
...
void CBCGPPopupMenu::DoPaint (CDC* pPaintDC)
{
	...
	if (m_iLogoWidth > 0)
	{
		...
		case MENU_LOGO_TOP:
			if (!m_rectResize.IsRectEmpty () && m_bIsResizeBarOnTop)
			{
				rectLogo.top += m_rectResize.Height();
			}
		...
		case MENU_LOGO_BOTTOM:
			if (!m_rectResize.IsRectEmpty () && !m_bIsResizeBarOnTop)
			{
				rectLogo.bottom -= m_rectResize.Height();
			}
		...
		if (m_bGutterLogo && pMenuBar != NULL)
		{
			CRect rectGutter = rectLogo;
			rectGutter.right = rectGutter.left + pMenuBar->GetGutterWidth();
			rectGutter.InflateRect(0, 1);

			rectLogo.left = rectGutter.right;

			CBCGPVisualManager::GetInstance ()->OnFillBarBackground(pPaintDC, pMenuBar, rectGutter, rectGutter);
		}

		if (!OnDrawLogo(pPaintDC, rectLogo))
		{
			pPaintDC->FillRect(rectLogo, &globalData.brBarFace);
		}
	}
	...
	if (m_bScrollable)
	{
		...
		if (IsScrollDnAvailable ())
		{
			if (pMenuBar != NULL)
			{
				...
				CBCGPVisualManager::GetInstance ()->OnFillBarBackground (pPaintDC, pMenuBar, rectFill, rectFill);
			}
...
//*************************************************************************************
BOOL CBCGPPopupMenu::OnDrawLogo(CDC* pDC, const CRect& rectLogo)
{
	CFrameWnd* pWndMain = BCGCBProGetTopLevelFrame(this);
	...
	if (pMainFrame != NULL)
	{
		pMainFrame->OnDrawMenuLogo (pDC, this, rectLogo);
	}
	else	// Maybe, SDI frame...
	{
		CBCGPFrameWnd* pFrame = DYNAMIC_DOWNCAST (CBCGPFrameWnd, pWndMain);
		if (pFrame != NULL)
		{
			pFrame->OnDrawMenuLogo (pDC, this, rectLogo);
		}
		else	// Maybe, OLE frame...
		{
			CBCGPOleIPFrameWnd* pOleFrame = 
			...
			if (pOleFrame != NULL)
			{
				pOleFrame->OnDrawMenuLogo (pDC, this, rectLogo);
			}
			else
			{
				CBCGPOleDocIPFrameWnd* pOleDocFrame = 
				...
				if (pOleDocFrame != NULL)
				{
					pOleDocFrame->OnDrawMenuLogo (pDC, this, rectLogo);
				}
			}
		}
	}
...
{
	if (m_pMessageWnd->GetSafeHwnd() != NULL)
	{
...
//************************************************************************************
void CBCGPPopupMenu::EnableMenuLogo (int iLogoSize, LOGO_LOCATION nLogoLocation/* = MENU_LOGO_LEFT*/, BOOL bDrawGutter/* = FALSE*/)
{
	m_iLogoWidth = iLogoSize;
	m_nLogoLocation = nLogoLocation;
	m_bGutterLogo = bDrawGutter && (m_nLogoLocation == CBCGPPopupMenu::MENU_LOGO_TOP || m_nLogoLocation == CBCGPPopupMenu::MENU_LOGO_BOTTOM);

	CBCGPPopupMenuBar* pMenuBar = GetMenuBar ();
	if (pMenuBar != NULL)
	{
		pMenuBar->m_bGutterLogo = m_bGutterLogo;
	}
...
{
	CBCGPDialog* pDlg = NULL;
	CBCGPPropertyPage* pPropPage = NULL;
	CBCGPPropertySheet* pPropSheet = NULL;

	if (m_pMessageWnd->GetSafeHwnd() != NULL && ::IsWindow(m_pMessageWnd->GetSafeHwnd()))
	{
		pDlg = DYNAMIC_DOWNCAST (CBCGPDialog, m_pMessageWnd);
		pPropPage = DYNAMIC_DOWNCAST (CBCGPPropertyPage, m_pMessageWnd);
	}
	else
	{
		CWnd* pWndMain = m_bUseAppMainWndInNotifyParentDlg && AfxGetApp() != NULL ? AfxGetApp()->GetMainWnd() : AfxGetMainWnd();

		pDlg = DYNAMIC_DOWNCAST (CBCGPDialog, pWndMain);
		if (pDlg == NULL)
		{
			pPropSheet = DYNAMIC_DOWNCAST(CBCGPPropertySheet, pWndMain);
		}
	}

	if (pDlg == NULL && pPropPage == NULL && pPropSheet == NULL)
	{
		return FALSE;
	}
	...
	if (pPropSheet != NULL)
	{
		pPropSheet->SetActiveMenu (bActivate ? this : NULL);
	}
...
//************************************************************************************
HRESULT CBCGPPopupMenu::get_accParent(IDispatch **ppdispParent)
{
	...
	*ppdispParent = NULL;
	...
	{
		return AccessibleObjectFromWindow(pOwner->GetSafeHwnd(), (DWORD)OBJID_WINDOW, IID_IAccessible, (void**)ppdispParent);
	}

	return S_FALSE;
}
//************************************************************************************
HRESULT CBCGPPopupMenu::get_accChildCount(long *pcountChildren)
{
	if( !pcountChildren )
    {
        return E_INVALIDARG;
...
//************************************************************************************
HRESULT CBCGPPopupMenu::get_accChild(VARIANT varChild, IDispatch **ppdispChild)
{
	if (!ppdispChild)
	{
		return E_INVALIDARG;
	...
	if (varChild.vt == VT_I4 && varChild.lVal != CHILDID_SELF)
	{
		...
		if (pMenuBar != NULL)
		{
			return AccessibleObjectFromWindow(pMenuBar->GetSafeHwnd(), (DWORD)OBJID_CLIENT, IID_IAccessible, (void**) ppdispChild);
		}
	}
...
//************************************************************************************
HRESULT CBCGPPopupMenu::get_accRole(VARIANT varChild, VARIANT *pvarRole)
{
	if( !pvarRole )
    {
        return E_INVALIDARG;
...
//************************************************************************************
HRESULT CBCGPPopupMenu::get_accState(VARIANT varChild, VARIANT *pvarState)
{
	if( !pvarState )
    {
        return E_INVALIDARG;
