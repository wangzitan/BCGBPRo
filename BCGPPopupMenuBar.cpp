...
//***************************************************************
CSize CBCGPPopupMenuBar::GetPopupMenuImageSize() const
{
	if (m_bToolBarImagesInDropDownMenus)
	{
		return globalUtils.ScaleByDPI(GetImageSize());
	}

	return GetMenuImageSize();
}
//***************************************************************
void CBCGPPopupMenuBar::AdjustLocations ()
{
	...
	CSize sizeMenuButton = GetPopupMenuImageSize();
	...
	for (POSITION pos = m_Buttons.GetHeadPosition (); pos != NULL;)
	{
		...
		if (CBCGPToolBar::m_bUpdateUnpermittedCommands && !CBCGPToolBar::IsCommandPermitted(pButton->m_nID))
		{
		...
		if (pButton->m_nStyle & TBBS_SEPARATOR)
		{
			...
			rectButton.bottom = rectButton.top + globalUtils.ScaleByDPI(iSeparatorHeight);
		}
		else
		{
...
{
	return CSize(globalUtils.ScaleByDPI(iEmptyMenuWidth), globalUtils.ScaleByDPI(iEmptyMenuHeight));
}
//***************************************************************************************
CSize CBCGPPopupMenuBar::CalcSize (BOOL /*bVertDock*/)
{
	...
	BOOL bIsEmpty = m_Buttons.IsEmpty();

	if (CBCGPToolBar::m_bUpdateUnpermittedCommands)
	{
		bIsEmpty = TRUE;
		...
		for (POSITION pos = m_Buttons.GetHeadPosition (); pos != NULL;)
		{
			...
			if (CBCGPToolBar::IsCommandPermitted(pButton->m_nID))
			{
				bIsEmpty = FALSE;
				break;
			}
	...
	else
	{
		...
		for (POSITION pos = m_Buttons.GetHeadPosition (); pos != NULL;)
		{
			...
			if (pButton->m_nStyle & TBBS_SEPARATOR)
			{
				iHeight = globalUtils.ScaleByDPI(iSeparatorHeight);
			}
			else
			{
				...
				{
					sizeButton.cx += globalUtils.ScaleByDPI(iMinTabSpace);
				}
	...
	for (int i = 0; i < iCount; i ++)
	{
        ...
        else
        {
				...
				CBCGPToolBar::IsCommandPermitted (uiCmd))
			{
				...
				UINT nHotKey = CBCGPToolbarButton::GetHotKey(strText);
				if (nHotKey != 0)
				{
					m_HiddenItemsAccel.SetAt(nHotKey, uiCmd);
				}
			}
		}
	}
...
int CBCGPPopupMenuBar::GetGutterWidth ()
{
	...
	int cx = 0;
	int cxImage = GetPopupMenuImageSize().cx;
