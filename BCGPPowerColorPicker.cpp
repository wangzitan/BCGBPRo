...
void CBCGPColorPickerCtrl::DrawCursor (CDC* pDC, const CRect& rect)
{
	...
	const BOOL bIsFocused = (GetFocus() == this);

	if (m_COLORTYPE == PICKER)
	{
		COLORREF colorFocus = bIsFocused ? colorBlack : colorWhite;
	...
	else if (m_COLORTYPE == PICKERH)
	{
		...
		const int iXMiddle = rectArrow.CenterPoint().x;

		rectArrow.DeflateRect (0, rectArrow.Height () / 3);
		rectArrow.left = iXMiddle - rectArrow.Height ();
		rectArrow.right = iXMiddle + rectArrow.Height ();

		const int iHalfWidth = (int)(0.5 + 0.5 * rectArrow.Width());
		...
		pts[2].y = rectArrow.bottom;

		CBrush brArrow (bIsFocused ? colorBlack : colorWhite);
		CPen penArrow (PS_SOLID, 1, bIsFocused ? colorWhite : colorBlack);
	...
	else if (m_COLORTYPE == LUMINANCE)
	{
		...
		CBrush br (bIsFocused ? clrLine : (m_bVisualManagerStyle ? globalData.clrBarShadow : globalData.clrBtnShadow));
...
void CBCGPColorPickerCtrl::DrawItem (LPDRAWITEMSTRUCT lpDIS)
{
	...
	case PICKERH:
		...
		{
			pDC->Draw3dRect (rectClient, globalData.clrBarShadow, globalData.clrBarShadow);
		}
		else
		{
...
HRESULT CBCGPColorPickerCtrl::get_accName(VARIANT varChild, BSTR *pszName)
{
...
HRESULT CBCGPColorPickerCtrl::get_accValue(VARIANT varChild, BSTR *pszValue)
{
	...
	{
		CString str;
		if (!CBCGPColorBar::GetColorName(m_colorNew, str))
		{
			str.Format (_T("Hex={%02X,%02X,%02X}"), 
				GetRValue (m_colorNew), GetGValue (m_colorNew), GetBValue (m_colorNew));
		}
