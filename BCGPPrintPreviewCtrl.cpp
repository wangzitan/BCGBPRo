...
{
	if (DYNAMIC_DOWNCAST(CHtmlView, m_pPrintView) != NULL)
	{
		return FALSE;
	}
...
BOOL CBCGPPrintPreviewCtrl::OnMouseWheel(UINT nFlags, short zDelta, CPoint /*pt*/) 
{
	...
	{
		int nZoom = bcg_clamp((int)(GetZoom() * 100.0) + ((zDelta > 0) ? 10 : -10), 10, m_nMaxZoomLevel);
		if (nZoom != m_nZoomType)
		{
			SetZoomType(nZoom, TRUE);
			OnZoomChanged();
		}
