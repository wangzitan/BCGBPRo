...
IMPLEMENT_DYNAMIC(CBCGPProgressDlgCtrl, CBCGPProgressCtrl)

void CBCGPProgressDlgCtrl::OnAnimationIdle()
{
	if (m_pDlg != NULL && ::IsWindow(m_pDlg->GetSafeHwnd()) && !m_pDlg->m_bCancelled)
	{
		m_pDlg->PostMessage(UM_PUMPMESSAGE);
	}
}
...
CBCGPProgressDlg::~CBCGPProgressDlg()
{
	...
	if (m_pWndProgressCustom != NULL)
	{
		delete m_pWndProgressCustom;
	}
}
//**************************************************************************************
...
BOOL CBCGPProgressDlg::OnInitDialog() 
{
	...
	if (m_Params.m_pProgressCtrlRTI != NULL)
	{
		if (!m_Params.m_pProgressCtrlRTI->IsDerivedFrom(RUNTIME_CLASS(CBCGPProgressDlgCtrl)))
		{
			ASSERT(FALSE);
		}
		else
		{
			m_pWndProgressCustom = DYNAMIC_DOWNCAST(CBCGPProgressDlgCtrl, m_Params.m_pProgressCtrlRTI->CreateObject());
			if (m_pWndProgressCustom == NULL)
			{
				ASSERT(FALSE);
			}
			...
			{
				m_pWndProgressCustom->SubclassWindow(m_wndProgress.UnsubclassWindow());

				m_pWndProgressCustom->m_pDlg = this;
				m_pWndProgressCustom->m_bVisualManagerStyle = IsVisualManagerStyle();
			}
		}
	}

    GetProgressCtrl().SetRange32 (m_Params.m_nRangeMin, m_Params.m_nRangeMax);
    GetProgressCtrl().SetStep (m_Params.m_nStep);
    GetProgressCtrl().SetPos (m_Params.m_nRangeMin);

	int nProgressNewHeight = m_Params.m_nProgressHeight;

	CRect rectProgress;
	GetProgressCtrl().GetWindowRect(rectProgress);

	if (m_Params.m_bShowInfiniteProgress)
	{
		GetProgressCtrl().m_MarqueeStyle = m_Params.m_MarqueeStyle;
		GetProgressCtrl().m_clrMarquee = m_Params.m_clrMarquee;
		GetProgressCtrl().m_clrMarqueeGradient = m_Params.m_clrMarqueeGradient;

		GetProgressCtrl().ModifyStyle (0, PBS_MARQUEE);

		if (GetProgressCtrl().m_MarqueeStyle == CBCGPProgressCtrl::BCGP_MARQUEE_GRADIENT && nProgressNewHeight == 0)
		{
			// Deflate progress for the better look:
			int nProgressHeight = globalUtils.ScaleByDPI(4);

			if (nProgressHeight < rectProgress.Height())
			{
				nProgressNewHeight = nProgressHeight;
			}
		}

		GetProgressCtrl().PostMessage (PBM_SETMARQUEE, 1, 0);

		m_bAnimation = (GetProgressCtrl().m_MarqueeStyle == CBCGPProgressCtrl::BCGP_MARQUEE_DOTS);
	}

	int cyProgressExtra = 0;

	if (nProgressNewHeight > 0)
	{
		ScreenToClient(rectProgress);

		cyProgressExtra = max(0, nProgressNewHeight - rectProgress.Height());
		
		if (cyProgressExtra == 0)
		{
			rectProgress.top = rectProgress.CenterPoint().y - nProgressNewHeight / 2;
		}
		else
		{
			cy += cyProgressExtra;
		...
		rectProgress.bottom = rectProgress.top + nProgressNewHeight;
		GetProgressCtrl().MoveWindow(rectProgress, FALSE);
	}
	...
	if (cyOffset != 0)
	{
		...
		OffsetWnd (&GetProgressCtrl(), cyOffset);
	...
	else if (!m_Params.m_bShowPercentage || m_Params.m_bShowInfiniteProgress)
	{
		...
		GetProgressCtrl().SetWindowPos (NULL, -1, -1, rectProgress.Width (), rectProgress.Height (),
			SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE);
	}

	int cyOffsetBottom = cyOffset + cyProgressExtra;

	if (cyOffsetBottom != 0)
	{
		OffsetWnd (&m_wndMessage2, cyOffsetBottom);
		OffsetWnd (&m_wndLine, cyOffsetBottom);
		OffsetWnd (&m_wndCancel, cyOffsetBottom);
	}
...
long CBCGPProgressDlg::SetPos(long nPos)
{
	...
	long lResult = GetProgressCtrl().SetPos(nPos);
	UpdatePercent(nPos);
...
long CBCGPProgressDlg::OffsetPos (long nPos)
{
    ...
    long lResult = GetProgressCtrl().OffsetPos(nPos);
...
long CBCGPProgressDlg::StepIt(BOOL bWaitForMessages)
{
    ...
    long lResult = GetProgressCtrl().StepIt();
...
void CBCGPProgressDlg::UpdatePercent (long nNewPos)
{
	if (m_Params.m_bShowInfiniteProgress)
	{
		...
		GetProgressCtrl().OnUpdatePos(m_nCurPos);
		return;
	}

	GetProgressCtrl().OnUpdatePos(nNewPos);
...
//**************************************************************************************
BOOL CBCGPProgressDlg::PreTranslateMessage(MSG* pMsg) 
{
	if (pMsg->message == WM_NCRBUTTONUP)
	{
		// Don't show the system menu while progress dialog is running
		return TRUE;
	}
