...
public:
	...
	CRuntimeClass*	m_pProgressCtrlRTI;	// Should be derived from CBCGPProgressDlgCtrl
	...
	CBCGPProgressDlgParams& operator= (const CBCGPProgressDlgParams& src)
	{
		...
		m_nProgressHeight	= src.m_nProgressHeight;
		...
		m_pProgressCtrlRTI	= src.m_pProgressCtrlRTI;
...
class BCGCBPRODLLEXPORT CBCGPProgressDlgCtrl : public CBCGPProgressCtrl
{
	DECLARE_DYNAMIC(CBCGPProgressDlgCtrl)

	friend class CBCGPProgressDlg;

	CBCGPProgressDlgCtrl(CBCGPProgressDlg* pDlg = NULL) :
		m_pDlg(pDlg)
	{
	}

	virtual void OnAnimationIdle();
	virtual void OnUpdatePos(int nNewPos)
	{
		UNREFERENCED_PARAMETER(nNewPos);
	}
	
	CBCGPProgressDlg* m_pDlg;
};
...
// Operations:
	...
	CBCGPProgressDlgCtrl& GetProgressCtrl()
	{
		return m_pWndProgressCustom != NULL ? (*m_pWndProgressCustom) : m_wndProgress;
	}
...
protected:
	...
	CBCGPProgressDlgCtrl*	m_pWndProgressCustom;
};
