...
BOOL CBCGPProp::AddSubItem (CBCGPProp* pProp)
{
	...
	if (pProp->IsToggleControl())
	{
		pProp->m_dwFlags = PROP_HAS_BUTTON;
	}
...
CBCGPProp* CBCGPProp::HitTest (CPoint point, ClickArea* pnArea)
{
	...
	if (m_Rect.PtInRect (point))
	{
		...
		if (pnArea != NULL)
		{
			if (IsGroup () && point.x < m_Rect.left + m_pWndList->GetRowHeight(TRUE))
			{
				*pnArea = ClickExpandBox;
			}
			else if (IsGroup () && !m_bIsValueList && m_pWndList->IsGroupNameFullWidth() && point.x > m_Rect.left + m_pWndList->GetRowHeight(TRUE))
			{
...
//*******************************************************************************************
BOOL CBCGPProp::IsModified(BOOL bIncludeSubItems /*= FALSE*/) const
{
	ASSERT_VALID (this);

	if (!bIncludeSubItems || m_bIsModified)
	{
...
void CBCGPProp::Repos (int& y)
{
	...
	if (bShowProperty)
	{
		int dx = m_pWndList->m_bAlphabeticMode ?
			m_pWndList->GetRowHeight(TRUE) :
			GetHierarchyLevel () * m_pWndList->GetRowHeight(TRUE);
		...
		m_nRow = m_pWndList->m_nCurrReposedRow++;
		...
		if (!m_rectButton.IsRectEmpty () && !IsToggleControl())
		{
...
void CBCGPProp::OnDrawValue (CDC* pDC, CRect rect)
{
	...
	{
		uiDTFlags |= DT_WORDBREAK;
		rect.DeflateRect(0, TEXT_MARGIN / 2 + m_pWndList->m_nRowPadding);
	}
...
void CBCGPProp::OnDrawButton (CDC* pDC, CRect rect)
{
	...
	if (IsCheckBox())
	{
		...
		if (m_pWndList->DrawControlBarColors())
		{
			bDrawThemed = FALSE;
		}
		else
		{
			bDrawThemed = visualManager->CBCGPVisualManager::DrawCheckBox(pDC, rect, m_bButtonIsHighlighted && m_bEnabled, (bool)m_varValue, m_bEnabled, FALSE);
		}

		if (!bDrawThemed)
		{
			CBCGPVisualManager::GetInstance ()->OnDrawCheckBoxEx(pDC, rect, (bool)m_varValue, m_bButtonIsHighlighted && m_bEnabled, FALSE, m_bEnabled);
		}
	...
	if (IsSwitch())
	{
		const BOOL bOn = (bool)m_varValue;

		if (m_pWndList->DrawControlBarColors())
		{
			CBCGPVisualManager::GetInstance ()->OnDrawSwitch(pDC, rect, bOn, m_bButtonIsHighlighted && m_bEnabled, FALSE, m_bEnabled);
		}
		else
		{
			visualManager->CBCGPVisualManager::OnDrawSwitch(pDC, rect, bOn, m_bButtonIsHighlighted && m_bEnabled, FALSE, m_bEnabled);
		}

		CString strSwitchLabel = bOn ? m_strSwitchOn : m_strSwitchOff;
		if (!strSwitchLabel.IsEmpty())
		{
			CRect rectLabel = m_Rect;
			UINT uFlags = DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS;
			
			if (m_pWndList->m_nBooleanPropertiesHorzAlign == TA_RIGHT)
			{
				rectLabel.left = m_pWndList->m_rectList.left + m_pWndList->m_nLeftColumnWidth;
				rectLabel.right = rect.left;

				uFlags |= DT_RIGHT;
			}
			else
			{
				rectLabel.left = rect.right;
			}
			
			rectLabel.DeflateRect(TEXT_MARGIN, 0);

			pDC->DrawText(strSwitchLabel, rectLabel, uFlags);

			m_bValueIsTrancated = pDC->GetTextExtent(strSwitchLabel).cx > rectLabel.Width();
		}

		return;
	}

	BOOL bControlBarColors = m_pWndList->DrawControlBarColors() && !m_pWndList->IsButtonsDefaultTheme();

	if (m_dwFlags & PROP_HAS_LIST)
	{
		visualManager->OnDrawPropListComboButton(pDC, rect, this, bControlBarColors, m_bButtonIsFocused, m_bEnabled, m_bButtonIsDown, m_bButtonIsHighlighted);
		return;
	}

	COLORREF clrText = visualManager->OnDrawPropListPushButton(pDC, rect, this, bControlBarColors, m_bButtonIsFocused, m_bEnabled, m_bButtonIsDown, m_bButtonIsHighlighted);
...
BOOL CBCGPProp::OnEdit (LPPOINT /*lptClick*/)
{
	...
	if (m_pWndInPlace != NULL)
	{
		...
		if (m_dwFlags & PROP_HAS_LIST)
		{
			const int nOffset = globalUtils.ScaleByDPI(4);

			CRect rectCombo = m_Rect;
			rectCombo.left = rectEdit.left - nOffset;

			m_pWndCombo = CreateCombo (m_pWndList, rectCombo);
			ASSERT_VALID (m_pWndCombo);

			CBCGPComboBox* pWndCombo = DYNAMIC_DOWNCAST(CBCGPComboBox, m_pWndCombo);
			if (pWndCombo != NULL)
			{
				const int nVertPadding = m_pWndList->GetRowPadding();
				if (nVertPadding > 0)
				{
					pWndCombo->SetVerticalPadding(nVertPadding);
				}

				pWndCombo->SetHorizontalPadding(pWndCombo->GetHorizontalPadding() + nOffset);
			}
			...
			for (POSITION pos = m_lstOptions.GetHeadPosition (); pos != NULL; nIndex++)
			{
				OnAddComboBoxItem(nIndex, m_lstOptions.GetNext (pos), GetOptionData(nIndex));
			}
		}
...
//******************************************************************************************
int CBCGPProp::OnAddComboBoxItem(int nIndex, const CString& strItem, DWORD_PTR dwItemData)
{
...
void CBCGPProp::AdjustButtonRect ()
{
	...
	if (IsToggleControl())
	{
		CSize sizeControl(0, 0);
		
		if (IsCheckBox())
		{
			if (m_pWndList->DrawControlBarColors())
			{
				sizeControl = visualManager->GetCheckRadioDefaultSize();
			}
			else
			{
				sizeControl = visualManager->CBCGPVisualManager::GetCheckRadioDefaultSize();
			}
		}
		else
		{
			sizeControl.cy = m_Rect.Height() - max(globalUtils.ScaleByDPI(2), m_Rect.Height() / 4);
			sizeControl.cx = 2 * sizeControl.cy;
		}

		int xLeft = m_pWndList->m_rectList.left + m_pWndList->m_nLeftColumnWidth;
		int xRight = m_pWndList->m_rectList.right;

		switch (m_pWndList->m_nBooleanPropertiesHorzAlign)
		{
		case TA_LEFT:
			m_rectButton.left = xLeft + TEXT_MARGIN;
			break;

		case TA_CENTER:
			m_rectButton.left = max(xLeft, (xRight + xLeft - sizeControl.cx) / 2);
			break;
			
		case TA_RIGHT:
			m_rectButton.left = max(xLeft, xRight - sizeControl.cx - TEXT_MARGIN);
			break;
		}

		m_rectButton.right = m_rectButton.left + sizeControl.cx;

		m_rectButton.top += max(0, (int)(0.5 + 0.5 * (m_rectButton.Height() - sizeControl.cy)));
		m_rectButton.bottom = m_rectButton.top + sizeControl.cy;

		if (IsSwitch())
		{
			m_nValueToolTipXOffset = m_rectButton.right - xLeft;
		}
...
void CBCGPProp::AdjustInPlaceEditRect (CRect& rectEdit, CRect& rectSpin)
{
	...
	if (m_dwFlags & PROP_HAS_SPIN)
	{
		...
		rectSpin.left = rectSpin.right - m_pWndList->GetRowHeight(TRUE);
		rectSpin.top ++;
...
CSpinButtonCtrl* CBCGPProp::CreateSpinControl (CRect rectSpin)
{
	...
	pWndSpin->m_bVisualManagerStyle = m_pWndList->DrawControlBarColors();
...
BOOL CBCGPProp::IsButtonVisible() const
{
	...
	return m_pWndList->GetCurSel() == this || (m_pWndList->IsContextMenuEnabled() && (m_dwFlags & PROP_HAS_LIST) == 0) || IsToggleControl();
}
//****************************************************************************************
void CBCGPProp::OnClickButton (CPoint /*point*/)
{
	...
	if (IsToggleControl())
	{
		m_varValue = (bool) !(bool)m_varValue;

		if (IsSwitch() && (!m_strSwitchOn.IsEmpty() || !m_strSwitchOff.IsEmpty()))
		{
			Redraw();
		}
		else
...
void CBCGPProp::OnClickMenuButton (CPoint point)
{
...

	{
		...
		if (!m_pWndList->m_lstCustomMenuItems.IsEmpty())
		{
			...
			{
				strItem = m_pWndList->m_lstCustomMenuItems.GetNext (pos);
				menu.AppendMenu (MF_STRING, nID, strItem);

				UINT nState = OnGetMenuCustomItemState(nCustomMenuIndex);
...
HBRUSH CBCGPProp::OnCtlColor(CDC* pDC, UINT /*nCtlColor*/)
{
	...
	const COLORREF clrAlt = GetAlternateColor();

	if (clrAlt != CLR_NONE)
	{
		pDC->SetBkColor(clrAlt);

		if (m_pWndList->m_brAlternate.GetSafeHandle() != NULL)
		{
			LOGBRUSH lbr;
			m_pWndList->m_brAlternate.GetLogBrush(&lbr);
			
			if (lbr.lbColor != clrAlt)
			{
				m_pWndList->m_brAlternate.DeleteObject();
				m_pWndList->m_brAlternate.CreateSolidBrush(clrAlt);
			}
		}
		else
		{
		...
		return (HBRUSH)m_pWndList->m_brAlternate.GetSafeHandle();
	}
...
//**************************************************************************************
BOOL CBCGPProp::IsDroppedDown() const
{
	ASSERT_VALID(this);	

	return m_pWndCombo->GetSafeHwnd() != NULL && m_pWndCombo->GetDroppedState();
}
//**************************************************************************************
...
//**************************************************************************************
COLORREF CBCGPProp::GetAlternateColor() const
{
	ASSERT_VALID(this);
	ASSERT_VALID(m_pWndList);

	if ((!IsGroup() || m_bIsValueList) && m_pWndList->m_bAlternateRowColor && m_nRow >= 0 && (m_nRow % 2) != 0)
	{
		return m_pWndList->GetAlternateColor();
	}

	return CLR_NONE;
}
//**************************************************************************************
void CBCGPProp::SetSwitchLabels(LPCTSTR lpszOn, LPCTSTR lpszOff)
{
	ASSERT_VALID(this);

	m_strSwitchOn = lpszOn == NULL ? _T("") : lpszOn;
	m_strSwitchOff = lpszOff == NULL ? _T("") : lpszOff;
}
//**************************************************************************************
...
BOOL CBCGPProp::SetACCData (CWnd* pParent, CBCGPAccessibilityData& data)
{
	...
	CBCGPRAII<BOOL> raiiAccData(m_bFillAccData, FALSE);
...
//**********************************************************************************
BOOL CBCGPProp::NoInplaceEdit() const
{
	return IsToggleControl();
}
//**********************************************************************************
void CBCGPProp::OnUpdateStyle()
{
	ASSERT_VALID (this);
	ASSERT_VALID (m_pWndList);
...
//**********************************************************************************
BOOL CBCGPProp::IsCheckBox() const
{
	if (m_pWndList == NULL)
	{
		return FALSE;
	...
	ASSERT_VALID (m_pWndList);
	return (m_varValue.vt == VT_BOOL && m_pWndList->GetBooleanPropertiesStyle() == CBCGPPropList::BOOL_PROPS_CHECK_BOX);
}
//**********************************************************************************
BOOL CBCGPProp::IsSwitch() const
{
	if (m_pWndList == NULL)
	{
		return FALSE;
	...
	ASSERT_VALID (m_pWndList);
	return (m_varValue.vt == VT_BOOL && m_pWndList->GetBooleanPropertiesStyle() == CBCGPPropList::BOOL_PROPS_SWITCH);
}
...
BOOL CBCGPColorProp::OnEdit (LPPOINT /*lptClick*/)
{
	...
	pWndEdit->SetValidChars(m_strValidInputCharacters.IsEmpty() ? _T("01234567890ABCDEFabcdef") : (LPCTSTR)m_strValidInputCharacters);
...
//********************************************************************************
void CBCGPColorProp::SetWebColors()
{
...
void CBCGPBrushProp::OnDrawValue (CDC* pDC, CRect rect)
{
	...
	if (m_hbmp == NULL && !m_Brush.IsEmpty())
	{
		...
		if (pGM != NULL)
		{
			...
			if (m_hbmp != NULL)
			{
				CBCGPRect rectDraw(CBCGPPoint(), size);
				...
				pGM->BindDC(&dcMem, rectDraw);
				pGM->BeginDraw();

				pGM->FillRectangle(rectDraw, CBCGPBrush(CBCGPColor::White));
				pGM->FillRectangle(rectDraw, m_Brush);
...
CString CBCGPTextFormatProp::FormatProperty ()
{
	...
	if (dblSize < 0.0)
	{
		...
		if (nLogY != 0)
		{
			str.Format( _T("%s(%i)"), (LPCTSTR)m_TextFormat.GetFontFamily(),
	...
	{
		str.Format( _T("%s(%i)"), (LPCTSTR)m_TextFormat.GetFontFamily(), (int)dblSize);
	}
...
CBCGPDateTimePropAttributes::CBCGPDateTimePropAttributes()
{
	...
	m_bDisplayCalendarTodayButton = dateTime.IsCalendarTodayButtonEnabled();
}
...
void CBCGPDateTimeProp::OnDrawValue (CDC* pDC, CRect rect)
{
	...
	m_wndDateTime.m_bVisualManagerStyle = m_pWndList->DrawControlBarColors();
	...
	m_wndDateTime.SetBackgroundColor (clrFill, FALSE);
...
CWnd* CBCGPDateTimeProp::CreateInPlaceEdit (CRect rectEdit, BOOL& bDefaultFormat)
{
	...
	pDateTime->SetReadOnly(!m_bAllowEdit);
	...
	pDateTime->Create (_T(""), WS_CHILD | BS_NOTIFY, rectEdit, m_pWndList, BCGPROPLIST_ID_INPLACE);
	pDateTime->SetFont (
		IsModified () && m_pWndList->m_bMarkModifiedProperties ? 
			&m_pWndList->m_fontBold : m_pWndList->GetFont ());
	...
	pDateTime->EnableCalendarTodayButton(m_Attributes.m_bDisplayCalendarTodayButton);

	pDateTime->ShowWindow(SW_SHOWNOACTIVATE);
...
void CBCGPDateTimeProp::OnPosSizeChanged (CRect /*rectOld*/)
{
	...
	if (m_wndDateTime.GetSafeHwnd () == NULL)
	{
		...
		m_wndDateTime.SetReadOnly(!m_bAllowEdit);
...
//********************************************************************************
BOOL CBCGPDateTimeProp::IsEditAvailable()
{
	if (!m_bAllowEdit)
	{
		return FALSE;
	...
	return CBCGPProp::IsEditAvailable();
}
//**********************************************************************************
BOOL CBCGPDateTimeProp::IsPasteAvailable()
{
	if (!m_bAllowEdit)
	{
		return FALSE;
	...
	return CBCGPProp::IsPasteAvailable();
}
//********************************************************************************
...
CBCGPBitwiseProp::CBCGPBitwiseProp(const CString& strName, const CStringArray& arFlagNames, ULONG ulFlags, DWORD_PTR dwData, LPCTSTR lpszDescr, const CStringArray* parFlagDescriptions, const CArray<ULONG, ULONG>* parAllowedFlags) :
	CBCGPProp (strName, _variant_t(0L), lpszDescr, dwData)
{
	Rebuild(arFlagNames, ulFlags, parFlagDescriptions, parAllowedFlags);
}
//**********************************************************************************
CBCGPBitwiseProp::CBCGPBitwiseProp(const CString& strName, UINT nID, const CStringArray& arFlagNames, ULONG ulFlags, DWORD_PTR dwData, LPCTSTR lpszDescr, const CStringArray* parFlagDescriptions, const CArray<ULONG, ULONG>* parAllowedFlags) :
...
//**********************************************************************************	
void CBCGPBitwiseProp::Rebuild(const CStringArray& arFlagNames, ULONG ulFlags, const CStringArray* parFlagDescriptions, const CArray<ULONG, ULONG>* parAllowedFlags)
{
	m_bGroup = TRUE;
	...
	int nSize = (int)arFlagNames.GetSize();
	if (nSize > 31)
	{
	...
	m_arAllowedFlags.RemoveAll();

	if (parAllowedFlags != NULL)
	{
		if (parAllowedFlags->GetSize() != arFlagNames.GetSize())
		{
			ASSERT(FALSE);
			parAllowedFlags = NULL;
		}
		else
		{
			m_arAllowedFlags.Append(*parAllowedFlags);
		}
	}
	...
	{
		bool bValue = m_arAllowedFlags.GetSize() > 0 ? ((m_arAllowedFlags[i] & ulFlags) == m_arAllowedFlags[i]) : ((nShift & ulFlags) != 0);
...
CString CBCGPBitwiseProp::FormatProperty()
{
	...
	for (POSITION pos = m_lstSubItems.GetHeadPosition (); pos != NULL; i++)
	{
		CBCGPProp* pProp = m_lstSubItems.GetNext (pos);
		ASSERT_VALID (pProp);
		...
		{
			if (i < m_arAllowedFlags.GetSize())
			{
				ulFlags |= m_arAllowedFlags[i];
			}
			else
			{
				ulFlags |= nShift;
	...
	m_wndToolBar(m_ToolBarCustomImages)
{
	...
	m_BooleanPropertiesStyle = BOOL_PROPS_LABEL;
	m_nBooleanPropertiesHorzAlign = TA_LEFT;
...
void CBCGPPropList::Init ()
{
	...
	hdItem.pszText = (LPTSTR)_T("Property");
	hdItem.cchTextMax = 100;

	GetHeaderCtrl ().InsertItem (0, &hdItem);

	hdItem.fmt = HDF_LEFT;
	hdItem.pszText = (LPTSTR)_T("Value");
	hdItem.cchTextMax = 100;
...
//******************************************************************************************
COLORREF CBCGPPropList::GetAlternateColor()
{
	...
	COLORREF clrAlternate = (m_clrGroupBackground != (COLORREF)-1) ? m_clrGroupBackground : (m_bVSDotNetLook ?  m_clrGray : globalData.clrBarLight);
	
	if (CBCGPDrawManager::IsDarkColor(clrAlternate))
	{
		clrAlternate = m_bVSDotNetLook ? CBCGPDrawManager::ColorMakeDarker(clrAlternate) : CBCGPDrawManager::ColorMakeLighter(clrAlternate);
	}
	else if (CBCGPDrawManager::IsPaleColor(clrAlternate))
	{
		clrAlternate = CBCGPDrawManager::ColorMakeDarker(clrAlternate, .025);
	}
	else
	{
		clrAlternate = CBCGPDrawManager::ColorMakePale(clrAlternate, .97);
	}

	return clrAlternate;
}
//******************************************************************************************
BOOL CBCGPPropList::OnGestureEventPan(const CPoint& ptFrom, const CPoint& ptTo, CSize& sizeOverPan)
{
	CBCGPPopupMenu* pMenuActive = CBCGPPopupMenu::GetSafeActivePopupMenu();
	if (pMenuActive != NULL && !globalUtils.IsPlacedOnActiveMenu(this))
	{
	...
	const int nDelta = (ptFrom.y - ptTo.y);

	int nVertScrollOffset = m_nVertScrollOffset + nDelta;
	int nVertScrollOffsetPrev = m_nVertScrollOffset;

	SetScrollPos(SB_VERT, nVertScrollOffset, FALSE);
	OnVScroll(SB_THUMBPOSITION, nVertScrollOffset, NULL);

	if (nVertScrollOffsetPrev == m_nVertScrollOffset)
	{
...
//******************************************************************************************
CWnd* CBCGPPropList::DoCreateOnPlaceHolder(CWnd* pParent, UINT nID, DWORD dwStyle, const CRect& rect, LPVOID /*lParam*/)
{
...
void CBCGPPropList::InitToolBar()
{
	...
	m_wndFilter.SetSimplifiedBrowseIcon(m_bFilterSimplifiedIcon);
	m_wndFilter.ModifyStyleEx(0, WS_EX_CLIENTEDGE, SWP_FRAMECHANGED);

	CBCGPGestureConfig gestureConfig;
	gestureConfig.EnablePan();
	
	bcgpGestureManager.SetGestureConfig(m_wndFilter.GetSafeHwnd(), gestureConfig);
}
//******************************************************************************************
...
void CBCGPPropList::AdjustLayout ()
{
	...
	m_nRowHeight = tm.tmHeight + TEXT_MARGIN + m_nRowPadding * 2;
	...
	if (m_bToolBar || m_bFilterBox)
	{
		...
		const int nFilterBoxWidth = rectClient.Width() - nToolbarWidth - 2 * nMarginX;

		if (m_bFilterBox && nFilterBoxWidth > globalUtils.ScaleByDPI(40))
		{
			...
			const int nExtraHeight = (int)globalUtils.ScaleByDPI(1.5);

			const int nFilterBoxHeight = max(m_nToolbarHeight - globalUtils.ScaleByDPI(2), rectEdit.Height() + nExtraHeight);
			const int nYOffset = max(0, (nMarginY + m_nToolbarHeight - nFilterBoxHeight) / 2);

			m_wndFilter.SetVerticalAlignment(TA_CENTER);

			m_wndFilter.SetWindowPos (NULL, rectClient.left + nToolbarWidth + nMarginX, rectClient.top + nYOffset, 
				nFilterBoxWidth, nFilterBoxHeight, SWP_NOZORDER | SWP_NOACTIVATE | SWP_FRAMECHANGED);
	...
	if (m_bHeaderCtrl)
	{
		m_nHeaderHeight = GetRowHeight(TRUE) + globalUtils.ScaleByDPI(4);
	...
	{
		m_nCommandsHeight = max (m_nCommandsHeight, GetRowHeight(TRUE));
		m_nCommandsHeight = min (m_nCommandsHeight, rectClient.Height () - GetRowHeight(TRUE));
	...
	{
		m_nDescrHeight = max (m_nDescrHeight, GetRowHeight(TRUE));
		m_nDescrHeight = min (m_nDescrHeight, rectClient.Height () - GetRowHeight(TRUE));
	...
	if (HasCommands () && m_bAreCommandsVisible && m_nCommandsHeight != -1 && rectClient.Height() > 0)
	{
		...
		int y = rectCommands.top + GetRowHeight(TRUE) / 4;
...
void CBCGPPropList::ReposProperties ()
{
	...
	int y = m_rectList.top - GetRowHeight() * m_nVertScrollOffset - 1;
...
void CBCGPPropList::CalcEditMargin ()
{
	...
	m_nEditTopMargin = max(0, (int)(.5 + 0.5 * (GetRowHeight() - rectEdit.Height() + 2)));

	editDummy.SetFont (&m_fontBold);
	m_nBoldEditLeftMargin = LOWORD (editDummy.GetMargins ());

	editDummy.GetClientRect(rectEdit);
	m_nBoldEditTopMargin = max(0, (int)(.5 + 0.5 * (GetRowHeight() - rectEdit.Height() + 2)));

	editDummy.DestroyWindow ();
}
//******************************************************************************************
void CBCGPPropList::AdjustAreas()
{
	BOOL bAdjustLayout = FALSE;
	
	if (m_nDescrHeight == -1)
	{
		m_nDescrHeight = GetRowHeight(TRUE) * m_nDescrRows + GetRowHeight(TRUE) / 2;
		bAdjustLayout = TRUE;
	}
	
	if (m_nCommandsHeight == -1 && !m_lstCommands.IsEmpty () && m_bAreCommandsVisible)
	{
		m_nCommandsHeight = GetRowHeight(TRUE) * m_nCommandRows + GetRowHeight(TRUE) / 2;
		bAdjustLayout = TRUE;
	}
...
void CBCGPPropList::OnDrawList (CDC* pDC)
{
	...
	if (m_bOutOfFilter)
	{
		...
		rectText.top += min(GetRowHeight() * 3, rectText.Height() / 3);
	...
	const CList<CBCGPProp*, CBCGPProp*>& lst = m_bAlphabeticMode ? m_lstTerminalProps : m_lstProps;
...
void CBCGPPropList::OnDrawDescription (CDC* pDC, CRect rect)
{
	...
	if (m_pSel->m_bDescriptionTruncated && m_bDescriptionShowMoreIndicator)
	{
		...
		rectMore.left = rectMore.right - GetRowHeight(TRUE);
		rectMore.top = rectMore.bottom - GetRowHeight(TRUE);
...
BOOL CBCGPPropList::OnDrawProperty (CDC* pDC, CBCGPProp* pProp) const
{
	...
	if (!pProp->m_Rect.IsRectEmpty ())
	{
		...
		if (pProp->m_Rect.bottom >= m_rectList.top)
		{
			...
			COLORREF clrAlternate = pProp->GetAlternateColor();
			
			if (clrAlternate != CLR_NONE)
			{
				CBrush br(clrAlternate);
				
				CRect rectFill = pProp->m_Rect;
				rectFill.top++;
				rectFill.top = max(rectFill.top, m_rectList.top);
				
				pDC->FillRect(rectFill, &br);

				pDC->MoveTo (nXCenter, rectFill.top);
				pDC->LineTo (nXCenter, rectFill.bottom);
			}
			...
			if (m_bVSDotNetLook)
			{
				...
				{
					rectLeft.right = rectLeft.left + GetRowHeight(TRUE);
				}
				...
				pProp->HasValueField ())
			{
				...
				if (m_bContextMenu && pProp->IsMenuButtonVisible() && rectName.right - m_rectList.left >= 2 * GetRowHeight(TRUE))
				{
					pProp->m_rectMenuButton = rectName;
					rectName.right -= GetRowHeight(TRUE);
					...
					if (pProp->GetRowsNumber() > 1)
					{
						...
						pProp->m_rectMenuButton.top += max(0, (GetRowHeight(TRUE) - nMenuSize) / 2);
			...
			if (pProp->IsGroup ())
			{
				...
				rectName.left += GetRowHeight(TRUE);
			...
			if (!pProp->m_rectButton.IsRectEmpty () && !pProp->IsToggleControl())
			{
			...
			{
				BOOL bDrawBottomLine = TRUE;
				
				if (m_bAlternateRowColor && !globalData.IsHighContastMode())
				{
					if (!pProp->IsGroup() || pProp->m_bIsValueList)
					{
...
void CBCGPPropList::OnHeaderItemChanged(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (pHeader->iItem == 0)
	{
		...
		m_nLeftColumnWidth = min (max (GetRowHeight(TRUE), hdItem.cxy - 2), rectClient.Width () - ::GetSystemMetrics (SM_CXHSCROLL) - 5);
...
//******************************************************************************************
void CBCGPPropList::EnableSearchBox(BOOL bEnable, LPCTSTR lpszPrompt, BOOL bSimplifiedIcon)
{
	ASSERT_VALID (this);
	...
	m_bFilterSimplifiedIcon = bSimplifiedIcon;
...
//******************************************************************************************
void CBCGPPropList::EnableGridLines(BOOL bHorizontal /*= TRUE*/, BOOL bVertical /*= TRUE*/)
{
	ASSERT_VALID(this);
...
void CBCGPPropList::TrackHeader (int nOffset)
{
	...
	else
	{
		...
		if (m_bShowDragContext)
		{
			...
			m_nLeftColumnWidth = min (max (GetRowHeight(TRUE), nOffset), rectClient.Width () - ::GetSystemMetrics (SM_CXHSCROLL) - 5);
...
void CBCGPPropList::TrackDescr (int nOffset)
{
	...
	else
	{
		...
		nOffset = max (nOffset, rectClient.top + GetRowHeight(TRUE) + m_nToolbarHeight + m_nHeaderHeight);
		nOffset = min (nOffset, rectClient.bottom - GetRowHeight(TRUE));

		m_rectTrackDescr = rectClient;
		m_rectTrackDescr.top = nOffset - 1;
		m_rectTrackDescr.bottom = m_rectTrackDescr.top + globalUtils.ScaleByDPI(2);
...
void CBCGPPropList::TrackCommands (int nOffset)
{
	...
	else
	{
		...
		m_rectTrackCommands.bottom = m_rectTrackCommands.top + globalUtils.ScaleByDPI(2);
...
void CBCGPPropList::TrackToolTip (CPoint point)
{
	...
	if (point.x < m_rectList.left + m_nLeftColumnWidth)
	{
		...
		if (pProp->IsGroup ())
		{
			rectToolTip.left += GetRowHeight(TRUE);
	...
	else
	{
		...
		rectToolTip.left = m_rectList.left + m_nLeftColumnWidth + pProp->m_nValueToolTipXOffset + globalUtils.ScaleByDPI(1);
		bIsValueTT = TRUE;
	}
...
BOOL CBCGPPropList::PreTranslateMessage(MSG* pMsg) 
{
	...
	{
		if (globalUtils.ProcessCtrlEditAccelerators(&m_wndFilter, (UINT)pMsg->wParam, TRUE))
		{
			return TRUE;
		}
	...
	if (m_pSel != NULL && m_pSel->m_bInPlaceEdit)
	{
		...
		const BOOL bIsDroppedDown = m_pSel->IsDroppedDown();

		if (pMsg->message == WM_SYSKEYDOWN && bIsDroppedDown)
		{
			m_pSel->OnEndEdit();
			return TRUE;
		}
		...
		if (pMsg->message == WM_KEYDOWN && (!bIsDroppedDown || pMsg->wParam == VK_RETURN))
		{
			...
			default:
				...
				if (m_pSel->m_bEnabled && !m_pSel->m_bAllowEdit)
				{
...
void CBCGPPropList::SetScrollSizes ()
{
	...
	if (GetRowHeight(TRUE) <= 0 || !m_bVerticalScrollBarEnabled)
	{
		m_nVertScrollPage = 0;
		m_nVertScrollTotal = 0;
		m_nVertScrollOffset = 0;
	...
	{
		m_nVertScrollPage = m_rectList.Height () / GetRowHeight() - 1;
...
void CBCGPPropList::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	...
	int dy = GetRowHeight() * (nPrevOffset - m_nVertScrollOffset);
...
void CBCGPPropList::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	case VK_F4:
		...
		{
			if (((m_pSel->m_dwFlags) & PROP_HAS_BUTTON) && !m_pSel->IsToggleControl())
			{
				CString strPrevVal = m_pSel->FormatProperty ();
	...
	case VK_NEXT:
		{
			...
			if (m_nVertScrollPage != 0)
			{
				...
				CPoint point (m_pSel->m_Rect.right - 1, 
					m_pSel->m_Rect.top + m_nVertScrollPage * GetRowHeight());
...
void CBCGPPropList::EnsureVisible (CBCGPProp* pProp, BOOL bExpandParents/* = FALSE*/)
{
	...
	{
		OnVScroll (SB_THUMBPOSITION, m_nVertScrollOffset + (-m_rectList.top + rect.top) / GetRowHeight(), NULL);
	}
...
//****************************************************************************************
BOOL CBCGPPropList::OnDrawPaletteColorBox(CDC* pDC, const CRect rectColor, COLORREF color, BOOL bIsAutomatic, BOOL bIsSelected, BOOL bOnPopupMenu)
{
	ASSERT_VALID (this);
	...
	return pColorProp->OnDrawPaletteColorBox(pDC, rectColor, color, bIsAutomatic, bIsSelected, bOnPopupMenu);
}
//****************************************************************************************
...
//****************************************************************************************
void CBCGPPropList::SetButtonsDefaultTheme(BOOL bSet)
{
	ASSERT_VALID (this);
	
	m_bButtonsDefaultTheme = bSet;
...
BOOL CBCGPPropList::OnNeedTipText(UINT /*id*/, NMHDR* pNMH, LRESULT* /*pResult*/)
{
	...
	if (m_rectDescr.PtInRect(point))
	{
		...
		{
			strTipText = m_pSel->GetDescription().IsEmpty() ? CString(m_pSel->GetName()) : m_pSel->GetDescription();
		}
		else
		{
			strTipText.Empty();
...
void CBCGPPropList::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if (m_bTracking)
	{
		...
		m_nLeftColumnWidth = min (max (GetRowHeight(TRUE), point.x), rectClient.Width () - ::GetSystemMetrics (SM_CXHSCROLL) - 5);
	...
	if (m_bTrackingDescr)
	{
		...
		point.y = max (point.y, GetRowHeight(TRUE) + m_nToolbarHeight + m_nHeaderHeight);
		m_nDescrHeight = rectClient.Height () - point.y + 2;
		m_nDescrHeight = max (GetRowHeight(TRUE), m_nDescrHeight);
	...
	if (m_bTrackingCommands)
	{
		...
		m_nCommandsHeight = max (GetRowHeight(TRUE), m_nCommandsHeight);
...
//********************************************************************************
void CBCGPPropList::SetRowPadding(int nPadding)
{
	m_nRowPadding = max(0, nPadding);
...
BOOL CBCGPPropList::ProcessClipboardAccelerators (UINT nChar)
{
	...
	return globalUtils.ProcessCtrlEditAccelerators(m_pSel->m_pWndInPlace, nChar, m_pSel->m_bAllowEdit && m_pSel->IsEnabled());
}
//************************************************************************************
...
//**************************************************************************
LRESULT CBCGPPropList::OnGetGestureConfig(WPARAM, LPARAM)
{
	return 1L;	
}
//**************************************************************************
void CBCGPPropList::SetBooleanPropertiesStyle(BooleanPropertiesStyle style, int nHorzAlign/* = TA_LEFT*/)
{
	if (m_BooleanPropertiesStyle == style && m_nBooleanPropertiesHorzAlign == nHorzAlign)
	{
		return;
	}

	m_BooleanPropertiesStyle = style;
	m_nBooleanPropertiesHorzAlign = nHorzAlign;
...
//**************************************************************************
void CBCGPPropList::SetAlternateRowColor(BOOL bSet /*= TRUE*/, BOOL bRedraw/* = TRUE*/)
{
	m_bAlternateRowColor = bSet;	
...
//**************************************************************************
LRESULT CBCGPPropList::OnChangeVisualManager (WPARAM, LPARAM)
{
	if (m_wndFilter.GetSafeHwnd() != NULL && m_wndFilter.IsWindowVisible())
	{
		m_wndFilter.SendMessage(BCGM_CHANGEVISUALMANAGER);
	}
