...
public:
	...
	void SetSwitchLabels(LPCTSTR lpszOn, LPCTSTR lpszOff);
...
public:
	...
	virtual BOOL NoInplaceEdit() const;
...
public:
	...
	BOOL IsModified (BOOL bIncludeSubItems = FALSE) const;
	...
	COLORREF GetAlternateColor() const;
...
protected:
	...
	CString			m_strSwitchOn;	// Switch "On" text label
	CString			m_strSwitchOff;	// Switch "Off" text label
...
protected:
	...
	CString						m_strValidInputCharacters;
};
...
struct BCGCBPRODLLEXPORT CBCGPDateTimePropAttributes
{
	...
	BOOL			m_bDisplayCalendarTodayButton;
};
...
public:
	CBCGPBitwiseProp(const CString& strName, const CStringArray& arFlagNames, ULONG ulFlags, DWORD_PTR dwData = 0, LPCTSTR lpszDescr = NULL, const CStringArray* parFlagDescriptions = NULL, const CArray<ULONG, ULONG>* parAllowedFlags = NULL);
	CBCGPBitwiseProp(const CString& strName, UINT nID, const CStringArray& arFlagNames, ULONG ulFlags, DWORD_PTR dwData = 0, LPCTSTR lpszDescr = NULL, const CStringArray* parFlagDescriptions = NULL, const CArray<ULONG, ULONG>* parAllowedFlags = NULL);

	virtual ~CBCGPBitwiseProp();

	virtual void SetValue (const _variant_t& varValue);
	void Rebuild(const CStringArray& arFlagNames, ULONG ulFlags, const CStringArray* parFlagDescriptions = NULL, const CArray<ULONG, ULONG>* parAllowedFlags = NULL);
...
protected:
	...
	CArray<ULONG, ULONG> m_arAllowedFlags;
};
...
public:
	enum BooleanPropertiesStyle
	{
		BOOL_PROPS_LABEL,
		BOOL_PROPS_CHECK_BOX,
		BOOL_PROPS_SWITCH,
	};
...
public:
	...
	void EnableSearchBox(BOOL bEnable = TRUE, LPCTSTR lpszPrompt = NULL, BOOL bSimplifiedIcon = FALSE);
	...
	void EnableGridLines(BOOL bHorizontal = TRUE, BOOL bVertical = TRUE);

	BOOL HasHorizontalGridLines() const
	{
		return m_bHorizontalGridLines;
	}

	BOOL HasVerticalGridLines() const
	{
		return m_bVerticalGridLines;
	}
	...
	void SetButtonsDefaultTheme(BOOL bSet = TRUE);
	BOOL IsButtonsDefaultTheme() const
	{
		return m_bButtonsDefaultTheme;
	}
	...
	int	GetRowHeight(BOOL bNoPadding = FALSE) const
	{
		return bNoPadding ? (m_nRowHeight - 2 * m_nRowPadding): m_nRowHeight;
	}

	void SetRowPadding(int nPadding);

	int GetRowPadding() const
	{
		return m_nRowPadding;
	}
	...
	BooleanPropertiesStyle GetBooleanPropertiesStyle() const
	{
		return m_BooleanPropertiesStyle;
	}

	void SetBooleanPropertiesStyle(BooleanPropertiesStyle style, int nHorzAlign = TA_LEFT);

	void SetAlternateRowColor(BOOL bSet = TRUE, BOOL bRedraw = TRUE);

	BOOL IsAlternateRowColor() const
	{
		return m_bAlternateRowColor;
	}
...
protected:
	...
	BOOL					m_bFilterSimplifiedIcon;

	PROP_HEADER_CTRL_CLASS	m_wndHeader;// Property list header control
	BOOL		m_bHeaderCtrl;			// Is header control visible?

	BOOL		m_bHorizontalGridLines;	// Is horizontal grid line visible?
	BOOL		m_bVerticalGridLines;	// Is vertical grid line visible?
	...
	BOOL		m_bButtonsDefaultTheme;	// TRUE - default OS look, FALSE - Visual Manager theme.
	...
	int			m_nRowPadding;			// Row vertical padding
	...
	BOOL		m_bAlternateRowColor;
	int			m_nCurrReposedRow;
	...
	CBrush					m_brAlternate;

	CBCGPGraphicsManager*	m_pGM;
	CBCGPToolBarImages		m_AdvImages;

	BooleanPropertiesStyle	m_BooleanPropertiesStyle;
	int						m_nBooleanPropertiesHorzAlign;
...
protected:
	...
	virtual COLORREF GetAlternateColor();
