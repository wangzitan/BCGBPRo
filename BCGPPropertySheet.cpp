...
void CBCGPPropSheetBtn::OnPaint() 
{
	...
	if (pParent->m_bSimplifiedBackIcon)
	{
		CBCGPMenuImages::IMAGE_STATE stateImage = (CBCGPMenuImages::IMAGE_STATE)CBCGPVisualManager::GetInstance()->GetNavImageState(
			IsHighlighted() || (GetFocus() == this), IsPressed() && IsHighlighted(), !IsWindowEnabled(), m_bOnGlass);
...
BOOL CBCGPPropSheetPane::OnSendCommand (const CBCGPToolbarButton* pButton)
{
	...
	{
		if (!m_pParent->SetActivePageWithEffects(nPageIndex))
		{
			return FALSE;
		}
...
BOOL CBCGPPropertySheet::OnInitDialog() 
{
	...
	else
	{
		if (pWndNavigator != NULL)
		{
			...
			else if (bIsCtrl && !m_bBackstageMode)
			{
				pTab->ShowWindow (SW_HIDE);
			}
			...
			{
				rectNavigator.OffsetRect(0, -1);	// Align navigation top with the page top
			}
			...
			pWndNavigator->SetWindowPos (pTab, 
...
#ifndef _BCGSUITE_
				if (CBCGPVisualManager::GetInstance()->IsDWMCaptionSupported() && !globalData.IsTitleBarAccentColor())
#endif
				{
...
void CBCGPPropertySheet::AdjustControlsLayout()
{
	...
	if (!IsLayoutEnabled() || !m_bIsReady || IsIconic())
	{
		return;
	}
...
int CBCGPPropertySheet::ReposButtons (BOOL bRedraw)
{
	...
	for (int iStep = 0; iStep < 2; iStep++)
	{
		for (int i = 0; i < sizeof (ids) / sizeof (ids [0]); i++)
		{
			...
			if (pButton != NULL)
			{
				if ((ids[i] == IDHELP && (m_psh.dwFlags & PSH_HASHELP) == 0) || (ids[i] == ID_APPLY_NOW && (m_psh.dwFlags & PSH_NOAPPLYNOW)))
				{
					CString strBtnText;
					pButton->GetWindowText(strBtnText);
					
					// The button is not visible - remove keyboard accelerator:
					strBtnText.Remove(_T('&'));
					
					pButton->SetWindowText(strBtnText);
					continue;
				}
				...
				if (m_nAeroHeight > 0 && ids [i] == ID_WIZBACK)
				{
					if (m_btnBack.GetSafeHwnd() == NULL)
					{
						...
						m_btnBack.SetWindowText(_T("Back"));
						m_btnBack.SetDrawText(FALSE, FALSE);
					}
...
CWnd* CBCGPPropertySheet::InitNavigationControl ()
{
	...
	if (m_look == PropSheetLook_Tree)
	{
		...
		{
			dwTreeStyle |= 0x0200 /* TVS_TRACKSELECT */ | TVS_HASBUTTONS | TVS_HASLINES | TVS_LINESATROOT | TVS_SHOWSELALWAYS | WS_TABSTOP;
		}
	...
	if (m_look == PropSheetLook_OneNoteTabs || m_look == PropSheetLook_Pointer || m_look == PropSheetLook_Underline || m_look == PropSheetLook_Slider || IsVisualManagerStyle ())
	{
		...
		case PropSheetLook_Underline:
			tabStyle = CBCGPTabWnd::STYLE_UNDERLINE;
			break;
#endif
		}
	...
	else
	{
		...
		if (pCategory != NULL)
		{
			...
			while (pCategory->m_hLastSelectedItem != NULL && !bIsPageSelected)
			{
				...
				{
					CBCGPPropertyPage* pTreePage = DYNAMIC_DOWNCAST (CBCGPPropertyPage,
						(CObject*) m_wndTree.GetItemData (pCategory->m_hLastSelectedItem));
					if (pTreePage != NULL)
					{
						if (IsPageTransitionAvailable())
						{
							SetActivePageWithEffects(GetPageIndex(pTreePage));
						}
						else
						{
							SetActivePage (pTreePage);
						}

						CRect rectItem;
						m_wndTree.GetItemRect (pTreePage->m_hTreeNode, rectItem, FALSE);
						m_wndTree.InvalidateRect (rectItem);
...
void CBCGPPropertySheet::EnableLayout(BOOL bEnable, CRuntimeClass* pRTC, BOOL bResizeBox)
{
	...
	if (bEnable && m_bFlickerLess && GetSafeHwnd() != NULL)
	{
		...
		for (int i = 0; i < GetPageCount (); i++)
		{
			...
			if (pPage->GetSafeHwnd() != NULL)
			{
				pPage->ModifyStyle(0, WS_CLIPCHILDREN | WS_CLIPSIBLINGS);
			}
		}
	}
}
...
BOOL CBCGPPropertySheet::SetActivePageWithEffects(int nPage)
{
	...
	const BOOL bIsAppWnd = AfxGetMainWnd()->GetSafeHwnd() == GetSafeHwnd();
	...
	for (int i = nPage1; i <= nPage2; i++)
	{
		...
		if (pPage != NULL)
		{
			if (pPage->GetSafeHwnd() == NULL || IsLayoutEnabled())
			{
				SetActivePage(pPage);
			}
	...
	if (m_bFlickerLess && IsLayoutEnabled())
	{
...
void CBCGPPropertySheet::OnPageTransitionFinished()
{
	...
	if (pActivePage->GetSafeHwnd() != NULL)
	{
		if (!pActivePage->IsWindowVisible())
		{
			pActivePage->ShowWindow(SWP_NOACTIVATE);
		}

		if (bNeedToRedraw)
		{
			pActivePage->RedrawWindow(NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ALLCHILDREN);
		}
	}
	...
	else if (m_wndOutlookBar.GetSafeHwnd() != NULL)
	{
		m_wndOutlookBar.RedrawWindow(NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ALLCHILDREN | RDW_ERASE);
	}
