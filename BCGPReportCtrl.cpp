...
void CBCGPReportRow::OnFillGroupBackground (CDC* pDC, CRect rectFill, BOOL bGroupUnderline)
{
	...
	if (!m_pWndList->IsHighlightGroups())
	{
		...
		else if (IsSelected() && m_pWndList->m_brSelBackground.GetSafeHandle () != NULL)
		{
			CBrush& br = m_pWndList->m_brSelBackground;
...
void CBCGPReportRow::OnDrawName (CDC* pDC, CRect rect)
{
	...
	if (IsSelected ())
	{
		...
		if (m_pWndList->m_ColorData.m_GroupSelColors.m_clrText != (COLORREF)-1)
		{
			...
			if (m_pWndList->m_ColorData.m_GroupSelColors.m_clrBackground == (COLORREF)-1)
			{
		...
		else
		{
			...
			{
				clrText = m_pWndList->GetGroupTextColor(IsSelected());
			}
		}
	}
	else
...
void CBCGPReportRow::OnDrawExpandBox (CDC* pDC, CRect rect)
{
	...
	else
	{
		...
		if (IsSelected() ? (m_pWndList->m_ColorData.m_GroupSelColors.m_clrBackground == (COLORREF)-1) : 
			(m_pWndList->m_ColorData.m_GroupColors.m_clrBackground == (COLORREF)-1))
		{
			visualManager->OnFillReportCtrlRowBackground (
				m_pWndList, pDC, rectFill, IsSelected (), IsGroup ());
		}
...
CBCGPReportCtrl::CBCGPReportCtrl()
{
	...
	m_bWholeRowSel = TRUE;
	m_bDisableWholeSelHeaderHighlighting = TRUE;
