...
void CBCGPRibbonBackstagePagePrint::OnSelectPrinter() 
{
		...
		lstrcmp((LPCTSTR)lpDevNames + lpDevNames->wOutputOffset, info.strPortName) != 0)
	{
		...
		LPDEVNAMES lpDevNames1 = (LPDEVNAMES)::GlobalLock(hDevNames);
		ASSERT(lpDevNames1 != NULL);
		...
		lpDevNames1->wDriverOffset = (WORD)tcOffset;
		memcpy((LPTSTR)lpDevNames1 + tcOffset, (LPCTSTR)info.strDriverName, drvNameLen * sizeof(TCHAR));
		tcOffset += drvNameLen;

		lpDevNames1->wDeviceOffset = (WORD)tcOffset;
		memcpy((LPTSTR)lpDevNames1 + tcOffset, (LPCTSTR)info.strPrinterName, ptrNameLen * sizeof(TCHAR));
		tcOffset += ptrNameLen;

		lpDevNames1->wOutputOffset = (WORD)tcOffset;
		memcpy((LPTSTR)lpDevNames1 + tcOffset, (LPCTSTR)info.strPortName, porNameLen * sizeof(TCHAR));
		lpDevNames1->wDefault = 0;

		::GlobalUnlock(lpDevNames1);

		dlgPrint->hDevNames = lpDevNames1;
