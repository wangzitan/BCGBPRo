...
class CBCGPRibbonCustomizationInfo;
...
#define BCGP_RIBBON_CAPTION_CUSTOM_BUTTON_DISPLAY_IN_NORMAL_MODE	0x0001
#define BCGP_RIBBON_CAPTION_CUSTOM_BUTTON_DISPLAY_IN_BACKSTAGE_MODE	0x0002
#define BCGP_RIBBON_CAPTION_CUSTOM_BUTTON_DISPLAY_IN_HIDDEN_MODE	0x0004
...
class BCGCBPRODLLEXPORT CBCGPRibbonAutoHideButton : public CBCGPRibbonCaptionButton
{
	friend class CBCGPRibbonBar;
...
protected:
	...
	virtual void OnLButtonUp (CPoint /*point*/) {}
	virtual BOOL CanBeAddedToQAT() const { return FALSE; }
};
...
class BCGCBPRODLLEXPORT CBCGPRibbonBar : public CBCGPControlBar
{
	...
	friend class CBCGPRibbonAutoHideButton;
...
public:
	...
	BOOL EnableStartPage(UINT nDlgTemplateID, CRuntimeClass* pDlgClass, BOOL bShowOnStartup = TRUE, const CString& strPageName = _T("StartPage"));
	...
	void SetAutoHideMode(BOOL bSet = TRUE);
	BOOL IsInAutoHideMode() const
	{
		return m_bAutoHideMode;
	}

	BOOL IsAutoHideModeAvailable() const
	{
	...
	void SetDefaultSystemMenu(BOOL bSet = TRUE)
	{
		m_bDefaultSystemMenu = bSet;
	}

	BOOL IsDefaultSystemMenu() const
	{
		return m_bDefaultSystemMenu;
	}
	...
	BOOL ReplaceRibbonElementByID(UINT uiCmdID, CBCGPBaseRibbonElement& newElement, BOOL bCopyContent = TRUE, BOOL bIncludeHidden = FALSE) const;
	...
	void SetTabButtonIcons(CBCGPToolBarImages* pImages);
	...
	void SetCaptionButtonIconStyle(BCGPRibbonIconStyle style);
	BCGPRibbonIconStyle GetCaptionButtonIconStyle() const
	{
		return m_CaptionButtonIconStyle;
	}
	...
	virtual BOOL IsDrawSystemIcon() const;
	virtual CSize GetSystemIconSize() const;
...
protected:
	...
	CWnd*					m_pWndAutoHidePopup;
	BOOL					m_bInShowMainMenuKeyTips;
	...
	CBCGPRibbonAutoHideButton m_AutoHideCaption;

	BCGPRibbonIconStyle		m_CaptionButtonIconStyle;
...
public:
	...
	virtual void PrepareCustomLabel(CString& strName) const;
...
public:
	...
	void ForceRecalcLayout (BOOL bRecalcLayoutInParentFrame = TRUE);
	...
	BOOL IsOnlyAutoHideButtonVisible() const
	{
		return m_bAutoHideMode && m_pWndAutoHidePopup == NULL && !IsBackstageViewActive();
	}
...
protected:
	...
	virtual BOOL OnProcessCustomizationData(CBCGPRibbonCustomizationInfo& /*info*/) { return TRUE; }
	virtual void ApplyCustomizationData(const CBCGPRibbonCustomizationData& data);

	virtual BOOL AssignKeyToPanelCustomCopy(CBCGPRibbonPanel* /*pPanel*/) const { return FALSE; }

	BOOL IsPanelValid(CBCGPRibbonDefaultPanelButton* pDefaultButton);

	LRESULT NotifyParent(UINT message, WPARAM wp = 0, LPARAM lp = 0L);

	void _GetClientRect(CRect& rect);
	void _InvalidateRect(const CRect& rect);
	void _UpdateWindow();
	BOOL _RedrawWindow(LPCRECT lpRectUpdate = NULL, CRgn* prgnUpdate = NULL, UINT flags = RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASE);

	void SetAutoHidePopup(CBCGPRibbonPanelMenuBar* pMenuBar);
	void ShowAutoHidePopup(BOOL bShow);
...
protected:
	...
	afx_msg LRESULT OnEraseChildControlBackground(WPARAM wp, LPARAM lp);
	DECLARE_MESSAGE_MAP()
};
...
public:
	...
	void SetImage (UINT uiBmpResID, BOOL bDPIAutoScale = FALSE);
	void SetImage (HBITMAP hBmp);
	void SetScenicImage (UINT uiBmpResID, BOOL bDPIAutoScale = FALSE);
...
protected:
	...
	virtual void OnClick (CPoint /*point*/)
	{
		...
		{
			m_parentRibbon.ToggleMinimizeState();
		}
	}
...
#define ToggleMimimizeState	ToggleMinimizeState
