...
//********************************************************************************
BOOL CBCGPRibbonButtonsGroup::Remove(int nIndex)
{
	ASSERT_VALID(this);
	...
	CBCGPBaseRibbonElement* pButton = m_arButtons[nIndex];
	ASSERT_VALID(pButton);
...
//********************************************************************************
BOOL CBCGPRibbonButtonsGroup::RemoveByID(UINT nCmdID)
{
	ASSERT_VALID(this);
...
void CBCGPRibbonButtonsGroup::OnAfterChangeRect (CDC* pDC)
{
	...
	int nButtonHeight = m_rect.Height () - nMarginTop - nMarginBottom;
	if (bIsOnQAT)
	{
		nButtonHeight -= szQATMargin.cy;
	}
	...
	for (int i = 0; i < m_arButtons.GetSize (); i++)
	{
		...
		if (pButton->CanBeResizedInGroupVertically() || sizeButton.cy > nButtonHeight)
		{
			sizeButton.cy = i != nCustomizeButtonIndex ? nButtonHeight : nButtonHeight - 1;
		}
		else
		{
			yOffset = (nButtonHeight - sizeButton.cy) / 2;
		}

		const int y = i != nCustomizeButtonIndex ? rectGroup.top + nMarginTop : rectGroup.top;

		pButton->m_rect = CRect (CPoint (x, y + yOffset), sizeButton);
...
//*****************************************************************************
void CBCGPRibbonButtonsGroup::DestroyCtrl()
{
	ASSERT_VALID (this);
	
	CBCGPBaseRibbonElement::DestroyCtrl();
...
//*************************************************************************************
void CBCGPRibbonButtonsGroup::SetAutohidePopupMode(BOOL bSet)
{
	CBCGPBaseRibbonElement::SetAutohidePopupMode(bSet);
	...
	for (int i = 0; i < m_arButtons.GetSize (); i++)
	{
		...
		pButton->SetAutohidePopupMode(bSet);
	}
}
//*************************************************************************************
