...
{
	m_iSelIndex = -1;
	m_nDropDownHeight = globalUtils.ScaleByDPI(iDefaultComboHeight);
	m_bHasEditBox = FALSE;
...
void CBCGPRibbonComboBox::OnAfterChangeRect (CDC* pDC)
{
	...
	if (m_rectCommand.Width () > cx + 1)
	{
		m_rectCommand.left = m_rectCommand.right - cx - 1;
	}
...
const int nDefaultFontListHeight = 400;
	...
	m_nPitchAndFamily (DEFAULT_PITCH)
{
	...
	m_nDropDownHeight = globalUtils.ScaleByDPI(nDefaultFontListHeight);
}
//**************************************************************************
CBCGPRibbonFontComboBox::CBCGPRibbonFontComboBox() :
	m_nFontType (DEVICE_FONTTYPE | RASTER_FONTTYPE | TRUETYPE_FONTTYPE),
											  ...
											  BOOL /*bHighlight*/)
{
	...
	rc.left += globalUtils.ScaleByDPI(nImageMargin);
	...
	rc.left += m_Images.GetImageSize().cx + globalUtils.ScaleByDPI(nImageMargin);
		...
		CSize sizeDefault)
{
	...
	size.cx += m_Images.GetImageSize().cx + 2 * globalUtils.ScaleByDPI(nImageMargin);
