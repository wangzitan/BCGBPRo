...
//*************************************************************************************************************
BOOL CBCGPRibbonCustomizationData::LoadFromBuffer(LPCTSTR lpszXMLBuffer, CBCGPRibbonBar* pRibbonBar)
{
	...
	if (pRibbonBar != NULL)
	{
		...
		if (!pRibbonBar->OnProcessCustomizationData(info))
		{
			return FALSE;
		}
	}
...
CBCGPRibbonPanel* CBCGPRibbonCustomPanel::CreateRibbonPanel(CBCGPRibbonBar* pRibbonBar, CBCGPRibbonCategory* pCategory)
{
	...
	if ((int)m_dwOriginal > 0)
	{
		...
		CBCGPRibbonPanel* pPanel = pOriginalPanel->CreateCustomCopy(pCategory);
		ASSERT_VALID(pPanel);
		
		if (pRibbonBar->AssignKeyToPanelCustomCopy(pPanel))
		{
