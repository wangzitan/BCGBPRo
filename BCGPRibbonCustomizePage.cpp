	...
	m_bCustomizeQAT(bCustomizeQAT)
{
	...
	SetLook(CBCGPPropertySheet::PropSheetLook_List, globalUtils.ScaleByDPI(150) /* List width */);
}
//**********************************************************************
...
//**********************************************************************
void CBCGPRibbonCustomizeRibbonPage::SaveCustomizationData()
{
	if (m_bIsDataSaved)
	{
		return;
	...
	// Check if parent property sheet has a QAT customization page and
	// save QAT data first
	...
	if (pParent != NULL)
	{
		...
		for (int iPage = 0; iPage < pParent->GetPageCount(); iPage++)
		{
			CBCGPRibbonCustomizeQATPage* pQATPage = DYNAMIC_DOWNCAST(CBCGPRibbonCustomizeQATPage, pParent->GetPage(iPage));
			if (pQATPage != NULL)
			{
				ASSERT_VALID(pQATPage);
				pQATPage->SaveCustomizationData();
				break;
			}
		}
	}
...
void CBCGPRibbonCustomizeRibbonPage::RenameCategory(CBCGPGridRow* pRow)
{
	...
	if (m_pSelCategoryDest->IsCustom())
	{
		m_pRibbonBar->PrepareCustomLabel(strNewName);
	}
