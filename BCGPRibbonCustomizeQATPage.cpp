...
void CBCGPRibbonCustomizeQATPage::OnRemove() 
{
	...
	if (nIndex >= 0)
	{
		CBCGPBaseRibbonElement* pElement = (CBCGPBaseRibbonElement*)m_wndQATList.GetItemData(nIndex);
		if (pElement != NULL && !pElement->CanBeRemovedFromQAT())
		{
			return;
		}
...
{
	const int nCurSel = m_wndQATList.GetCurSel();

	m_wndUp.EnableWindow(nCurSel > 0);
	m_wndDown.EnableWindow(nCurSel >= 0 && nCurSel < m_wndQATList.GetCount() - 1);
	
	BOOL bRemoveEnabled = FALSE;

	if (nCurSel >= 0)
	{
		bRemoveEnabled = TRUE;

		CBCGPBaseRibbonElement* pElement = (CBCGPBaseRibbonElement*)m_wndQATList.GetItemData(nCurSel);
		if (pElement != NULL && !pElement->CanBeRemovedFromQAT())
		{
	...
	m_wndRemove.EnableWindow(bRemoveEnabled);
}
//**********************************************************************
...
//**********************************************************************
void CBCGPRibbonCustomizeQATPage::SaveCustomizationData()
{
	if (m_bIsDataSaved)
	{
		return;
