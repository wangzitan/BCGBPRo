...
BOOL CBCGPRibbonInfoLoader::LoadFromBuffer (LPCTSTR lpszBuffer)
{
	...
	if (info.FromTag (lpszBuffer))
	{
		...
		{
			const BOOL bMainImage = images[i] == pMainImage || images[i] == pMainImageScenic;
			if (!LoadImage (*images[i], bMainImage))
			{
				ASSERT(FALSE);
			}

			if (bMainImage && images[i]->m_Image.IsValid() && images[i]->m_Image.IsSVGList())
			{
				if (images[i] == pMainImage)
				{
					images[i]->m_Image.SetImageSize(info.GetImageSize(CBCGPRibbonInfo::e_ImagesMB));
				}
				else
				{
					images[i]->m_Image.SetImageSize(info.GetImageSize(CBCGPRibbonInfo::e_ImagesSmall));
				}
			}
		}
	}
