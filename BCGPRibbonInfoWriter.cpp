...
static LPCTSTR s_szExtensionImagePNG = _T(".png");
static LPCTSTR s_szExtensionImageSVG = _T(".svg");

static LPCTSTR s_szTypeXML        = _T("BCGP_RIBBON_XML");
static LPCTSTR s_szTypeImagePNG   = _T("PNG");
static LPCTSTR s_szTypeImageSVG   = _T("SVG");
...
void CBCGPRibbonInfoWriter::CorrectImage (CBCGPRibbonInfo::XImage& image, const CString& strID, const CString& strFile)
{
	if (image.m_Image.IsValid ())
	{
		...
		{
			BOOL bIsSVG = image.m_Image.IsSVGList();

			image.m_ID.m_Name = str;
			AddResource (XResourceFile (image.m_ID.m_Name, 
				bIsSVG ? s_szTypeImageSVG : s_szTypeImagePNG,
				m_ResourceFolder + strFile + (bIsSVG ? s_szExtensionImageSVG : s_szExtensionImagePNG)));
		}
	}
	else
	{
...
BOOL CBCGPRibbonInfoWriter::Prepare ()
{
	...
	// Image files
	{
		LPCTSTR szTypeImage[] = {s_szTypeImageSVG, s_szTypeImagePNG};

		for (int nStep = 0; nStep < 2; nStep++)
		{
			const XResourceFileArray* array = GetResourcesArray (szTypeImage[nStep]);
			if (array != NULL)
			{
...

	{
		...
		for (int nStep = 0; nStep < 2; nStep++)
		{
			...
			if (array != NULL && array->GetSize () > 0)
			{
				text.Add (s_szCommentLine);
				text.Add (s_szComment);
				text.Add (_T("// ") + CString (szTypeImage[nStep]));
				text.Add (s_szComment);
				text.Add (_T(""));
	...
	for (int nStep = 0; nStep < 2; nStep++)
	{
		...
		for (int i = 0; i < images.GetSize (); i++)
		{
			...
			if (nStep == 0)
			{
				...
				CBCGPSVGImageList list;

				bRes = images[i]->m_Image.ExportToSVGImageList(list);
				if (!bRes)
				{
					break;
				}

				bRes = list.SaveToFile(GetFileName (array->GetAt (nIndex)->GetPath ()));
				if (!bRes)
				{
					break;
				}
...
CString CBCGPRibbonInfoWriter::XResourceFile::Format () const
{
	...
	if (IsValid ())
	{
		...
		str.Format (_T("%s %s \"%s\""), (LPCTSTR)GetID (), (LPCTSTR)GetType (), (LPCTSTR)strPath);
	}
