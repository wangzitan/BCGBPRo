...
#define BACKSTAGE_BUTTON_MARGIN_X	30
//wlg add next line
#define BACKSTAGE_BUTTON_MARGIN_Y	20
...
static CString PrepareDisplayName(int nIndex, const CString& strFilePath)
{
	...
	if (nIndex == 9)
	{
		strOut.Format (_T("1&0 %s"), name);
	}
	else if (nIndex < 9)
	{
		strOut.Format (_T("&%d %s"), nIndex + 1, name);
	}
	else
	{
		strOut = name;
...
void CBCGPRibbonMainPanel::Repos (CDC* pDC, const CRect& rect)
{
	...
	for (i = iMenuStart; i < iMenuFinish; i++)
	{
		...
		sizeElem.cy +=  BACKSTAGE_BUTTON_MARGIN_Y;

		CRect rectElem = CRect
			(CPoint (rect.left - m_nScrollOffsetHorz + m_nXMargin, rect.top + y + m_nYMargin+ BACKSTAGE_BUTTON_MARGIN_Y*2),
			sizeElem);
	...
	if (m_pElemOnRight != NULL && !m_bSearchMode)
	{
		...
		m_rectMenuElements.bottom += GetExtraHeight();
	...
	if (m_nBottomElementsNum > 0 && !m_bSearchMode)
	{
		...
		for (int nCount = 0; nCount < m_nBottomElementsNum; nCount++)
		{
			...
			CRect rectElem = CRect (CPoint (x - sizeElem.cx, y - sizeElem.cy), sizeElem);
...
void CBCGPRibbonMainPanel::DoPaint (CDC* pDC)
{
	...
	else if (!IsBackstageRightPaneActive())
	{
		...
		{
			CBCGPVisualManager::GetInstance ()->OnFillRibbonBackstageForm(pDC, pParentWnd, rectRight);
		}
	}
...
void CBCGPRibbonMainPanel::DrawMainButton (CDC* pDC, CWnd* pWnd)
{
	...
	if (rectMainButtonSaved.IsRectEmpty())
	{
		return;
	}
