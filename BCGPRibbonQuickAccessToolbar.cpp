...
CSize CBCGPRibbonQuickAccessToolbar::GetRegularSize (CDC* pDC)
{
	...
	const BOOL bIsOnTop = m_pRibbonBar->IsQuickAccessToolbarOnTop();

	if (bIsOnTop && m_arButtons.GetSize () > 1)
	{
	...
	if (size.cy > 0)
	{
		size.cy += CBCGPVisualManager::GetInstance ()->GetRibbonQATExtraHeight(bIsOnTop);
	}
...
void CBCGPRibbonQuickAccessToolbar::Add (CBCGPBaseRibbonElement* pElem)
{
	...
	if (pElem->m_bIsAutohidePopupMode)
	{
		pButton->DestroyCtrl();
		pButton->SetAutohidePopupMode(TRUE);
		pButton->SetParentMenu(m_pParentMenu);
	}
	else if (DYNAMIC_DOWNCAST(CBCGPPopupMenuBar, pElem->GetParentWnd()) != NULL)
	{
