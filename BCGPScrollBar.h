...
// Operations
public:
	BOOL DoGesturePan(CPoint ptFrom, CPoint ptTo);
...
protected:
	...
	virtual void PreSubclassWindow()
	{
		...
		m_dwScrool = T::GetStyle() & (WS_HSCROLL | WS_VSCROLL);
		m_dwScroolEx = T::GetExStyle() & WS_EX_LEFTSCROLLBAR;
	}
...
inline BOOL TBCGPInternalScrollBarWrapperWnd<CListCtrl>::InternalSB_OnThumbTrack(int nBar, int nTrackPos)
{
		...
		const BOOL bVert = nBar == SB_VERT;
		const DWORD dwType = (GetStyle() & LVS_TYPEMASK);

		if (dwType == LVS_REPORT && bVert)
		{
			CRect rect;
			GetItemRect(0, rect, LVIR_BOUNDS);
			nTrackPos *= rect.Height();
		}
		else if (dwType == LVS_LIST && !bVert)
		{
			CRect rect;
			GetItemRect(0, rect, LVIR_BOUNDS);
			nTrackPos *= rect.Width();
		}

		ListView_Scroll(GetSafeHwnd(), bVert ? 0 : nTrackPos, bVert ? nTrackPos : 0);
