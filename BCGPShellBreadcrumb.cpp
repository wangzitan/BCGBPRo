...
BOOL CBCGPShellBreadcrumb::SelectShellItem (LPCITEMIDLIST lpidl)
{
	...
	HBREADCRUMBITEM hItem = FindShellItem(lpidl);
	if (hItem == NULL)
	{
		return FALSE;
	}
...
BOOL CBCGPShellBreadcrumb::RenameShellItem(LPCITEMIDLIST lpidl, const CString& strNewName)
{
	ASSERT_VALID (this);
...
void CBCGPShellBreadcrumb::ShellObjectToItemInfo (BREADCRUMBITEMINFO* pItemInfo, LPITEMIDLIST pidlCurrent, LPSHELLFOLDER pParentFolder, LPITEMIDLIST pidlParent, BOOL bConcatenateItem)
{
	...
	{
		pItemParam->pidlFQ = bConcatenateItem ? pShellManager->ConcatenateItem (pidlParent, pidlCurrent) : pidlParent;
...
void CBCGPShellBreadcrumb::GetItemChildrenDynamic (HBREADCRUMBITEM hParentItem)
{
	...
	OnAfterEnumFolderItems(hParentItem, arrSubItems);
...
#ifndef _BCGSUITE_
		m_pRelatedShellList->m_hwndRelatedBreadCrumb = GetSafeHwnd();
#endif
...
HBREADCRUMBITEM CBCGPShellBreadcrumb::FindShellItem(LPCITEMIDLIST lpidl)
{
	ASSERT_VALID (this);
...
#ifndef _BCGSUITE_
BOOL CBCGPShellBreadcrumb::AddExistingFolder(HBREADCRUMBITEM hParentItem, LPITEMIDLIST lpidl)
{
	ASSERT_VALID(this);
	...
	itemInfo.pszText = new TCHAR [MAX_PATH + 1];
	itemInfo.cchTextMax = MAX_PATH;
	itemInfo.hParentItem = hParentItem;
	
	ShellObjectToItemInfo (&itemInfo, g_pShellManager->CopyItem(lpidl, TRUE), pParentFolder, lpidl, FALSE);
	SendMessage(BCCM_INSERTITEM, (WPARAM)hParentItem, (LPARAM)&itemInfo);
