...
public:
	...
	BOOL RenameShellItem (LPCITEMIDLIST lpidl, const CString& strNewName);
...
protected:
	...
	static void ShellObjectToItemInfo (BREADCRUMBITEMINFO* pItemInfo, LPITEMIDLIST pidlCurrent, LPSHELLFOLDER pParentFolder, LPITEMIDLIST pidlParent, BOOL bConcatenateItem = TRUE);

	virtual CEdit* CreateInplaceEdit(CWnd* pParent, const CRect& rect, UINT uiControlID);

	HBREADCRUMBITEM FindShellItem (LPCITEMIDLIST lpidl);

#if (!defined _BCGSUITE_) && (!defined _BCGSUITE_INC_)
	BOOL AddExistingFolder(HBREADCRUMBITEM hParentItem, LPITEMIDLIST lpidl);
#endif

	virtual void OnAfterEnumFolderItems(HBREADCRUMBITEM hFolder, CArray<BREADCRUMBITEMINFO, const BREADCRUMBITEMINFO&>& arrSubItems)
	{
		UNREFERENCED_PARAMETER(hFolder);
		UNREFERENCED_PARAMETER(arrSubItems);
	}
