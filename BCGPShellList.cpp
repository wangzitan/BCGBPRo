...
HRESULT CBCGPShellList::DisplayFolder (LPBCGCBITEMINFO pItemInfo)
{
	...
	if (m_psfCurFolder != NULL)
	{
		...
		BOOL bWndVisible = IsWindowVisible();
		if (bWndVisible)
		{
			SetRedraw (FALSE);
		}
									...
									LPITEMIDLIST pidlParent)
{
	...
	if (SUCCEEDED (hRes))
	{
		...
		{
			InsertShellItem(pParentFolder, pidlParent, pidlTemp);
			dwFetched = 0;
		}
...
//***************************************************************************************
int CBCGPShellList::InsertShellItem(LPSHELLFOLDER pParentFolder, LPITEMIDLIST pidlParent, LPITEMIDLIST pidlNew, BOOL bConcatenateItem)
{
	ASSERT_VALID(g_pShellManager);
	...
	pItem->pidlRel = pidlNew;
	pItem->pidlFQ = bConcatenateItem ? g_pShellManager->ConcatenateItem (pidlParent, pidlNew) : pidlParent;
	...
	pParentFolder->GetAttributesOf (1, (LPCITEMIDLIST*)&pidlNew, &dwAttr);
	...
	UpdateItemColumns(iItem, pItem);
	return iItem;
}
//***************************************************************************************
void CBCGPShellList::UpdateItemColumns(int iItem, LPBCGCBITEMINFO pItem)
{
	ASSERT(pItem != NULL);

	const int nColumns = (GetStyle() & LVS_TYPEMASK) == LVS_REPORT ? m_wndHeader.GetItemCount() : 1;
...
//***************************************************************************************
void CBCGPShellList::ReleaseItem(LPBCGCBITEMINFO pItem)
{
	if (pItem == NULL)
	{
		ASSERT(FALSE);
...
//****************************************************************************************
BOOL CBCGPShellList::CreateFolder(const CString& strFolderName, LPSECURITY_ATTRIBUTES lpSecurityAttributes /*= NULL*/, BOOL bSelect /*= TRUE*/)
{
	ASSERT_VALID(g_pShellManager);
	...
	CString strPath;
	if (!GetCurrentFolder(strPath) || strPath.IsEmpty())
	{
		return FALSE;
	}
	...
	HTREEITEM hRelatedTreeItem = NULL;
	HBREADCRUMBITEM hRelatedBreadcrumbItem = NULL;
	
	CBCGPShellTree* pTree = NULL;
	CBCGPShellBreadcrumb* pBreadcrumb = NULL;

	LPITEMIDLIST pidlParent = NULL;
	if (SUCCEEDED(g_pShellManager->ItemFromPath(strPath, pidlParent)))
	{
		if ((pTree = GetRelatedTree ()) != NULL)
		{	
			hRelatedTreeItem = pTree->FindVisibleItem(pidlParent);

			if (!pTree->ItemHasChildren(hRelatedTreeItem))
			{
				hRelatedTreeItem = NULL;
			}
		}

		if ((pBreadcrumb = GetRelatedBreadCrumb()) != NULL)
		{	
			hRelatedBreadcrumbItem = pBreadcrumb->FindShellItem(pidlParent);
		}
	}
	...
	if (FAILED(g_pShellManager->ItemFromPath(strPath, pidl)))
	{
		return FALSE;
	}
	
	int iItem = InsertShellItem(m_psfCurFolder, pidl, g_pShellManager->CopyItem(pidl, TRUE), FALSE);
	if (iItem < 0)
	{
		return FALSE;
	}
	...
	if (pTree != NULL && hRelatedTreeItem != NULL)
	{
		pTree->AddExistingFolder(hRelatedTreeItem, strPath);
	}

	if (pBreadcrumb != NULL && hRelatedBreadcrumbItem != NULL)
	{
		pBreadcrumb->AddExistingFolder(hRelatedBreadcrumbItem, pidl);
	}
...
void CBCGPShellList::OnDestroy() 
{
	...
	CBCGPShellBreadcrumb* pBreadCrumb = GetRelatedBreadCrumb();
	if (pBreadCrumb != NULL)
	{
		ASSERT_VALID(pBreadCrumb);
		pBreadCrumb->m_pRelatedShellList = NULL;
	}
...
CBCGPShellTree* CBCGPShellList::GetRelatedTree () const
{
	...
	return DYNAMIC_DOWNCAST(CBCGPShellTree, CWnd::FromHandlePermanent(m_hwndRelatedTree));
}
//*********************************************************************************
CBCGPShellBreadcrumb* CBCGPShellList::GetRelatedBreadCrumb() const
{
	if (m_hwndRelatedBreadCrumb == NULL)
	{
		return NULL;
	}
	
	return DYNAMIC_DOWNCAST(CBCGPShellBreadcrumb, CWnd::FromHandlePermanent(m_hwndRelatedBreadCrumb));
}
//*********************************************************************************
...
//**********************************************************************************
BOOL CBCGPShellList::RenameItem(int iItem, const CString& strName)
{
    ASSERT_VALID(g_pShellManager);
	...
	if (SUCCEEDED(g_pShellManager->ItemFromPath(strOld, pidlOld)))
	{
		if ((pTree = GetRelatedTree()) != NULL)
		{	
			hRelatedTreeItem = pTree->FindVisibleItem(pidlOld);
		}

		if ((pBreadcrumb = GetRelatedBreadCrumb()) != NULL)
		{	
			hRelatedBreadcrumbItem = pBreadcrumb->FindShellItem(pidlOld);
		}
	}

    CString strNew = strPath + strName;
    ...
    LPITEMIDLIST pidl;
    if (FAILED(g_pShellManager->ItemFromPath(strNew, pidl)))
    {
        ASSERT(FALSE);
    ...
    pItem->pidlRel = g_pShellManager->CopyItem(pidl, TRUE);
    pItem->pidlFQ = pidl;
	...
	if (pTree != NULL && hRelatedTreeItem != NULL)
	{
		pTree->SetItemText(hRelatedTreeItem, strName);
	}

	if (pBreadcrumb != NULL && hRelatedBreadcrumbItem != NULL)
	{
		pBreadcrumb->SetItemText(hRelatedBreadcrumbItem, strName);
	}

    return TRUE;
} 
//**********************************************************************************
void CBCGPShellList::OnEndLabelEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
	ASSERT (pNMHDR != NULL);
	...
	if (pItem != NULL)
	{
		RenameItem(pItem->iItem, pItem->pszText);
	}
