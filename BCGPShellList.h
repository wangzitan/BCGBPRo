...
class BCGCBPRODLLEXPORT CBCGPShellList : public CBCGPListCtrl
{
	...
	friend class CBCGPShellBreadcrumb;
...
public:
	...
	BOOL CreateFolder(const CString& strFolderName, LPSECURITY_ATTRIBUTES lpSecurityAttributes = NULL, BOOL bSelect = TRUE);
	BOOL RenameItem(int iItem, const CString& strName);
...
protected:
	...
	CBCGPShellBreadcrumb* GetRelatedBreadCrumb() const;

	int InsertShellItem(LPSHELLFOLDER pParentFolder, LPITEMIDLIST pidlParent, LPITEMIDLIST pidlNew, BOOL bConcatenateItem = TRUE);
	void UpdateItemColumns(int iItem, LPBCGCBITEMINFO pItem);
};
