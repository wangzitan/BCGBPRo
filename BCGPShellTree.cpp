									...
									LPITEMIDLIST	pidlParent)
{
	...
	{
		InsertShellItem(hParentItem, pParentFolder, pidlParent, pidlTemp);
		dwFetched = 0;
	}
...
//***************************************************************************************
HTREEITEM CBCGPShellTree::InsertShellItem(HTREEITEM hParentItem, LPSHELLFOLDER pParentFolder, LPITEMIDLIST pidlParent, LPITEMIDLIST pidlNew, BOOL bConcatenateItem)
{
	TVITEM tvItem;
	ZeroMemory (&tvItem, sizeof (tvItem));
	...
	pParentFolder->GetAttributesOf (1, (LPCITEMIDLIST*) &pidlNew, &dwAttribs);
...
//****************************************************************************************
HTREEITEM CBCGPShellTree::AddExistingFolder(HTREEITEM hParentItem, const CString& strPath)
{
	LPBCGCBITEMINFO pParentItem = (LPBCGCBITEMINFO)GetItemData(hParentItem);
	if (pParentItem == NULL)
	{
		ASSERT(FALSE);
		return NULL;
	...
	// of the namespace, so the parent of this item is the desktop folder
	//--------------------------------------------------------------------
	if (pParentItem->pParentFolder == NULL)
	{
		hr = SHGetDesktopFolder (&pParentFolder);
	}
	else
	{
		...
		// Otherwise we need to get the IShellFolder for this item:
		//---------------------------------------------------------
		hr = pParentItem->pParentFolder->BindToObject (pParentItem->pidlRel, NULL, IID_IShellFolder, (LPVOID*) &pParentFolder);
	}
	...
	return InsertShellItem(hParentItem, pParentFolder, pidl, g_pShellManager->CopyItem(pidl, TRUE), FALSE);
}
//****************************************************************************************
...
BOOL CBCGPShellTree::SelectPath (LPCITEMIDLIST lpidl)
{
	...
	else
	{
		...
		{
			LPITEMIDLIST lpidlCurr1 = lstItems.GetNext (pos);
			...
			if (htreeItem != NULL)
			{
					...
					hTreeChild = GetNextSiblingItem (hTreeChild))
				{
					...
					if (SHGetFileInfo ((LPCTSTR) pItem->pidlFQ, 0, &sfi1, sizeof (sfi1), SHGFI_PIDL | SHGFI_DISPLAYNAME) &&
						SHGetFileInfo ((LPCTSTR)lpidlCurr1, 0, &sfi2, sizeof (sfi2), SHGFI_PIDL | SHGFI_DISPLAYNAME) &&
			...
			g_pShellManager->FreeItem (lpidlCurr1);
		}
	}
...
//****************************************************************************************
BOOL CBCGPShellTree::CreateFolder(const CString& strFolderName, LPSECURITY_ATTRIBUTES lpSecurityAttributes /*= NULL*/, BOOL bSelect /*= TRUE*/)
{
	ASSERT_VALID(g_pShellManager);
	...
	BOOL bIsEmptyParent = GetChildItem(hItem) == NULL;
	...
	CString strPath;
	if (!GetItemPath(strPath) || strPath.IsEmpty())
	{
		return FALSE;
	}
	...
	if (bIsEmptyParent)
	{
		...
		if (bSelect)
		{
			for (HTREEITEM hTreeChild = GetChildItem(hItem); hTreeChild != NULL; hTreeChild = GetNextSiblingItem(hTreeChild))
			{
				if (GetItemText(hTreeChild) == strFolderName)
				{
					EnsureVisible(hTreeChild);
					SelectItem(hTreeChild);
					break;
				}
			}
		}
	...
	else
	{
		...
		HTREEITEM hNewItem = InsertShellItem(GetSelectedItem(), pParentFolder, pidl, g_pShellManager->CopyItem(pidl, TRUE), FALSE);
		if (hNewItem == NULL)
		{
			return FALSE;
		}
...
//**********************************************************************************
BOOL CBCGPShellTree::RenameItem(HTREEITEM hItem, const CString& strName)
{
    ASSERT_VALID(g_pShellManager);
    ...
    CString strOld;
    if (!GetItemPath(strOld, hItem))
    {
        return FALSE;
    }
	
	CString strPath;
    if (!GetItemPath(strPath, hParentItem))
    {
        return FALSE;
    }
...
//**********************************************************************************
HTREEITEM CBCGPShellTree::FindVisibleItem(LPCITEMIDLIST lpidl, HTREEITEM hStart)
{
	ASSERT_VALID (this);
	ASSERT_VALID (g_pShellManager);
	...
	HTREEITEM htreeItem = hStart == NULL ? GetRootItem() : hStart;
	...
	if (SHGetFileInfo ((LPCTSTR) pItem->pidlFQ, 0, &sfi1, sizeof (sfi1), SHGFI_PIDL | SHGFI_DISPLAYNAME) &&
		SHGetFileInfo ((LPCTSTR) lpidl, 0, &sfi2, sizeof (sfi2), SHGFI_PIDL | SHGFI_DISPLAYNAME) &&
		lstrcmpi (sfi1.szDisplayName, sfi2.szDisplayName) == 0)
	{
		return htreeItem;
	}

	for (HTREEITEM hTreeChild = GetChildItem(htreeItem); hTreeChild != NULL; hTreeChild = GetNextSiblingItem (hTreeChild))
	{
		HTREEITEM htreeItemFound = FindVisibleItem(lpidl, hTreeChild);
		if (htreeItemFound != NULL)
		{
...
//**********************************************************************************
void CBCGPShellTree::OnEndLabelEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
	ASSERT (pNMHDR != NULL);
	...
	if (pItem != NULL)
	{
		RenameItem(pItem->hItem, pItem->pszText);
	}
