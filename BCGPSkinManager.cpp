...
BOOL CBCGPSkinManager::LoadState (LPCTSTR lpszProfileName)
{
	...
	{
		TRACE(_T("Can't load skin library '%s'"), (LPCTSTR)strLibraryPath);
		return FALSE;
	}
...
BOOL CBCGPSkinManager::DownloadSkins ()
{
	...
	if (hInstDLL == NULL)
	{
		...
		CString strError;
		strError.Format (IDS_BCGBARRES_CANT_LOAD_DLL_FMT, (LPCTSTR)m_strDownloadDllName);
		...
		else
		{
			...
			if (pfDownloadSkins == NULL)
			{
				...
				strError.Format (IDS_BCGBARRES_CANNT_FIND_ENTRY, 
					(LPCTSTR)m_strDownloadDllName, _T("BCGCBDownloadSkins"));
