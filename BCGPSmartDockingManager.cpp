...
CBCGPSmartDockingManager::CBCGPSmartDockingManager () :
	...
	m_pwndOwner (NULL),
	m_pDockingWnd (NULL),
	m_wndPlaceMarker(CBCGPSDPlaceMarkerWnd::BCGPSDPlaceMarkerWnd_Rectangle, 
		CBCGPDockManager::GetSmartDockingParams ().m_bUseThemeColorInShading ? globalData.clrActiveCaption : RGB (47, 103, 190), 100)
{
		...
		CRuntimeClass* prtCentralGroup)
{
    ...
    m_wndPlaceMarker.Create(rectEmpty, m_pwndOwner, FALSE);
...
void CBCGPSmartDockingManager::Show (BOOL bShow)
{
	if (m_bStarted && m_bShown != bShow)
	{
		...
		BOOL bTabbed = m_wndPlaceMarker.GetStyle() == CBCGPSDPlaceMarkerWnd::BCGPSDPlaceMarkerWnd_RectangleWithTab || m_wndPlaceMarker.GetStyle() == CBCGPSDPlaceMarkerWnd::BCGPSDPlaceMarkerWnd_FrameWithTab;
		if (!bShow && !bTabbed)
        {
            m_wndPlaceMarker.Hide ();
        }
...
void CBCGPSmartDockingManager::ShowPlaceAt (CRect rect)
{
    ...
    {
		m_wndPlaceMarker.ShowAt(rect, CBCGPSDPlaceMarkerWnd::BCGPSDPlaceMarkerWnd_Rectangle);
	}
}
...
void CBCGPSmartDockingManager::ShowTabbedPlaceAt (CRect rect, int /*nTabXOffset*/, int /*nTabWidth*/, int /*nTabHeight*/)
{
    if (m_bStarted)
    {
		m_wndPlaceMarker.ShowAt(rect, CBCGPSDPlaceMarkerWnd::BCGPSDPlaceMarkerWnd_RectangleWithTab);
	}
}
