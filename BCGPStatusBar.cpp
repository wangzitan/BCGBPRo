...
//********************************************************************************
BOOL CBCGPStatusBar::PreTranslateMessage(MSG* pMsg)
{
   	switch (pMsg->message)
	{
	case WM_KEYDOWN:
...
//*******************************************************************************
CString CBCGPStatusBar::GetTipDescription(int nIndex) const
{
	ASSERT_VALID(this);
	...
	CString s = pSBP->lpszDescr == NULL ? _T("") : pSBP->lpszDescr;
	return s;
}
//*******************************************************************************
void CBCGPStatusBar::SetTipDescription(int nIndex, LPCTSTR pszTipDescr)
{
	ASSERT_VALID(this);
	...
	if (pSBP->lpszDescr != NULL)
	{
		if (pszTipDescr != NULL && lstrcmp(pSBP->lpszDescr, pszTipDescr) == 0)
		{
			return;        // nothing to change
		}
		
		free((LPVOID)pSBP->lpszDescr);
	}
	else if (pszTipDescr == NULL || *pszTipDescr == '\0')
	{
		return; // nothing to change
	}
	
	if (pszTipDescr == NULL || *pszTipDescr == '\0')
	{
		pSBP->lpszDescr = NULL;
	}
	else
	{
		pSBP->lpszDescr = _tcsdup(pszTipDescr);
	}
}
//*******************************************************************************
...
//****************************************************************************
BOOL CBCGPStatusBar::OnTTNeedTipText (UINT /*id*/, NMHDR* pNMH, LRESULT* /*pResult*/)
{
	...
	CBCGStatusBarPaneInfo* pSBP = _GetPanePtr(nItem);
	if (pSBP == NULL)
	{
		return FALSE;
	}
	...
	if (pSBP->lpszDescr != NULL)
	{
		...
		if (pToolTip != NULL)
		{
			...
			pToolTip->SetDescription(pSBP->lpszDescr);
		}
	}
...
//**************************************************************************
LRESULT CBCGPStatusBar::OnBCGUpdateToolTips(WPARAM wp, LPARAM)
{
	UINT nTypes = (UINT) wp;
...
//**************************************************************************
void CBCGPStatusBar::RebuildToolTips()
{
	if (m_pToolTip->GetSafeHwnd () == NULL)
	{
		return;
