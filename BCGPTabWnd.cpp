...
CBCGPTabWnd::CBCGPTabWnd()
{
	...
	m_TabDocumentsMenuStyle = TAB_DOCUMENTS_MENU_DOWN_ARROW;
	...
	m_nTabAnimationVal = INT_MAX;
	m_bTabAnimationSupport = TRUE;
	m_bProcessAnimation = FALSE;
	m_dblTabAnimationTime = 0.3;

	m_bTabsHandCursor = FALSE;
	m_DotShape = TAB_DOT_ELLIPSE;

	m_nTabStateAnimationCount = 0;
	m_nTabStateAnimationDelay = 150;
						 ...
						 BOOL bCloseBtn /* = FALSE */)
{
	...
	m_bIsUnderlineStyle = (style == STYLE_UNDERLINE);
	...
	m_bScroll = (m_bFlat || style == STYLE_3D_SCROLLED || 
				 ...
				 style == STYLE_3D_ROUNDED_SCROLL || style == STYLE_POINTER || style == STYLE_UNDERLINE);
...
void CBCGPTabWnd::OnDraw(CDC* pDC)
{
	...
	if ((!m_bDrawFrame || m_bIsPointerStyle || m_bIsDotsStyle || m_bIsUnderlineStyle) && !bBackgroundIsReady)
	{
	...
	if (!m_bIsPointerStyle && !m_bIsDotsStyle && !m_bIsUnderlineStyle)
	{
	...
	else if (m_bIsPointerStyle)
	{
		...
		if (m_iActiveTab >= 0)
		{
			...
			if (m_nTabAnimationVal != INT_MAX)
			{
				rectActiveTab.OffsetRect(m_nTabAnimationVal - rectActiveTab.left, 0);
			}
		}

		CBCGPVisualManager::GetInstance ()->OnDrawTabsPointer(pDC, this, rectTabs, GetPointerAreaHeight(), rectActiveTab);
	}
	else if (m_bIsUnderlineStyle)
	{
		CRect rectActiveTab(0, 0, 0, 0);

		int nPointerAreaHeight = GetPointerAreaHeight();
		int yLine = m_location == LOCATION_TOP ? rectTabs.bottom - nPointerAreaHeight / 2 : rectTabs.top + nPointerAreaHeight / 2 - 1;

		CBCGPPenSelector ps(*pDC, CBCGPVisualManager::GetInstance ()->GetTabPointerColor(this));
		
		pDC->MoveTo(rectTabs.left, yLine);
		pDC->LineTo(rectTabs.right, yLine);
	}
	...
	if (m_rectTabsArea.Width () > 5 && m_rectTabsArea.Height () > 5)
	{
			...
			if (i != m_iActiveTab || m_bIsDotsStyle)	// Draw active tab last
			{
				...
				else if (m_bFlat)
				{
					...
					if (bIsSelected)
					{
						...
						pOldBrushFlat = pDC->SelectObject (&m_brActiveTab);
					...
					if (pOldBrushFlat != NULL)
					{
						pDC->SelectObject(pOldBrushFlat);
		...
		if (m_iActiveTab >= 0 && !m_bIsDotsStyle)
		{
			...
			if (m_bFlat)
			{
				...
				//---------------------------------
				int xLeft = max (	m_rectTabsArea.left + 1,
				...
				if (pTabActive->m_rect.right > m_rectTabsArea.left + 1)
				{
					...
					int xRight = pTabActive->m_rect.right;

					if (!m_rectTabSplitter.IsRectEmpty ())
					{
						xLeft = min(xLeft, m_rectTabSplitter.left);
						xRight = min(xRight, m_rectTabSplitter.left);
					}

					const int yLine = (m_location == LOCATION_BOTTOM) ? pTabActive->m_rect.top : pTabActive->m_rect.bottom;
					
					pDC->MoveTo(xLeft, yLine);
					pDC->LineTo(xRight, yLine);
	...
	if (m_iActiveTab >= 0 && m_bIsUnderlineStyle)
	{
		CBCGPTabInfo* pTabActive = (CBCGPTabInfo*) m_arTabs [m_iActiveTab];
		ASSERT_VALID (pTabActive);

		CRect rectActiveTab = pTabActive->GetRect();
		...
		if (!HasImage(m_iActiveTab) && pTabActive->m_rectClose.IsRectEmpty())
		{
			int nFullActiveTabWidthWidth = pDC->GetTextExtent(pTabActive->m_strText).cx;
			if (nFullActiveTabWidthWidth < rectActiveTab.Width())
			{
				rectActiveTab.left = rectActiveTab.CenterPoint().x - nFullActiveTabWidthWidth / 2;
				rectActiveTab.right = rectActiveTab.left + nFullActiveTabWidthWidth;
			}
		}
		
		const int nPointerAreaHeight = GetPointerAreaHeight();
		const int nVertOffset = nPointerAreaHeight * 3 / 4;

		if (m_location == LOCATION_TOP)
		{
			rectActiveTab.top = rectActiveTab.bottom - nPointerAreaHeight / 2;
			rectActiveTab.OffsetRect(0, nVertOffset);
		}
		else
		{
			rectActiveTab.bottom = rectActiveTab.top + nPointerAreaHeight / 2;
			rectActiveTab.OffsetRect(0, -nVertOffset);
		}
		...
		for (POSITION pos = m_lstButtons.GetHeadPosition (); pos != NULL;)
		{
			...
			if (pWndBtn->IsWindowVisible())
			{
				...
				if (rectBtn.left > rectActiveTab.left)
				{
					rectActiveTab.right = min(rectActiveTab.right, rectBtn.left);
				}
			}			
		}

		CBCGPVisualManager::GetInstance()->OnDrawTabUnderline(pDC, this, rectActiveTab, m_iActiveTab);
	}

	if (!m_rectTabSplitter.IsRectEmpty ())
	{
		CBCGPVisualManager::GetInstance()->OnDrawTabSplitter(pDC, this, m_rectTabSplitter);
	}
...
//***************************************************************************************
void CBCGPTabWnd::CleanUp()
{
	CBCGPBaseTabWnd::CleanUp();

	if (m_nTabStateAnimationCount > 0)
	{
		m_nTabStateAnimationCount = 0;
...
void CBCGPTabWnd::OnSize(UINT nType, int cx, int cy) 
{
	...
	const BOOL bWndVisible = IsWindowVisible();
	if (bWndVisible)
	{
		SetRedraw(FALSE);
	}
...
BOOL CBCGPTabWnd::SetActiveTab (int iTab)
{
	...
	if (m_bProcessAnimation && m_bTabAnimationSupport && m_dblTabAnimationTime > 0.0 && (IsPointerStyle() || IsUnderlineStyle()) && !m_bIsMDITab)
	{
		...
		{
			StartAnimation((double)rectActivePrev.left, (double)rectActive.left, m_dblTabAnimationTime, BCGPANIMATION_AccelerateDecelerate);
		}
...
void CBCGPTabWnd::Draw3DTab (CDC* pDC, CBCGPTabInfo* pTab, BOOL bActive)
{
	...
	if ((m_bIsOneNoteStyle || m_bIsVS2005Style || m_bLeftRightRounded) && pTab->m_rect.left < m_rectTabsArea.left && !m_bAlowPartialDrawing)
	{
		return;
	}
	...
	{
		CBCGPVisualManager::GetInstance ()->OnDrawTab(pDC, pTab->m_rect, m_iCurTab, bActive || IsTabSelected(m_iCurTab), this);
	}
}
//***************************************************************************************
...
//***************************************************************************************
BOOL CBCGPTabWnd::SetTabStateAnimationImages(UINT uiAnimImagesResID, int cx, BOOL bDPIAutoScale, COLORREF clrTransp)
{
	CBCGPToolBarImages images;
	images.SetTransparentColor(clrTransp);

	if (images.Load(uiAnimImagesResID))
	{
	...
	return SetTabStateAnimationImages(images);
}
//***************************************************************************************
BOOL CBCGPTabWnd::SetTabStateAnimationImages(const CBCGPToolBarImages& animImages)
{
	return ((CBCGPToolBarImages&)animImages).CopyTo(m_StateAnimationImages);
}
//***************************************************************************************
BOOL CBCGPTabWnd::StartTabStateAnimation(int iTab, BOOL bRedraw)
{
	if (GetTabStateAnimationFramesCount() == 0)
	{
		TRACE0("CBCGPTabWnd::StartTabStateAnimation: you've to specify tab animation images first - call CBCGPTabWnd::SetTabStateAnimationImages\n");
		ASSERT(FALSE);
		return FALSE;
	}
	...
	if (iTab < 0 || iTab >= m_iTabsNum)
	{
		TRACE(_T("CBCGPTabWnd::StartTabStateAnimation: illegal tab number %d\n"), iTab);
		return FALSE;
	}
	...
	if (pTabInfo->m_nStateAnimationStep >= 0)
	{
		return FALSE;
	}

	pTabInfo->m_nStateAnimationStep = 0;

	if (m_nTabStateAnimationCount == 0)
	{
		SetTimer(IdTabAnimation, m_nTabStateAnimationDelay, NULL);
	}

	m_nTabStateAnimationCount++;

	if (bRedraw)
	{
		RedrawWindow(pTabInfo->m_rect);
	}

	return TRUE;
}
//***************************************************************************************
void CBCGPTabWnd::StopTabStateAnimation(int iTab, BOOL bRedraw)
{
	if (iTab < 0 || iTab >= m_iTabsNum)
	{
		TRACE(_T("CBCGPTabWnd::StopTabStateAnimation: illegal tab number %d\n"), iTab);
		return;
	}
	...
	pTabInfo->m_nStateAnimationStep = -1;
...
//***************************************************************************************
BOOL CBCGPTabWnd::IsTabStateAnimation(int iTab) const
{
	if (iTab < 0 || iTab >= m_iTabsNum)
	{
		return FALSE;
	...
	return (pTabInfo->m_nStateAnimationStep >= 0);
}
//***************************************************************************************
void CBCGPTabWnd::ResetTabStateAnimationStep(int iTab)
{
	if (iTab < 0 || iTab >= m_iTabsNum)
	{
		return;
...
void CBCGPTabWnd::RecalcLayout ()
{
	...
	if (m_bScroll)
	{
		...
		{
			nScrollBtnWidth = 4 * nTabsHeight / 3;
		}
		else
		{
	...
	else
	{
		...
		if (m_bScroll)
		{
			...
			int nButtonsHeight1 = m_bScrollButtonFullSize ? (nTabsHeight - 2) : nScrollBtnWidth;

			// Reposition scroll buttons:
			ReposButtons (	
				CPoint (m_rectTabsArea.right + 1, m_rectTabsArea.CenterPoint ().y - nButtonsHeight1 / 2),
				CSize (nScrollBtnWidth, nButtonsHeight1), bHideTabs, nButtonMargin);
		}
	}
...
void CBCGPTabWnd::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if (m_bTrackSplitter)
	{
		...
		CRect rectRedraw = m_rectTabSplitter;
		rectRedraw.InflateRect (abs(m_rectTabSplitter.left - nSplitterLeftPrev), GetTabBorderSize ());
...
void CBCGPTabWnd::UpdateScrollButtonsState ()
{
	...
	if (m_bTabDocumentsMenu)
	{
		CBCGPMenuImages::IMAGE_STATE state = m_bIsOneNoteStyle || m_bIsVS2005Style || m_bLeftRightRounded ? CBCGPMenuImages::ImageBlack : CBCGPMenuImages::ImageDkGray;

		switch (m_TabDocumentsMenuStyle)
		{
		case TAB_DOCUMENTS_MENU_DOWN_ARROW:
		default:
		...
		case TAB_DOCUMENTS_MENU_ELLIPSIS:
			m_btnScrollRight.SetStdImage(CBCGPMenuImages::IdEllipsis, state);
			break;

		case TAB_DOCUMENTS_MENU_HAMBURGER:
			m_btnScrollRight.SetStdImage(CBCGPMenuImages::IdHamburger, state);
			break;
		}
	}
...
//***************************************************************************************
void CBCGPTabWnd::EnableTabDocumentsMenu (BOOL bEnable/* = TRUE*/, TabsDocumentsMenuStyle style/* = TAB_DOCUMENTS_MENU_DOWN_ARROW*/)
{
	ASSERT_VALID (this);
...
void CBCGPTabWnd::OnShowTabDocumentsMenu (CPoint point)
{
	...
	for (int i = 0; i < m_iTabsNum; i ++)
	{
		...
		//--------------------------------
		if (globalUtils.m_bRemoveDoubleAmpersands)
		{
...
void CBCGPTabWnd::DrawResizeDragRect (CRect& rectNew, CRect& /*rectOld*/)
{
	if (m_pDragRectWnd == NULL)
	{
		...
		{
			m_pDragRectWnd = new CBCGPSDPlaceMarkerWnd(CBCGPSDPlaceMarkerWnd::BCGPSDPlaceMarkerWnd_Rectangle);
			
			if (!m_pDragRectWnd->Create(rectNew, this))
			{
				ASSERT(FALSE);
			}
		}
	...
	else
	{
		...
		{
			m_pDragRectWnd->SetWindowPos(&CWnd::wndTopMost, rectNew.left, rectNew.top, -1, -1,
...
{
	if (GetVisibleTabsNum() == 0 || !IsPureFlatTab())
	{
		return 0;
	}

	int nTextMargin = 0;
	return m_bIsPointerStyle ? ((CBCGPTabWnd*)this)->GetTextHeight(nTextMargin) : m_bIsUnderlineStyle ? globalUtils.ScaleByDPI(6) : 0;
}
//*****************************************************************************
...
//**************************************************************************************
void CBCGPTabWnd::OnAnimationValueChanged(double dblOldValue, double dblNewValue)
{
	UNREFERENCED_PARAMETER(dblOldValue);

	int nTabAnimationVal = (int)dblNewValue;
	if (nTabAnimationVal != m_nTabAnimationVal)
	{
		m_nTabAnimationVal = nTabAnimationVal;
...
//**************************************************************************************
void CBCGPTabWnd::OnAnimationFinished()
{
	m_nTabAnimationVal = INT_MAX;
}
//**************************************************************************************
CWnd* CBCGPTabWnd::DoCreateOnPlaceHolder(CWnd* pParent, UINT nID, DWORD dwStyle, const CRect& rect, LPVOID /*lParam*/)
{
	if (!Create(STYLE_3D, rect, pParent, nID, CBCGPBaseTabWnd::LOCATION_TOP, FALSE))
	{
		return NULL;
	}
...
//**************************************************************************************
void CBCGPTabWnd::EnableTabAnimation(BOOL bEnable, double dblAnimationTime)
{
	m_bTabAnimationSupport = bEnable;
	m_dblTabAnimationTime = dblAnimationTime;
}
//**************************************************************************************
...
void CBCGPTabWnd::OnTimer(UINT_PTR nIDEvent) 
{
	if (nIDEvent == IdTabAnimation)
	{
		for (int i = 0; i < m_iTabsNum; i++)
		{
			...
			if (pTab->m_nStateAnimationStep >= 0)
			{
				pTab->m_nStateAnimationStep++;

				if (pTab->m_nStateAnimationStep >= GetTabStateAnimationFramesCount())
				{
					pTab->m_nStateAnimationStep = 0;
				}
