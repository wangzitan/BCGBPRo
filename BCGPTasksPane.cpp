...
void CBCGPTasksPaneToolBar::Disconnect()
{
	#if _MSC_VER < 1300
...
int CBCGPTasksPane::ReposTasks (BOOL bCalcHeightOnly/* = FALSE*/)
{
	...
	for (pos = m_lstTaskGroups.GetHeadPosition (); pos != NULL;)
	{
		...
		{
			CFont* pFontBoldOld = dc.SelectObject (&globalData.fontBold);
			CSize sizeText = dc.GetTextExtent (pGroup->m_strName);
			dc.SelectObject (pFontBoldOld);
			...
			if (IsToolBox ())
			{
				...
				{
					nHeight = globalUtils.ScaleByDPI(18);
				}
			}
				...
				if (pTask->m_hwndTask == NULL)
				{
						if (pTask->m_bVisible)
						{
							...
							if ((pTask->m_uiCommandID == 0) ? m_bWrapLabels : m_bWrapTasks)
							{
								...
								CFont* pFontUnderlineOld = dc.SelectObject (&globalData.fontUnderline);
								int cy = dc.DrawText (pTask->m_strName, rectText, DT_CALCRECT | DT_WORDBREAK);
								dc.SelectObject (pFontUnderlineOld);
...
//******************************************************************************************
BOOL CBCGPTasksPane::SetIconsList (UINT uiImageListResID, int cx, COLORREF clrTransparent, BOOL bAutoScale, int cy)
{
	ASSERT_VALID(this);
	...
	{
		images.SetImageSize(CSize(cx, cy == 0 ? cx : cy));
	}
...
//****************************************************************************************
void CBCGPTasksPane::AdjustScroll (BOOL bReposTasks/* = TRUE*/, BOOL bRedraw/* = TRUE*/)
{
	ASSERT_VALID (this);
	...
	const BOOL bWrapItemText = m_bWrapTasks || m_bWrapLabels;

	BYTE bAdjustForScroll = TRUE;
	if (!m_bUseScrollButtons && bWrapItemText)
	{
	...
	{
		if (!bWrapItemText || !bAdjustForScroll)
		{
			m_rectTasks.right -= cxScroll;
			ReposTasks();
		...
		m_wndScrollVert.SetWindowPos(NULL, m_rectTasks.right, m_rectTasks.top, cxScroll, m_rectTasks.Height (), SWP_SHOWWINDOW | SWP_NOZORDER | SWP_NOACTIVATE);
	}
	else
	{
		if (!m_bUseScrollButtons && bWrapItemText)
...
//********************************************************************************
LRESULT CBCGPTasksPane::OnGetGestureConfig(WPARAM, LPARAM)
{
	return 1L;	
}
//********************************************************************************
