...
BOOL CBCGPToolBar::m_bUpdateUnpermittedCommands = FALSE;
	...
	m_bIgnoreSetText (FALSE)
{
	...
	m_bToolBarImagesInDropDownMenus = FALSE;
	m_bHideEmptyMenu	= FALSE;
}
...
//*****************************************************************************************
void CBCGPToolBar::SetUseToolBarImagesInDropDownMenus(BOOL bSet /*= TRUE*/)
{
	m_bToolBarImagesInDropDownMenus = bSet;
}
//*****************************************************************************************
...
int CBCGPToolBar::InsertButton (CBCGPToolbarButton* pButton, int iInsertAt)
{
	...
	if (!IsCommandPermitted (pButton->m_nID) && !m_bUpdateUnpermittedCommands)
	{
		return -1;
	}
...
void CBCGPToolBar::DoPaint(CDC* pDCPaint)
{
	...
	if ((GetStyle () & TBSTYLE_TRANSPARENT) == 0 || IsFloating())
	{
		CBCGPVisualManager::GetInstance ()->OnFillBarBackground(pDC, this, rectClient, rectClip);
	}
	else
	{
	...
	CSize sizeImageDest = m_bMenuMode && !m_bToolBarImagesInDropDownMenus ? m_sizeMenuImage : GetImageSize ();
	if (dblImageScale != 1.)
	{
		if (m_bMenuMode && !m_bToolBarImagesInDropDownMenus && sizeImageDest == CSize (-1, -1))
		{
	...
	for (POSITION pos = m_Buttons.GetHeadPosition (); pos != NULL; iButton ++)
	{
		...
		if (pDC->RectVisible(&rect))
		{
			...
			if (bDrawImages)
			{
				const BOOL bIsSVGImage = pImages->IsSVGImage(pButton->GetImage());
				...
				{
					if (m_bMenuMode && !m_bToolBarImagesInDropDownMenus)
					{
						if (bDisabled && !bIsSVGImage && pDisabledMenuImages->GetCount () > 0)
						{
						...
						{
							bDrawDisabledImages = (bDisabled && !bIsSVGImage && pDisabledImages->GetCount () > 0);
		...
		if (pTI != NULL)
		{
			...
			if (!OnUserToolTip (pButton, strTipText))
			{
					...
					!pButton->m_strText.IsEmpty ())
				{
					...
					globalUtils.RemoveSingleAmp(strTipText, FALSE);
				}
				else
				{
...
void CBCGPToolBar::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (nHit != m_iHighlighted && nHit >= 0 && GetDroppedDownMenu () != NULL)
	{
		if (DYNAMIC_DOWNCAST(CBCGPToolbarMenuButton, GetButton(nHit)) == NULL)
		{
			return;
		}
	}
	...
	if (m_bMenuMode && m_iHighlighted == -1 && globalData.IsAccessibilitySupport ())
	{
		const int nIndex = HitTest(point);
			...
			m_iHighlighted != -1) 
		{
			...
			if (globalData.IsAccessibilitySupport () && m_bMenuMode)
			{
				...
				if (nIndex != m_iAccHotItem)
				{
					...
					KillTimer (uiAccNotifyEvent);
					SetTimer (uiAccNotifyEvent, bDropDown ? uiAccPopupTimerDelay : uiAccTimerDelay, NULL);
				}	
			}
		}
...
void CBCGPToolBar::OnLButtonUp(UINT nFlags, CPoint point)
{
		...
		iButtonCapture < m_Buttons.GetCount ())	// Button may disappear now!
	{
		...
		{
			pButton = GetButton (iButtonCapture);
			if (pButton != NULL)
			{
				pButton->m_nStyle &= ~TBBS_PRESSED;
			}
...
BOOL CBCGPToolBar::SetCustomizeMode (BOOL bSet)
{
	...
	//---------------------------------------------------------------------
	for (BOOL bToolbarsListWasChanged = TRUE; bToolbarsListWasChanged;)
	{
		INT_PTR iOrigCount = gAllToolbars.GetCount (); bToolbarsListWasChanged = FALSE;
	...
	for (posTlb = gAllToolbars.GetHeadPosition (); posTlb != NULL;)
	{
		...
		if (!bSet && pToolBar->m_bHideEmptyMenu)
		{
...
void CBCGPToolBar::Serialize (CArchive& ar)
{
	...
	try
	{
		...
		if (ar.IsLoading ())
		{
			...
			//--------------------------
			if (!m_bUpdateUnpermittedCommands)
			{
...
CSize CBCGPToolBar::CalcSize (BOOL bVertDock)
{
	...
	for (POSITION pos = m_Buttons.GetHeadPosition (); pos != NULL;)
	{
		...
		if (m_bUpdateUnpermittedCommands && !IsCommandPermitted(pButton->m_nID))
		{
			continue;
		}

		if (m_bHideEmptyMenu && !IsCustomizeMode() && pButton->IsEmptyDropDownMenu())
		{
			continue;
		}
...
void CBCGPToolBar::AdjustLocations ()
{
	...
	for (POSITION pos = m_Buttons.GetHeadPosition (); pos != NULL;)
	{
		...
		else if (m_bUpdateUnpermittedCommands && !IsCommandPermitted(pButton->m_nID))
		{
			sizeButton = CSize (0, 0);
			bVisible = FALSE;
		}
		else if (m_bHideEmptyMenu && !IsCustomizeMode() && pButton->IsEmptyDropDownMenu())
		{
			sizeButton = CSize (0, 0);
			bVisible = FALSE;
		}
...
void CBCGPToolBar::RebuildAccelerationKeys ()
{
	...
	for (POSITION pos = m_Buttons.GetHeadPosition (); pos != NULL;)
	{
		...
		UINT nHotKey = CBCGPToolbarButton::GetHotKey(pButton->m_strText);
		if (nHotKey != 0)
		{
			m_AcellKeys.SetAt(nHotKey, pButton);
		}
	}
}
//****************************************************************************************
...
//***************************************************************************************
void CBCGPToolBar::SetNonPermittedCommands (CList<UINT, UINT>& lstCommands, BOOL bUpdateExistingToolBars)
{
	m_lstUnpermittedCommands.RemoveAll ();
	m_lstUnpermittedCommands.AddTail (&lstCommands);

	m_bUpdateUnpermittedCommands = bUpdateExistingToolBars;

	if (bUpdateExistingToolBars)
	{
		for (POSITION posTlb = gAllToolbars.GetHeadPosition (); posTlb != NULL;)
		{
			CBCGPToolBar* pToolBar = (CBCGPToolBar*) gAllToolbars.GetNext (posTlb);
...
BOOL CBCGPToolBar::SaveParameters (LPCTSTR lpszProfileName)
{
	...
	CString strSection;
	strSection.Format (REG_PARAMS_FMT, (LPCTSTR)strProfileName);
...
void CBCGPToolBar::UpdateTooltips ()
{
	...
	{
		TRACE(_T("CBCGPToolBar::UpdateTooltips: cannot delete all tools\n"));
	}
#endif
...
LRESULT CBCGPToolBar::OnPromptReset(WPARAM, LPARAM)
{
...

	{
		...
		strPrompt.Format (IDS_BCGBARRES_RESET_TOOLBAR_FMT, (LPCTSTR)strCaption);
	}
