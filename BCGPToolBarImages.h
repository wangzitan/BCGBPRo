...
public:
	...
	int AddImageList (const CBCGPToolBarImages& src, BOOL bSetBitPerPixel = FALSE);
	int AddImage (const CBCGPToolBarImages& imageList, int nIndex);
	int AddIcon (HICON hIcon, BOOL bAlphaBlend = FALSE);
	
#ifndef BCGP_EXCLUDE_SVG
	int AddSVG(CBCGPSVGImage* pSVG);
	int AddSVGList(const CBCGPSVGImageList& svg);
	BOOL UpdateSVG(int iImage, CBCGPSVGImage* pSVG);
	BOOL ExportToSVGImageList(CBCGPSVGImageList& imageList);
#endif
	...
	HICON ExtractSimplifiedIcon(int nIndex);

#ifndef BCGP_EXCLUDE_SVG
	CBCGPSVGImage* GetSVG(int nIndex) const;
#endif
	...
	static HICON SimplifyIcon(HICON hIcon, BOOL bDefaultSize = TRUE);
	...
	void AdaptColors(COLORREF clrBase, COLORREF clrTone, double dOpacity); // dOpacity [0.0 ... 1.0]
	...
	BOOL IsSVGImage(int iImage) const
	{
#ifndef BCGP_EXCLUDE_SVG
		return iImage >= 0 && iImage < m_arSVG.GetSize() && m_arSVG[iImage] != NULL;
#else
		UNREFERENCED_PARAMETER(iImage);
		return FALSE;
#endif
	}
	
	BOOL IsSVGList() const
	{
#ifndef BCGP_EXCLUDE_SVG
		return m_iCount > 0 && m_iCount == m_nSVGCount;
#else
		return FALSE;
#endif
	}
...
public:
	...
	static double		m_dblColorTolerance;// Used by 'AdaptColors' method
...
#ifndef BCGP_EXCLUDE_SVG
	CArray<CBCGPSVGImage*,CBCGPSVGImage*> m_arSVG;		// Array of SVG images
#endif
	int					m_nSVGCount;		// Count of SVG images
