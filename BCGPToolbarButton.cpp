								...
								BOOL bDrawBorder, BOOL bGrayDisabledButtons)
{
	...
	if ((m_bTextBelow && bHorz) || IsDrawText ())
	{
		...
		if (bHorz)
		{
			...
			{
				if (strText.Find(_T("&&")) >= 0 && globalUtils.m_bRemoveDoubleAmpersands)
				{
		...
		else
		{
			...
			{
				strText.Replace (_T("&&"), strDummyAmpSeq);
				iAmpIndex = strText.Find (_T('&'));	// Find a SINGLE '&'
				strText.Remove (_T('&'));
				strText.Replace (strDummyAmpSeq, _T("&&"));
			}
			...
			if (iAmpIndex >= 0 && (globalData.m_bUnderlineKeyboardShortcuts && !CBCGPToolBar::IsCustomizeMode ()))
			{
				...
				int x1 = rect.CenterPoint ().x - sizeText.cy / 2;
				...
				pDC->MoveTo (x1, rectText.top + y1);
				pDC->LineTo (x1, rectText.top + y2);
...
//*******************************************************************************
UINT CBCGPToolbarButton::GetHotKey(const CString& strLabelIn)
{
	if (!strLabelIn.IsEmpty())
	{
		CString strLabel = strLabelIn;

		// Remove double ampersands (the hot key can be located after them):
