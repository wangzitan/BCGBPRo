		...
		default:
			...
			else
			{
				...
				if (lpszMenuPath != NULL)
				{
					...
					for (int iStrCount = 0; iStrCount < lstrlen (pszCustom) - 1; iStrCount++)
					{
						if (pszCustom [iStrCount] == _TCHAR(' '))
						{
							CharUpperBuff (&pszCustom [iStrCount + 1], 1);
						}
					}
				...
				AddButton (lpszCategory == NULL ? (LPCTSTR)m_strAllCommands : lpszCategory, button);
			}
		}
	}
