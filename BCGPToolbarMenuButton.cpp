			...
			BOOL bDrawBorder, BOOL bGrayDisabledButtons)
{
	...
	if (m_bDrawDownArrow)
	{
		if (bHorz)
		{
			int nArrowSize = sizeImage.cx + nSeparatorSize - 2 + sizeExtra.cx;
			nArrowSize = max(nArrowSize, rectParent.Height() / 4);

			rectParent.right -= nArrowSize;
		...
		{
			int nArrowSize = sizeImage.cy + nSeparatorSize - 1;
			nArrowSize = max(nArrowSize, rectParent.Width() / 4);

			rectParent.bottom -= nArrowSize;
...
SIZE CBCGPToolbarMenuButton::OnCalculateSize (CDC* pDC, const CSize& sizeDefault, BOOL bHorz)
{
	...
	if (m_bDrawDownArrow || m_bMenuMode)
	{
		...
		{
			nArrowSize = (bHorz) ? CBCGPMenuImages::Size ().cx : CBCGPMenuImages::Size ().cy;
					...
					BOOL bContentOnly)
{
	...
	if (pParentMenu != NULL)
	{
		if (pParentMenu->IsToolBarImagesInDropDownMenus())
		{
			sizeMenuImage = pParentMenu->GetImageSize();

			if (!CBCGPToolBar::m_bDontScaleImages)
			{
				sizeMenuImage = globalUtils.ScaleByDPI(sizeMenuImage);
			}
		}
	...
	if (bIsRarelyUsed)
	{
		...
		{
			CBCGPPopupMenu* pParentMenu1 = DYNAMIC_DOWNCAST (CBCGPPopupMenu, 
				pParentMenuBar->GetParent ());
			if (pParentMenu1 != NULL && pParentMenu1->HideRarelyUsedCommands ())
			{
				bIsRarelyUsed = TRUE;
			}
		}
	...
	if (m_bAlwaysCallOwnerDraw || !bImageIsReady)
	{
		...
		if (pParentMenuBar != NULL)
		{
			CBCGPPopupMenu* pParentMenu1 = DYNAMIC_DOWNCAST (CBCGPPopupMenu, pParentMenuBar->GetParent ());
			if (pParentMenu1 != NULL)
			{
				CBCGPBreadcrumb* pBreadcrumb = DYNAMIC_DOWNCAST(CBCGPBreadcrumb, pParentMenu1->GetSafeOwner());
				if (pBreadcrumb != NULL)
				{
		...
		if (!bImageIsReady)
		{
			...
			{
				CBCGPRibbonPanelMenu* pRibbonPanel = DYNAMIC_DOWNCAST(CBCGPRibbonPanelMenu, pParentFrame);
				if (pRibbonPanel != NULL)
				{
					bImageIsReady = pRibbonPanel->OnDrawMenuImage (pDC, this, rectImage);
				}
			}
#endif
		}
...
BOOL CBCGPToolbarMenuButton::OpenPopupMenu (CWnd* pWnd)
{
	...
	if (!m_bQuickCustomMode && m_pPopupMenu->GetMenuBar() != NULL)
	{
		CBCGPToolBar* pParentToolBar = DYNAMIC_DOWNCAST (CBCGPToolBar, m_pWndParent);
		if (pParentToolBar != NULL && pParentToolBar->IsToolBarImagesInDropDownMenus())
		{
			m_pPopupMenu->GetMenuBar()->SetUseToolBarImagesInDropDownMenus();
		}
	}
...
//**************************************************************************************
BOOL CBCGPToolbarMenuButton::IsEmptyDropDownMenu() const
{
	if (m_listCommands.IsEmpty() && IsEmptyMenuAllowed())
	{
		return FALSE;
	}
	...
	for (POSITION pos = m_listCommands.GetHeadPosition (); pos != NULL;)
	{
		...
		UINT nID = pItem->m_nID;

		if (nID == 0 || nID == (UINT)-1)
		{
			if (!pItem->IsEmptyDropDownMenu())
			{
				return FALSE;
			}
		}
		else if (CBCGPToolBar::IsCommandPermitted(nID))
		{
			return FALSE;
		}
	}
