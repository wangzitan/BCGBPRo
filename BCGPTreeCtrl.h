...
protected:
	...
	afx_msg LRESULT OnSetImageList(WPARAM wp, LPARAM lp);
	DECLARE_MESSAGE_MAP()

protected:
	CImageList	m_ilState;
	BOOL		m_bImageListStateChanged;

	virtual void UpdateImageListState();
};
...
public:
	...
	void EnableTreeCtrlNotifications(BOOL bEnable = TRUE);
	BOOL TreeCtrlNotificationsEnabled() const
	{
		return m_bTreeCtrlNotifications;
	}
...
public:
	...
	{
		CBCGPGridCtrl::SetCurSel(pItem, bRedraw);
	}
...
protected:
	...
	CBCGPGridRow* TreeItem(HTREEITEM hItem) const    { return hItem == TVI_ROOT ? NULL : (CBCGPGridRow*)hItem; }

	CBCGPGridRow* NextRow(CBCGPGridRow* pRow, UINT nCode) const;

	void GetPositionInsertAfter(CBCGPGridRow* pParent, CBCGPGridRow* pInsertAfter, ULONG_PTR nFlags, POSITION& pos, POSITION& posParent, POSITION& posSub) const;
	...
	BOOL SendTreeNotification(UINT nCode, CBCGPGridRow* pOld, CBCGPGridRow* pNew, UINT nAction = 0) const;

	BOOL	m_bNeedToRepos;
	BOOL	m_bTreeCtrlNotifications;
};
