...
CBCGPVisualInfo::XGaugeElement* CBCGPVisualCollector::CollectGaugeElement(const CBCGPGaugeImpl& element)
{
	...
	if (element.IsKindOf (RUNTIME_CLASS (CBCGPCircularProgressIndicatorImpl)))
	{
		CBCGPVisualInfo::XElementCircularProgress* pNewInfo = new CBCGPVisualInfo::XElementCircularProgress;
		info = pNewInfo;

		CollectBaseGaugeElement (element, *info);

		CBCGPCircularProgressIndicatorImpl* pElement = (CBCGPCircularProgressIndicatorImpl*)&element;
		ASSERT_VALID (pElement);

		pNewInfo->m_ProgressColors = pElement->GetColors ();
		pNewInfo->m_Options = pElement->GetOptions();

		if (!pElement->IsMarqueeStyle())
		{
			pElement->GetRange(pNewInfo->m_dblStartValue, pNewInfo->m_dblFinishValue);
			pNewInfo->m_dblStep = pElement->GetStep();
			pNewInfo->m_dblValue = pElement->GetPos();
		}

		pNewInfo->m_strLabel = pElement->GetLabel();
	}
	else if (element.IsKindOf (RUNTIME_CLASS (CBCGPCircularGaugeImpl)))
	{
		...
		for (i = 0; i < element.GetScalesCount (); i++)
		{
			...
			pScaleInfo->m_bEquidistantLabels     = pElement->IsEquidistantLabels (i);
		...
		for (i = 0; i < pElement->GetLevelBarsCount (); i++)
		{
			CBCGPGaugeLevelBarObject* pLevelBar = (CBCGPGaugeLevelBarObject*)pElement->GetLevelBar (i);
			CBCGPVisualInfo::XGaugeLevelBar* pLevelBarInfo = new CBCGPVisualInfo::XGaugeLevelBar;

			pLevelBarInfo->m_nScale              = pLevelBar->GetScale ();
			pLevelBarInfo->m_brFill              = pLevelBar->GetFillBrush ();
			pLevelBarInfo->m_brOutline           = pLevelBar->GetOutlineBrush ();
			pLevelBarInfo->m_brValue             = pLevelBar->GetFillValueBrush ();
			pLevelBarInfo->m_dblValue            = pLevelBar->GetValue();
			pLevelBarInfo->m_dblWidth            = pLevelBar->GetWidth();
			pLevelBarInfo->m_dblOffsetFromFrame  = pLevelBar->GetOffsetFromFrame ();
			pLevelBarInfo->m_Style               = pLevelBar->GetStyle();
			pNewInfo->m_arLevelBars.Add(pLevelBarInfo);
		}
	...
	else if (element.IsKindOf (RUNTIME_CLASS (CBCGPNumericIndicatorImpl)))
	{
		...
		pNewInfo->m_Colors            = pElement->GetColors ();
		pNewInfo->m_fmtText           = pElement->GetTextFormat ();
		pNewInfo->m_bTextFormatAutoSize = pElement->IsTextFormatAutoSize();
		if (pNewInfo->m_bTextFormatAutoSize)
		{
			pNewInfo->m_fmtText.SetFontSize(pElement->m_fOriginalTextSize);
		}
