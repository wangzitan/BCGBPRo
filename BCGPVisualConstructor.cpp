...
CBCGPBaseVisualObject* CBCGPVisualConstructor::CreateElement (const CBCGPVisualInfo::XElement& info, const CBCGPVisualContainer* container) const
{
	...
	else if (info.GetElementName ().Compare (CBCGPVisualInfo::s_szChart) == 0)
	{
		...
		info.GetElementName ().Compare (CBCGPVisualInfo::s_szCircularProgressInd) == 0 ||
...
CBCGPGaugeImpl* CBCGPVisualConstructor::CreateGaugeElement (const CBCGPVisualInfo::XGaugeElement& info, const CBCGPVisualContainer* /*container*/) const
{
	...
	else if (info.GetElementName ().Compare (CBCGPVisualInfo::s_szKnob) == 0)
	{
	...
	else if (info.GetElementName ().Compare (CBCGPVisualInfo::s_szCircularProgressInd) == 0)
	{
		pElement = new CBCGPCircularProgressIndicatorImpl;
	}
...
void CBCGPVisualConstructor::ConstructGaugeElement (CBCGPGaugeImpl& element, const CBCGPVisualInfo::XGaugeElement& info) const
{
		...
		info.GetElementName ().Compare (CBCGPVisualInfo::s_szAnalogClock) == 0)
	{
		...
		for (i = 0; i < (int)infoElement.m_arScales.GetSize (); i++)
		{
			...
			pNewElement->EnableEquidistantLabels (pScaleInfo->m_bEquidistantLabels, index);
		}
		...
		for (i = 0; i < (int)infoElement.m_arLevelBars.GetSize (); i++)
		{
			const CBCGPVisualInfo::XGaugeLevelBar* pLevelBarInfo = 
				(const CBCGPVisualInfo::XGaugeLevelBar*)infoElement.m_arLevelBars[i];

			pNewElement->AddLevelBar (pLevelBarInfo->m_brFill, pLevelBarInfo->m_brOutline, pLevelBarInfo->m_brValue, pLevelBarInfo->m_nScale, 
				pLevelBarInfo->m_dblWidth, pLevelBarInfo->m_dblOffsetFromFrame, pLevelBarInfo->m_Style, FALSE);

			pNewElement->SetLevelBarValue(pLevelBarInfo->m_dblValue, i, 0, FALSE);
		}
	...
	{
		const CBCGPVisualInfo::XElementCircularProgress& infoElement = 
			(const CBCGPVisualInfo::XElementCircularProgress&)info;

		CBCGPCircularProgressIndicatorImpl* pNewElement = (CBCGPCircularProgressIndicatorImpl*)&element;

		pNewElement->SetColors (infoElement.m_ProgressColors);
		pNewElement->SetOptions (infoElement.m_Options);

		if (!pNewElement->IsMarqueeStyle())
		{
			pNewElement->SetRange(infoElement.m_dblStartValue, infoElement.m_dblFinishValue);
			pNewElement->SetStep(infoElement.m_dblStep);
			pNewElement->SetPos(infoElement.m_dblValue);
		}
		pNewElement->SetLabel(infoElement.m_strLabel);
	}
	else if (info.GetElementName ().Compare (CBCGPVisualInfo::s_szLinearGauge) == 0)
	{
	...
	else if (info.GetElementName ().Compare (CBCGPVisualInfo::s_szNumericInd) == 0)
	{
		...
		{
			pNewElement->SetTextFormat (infoElement.m_fmtText);
			pNewElement->SetTextFormatAutoSize(infoElement.m_bTextFormatAutoSize);
		}
