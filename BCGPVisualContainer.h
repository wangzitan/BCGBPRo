...
public:
	...
	CBCGPAnimationManager::BCGPAnimationType GetDataAnimationType() const
	{
		return m_DataAnimationType;
	}

	void SetDataAnimationType(CBCGPAnimationManager::BCGPAnimationType type)
	{
		m_DataAnimationType = type;
	}
...
// Overrides:
	...
	virtual BOOL IsReturnKeySupported() { return FALSE; }
...
protected:
	...
	CBCGPAnimationManager::BCGPAnimationType	m_DataAnimationType;
...
public:
	...
	virtual void CopyFrom(const CBCGPVisualScrollBarColorTheme & src)
	{
		...
		m_brButtonHighlighted = src.m_brButtonHighlighted;
		...
		if (m_brButtonHighlighted.IsEmpty() && !m_brButton.IsEmpty())
		{
			m_brButtonHighlighted = m_brButton;
			
			if (m_brButton.GetColor().IsDark())
			{
				m_brButtonPressed.MakeLighter(.8);
			}
			else
			{
				m_brButtonPressed.MakeDarker(.8);
			}
		}
	...
	CBCGPBrush	m_brButtonHighlighted;
...
public:
	...
	virtual BOOL OnMouseMove(const CBCGPPoint& pt);
...
// Operations:
public:
	void Redraw(CBCGPRect* pRect = NULL);
	void Reset();
	void ReposThumb();
	...
	BOOL DoGestureEventPan(const CBCGPPoint& ptFrom, const CBCGPPoint& ptTo);
...
protected:
	...
	BOOL					m_bIsThumbHighlighted;
	BOOL					m_bIsPrevHighlighted;
	BOOL					m_bIsNextHighlighted;
	double					m_dblTotal;
	double					m_dblOffset;
	double					m_dblStep;
	HBITMAP					m_hBitmap;
