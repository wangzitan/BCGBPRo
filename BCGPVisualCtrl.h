...
struct BCGCBPRODLLEXPORT CBCGPVisualPrintOptions
{
	CBCGPVisualPrintOptions()
	{
		m_bPrintScreenshot = FALSE;
		m_bUseGDI = FALSE;
	}

	BOOL	m_bPrintScreenshot;
	BOOL	m_bUseGDI;
};

// CBCGPBaseVisualCtrl flags:
#define BCGP_VISUAL_POPUP_DONT_CLOSE_ON_MOUSE_CLICK	0x0001
...
public:
	...
	BOOL CreatePopup(const CRect& rect, BYTE nTransparency = 255 /* from 0 to 255 */, CWnd* pWndOwner = NULL, UINT nFlags = 0);
...
public:
	...
	virtual void OnNcMouseMove(const CBCGPPoint& pt) = 0;
	...
	virtual BOOL IsReturnKeySupported() = 0;
	...
	virtual void DoEraseBackground(CDC* pDC, const CRect& rectClient);
	virtual void DoPaint(CDC* pDC, CRect rectClip = CRect(0, 0, 0, 0));

	// Printing support
	void SetPrintOptions(const CBCGPVisualPrintOptions& options)
	{
		m_PrintOptions = options;
	}

	const CBCGPVisualPrintOptions& GetPrintOptions() const
	{
...
protected:
	...
	CBCGPVisualPrintOptions	m_PrintOptions;
	BOOL					m_bIgnoreMinSize;
...
public:
	...
	virtual void SetRect(const CBCGPRect& rect, BOOL bRedraw = FALSE)
	{
		...
		if (pVisualObject != NULL)
		{
			...
			if (sizeMin.IsEmpty() || m_bIgnoreMinSize)
			{
			...
			else
			{
				...
				rectMin.SetWidth(bcgp_max(rect.Width(), sizeMin.cx));
				rectMin.SetHeight(bcgp_max(rect.Height(), sizeMin.cy));
	...
	virtual void OnNcMouseMove(const CBCGPPoint& pt)
	{
		CBCGPBaseVisualObject* pVisualObject = GetVisualObject();
		if (pVisualObject != NULL)
		{
			pVisualObject->OnNcMouseMove(pt);
		}
	}
	...
	virtual BOOL IsReturnKeySupported()
	{
		...
		if (pVisualObject != NULL)
		{
			return pVisualObject->IsReturnKeySupported();
		}
...
protected:
	...
	virtual void OnNcMouseMove(const CBCGPPoint& pt)
	{
		...
		if (pVisualContainer != NULL)
		{
			pVisualContainer->OnNcMouseMove(pt);
		}
	}
	...
	virtual BOOL IsReturnKeySupported()
	{
		...
		if (pVisualContainer != NULL)
		{
			return pVisualContainer->IsReturnKeySupported();
		}
