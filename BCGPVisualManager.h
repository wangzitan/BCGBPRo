...
struct CBCGPSwitchColors;
class CBCGPInfoBoxRenderer;
class CBCGPLightBoxDialog;
...
public:
	...
	static CString m_strStylePath;

// Operations:
public:
	static void RedrawAll ();
	static void RedrawMDIArea(CMDIFrameWnd* pFrameWnd);
...
public:
	...
	virtual BOOL FillCheckImageList(CImageList& ilState, COLORREF clrBkgnd, const CArray<int, int>& states, BOOL bIsDisabled) const;
	...
	virtual void OnFillBarBackground (CDC* pDC, CBCGPBaseControlBar* pBar,
									...
									BOOL bNCArea = FALSE);
	virtual BOOL OnFillParentBarBackground (CWnd* pWnd, CDC* pDC, LPRECT rectClip = NULL, BOOL bNCArea = FALSE);
	...
	virtual CSize GetFrameCaptionButtonOffset(CWnd* /*pWnd*/) { return CSize(0, 0);	}
	...
	virtual void OnDrawTabUnderline(CDC* pDC, CBCGPTabWnd* pTabWnd, CRect rect, int iTab);
	virtual void OnDrawTabSplitter(CDC* pDC, CBCGPTabWnd* pTabWnd, CRect rectTabSplitter);
	...
	virtual void OnDrawPropListComboButton(CDC* pDC, CRect rect, CBCGPProp* pProp, BOOL bControlBarColors, BOOL bFocused, BOOL bEnabled, BOOL bButtonIsDown, BOOL bButtonIsHighlighted);
	virtual COLORREF OnDrawPropListPushButton(CDC* pDC, CRect rect, CBCGPProp* pProp, BOOL bControlBarColors, BOOL bFocused, BOOL bEnabled, BOOL bButtonIsDown, BOOL bButtonIsHighlighted);
	...
	virtual void OnDrawSplitterBar(CDC* pDC, CBCGPSplitterWnd* pSplitterWnd, CRect& rect);
	...
	virtual void GetSwitchColors(CBCGPSwitchColors& colors, BOOL bIsVertical);

	// WinUITiles:
	virtual void GetWinUITilesColors(CBCGPWinUITilesColors& colors);

	// Info box:
	virtual void GetInfoBoxColors(CBCGPInfoBoxRenderer* pCtrl, COLORREF& clrBackground, COLORREF& clrForeground, COLORREF& clrFrame, COLORREF& clrLink);

	// Light box dialog:
	virtual void OnDrawLightBoxDialogCloseButton(CDC* pDC, CBCGPLightBoxDialog* pDlg, CRect rectClose, BOOL bIsHighlighted, BOOL bIsPressed);
	...
	virtual void OnDrawSwitch(CDC *pDC, CRect rect, BOOL bOn, BOOL bHighlighted, BOOL bPressed, BOOL bEnabled);
	...
	virtual void GetEditSimplifiedIconColors(COLORREF& clrSimple, COLORREF& clrHot, BOOL bIsCloseButton);
	...
	virtual void RemoveActivateFlag(CWnd* pWnd);
	...
	virtual BOOL IsRibbonPanelHasHighlightedState()	{	return TRUE; }
	...
	virtual int GetRibbonQATExtraHeight(BOOL bIsOnTop)	{	return bIsOnTop ? 1 : 3; }
	...
	virtual BOOL IsSimplifiedRibbonCaptionButtonIcon(CBCGPRibbonButton* /*pButton*/)	{ return FALSE; }
	...
	virtual void OnDrawRibbonAutoHideButton(CDC* pDC, const CRect& rect, BOOL bIsHighlighted, COLORREF clrFill, COLORREF clrText);
	...
	virtual int GetNavImageState(BOOL bIsHighlighted, BOOL bIsPressed, BOOL bIsDisabled, BOOL bIsDrawOnGlass);
...
// Attributes:
public:
	static BOOL					m_bMDIAreaFullRedrawOnActivate;
...
protected:
	...
	CBCGPToolBarImages*	m_pImagesSwitchOn;
	CBCGPToolBarImages*	m_pImagesSwitchOff;

	CSize	GetSystemBorders (BOOL bRibbonPresent) const;

	virtual BOOL CanSetActivateFlag(CWnd* pWnd) const;
	virtual void SetActivateFlag(CWnd* pWnd, BOOL bActive);
