...
const CBCGPVisualManager2007::Style c_StyleDefault = CBCGPVisualManager2007::VS2007_LunaBlue;
...
BOOL CBCGPVisualManager2007::SetStyle (Style style, LPCTSTR lpszPath)
{
	...
	if ((lpszPath != NULL && _tcslen (lpszPath) > 0) || !m_strStylePath.IsEmpty())
	{
		strStyleDLLPath = m_strStylePath.IsEmpty() ? lpszPath : m_strStylePath;
...
//*****************************************************************************
BOOL CBCGPVisualManager2007::CanSetActivateFlag(CWnd* pWnd) const
{
	ASSERT_VALID (pWnd);
	...
	return !globalData.DwmIsCompositionEnabled() || !IsDWMCaptionSupported() || bIsRibbonCaption;
}
//*****************************************************************************
	...
	else
	{
		...
		if (!pRenderer->IsValid ())
		{
			...
			COLORREF colorBorder = bRibbon 
								...
								: CBCGPToolBarImages::m_bIsDrawOnGlass ? m_clrComboBtnStart : m_clrComboBtnBorder;
			if (bDisabled)
			{
				color1 = bRibbon 
							...
							CBCGPToolbarComboBoxButton* pButton)
{
	...
	else if (CBCGPToolBarImages::m_bIsDrawOnGlass)
	{
		colorBorder = m_clrComboBtnStart;
	}
...
{
	if (!CanDrawImage () || pTabWnd->IsDialogControl (TRUE) || pTabWnd->IsPureFlatTab())
	{
											  ...
											  CRect rectCaption, CRect rectText)
{
	...
	DrawNcText (pDC, rectText, strTitle, strDocument, bPrefix, bActive, bIsRTL, IsRibbonCaptionTextCentered(pBar) && !pBar->IsScenicLook (), bGlass, nGlassGlowSize, clrTextGlass);
...
//*****************************************************************************
void CBCGPVisualManager2007::OnDrawRibbonMainButtonLabel(CDC* pDC, CBCGPRibbonButton* pButton, CRect rectText, const CString strText)
{
	if (!CanDrawImage ())
	{
		CBCGPVisualManager2003::OnDrawRibbonMainButtonLabel(pDC, pButton, rectText, strText);
		return;
	}
	...
	pButton->DoDrawText(pDC, strText, rectText, DT_SINGLELINE | DT_CENTER | DT_VCENTER, 
		pButton->IsDisabled() ? m_clrRibbonCategoryTextDisabled : m_clrRibbonCategoryTextActive);
}
//*****************************************************************************
					...
					CBCGPRibbonButton* pButton)
{
	...
	else if (bDefaultPanelButton)
	{
		...
		if (index != -1)
		{
			...
			CBCGPBitmapCache* pCache1 = &m_cacheRibbonBtnDefault;
			...
			if (pCategory != NULL)
			{
				...
				if (pCategory->GetTabColor () != BCGPCategoryColor_None)
				{
					...
					pCache1 = &context.m_cacheBtnDefault;
				}
			}
			...
			{
				CSize size (params.m_rectImage.Width (), rect.Height ());
				nCacheIndex = pCache1->FindIndex (size);
				if (nCacheIndex == -1)
				{
					nCacheIndex = pCache1->CacheY (size.cy, *pRenderer);
				}
			}

			if (nCacheIndex != -1 && pCache1 != NULL)
			{
				pCache1->Get(nCacheIndex)->DrawY (pDC, rect,
					CSize (params.m_rectInter.left, params.m_rectImage.right - params.m_rectInter.right),
					index);
					...
					BOOL bIsDisabled)
{
	...
	else if (bIsHighlighted && !bIsReadOnly)
	{
...
//*******************************************************************************
void CBCGPVisualManager2007::OnDrawPlannerTimeLine (CDC* pDC, CBCGPPlannerView* pView, CRect rect)
{
	if (!CanDrawImage ())
	{
		CBCGPVisualManager2003::OnDrawPlannerTimeLine (pDC, pView, rect);
		return;
	}
	...
	{
		CBCGPDrawManager dm (*pDC);
		dm.FillGradient (rect, m_clrPlannerTodayFill, m_clrPlannerNcArea, TRUE);
	...
	{
		CBCGPDrawManager dm (*pDC);
		dm.FillGradient (rect, m_clrPlannerNcArea, m_clrPlannerTodayFill, FALSE);
...
//********************************************************************************
void CBCGPVisualManager2007::GetEditSimplifiedIconColors(COLORREF& clrSimple, COLORREF& clrHot, BOOL bIsCloseButton)
{
	if (!CanDrawImage ())
	{
		CBCGPVisualManager2003::GetEditSimplifiedIconColors(clrSimple, clrHot, bIsCloseButton);
		return;
	}

	clrSimple = CBCGPDrawManager::ColorMakeDarker(globalData.clrBarShadow, .3);
	clrHot = bIsCloseButton ? RGB(213, 138, 0) : RGB(255, 165, 0);
}
//********************************************************************************
BOOL CBCGPVisualManager2007::OnDrawBrowseButton (CDC* pDC, CRect rect, CBCGPEdit* pEdit, CBCGPVisualManager::BCGBUTTON_STATE state, COLORREF& clrText)
{
	...
	BOOL bDisabled = pEdit->GetSafeHwnd() != NULL && !pEdit->IsWindowEnabled ();
...
void CBCGPVisualManager2007::OnDrawPropListComboButton(CDC* pDC, CRect rect, CBCGPProp* pProp, BOOL bControlBarColors, BOOL bFocused, BOOL bEnabled, BOOL bButtonIsDown, BOOL bButtonIsHighlighted)
{
	if (!CanDrawImage () || !bControlBarColors)
	{
		CBCGPVisualManager2003::OnDrawPropListComboButton(pDC, rect, pProp, bControlBarColors, bFocused, bEnabled, bButtonIsDown, bButtonIsHighlighted);
		return;
	}
	...
	m_ctrlRibbonBtnPush.Draw (pDC, rect, !bEnabled ? 6 : bButtonIsDown ? 2 : bButtonIsHighlighted ? 1 : 0);

	CBCGPMenuImages::Draw (pDC, CBCGPMenuImages::IdArowDown, rect,
		bEnabled ? CBCGPMenuImages::ImageBlack : CBCGPMenuImages::ImageGray);
}

COLORREF CBCGPVisualManager2007::OnDrawPropListPushButton(CDC* pDC, CRect rect, CBCGPProp* pProp, BOOL bControlBarColors, BOOL bFocused, BOOL bEnabled, BOOL bButtonIsDown, BOOL bButtonIsHighlighted)
{
	if (!CanDrawImage () || !bControlBarColors)
	{
	...
	return !bEnabled ? m_clrRibbonBarBtnTextDisabled : bButtonIsHighlighted ? m_clrRibbonBarBtnTextHighlighted : m_clrRibbonBarBtnText;
}
...
//*****************************************************************************
void CBCGPVisualManager2007::GetSwitchColors(CBCGPSwitchColors& colors, BOOL bIsVertical)
{
	CBCGPVisualManager2003::GetSwitchColors(colors, bIsVertical);
	...
	if (colors.IsGDICtrl())
	{
		colors.m_brOutline = CBCGPDrawManager::ColorMakeDarker(globalData.clrBarDkShadow);
		return;
	}

	colors.m_brFillThumb.SetColors(m_clrComboBtnStart, m_clrComboBtnFinish, bIsVertical ? CBCGPBrush::BCGP_GRADIENT_VERTICAL : CBCGPBrush::BCGP_GRADIENT_HORIZONTAL);
	colors.m_brLabelOff = globalData.clrBarText;
	colors.m_brLabelOn = m_clrMenuTextHighlighted;

	colors.m_brFillOn.SetColors(m_clrComboBtnHighlightedStart, m_clrComboBtnHighlightedFinish, bIsVertical ? CBCGPBrush::BCGP_GRADIENT_VERTICAL : CBCGPBrush::BCGP_GRADIENT_HORIZONTAL);
}
//*****************************************************************************
...
//*****************************************************************************
int CBCGPVisualManager2007::GetNavImageState(BOOL bIsHighlighted, BOOL bIsPressed, BOOL bIsDisabled, BOOL bIsDrawOnGlass)
{
	if (!CanDrawImage ())
	{
		return CBCGPVisualManager2003::GetNavImageState(bIsHighlighted, bIsPressed, bIsDisabled, bIsDrawOnGlass);
	}

	CBCGPMenuImages::IMAGE_STATE stateImage = CBCGPMenuImages::ImageBlack2;
	...
	{
		stateImage = CBCGPMenuImages::ImageBlack;
	}
