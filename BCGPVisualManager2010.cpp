...
//*****************************************************************************
void CBCGPVisualManager2010::OnDrawRibbonMainButtonLabel(CDC* pDC, CBCGPRibbonButton* pButton, CRect rectText, const CString strText)
{
											  ...
											  CRect rectCaption, CRect rectText)
{
	...
	DrawNcText (pDC, rectText, strTitle, strDocument, bPrefix, bActive, bIsRTL, IsRibbonCaptionTextCentered(pBar), bGlass, nGlassGlowSize, clrTextGlass);
...
COLORREF CBCGPVisualManager2010::OnFillRibbonButton(CDC* pDC, CBCGPRibbonButton* pButton)
{
	...
	if (pButton->IsBackstageViewMode () && !pButton->IsQATMode())
	{
		...
		pRenderer = &m_ctrlRibbonBtnBsNormal;
		if (m_clrMainButton != (COLORREF)-1)
		{
			if (!m_ctrlRibbonBtnBsNormalColorized.IsValid())
			{
				...
				m_ctrlRibbonBtnBsNormalColorized.GetImages().AdaptColors(RGB(0, 0, 192), m_clrMainButton);
			}
		...
		if (pButton->GetBackstageAttachedView() != NULL)
		{
			...
			if (m_clrMainButton != (COLORREF)-1)
			{
				if (!m_ctrlRibbonBtnBsViewColorized.IsValid())
				{
					...
					m_ctrlRibbonBtnBsViewColorized.GetImages().AdaptColors(RGB(0, 0, 192), m_clrMainButton);
				}
											...
											BOOL bIsSysButton)
{
	...
	CPen pen(PS_SOLID, 1, CBCGPDrawManager::PixelAlpha (pBar->m_clrBarBackground, 80));
...
COLORREF CBCGPVisualManager2010::GetPropListGroupColor(CBCGPPropList* pPropList)
{
	ASSERT_VALID (pPropList);

	if (!CanDrawImage () || !pPropList->DrawControlBarColors ())
	{
		return CBCGPVisualManager2007::GetPropListGroupColor(pPropList);
	}

	return m_clrToolBarGradientDark;
}

COLORREF CBCGPVisualManager2010::GetPropListGroupTextColor (CBCGPPropList* pPropList)
{
	ASSERT_VALID (pPropList);
	
	if (!CanDrawImage () || !pPropList->DrawControlBarColors ())
	{
		return CBCGPVisualManager2007::GetPropListGroupTextColor(pPropList);
	}
