						...
						int iTab, BOOL bIsActive, const CBCGPBaseTabWnd* pTabWnd)
{
	...
	if (pTabWnd->IsFlatTab () || pTabWnd->IsOneNoteStyle () ||
		...
		pTabWnd->IsLeftRightRounded () || pTabWnd->IsPureFlatTab())
	{
												...
												CBCGPToolbarComboBoxButton* pButton)
{
	if (globalData.IsHighContastMode())
	{
		CBCGPVisualManager::OnDrawComboDropButton (pDC, rect, bDisabled, bIsDropped, bIsHighlighted, pButton);
		return;
	}
											  ...
											  CBCGPBaseTabWnd* pWndTab)
{
	...
	else if (pWndTab->IsDialogControl () || (pWndTab->IsPureFlatTab() && !pWndTab->IsMDITab()))
	{
...
//*********************************************************************************
void CBCGPVisualManagerXP::OnDrawTabUnderline(CDC* pDC, CBCGPTabWnd* pTabWnd, CRect rect, int iTab)
{
	if (globalData.m_nBitsPerPixel <= 8 || globalData.IsHighContastMode ())
	{
		CBCGPVisualManager::OnDrawTabUnderline(pDC, pTabWnd, rect, iTab);
		return;
	}

	COLORREF clrFill = m_clrHighlightDn;
	...
	CBCGPEdit* pEdit, CBCGPVisualManager::BCGBUTTON_STATE state, COLORREF& clrText)
{
	...
	if (globalData.m_nBitsPerPixel <= 8 || globalData.IsHighContastMode () || !bVMStyle)
	{
...
void CBCGPVisualManagerXP::OnDrawLightBoxDialogCloseButton(CDC* pDC, CBCGPLightBoxDialog* pDlg, CRect rectClose, BOOL bIsHighlighted, BOOL bIsPressed)
{
	if (globalData.m_nBitsPerPixel <= 8 || globalData.IsHighContastMode ())
	{
		CBCGPVisualManager::OnDrawLightBoxDialogCloseButton(pDC, pDlg, rectClose, bIsHighlighted, bIsPressed);
		return;
	}
	...
	COLORREF clrBack = pDlg->IsVisualManagerStyle() ? globalData.clrBarFace : globalData.clrBtnFace;
	if (bIsPressed)
	{
		clrBack = m_clrHighlightDn;
	}
	...
	CBCGPMenuImages::DrawByColor(pDC, CBCGPMenuImages::IdClose, rect, clrBack);
