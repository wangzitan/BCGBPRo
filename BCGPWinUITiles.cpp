...
//*****************************************************************************************
LPCTSTR CBCGPWinUITile::GetAccName() const
{
	ASSERT_VALID(this);
...
//*****************************************************************************************
LPCTSTR CBCGPWinUITile::GetAccDescription() const
{
	ASSERT_VALID(this);
	return m_strText;
}
		...
		else
		{
			...
			else
			{
				...
				if (pGroupCaption != NULL)
				{
					...
					for (POSITION pos = m_lstTilesSorted.GetHeadPosition(); pos != NULL;)
					{
						CBCGPWinUITile* pTileCurr = m_lstTilesSorted.GetNext(pos);
						ASSERT_VALID(pTileCurr);
						
						if (pTileCurr->IsVisible() && !pTileCurr->GetRect().IsRectEmpty() && pTileCurr->GetGroupID() == nID)
						{
							pNewSelected = pTileCurr;
							EnsureVisible(pNewSelected);
							break;
						}
					}
...
void CBCGPWinUITiles::OnChangeVisualManager()
{
	...
	m_ScrollBar.OnChangeVisualManager();
	Redraw();
}
//*****************************************************************************************
...
void CBCGPWinUITiles::RedrawObject(CBCGPWinUIBaseObject* pTile)
{
	if (pTile != NULL)
	{
		CRect rect = (CRect)pTile->m_rect;
		rect.InflateRect(1, 1);

		GetOwner()->RedrawWindow(rect);
	}
}
//*****************************************************************************************
...
BOOL CBCGPWinUITiles::OnGestureEventPan(const CBCGPPoint& ptFrom, const CBCGPPoint& ptTo, CBCGPSize& sizeOverPan)
{
	...
	if (m_ScrollBar.DoGestureEventPan(ptFrom, ptTo))
	{
		return TRUE;
	}
...
//*****************************************************************************
void CBCGPWinUITilesCtrl::SetWinUITilesRTI(CRuntimeClass* pWinUITilesRTI)
{
	if (pWinUITilesRTI != NULL && !pWinUITilesRTI->IsDerivedFrom(RUNTIME_CLASS(CBCGPWinUITiles)))
	{
		ASSERT(FALSE);
		pWinUITilesRTI = NULL;
	}

	m_pWinUITilesRTI = pWinUITilesRTI;
}
