...
public:
	...
	virtual LPCTSTR GetAccName() const;
	virtual LPCTSTR GetAccDescription() const;
...
public:
	...
	void SetWinUITilesRTI(CRuntimeClass* pWinUITilesRTI);
...
public:
	...
	{ 
		...
		{
			if (m_pWinUITilesRTI == NULL)
			{
				m_pWinUITiles = new CBCGPWinUITiles();
			}
			else
			{
				m_pWinUITiles = DYNAMIC_DOWNCAST(CBCGPWinUITiles, m_pWinUITilesRTI->CreateObject());
			}
		}
...
protected:
	...
	CRuntimeClass*		m_pWinUITilesRTI;
};
...
public:	
	...
	const CBCGPRect& GetGroupBounds() const	{ return m_rectGroup; }
