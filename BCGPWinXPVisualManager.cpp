												...
												CBCGPToolbarComboBoxButton* pButton)
{
	...
	if (bIsHighlighted || bIsDropped || globalData.IsHighContastMode() || CBCGPToolBarImages::m_bIsDrawOnGlass)
	{
		...
		{
			CBCGPDrawManager dm (*pDC);
			dm.DrawRect (rect, (COLORREF)-1, (bIsHighlighted || bIsDropped) ? globalData.clrHilite : globalData.clrBarShadow);
		}
		else
		{
...
void CBCGPWinXPVisualManager::OnFillTasksPaneBackground(CDC* pDC, CRect rectWorkArea)
{
	...
	if (globalData.bIsWindows8)
	{
		pDC->FillRect(rectWorkArea, &globalData.brBtnFace);
		return;
	}
								...
								BOOL bIsHighlighted, BOOL bIsSelected, BOOL bCanCollapse)
{
	...
	// -------------------------------
	COLORREF clrText = pGroup->m_clrText;
	COLORREF clrTextHot = pGroup->m_clrTextHot;
	...
	if (pGroup->m_bIsSpecial)
	{
		...
		{
			if ((*m_pfGetThemeColor) (m_hThemeExplorerBar, EBP_SPECIALGROUPHEAD, 0, TMT_TEXTCOLOR, &clrText) != S_OK)
			{
				clrText = globalData.clrWindow;
			}
		}

		if (clrTextHot == (COLORREF)-1)
		{
			if ((*m_pfGetThemeColor) (m_hThemeExplorerBar, EBP_SPECIALGROUPHEAD, TS_HOT, TMT_TEXTCOLOR, &clrTextHot) != S_OK)
			{
				clrTextHot = globalData.clrWindow;
			}
		}
	}
	else
	{
		...
		if (clrText == (COLORREF)-1)
		{
			if ((*m_pfGetThemeColor) (m_hThemeExplorerBar, EBP_NORMALGROUPHEAD, 0, TMT_TEXTCOLOR, &clrText) != S_OK)
			{
				clrText = m_clrTaskPaneHotText;
			}
		}

		if (clrTextHot == (COLORREF)-1)
		{
			if ((*m_pfGetThemeColor) (m_hThemeExplorerBar, EBP_NORMALGROUPHEAD, TS_HOT, TMT_TEXTCOLOR, &clrTextHot) != S_OK)
			{
				clrTextHot = m_clrTaskPaneHotText;
			}
		}
	}
	...
	COLORREF clrTextOld = pDC->SetTextColor(bCanCollapse && bIsHighlighted ? clrTextHot : clrText);
						...
						int iTab, BOOL bIsActive, const CBCGPBaseTabWnd* pTabWnd)
{
	...
	if (!m_b3DTabsXPTheme || m_hThemeTab == NULL || pTabWnd->IsFlatTab () || 
		...
		pTabWnd->IsLeftRightRounded () || pTabWnd->IsPureFlatTab() || pTabWnd->IsDotsStyle())
	{
										 ...
										 const CBCGPBaseTabWnd* pTabWnd)
{
	...
	if (!m_b3DTabsXPTheme || m_hThemeTab == NULL || pTabWnd->IsFlatTab () || 
		pTabWnd->IsOneNoteStyle () || pTabWnd->IsVS2005Style () ||
		pTabWnd->IsDialogControl () || pTabWnd->IsPureFlatTab() || pTabWnd->IsDotsStyle())
	{
...
void CBCGPWinXPVisualManager::OnDrawGridSelectAllMarker(CBCGPGridCtrl* pCtrl, CDC* pDC, CRect rect, int nPadding, BOOL bPressed)
{
	...
	(*m_pfDrawThemeBackground) (m_hThemeStatusBar, pDC->GetSafeHdc(), SP_GRIPPER, 0, &rect, 0);
}
//********************************************************************************
...
void CBCGPWinXPVisualManager::OnDrawLightBoxDialogCloseButton(CDC* pDC, CBCGPLightBoxDialog* pDlg, CRect rectClose, BOOL bIsHighlighted, BOOL bIsPressed)
{
	if (m_hThemeWindow == NULL || globalData.m_nBitsPerPixel <= 8)
	{
		CBCGPVisualManagerXP::OnDrawLightBoxDialogCloseButton(pDC, pDlg, rectClose, bIsHighlighted, bIsPressed);
		return;
	}
	
    int nState = bIsPressed ? CBS_PUSHED : bIsHighlighted ? CBS_HOT : 0;

	CSize sizeImage = CBCGPMenuImages::Size();
	CSize sizeButton = CSize(
		min(rectClose.Width(), 2 * sizeImage.cx),
		min(rectClose.Height(), 2 * sizeImage.cy));
	
	CRect rect (CPoint(
		rectClose.CenterPoint().x - sizeButton.cx / 2,
		rectClose.CenterPoint().y - sizeButton.cy / 2),
		sizeButton);

	(*m_pfDrawThemeBackground)(m_hThemeWindow, pDC->GetSafeHdc(), WP_CLOSEBUTTON, nState, &rect, 0);
}
