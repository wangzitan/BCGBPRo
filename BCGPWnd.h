...
public:
	...
	DWORD_PTR GetAccUserData() const 
	{
		return m_AccData.m_dwUserData; 
	}

	void SetAccUserData(DWORD_PTR dwUserData)
	{
		m_AccData.m_dwUserData = dwUserData; 
	}
...
class BCGCBPRODLLEXPORT CBCGPCtrlPlaceHolderHelper
{
public:
	BOOL CreateOnPlaceHolder(CWnd* pParent, UINT nIDPlaceHolder, UINT nID = 0, 
		DWORD dwStyle = WS_TABSTOP | WS_CHILD | WS_VISIBLE, BOOL bDestroyPlaceHolder = FALSE,
		LPVOID lParam = NULL);
	
protected:
	virtual CWnd* DoCreateOnPlaceHolder(CWnd* pParent, UINT nID, DWORD dwStyle, const CRect& rect, LPVOID lParam) = 0;
};
