...
UINT BCGM_ONERASECHILDCONTROLBACKGROUND	= ::RegisterWindowMessage (_T("BCGM_ONERASECHILDCONTROLBACKGROUND"));

extern CObList	gAllToolbars;

BOOL CBCGPMemDC::m_bUseMemoryDC = TRUE;
BOOL CBCGPMemDC::m_bUseBufferedPaint = TRUE;
	...
	m_dblScale		(dblScale)
{
	...
	if (m_bUseBufferedPaint && m_dblScale == 1.0 && globalData.m_pfBeginBufferedPaint != NULL && globalData.m_pfEndBufferedPaint != NULL && !dc.IsPrinting())
	{
		HDC hdcPaint = NULL;
...
BCGPGLOBAL_DATA::BCGPGLOBAL_DATA()
{
	...
	if (bIsWindows8)
	{
		typedef struct _BCGP_RTL_OSVERSIONINFO
		{
		...
		BCGP_RTL_OSVERSIONINFO;
		
		typedef LONG (WINAPI* BCGPF_RTLGETVERSIONPTR)(BCGP_RTL_OSVERSIONINFO*);

		HINSTANCE hinstNTDll = ::LoadLibrary(_T("ntdll.DLL"));
		if (hinstNTDll != NULL)
		{
			BCGPF_RTLGETVERSIONPTR pfRtlGetVersion = (BCGPF_RTLGETVERSIONPTR)::GetProcAddress(hinstNTDll, "RtlGetVersion");
			if (pfRtlGetVersion != NULL)
			{
				BCGP_RTL_OSVERSIONINFO info;
				memset(&info, 0, sizeof(info));
				info.dwOSVersionInfoSize = sizeof(info);

				if ((*pfRtlGetVersion)(&info) == 0)
				{
					bIsWindows10 = (info.dwMajorVersion >= 10);
				}
			}
	...
	m_clrDWMText = (COLORREF)-1;
	m_bDWMTextColorIsReady = FALSE;
...
void BCGPGLOBAL_DATA::UpdateUser32Wrappers()
{
	...
	if (m_hinstUser32 != NULL)
	{
		...
		m_pfPrintWindow = (BCGP_PRINTWINDOW)::GetProcAddress(m_hinstUser32, "PrintWindow");
	}
	else
	{
		m_pfChangeWindowMessageFilter = NULL;
		m_pfGetComboBoxInfo = NULL;
		m_pfPrintWindow = NULL;
	}
}
//************************************************************************************
void BCGPGLOBAL_DATA::UpdateDWMApiWrappers()
{
	...
	if (m_hinstDwmapiDLL != NULL)
	{
		...
		m_pfDwmGetColorizationColor = (BCGP_DWMGETCOLORIZATIONCOLOR) ::GetProcAddress(m_hinstDwmapiDLL, "DwmGetColorizationColor");
	}
	else
	{
		...
		m_pfDwmGetColorizationColor = NULL;
	}
}
//************************************************************************************
...
//*******************************************************************************************
HICON BCGPGLOBAL_DATA::GetCommonIcon(LPCTSTR iconID /* IDI_INFORMATION, IDI_WARNING, IDI_ERROR or IDI_QUESTION */)
{
#if (!defined _BCGPCALENDAR_STANDALONE) && !(defined _BCGPGRID_STANDALONE) && !(defined _BCGPEDIT_STANDALONE) && !(defined _BCGPCHART_STANDALONE)
	if (m_hiconWarning == NULL || m_hiconQuestion == NULL || m_hiconError == NULL || m_hiconInfo == NULL)
	{
		CBCGPLocalResource locaRes;

#ifndef BCGP_EXCLUDE_SVG
		CBCGPSVGImageList svgList;
		if (svgList.Load(IDB_BCGBARRES_INFOTIP_ICONS))
		{
			CBCGPToolBarImages images;
			images.SetImageSize(CSize(::GetSystemMetrics (SM_CYICON), ::GetSystemMetrics (SM_CYICON)));
			
			images.AddSVGList(svgList);
			
			if (images.GetCount() > 0)
			{
				if (m_hiconWarning == NULL)
				{
					m_hiconWarning = images.ExtractIcon(4);
				}

				if (m_hiconQuestion == NULL)
				{
					m_hiconQuestion = images.ExtractIcon(6);
				}

				if (m_hiconError == NULL)
				{
					m_hiconError = images.ExtractIcon(1);
				}

				if (m_hiconInfo == NULL)
				{
					m_hiconInfo = images.ExtractIcon(5);
				}
			}
		}
	...
	if (iconID == IDI_INFORMATION)
	{
		return m_hiconInfo;
	}
	else if (iconID == IDI_WARNING)
	{
		return m_hiconWarning;
	}
	else if (iconID == IDI_ERROR)
	{
		return m_hiconError;
	}
	else if (iconID == IDI_QUESTION)
	{
		return m_hiconQuestion;
	}
#else
...
void BCGPGLOBAL_DATA::CleanUp ()
{
	...
	CBCGPGraphicsManagerGdiPlus::Shutdown();
#endif
...
BOOL BCGPGLOBAL_DATA::DrawParentBackground (CWnd* pWnd, CDC* pDC, LPRECT rectClip)
{
	...
	if (pParent->SendMessage(BCGM_ONERASECHILDCONTROLBACKGROUND, (WPARAM)pDC->GetSafeHdc(), (LPARAM)pWnd->GetSafeHwnd()) == 0)
	{
#if (!defined _BCGPCALENDAR_STANDALONE) && !(defined _BCGPGRID_STANDALONE) && !(defined _BCGPEDIT_STANDALONE) && !(defined _BCGPCHART_STANDALONE)
...
//***********************************************************************************************
BOOL BCGPGLOBAL_DATA::DwmGetColorizationColor(DWORD* pcrColorization, BOOL* pfOpaqueBlend)
{
	if (m_pfDwmGetColorizationColor == NULL)
	{
		return FALSE;
	}

	HRESULT hres = (*m_pfDwmGetColorizationColor)(pcrColorization, pfOpaqueBlend);
	return hres == S_OK;
}
//***********************************************************************************************
...
//***********************************************************************************************
static BOOL CheckForColoredTitleBars(LPCTSTR lpszPath, BOOL& bRes)
{
#if (!defined _BCGPCALENDAR_STANDALONE) && !(defined _BCGPGRID_STANDALONE) && !(defined _BCGPEDIT_STANDALONE) && !(defined _BCGPCHART_STANDALONE)
	CBCGPRegistrySP regSP;
	CBCGPRegistry& reg = regSP.Create(FALSE, TRUE);
	
	const CString strKey(_T("ColorPrevalence"));
	DWORD dwColorPrevalence = 0;
	
	if (reg.Open(lpszPath) && reg.Read(strKey, dwColorPrevalence))
	{
		bRes = dwColorPrevalence != 0;
		return TRUE;
	}
#else
...
//***********************************************************************************************
COLORREF BCGPGLOBAL_DATA::GetDWMTextColor(BOOL bIsActive/* = TRUE*/, BOOL bFrameIsZoomed/* = FALSE*/, int* pnGlassGlowSize/* = NULL*/)
{
#if (!defined _BCGPCALENDAR_STANDALONE) && !(defined _BCGPGRID_STANDALONE) && !(defined _BCGPEDIT_STANDALONE) && !(defined _BCGPCHART_STANDALONE)
	if (bFrameIsZoomed && !bIsWindows7)
	{
		// Windows Vista has a special (black) background when frame is maximized

		if (pnGlassGlowSize != NULL)
		{
			*pnGlassGlowSize = 0;
		}
	...
	if (!bIsActive)
	{
		return bIsWindows10 ? clrGrayedText : (COLORREF)-1;
	}

	if (m_bDWMTextColorIsReady)
	{
		return m_clrDWMText;
	}

	m_clrDWMText = (COLORREF)-1;
	m_bDWMTextColorIsReady = TRUE;

	if (!IsTitleBarAccentColor())
	{
		return m_clrDWMText;
	}

	COLORREF clrTitleBar = GetWindowsAccentColor();
	if (clrTitleBar != (COLORREF)-1)
	{
		m_clrDWMText = CBCGPDrawManager::IsDarkColor(clrTitleBar) ? RGB(255, 255, 255) : RGB(10, 10, 10);
	}

#else
	UNREFERENCED_PARAMETER(bIsActive);
	UNREFERENCED_PARAMETER(bFrameIsZoomed);
	UNREFERENCED_PARAMETER(pnGlassGlowSize);
#endif
	return m_clrDWMText;
}
//***********************************************************************************************
void BCGPGLOBAL_DATA::ResetDWMTextColor()
{
	m_bDWMTextColorIsReady = FALSE;
	m_clrDWMText = (COLORREF)-1;
}
//***********************************************************************************************
COLORREF BCGPGLOBAL_DATA::GetWindowsAccentColor()
{
#if (!defined _BCGPCALENDAR_STANDALONE) && !(defined _BCGPGRID_STANDALONE) && !(defined _BCGPEDIT_STANDALONE) && !(defined _BCGPCHART_STANDALONE)
	DWORD dwColor = 0;
	BOOL opaque = FALSE;
	...
	const CString strKey(_T("ColorizationColor"));
	
	if (!reg.Open(_T("Software\\Microsoft\\Windows\\DWM")) || !reg.Read(strKey, dwColor))
	{
		if (!DwmGetColorizationColor(&dwColor, &opaque))
		{
			return ::GetSysColor(COLOR_HIGHLIGHT);
		}
	}

	COLORREF clr = ((dwColor & 0x00ff0000) >> 16) | ((dwColor & 0x0000ff00)) | ((dwColor & 0x000000ff) << 16);

	if (CBCGPDrawManager::IsPaleColor(clr))
	{
		clr = CBCGPDrawManager::ColorMakeDarker(clr, .3, TRUE);
	}
...
//***********************************************************************************************
BOOL BCGPGLOBAL_DATA::IsTitleBarAccentColor()
{
	if (!bIsWindows10)
	{
		return FALSE;
	...
	BOOL bIsTitleBarColorized = FALSE;
	
	if (!CheckForColoredTitleBars(_T("Software\\Microsoft\\Windows\\DWM"), bIsTitleBarColorized))
	{
		CheckForColoredTitleBars(_T("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize"), bIsTitleBarColorized);
	}

	return bIsTitleBarColorized;
}
//***********************************************************************************************
void BCGPGLOBAL_DATA::CheckForImmersiveColorSetChange(LPCTSTR lpszSection)
{
#if (!defined _BCGPCALENDAR_STANDALONE) && !(defined _BCGPGRID_STANDALONE) && !(defined _BCGPEDIT_STANDALONE) && !(defined _BCGPCHART_STANDALONE)
	if (lpszSection == NULL)
	{
	...
	if (strSection.CompareNoCase(_T("ImmersiveColorSet")) != 0)
	{
		return;
	}
	...
	CBCGPVisualManagerVS2012* pVM2012 = DYNAMIC_DOWNCAST(CBCGPVisualManagerVS2012, CBCGPVisualManager::GetInstance());
	if (pVM2012 != NULL && pVM2012->GetAccentColor() == CBCGPVisualManagerVS2012::VS2012_Default)
	{
		CWnd* pWndMain = AfxGetMainWnd();
		if (pWndMain->GetSafeHwnd() != NULL)
		{
			const BOOL bWndVisible = pWndMain->IsWindowVisible();
			if (bWndVisible)
			{
				pWndMain->SetRedraw(FALSE);
			}

			pWndMain->SendMessageToDescendants(WM_SYSCOLORCHANGE);
			pWndMain->SendMessage(BCGM_CHANGEVISUALMANAGER);

			if (bWndVisible)
			{
				pWndMain->SetRedraw(TRUE);
				pWndMain->RedrawWindow(NULL, NULL, RDW_INVALIDATE | RDW_FRAME | RDW_UPDATENOW | RDW_ALLCHILDREN);
			}
		}
	}
#else
...
//***********************************************************************************************
HICON BCGPGLOBAL_DATA::GetLinkIcon()
{
#if (!defined _BCGPCALENDAR_STANDALONE) && !(defined _BCGPGRID_STANDALONE) && !(defined _BCGPEDIT_STANDALONE) && !(defined _BCGPCHART_STANDALONE)
	if (m_hiconLink == NULL)
	{
...
BOOL BCGPGLOBAL_DATA::SetDPIAware ()
{
	...
	typedef enum _BCGPDPI_AWARENESS
	{ 
		BCGPDPI_AWARENESS_INVALID            = -1,
		BCGPDPI_AWARENESS_UNAWARE            = 0,
		BCGPDPI_AWARENESS_SYSTEM_AWARE       = 1,
		BCGPDPI_AWARENESS_PER_MONITOR_AWARE  = 2
	} BCGPDPI_AWARENESS;

	typedef HANDLE BCGPDPI_AWARENESS_CONTEXT;
	const BCGPDPI_AWARENESS_CONTEXT BCGPDPI_AWARENESS_CONTEXT_SYSTEM_AWARE         = (BCGPDPI_AWARENESS_CONTEXT)-2;

	// Windows 10
	typedef BCGPDPI_AWARENESS_CONTEXT (__stdcall * BCGPSETPROCESSDPIAWARENESSCONTEXT)(BCGPDPI_AWARENESS_CONTEXT dpiContext);

	BCGPSETPROCESSDPIAWARENESSCONTEXT pSetProcessDpiAwarenessContext = (BCGPSETPROCESSDPIAWARENESSCONTEXT)::GetProcAddress(m_hinstUser32, "SetProcessDpiAwarenessContext");
	if (pSetProcessDpiAwarenessContext != NULL)
	{
		BCGPDPI_AWARENESS_CONTEXT dpiContext = BCGPDPI_AWARENESS_CONTEXT_SYSTEM_AWARE;

		typedef BOOL (__stdcall * BCGPISVALIDDPIAWARENESSCONTEXT)(BCGPDPI_AWARENESS_CONTEXT dpiContext);
		...
		{
			if ((*pIsValidDpiAwarenessContext)(dpiContext))
			{
				bRes = (*pSetProcessDpiAwarenessContext)(dpiContext) != NULL;
			}
		}
	}
	...
	if (!bRes)
	{
		...
		HINSTANCE hinstShcore = LoadLibrary(_T("Shcore.dll"));
		if (hinstShcore != NULL)
		{
			BCGPSETPROCESSDPIAWARENESS pSetProcessDpiAwareness = (BCGPSETPROCESSDPIAWARENESS)::GetProcAddress(hinstShcore, "SetProcessDpiAwareness");
...
BOOL BCGPGLOBAL_DATA::PrintWindow(HWND hwnd, HDC hdcBlt, UINT nFlags)
{
	if (m_pfPrintWindow == NULL)
	{
		return FALSE;
	}

	return (*m_pfPrintWindow)(hwnd, hdcBlt, nFlags);
}
