...
typedef HRESULT (__stdcall * BCGP_DWMGETCOLORIZATIONCOLOR)(DWORD*, BOOL*);
...
typedef BOOL (WINAPI * BCGP_PRINTWINDOW)(HWND hwnd, HDC hdcBlt, UINT nFlags);
...
// Implementation
	...
	{
		return m_bUseBuiltIn32BitIcons && m_nBitsPerPixel >= 16;
	}
	...
	BOOL DwmGetColorizationColor(DWORD* pcrColorization, BOOL* pfOpaqueBlend);
	...
	COLORREF GetDWMTextColor(BOOL bIsActive = TRUE, BOOL bFrameIsZoomed = FALSE, int* pnGlassGlowSize = NULL);
	void ResetDWMTextColor();
	
	COLORREF GetWindowsAccentColor();
	BOOL IsTitleBarAccentColor();

	void CheckForImmersiveColorSetChange(LPCTSTR lpszSection);
	...
	// Common icons support:
	HICON GetCommonIcon(LPCTSTR iconID /* IDI_INFORMATION, IDI_WARNING, IDI_ERROR or IDI_QUESTION */);
...
protected:
	...
	BCGP_DWMGETCOLORIZATIONCOLOR		m_pfDwmGetColorizationColor;
	...
	BCGP_PRINTWINDOW					m_pfPrintWindow;
	...
	COLORREF							m_clrDWMText;
	BOOL								m_bDWMTextColorIsReady;
};
...
BCGCBPRODLLEXPORT extern UINT BCGM_ONERASECHILDCONTROLBACKGROUND;
