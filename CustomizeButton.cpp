			...
			BOOL /*bDrawBorder*/, BOOL /*bGrayDisabledButtons*/)
{
	...
	if (!m_bOnGlass && GetRValue (clrText) > 192 && GetGValue (clrText) >192 && GetBValue (clrText) > 192)
	{
...
SIZE CCustomizeButton::OnCalculateSize (CDC* /*pDC*/, const CSize& sizeDefault, BOOL bHorz)
{
	...
	int nMargin = CBCGPVisualManager::GetInstance ()->GetToolBarCustomizeButtonMargin ();
	const int xLargeIcons = CBCGPToolBar::IsLargeIcons () && !m_bIgnoreLargeIconsMode ? 2 : 1;

	if (bHorz)
	{
		return CSize (CBCGPMenuImages::Size ().cx * xLargeIcons + 2 * nMargin, sizeDefault.cy);
	}
	else
	{
		return CSize(sizeDefault.cx, CBCGPMenuImages::Size ().cy * xLargeIcons + 2 * nMargin);
	}
}
//*****************************************************************************************
